...
describe(':write on terminal buffer', function()
  ...
  it(':w with a bare *.mpack name saves state under stdpath("state")/term/', function()
    command('terminal')
    command('write test1.mpack')
    local fname = vim.fs.joinpath(fn.stdpath('state'), 'term', 'test1.mpack')
  ...
  it(':w with a path separator saves the *.mpack to that exact location', function()
    command('terminal')
    local path_dir = vim.fs.joinpath(xstate, 'terminal-write-dir')
    n.mkdir_p(path_dir)
    command('write ' .. vim.fs.joinpath(path_dir, 'test2.mpack'))
    eq(1, fn.filereadable(vim.fs.joinpath(path_dir, 'test2.mpack')))
    eq(0, fn.filereadable(vim.fs.joinpath(fn.stdpath('state'), 'term', 'test2.mpack')))
  end)
  ...
  it(':w *.mpack refuses to overwrite without !', function()
    ...
    command('write! test3.mpack')
    eq(1, fn.filereadable(vim.fs.joinpath(fn.stdpath('state'), 'term', 'test3.mpack')))
  end)

  it(':w *.mpack onto a directory gives E17', function()
    command('terminal')
    local path_dir = vim.fs.joinpath(xstate, 'test_dir')
    n.mkdir_p(vim.fs.joinpath(path_dir, 'dir.mpack'))
    local err = pcall_err(command, 'write ' .. vim.fs.joinpath(path_dir, 'dir.mpack'))
    ok(err:find('E17') ~= nil)
  end)
