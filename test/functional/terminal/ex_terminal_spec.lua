...
describe(':mksession with terminal buffers', function()
  local xstate = 'Xtest-functional-terminal'
  local session_file = 'test-terminal_session.vim'
  ...
  it('saves terminal state and :source restores it', function()
    command('terminal')
    command('mksession ' .. session_file)
    local dir = vim.fs.joinpath(fn.stdpath('state'), 'term')
    eq(1, #fn.readdir(dir))
    ok(t.read_file(session_file):find('term://', 1, true) ~= nil)
    command('%bwipeout!')

    n.clear({ env = { XDG_STATE_HOME = xstate } })
  ...
  it('does not save terminal state without "terminal" in sessionoptions', function()
    command('set sessionoptions-=terminal')
    command('terminal')
    command('mksession ' .. session_file)
    local dir = vim.fs.joinpath(fn.stdpath('state'), 'term')
    eq(0, fn.isdirectory(dir))
  end)

  it('does not save the state of an exited terminal', function()
    command('terminal echo hi')
    retry(3, nil, function()
      local chan = api.nvim_get_option_value('channel', { buf = 0 })
      eq(true, fn.jobwait({ chan }, 0)[1] ~= -1)
    end)
    command('mksession ' .. session_file)
    local dir = vim.fs.joinpath(fn.stdpath('state'), 'term')
    eq(0, fn.isdirectory(dir))
