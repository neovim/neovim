...
describe("'scrollback' option", function()
  ...
  it('error if setlocal on a normal buffer', function()
    command('new')
  ...
  it('no error if setlocal on a terminal buffer', function()
    command('terminal')
    execute('setlocal scrollback=32')
    eq('', eval("v:errmsg"))
    eq(32, curbufmeths.get_option('scrollback'))
  end)

  it('always -1 on a normal buffer', function()
    command('new')
