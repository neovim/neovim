...
describe('TUI', function()
  it('exit status 1 and error message with server --listen error #34365', function()
    ...
    if not is_os('win') then
      -- On Windows, the server is spawned with DETACHED_PROCESS and gets a hidden
      -- console (AllocConsole), so its stderr is not visible in the TUI terminal.
      if #addr_in_use <= 600 then
        screen:expect({
          any = vim.pesc(('%s: Failed to --listen: address already in use:'):format('nvim')),
          unchanged = true,
        })
      end
    end
