...
describe('TUI as a client', function()
  it('connects to remote instance (with its own TUI)', function()
    ...
    local screen_server = thelpers.screen_setup(
      0,
      string.format(
        [=[["%s", "--listen", "%s", "-u", "NONE", "-i", "NONE", "--cmd", "%s laststatus=2 background=dark"]]=],
        nvim_prog,
    ...
    local screen_client = thelpers.screen_setup(
      0,
      string.format([=[["%s", "--server", "%s", "--remote-ui"]]=], nvim_prog, server_pipe)
    )
  ...
  it('throws error when no server exists', function()
    ...
    local screen = thelpers.screen_setup(
      0,
      string.format([=[["%s", "--server", "127.0.0.1:2436546", "--remote-ui"]]=], nvim_prog),
      60
    )
