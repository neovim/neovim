...
describe('autocmd', function()
  ...
  describe('v:event is readonly #18063', function()
    it('during ChanOpen event', function()
      command('autocmd ChanOpen * let v:event.chan_info.id = 0')
      fn.jobstart({ 'cat' })
      retry(nil, nil, function()
        eq('E46: Cannot change read-only variable "v:event.chan_info"', api.nvim_get_vvar('errmsg'))
      end)
    end)

    it('during ChanOpen event', function()
      command('autocmd ChanInfo * let v:event.chan_info.id = 0')
