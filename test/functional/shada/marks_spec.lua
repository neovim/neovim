...
describe('ShaDa support code', function()
  ...
  it('does not restore the jump list with :keepjumps rshada', function()
    nvim_command('edit ' .. testfilename)
    nvim_command('normal! G')
    ...
    nvim_command('wshada')
    nvim_command('quit')
    nvim_command('keepjumps rshada')
    nvim_command('normal! \15')  -- <C-o>
    eq(testfilename, funcs.bufname('%'))
    eq({1, 0}, curwinmeths.get_cursor())
  end)

  it('does not restore the change list with :keepjumps rshada', function()
    nvim_command('edit ' .. testfilename)
    local saved_empty_changes = redir_exec('changes')
    nvim_command('normal! Gra')
    nvim_command('normal! ggrb')
    local saved_changes = redir_exec('changes')
    nvim_command('wqall!')
    set_additional_cmd('set shada=')
    reset()
    nvim_command('edit ' .. testfilename)
    nvim_command('set shada&')
    eq(saved_empty_changes, redir_exec('changes'))
    nvim_command('keepjumps rshada')
    eq(saved_empty_changes, redir_exec('changes'))
    nvim_command('rshada')
    eq(saved_changes, redir_exec('changes'))
  end)

  it('does not restore the . and ^ marks with :keepjumps rshada', function()
    nvim_command('edit ' .. testfilename)
    nvim_command('normal! GIa')
    nvim_command('wqall!')
    set_additional_cmd('set shada=')
    reset()
    nvim_command('edit ' .. testfilename)
