...
describe(':echo :echon :echomsg :echoerr', function()
  ...
  describe('used to represent funcrefs', function()
    ...
    it('dumps automatically created partials', function()
      assert_same_echo_dump("function('<SNR>2_Test2', {'f': function('<SNR>2_Test2')})", '{"f": Test2_f}.f', true)
      assert_same_echo_dump(
    ...
    it('does not crash or halt when dumping partials with reference cycles in self and arguments', function()
      meths.set_var('d', { v = true })
      meths.set_var('l', {})
      eval 'add(l, l)'
      eval 'add(l, function("Test1", l))'
  ...
  describe('used to represent dictionaries', function()
    ...
    it('dumps recursive dictionaries without the error', function()
      meths.set_var('d', { d = 1 })
      eval 'extend(d, {"d": d})'
      eq("\n{'d': {...@0}}\n{'d': {...@0}}", redir_exec 'echo String(d)')
      eq("\n{'out': {'d': {...@1}}}\n{'out': {'d': {...@1}}}", redir_exec 'echo String({"out": d})')
    end)
  end)
