...
describe(':write', function()
  ...
  it('errors out correctly', function()
    ...
    eq('\nE502: "." is a directory', redir_exec 'write .')
    ...
    funcs.setline(1, 'TTY')
    eq("Vim(write):E510: Can't make backup file (add ! to override)", exc_exec 'write')
    meths.set_option('backup', false)
    funcs.setfperm(fname, 'r--------')
    eq('Vim(write):E505: "Xtest-functional-ex_cmds-write" is read-only (add ! to override)', exc_exec 'write')
    if iswin() then
    ...
    eq("Vim(write):E166: Can't open linked file for writing", exc_exec 'write!')
  end)
end)
