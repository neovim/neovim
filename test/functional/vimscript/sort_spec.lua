...
describe('sort()', function()
  it('errors out when sorting special values', function()
    eq('Vim(call):E362: Using a boolean value as a Float', exc_exec('call sort([v:true, v:false], "f")'))
  end)

  it('sorts “wrong” values between -0.0001 and 0.0001, preserving order', function()
    meths.set_var('list', { true, false, NIL, {}, { a = 42 }, 'check', 0.0001, -0.0001 })
    ...
    eq("[-1.0e-4, function('tr'), v:true, v:false, v:null, [], {'a': 42}, 'check', 1.0e-4]", eval('string(g:list)'))
  end)
  ...
  it('can yield E702 and stop sorting after that', function()
    ...
    eq('Vim(let):E745: Using a List as a Number', pcall_err(command, 'let sl = sort([1, 0, [], 3, 2], "Cmp")'))
  end)
end)
