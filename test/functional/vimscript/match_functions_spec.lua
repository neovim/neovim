...
describe('setmatches()', function()
  ...
  it('does not fail if highlight group is not defined', function()
    ...
    eq(0, funcs.setmatches { { group = 1, pos1 = { 2 }, pos2 = { 6 }, id = 3, priority = 4, conceal = 5 } })
    eq({ { group = '1', pos1 = { 2 }, pos2 = { 6 }, id = 3, priority = 4, conceal = '5' } }, funcs.getmatches())
  end)
end)
...
describe('matchaddpos()', function()
  it('errors out on invalid input', function()
    ...
    eq('Vim(let):E5030: Empty list at position 0', exc_exec('let val = matchaddpos("PreProc", [[]])'))
    eq('Vim(let):E5030: Empty list at position 1', exc_exec('let val = matchaddpos("PreProc", [1, v:_null_list])'))
    eq(
