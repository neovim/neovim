  ...
  describe(funcname .. '() function', function()
    it('errors out when receives v:true/v:false/v:null', function()
      ...
      for _, var in ipairs { 'v:true', 'v:false' } do
        eq('Vim(call):E5299: Expected a Number or a String, Boolean found', exc_exec('call ' .. func:format(var)))
      end
      eq('Vim(call):E5300: Expected a Number or a String', exc_exec('call ' .. func:format 'v:null'))
    end)
    it('errors out when receives invalid argument', function()
      eq('Vim(call):E745: Expected a Number or a String, List found', exc_exec('call ' .. func:format '[]'))
      eq('Vim(call):E728: Expected a Number or a String, Dictionary found', exc_exec('call ' .. func:format '{}'))
      eq('Vim(call):E805: Expected a Number or a String, Float found', exc_exec('call ' .. func:format '0.0'))
      eq(
...
describe('setbufvar() function', function()
  it('throws the error or ignores the input when buffer was not found', function()
    ...
    command('edit ' .. fname2)
    eq(2, funcs.bufnr '%')
    eq('Vim(call):E93: More than one match for X', exc_exec 'call setbufvar("X", "&autoindent", 0)')
  end)
  ...
  it('may set variables', function()
    ...
    eq(true, bufmeths.get_var(buf1, 'number'))
    eq('Vim:E46: Cannot change read-only variable "b:changedtick"', pcall_err(funcs.setbufvar, 1, 'changedtick', true))
    eq(2, funcs.getbufvar(1, 'changedtick'))
  end)
end)
