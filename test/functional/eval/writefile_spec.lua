...
describe('writefile()', function()
  ...
  it('errors out with invalid arguments', function()
    ...
    eq('\nE119: Not enough arguments for function: writefile', redir_exec 'call writefile()')
    eq('\nE119: Not enough arguments for function: writefile', redir_exec 'call writefile([])')
    eq(
    ...
    for _, args in ipairs { '[], %s, "b"', '[], "' .. fname .. '", %s' } do
      eq('\nE806: using Float as a String', redir_exec(('call writefile(%s)'):format(args:format '0.0')))
      eq('\nE730: using List as a String', redir_exec(('call writefile(%s)'):format(args:format '[]')))
      eq('\nE731: using Dictionary as a String', redir_exec(('call writefile(%s)'):format(args:format '{}')))
      eq('\nE729: using Funcref as a String', redir_exec(('call writefile(%s)'):format(args:format 'function("tr")')))
    end
    eq('\nE5060: Unknown flag: «»', redir_exec(('call writefile([], "%s", "bs«»")'):format(fname)))
  ...
  it('does not write to file if error in list', function()
    ...
    eq(nil, read_file(fname))
    write_file(fname, 'TEST')
    eq('\nE745: Expected a Number or a String, List found', redir_exec(('call writefile(%s)'):format(args:format '[]')))
    eq('TEST', read_file(fname))
    eq(
