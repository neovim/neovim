...
describe('b:changedtick', function()
  ...
  it('fails to be changed by user', function()
    ...
    eq('\nE46: Cannot change read-only variable "b:changedtick"', redir_exec('let b:changedtick = ' .. ctn))
    eq('\nE46: Cannot change read-only variable "b:["changedtick"]"', redir_exec('let b:["changedtick"] = ' .. ctn))
    eq('\nE46: Cannot change read-only variable "b:.changedtick"', redir_exec('let b:.changedtick = ' .. ctn))
    eq('\nE46: Cannot change read-only variable "d.changedtick"', redir_exec('let d.changedtick = ' .. ctn))
    ...
    eq('\nE46: Cannot change read-only variable "b:.changedtick"', redir_exec 'unlet b:.changedtick')
    eq('\nE46: Cannot change read-only variable "b:["changedtick"]"', redir_exec 'unlet b:["changedtick"]')
    eq('\nE46: Cannot change read-only variable "d.changedtick"', redir_exec 'unlet d.changedtick')
  ...
  it('is listed in :let output', function()
    eq('\nb:changedtick         #2', redir_exec ':let b:')
  end)
  it('fails to unlock b:changedtick', function()
    eq(0, exc_exec 'let d = b:')
    eq(0, funcs.islocked 'b:changedtick')
    eq(0, funcs.islocked 'd.changedtick')
    eq('\nE940: Cannot lock or unlock variable b:changedtick', redir_exec 'unlockvar b:changedtick')
    eq('\nE46: Cannot change read-only variable "d.changedtick"', redir_exec 'unlockvar d.changedtick')
    eq(0, funcs.islocked 'b:changedtick')
    eq(0, funcs.islocked 'd.changedtick')
    eq('\nE940: Cannot lock or unlock variable b:changedtick', redir_exec 'lockvar b:changedtick')
    eq('\nE46: Cannot change read-only variable "d.changedtick"', redir_exec 'lockvar d.changedtick')
    eq(0, funcs.islocked 'b:changedtick')
    eq(0, funcs.islocked 'd.changedtick')
  end)
  ...
  it('cannot be changed by filter() or map()', function()
    eq(2, changedtick())
    eq('\nE795: Cannot delete variable filter() argument', redir_exec 'call filter(b:, 0)')
    eq('\nE742: Cannot change value of map() argument', redir_exec 'call map(b:, 0)')
    eq('\nE742: Cannot change value of map() argument', redir_exec 'call map(b:, "v:val")')
    eq(2, changedtick())
  end)
  it('cannot be remove()d', function()
    eq(2, changedtick())
    eq('\nE795: Cannot delete variable remove() argument', redir_exec 'call remove(b:, "changedtick")')
    eq(2, changedtick())
  end)
