...
describe('autoread file watcher', function()
  ...
  it('coalesces rapid changes via debouncing', function()
    ...
    -- 4 back-to-back writes well inside one 100ms debounce window.
    write_file(path, 'v2\n')
    write_file(path, 'v3\n')
    write_file(path, 'v4\n')
    ...
    -- Every fs_event restarts the 100ms timer, and 4 sub-millisecond write_file
    -- calls fit well inside one window, so the timer fires exactly once.
    sleep(50)
