...
describe('luaeval()', function()
  ...
  it('issues an error in some cases', function()
    ...
    startswith(
      'Vim(call):E5107: Error loading lua [string "luaeval()"]:',
      exc_exec('call luaeval("1, 2, 3")')
    )
    startswith(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:',
  ...
  it('correctly converts containers with type_idx', function()
    ...
    eq(
      { 42 },
      funcs.luaeval('{[vim.type_idx]=vim.types.array, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}')
    )
    eq(
      { foo = 2 },
      funcs.luaeval('{[vim.type_idx]=vim.types.dictionary, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}')
    )
    eq(
      10,
      funcs.luaeval('{[vim.type_idx]=vim.types.float, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}')
    )
  ...
  it('errors out correctly when doing incorrect things in lua', function()
    ...
    eq(
      'Vim(call):E5108: Error executing lua [NULL]',
