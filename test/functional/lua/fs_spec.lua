...
describe('vim.fs', function()
  ...
  describe('is_relative_to()', function()
    local function is_rel(path, root)
      return exec_lua([[return vim.fs.is_relative_to(...)]], path, root)
    end
    it('works', function()
      local p = 'a/b'
      eq(true, is_rel(p, 'a/'))
      eq(true, is_rel(p, 'a/'))
      eq(true, is_rel(p, 'a/b'))
      eq(false, is_rel(p, 'c'))
      eq(false, is_rel(p, 'a/b/c'))
      eq(false, is_rel(p, 'a/c'))
      eq(false, is_rel(p, '/a'))
    ...
    it('works on windows', function()
      if is_os('win') then
        t.skip(true, 'Bug in vim.fs.dirname prevents these from passing')
        local p = 'C:Foo/Bar'
        eq(true, is_rel(p, 'c:'))
        eq(true, is_rel(p, 'c:foO'))
        eq(true, is_rel(p, 'c:foO/'))
        eq(true, is_rel(p, 'c:foO/baR'))
        eq(false, is_rel(p, ''))
        eq(false, is_rel(p, 'd:'))
        eq(false, is_rel(p, '/'))
        eq(false, is_rel(p, 'Foo'))
        eq(false, is_rel(p, '/Foo'))
        eq(false, is_rel(p, 'C:/Foo'))
        eq(false, is_rel(p, 'C:Foo/Bar/Baz'))
        eq(false, is_rel(p, 'C:Foo/Baz'))
        p = 'C:/Foo/Bar'
        eq(true, is_rel(p, 'c:/'))
        eq(true, is_rel(p, 'c:/foO/'))
        eq(true, is_rel(p, 'c:/foO/baR'))
        eq(false, is_rel(p, 'c:'))
        eq(false, is_rel(p, 'C:/Baz'))
        eq(false, is_rel(p, 'C:/Foo/Bar/Baz'))
        eq(false, is_rel(p, 'C:/Foo/Baz'))
        eq(false, is_rel(p, 'C:Foo'))
        eq(false, is_rel(p, 'd:'))
        ...
        eq(false, is_rel(p, '/Foo'))
        eq(false, is_rel(p, '//C/Foo'))
        p = '//Server/Share/Foo/Bar'
        eq(true, is_rel(p, '//sErver/sHare'))
        eq(true, is_rel(p, '//sErver/sHare/'))
        eq(true, is_rel(p, '//sErver/sHare/Foo'))
        eq(true, is_rel(p, '//sErver/sHare/Foo/'))
        eq(true, is_rel(p, '//sErver/sHare/Foo/Bar'))
        eq(false, is_rel(p, '/Server/Share/Foo'))
        eq(false, is_rel(p, 'c:/Server/Share/Foo'))
        eq(false, is_rel(p, '//z/Share/Foo'))
        eq(false, is_rel(p, '//Server/z/Foo'))
      end
    end)
  end)
end)
