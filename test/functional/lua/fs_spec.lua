...
describe('vim.fs', function()
  ...
  describe('normalize()', function()
    local function normalize(path, opts)
      return exec_lua([[return vim.fs.normalize(...)]], path, opts)
    end
    it('removes trailing /', function()
      eq('/home/user', normalize('/home/user/'))
    end)
    it('works with /', function()
      eq('/', normalize('/'))
    end)
    it('works with ~', function()
      eq(normalize(assert(vim.uv.os_homedir())) .. '/src/foo', normalize('~/src/foo'))
    end)
    it('works with environment variables', function()
      ...
      eq(
        ...
        exec_lua(function()
          ...
          return normalize('$XDG_CONFIG_HOME/nvim')
        end)
      )
    end)
    ...
    it('preserves leading double slashes in POSIX paths', function()
      eq('//foo', normalize('//foo', posix_opts))
      eq('//foo/bar', normalize('//foo//bar////', posix_opts))
      eq('/foo', normalize('///foo', posix_opts))
      eq('//', normalize('//', posix_opts))
      eq('/', normalize('///', posix_opts))
      eq('/foo/bar', normalize('/foo//bar////', posix_opts))
    end)

    it('handles case-insensitiveness', function()
      eq('C:/foo/test', normalize([[C:\fOo\\tEst\]], win_opts))
      eq([[//кириллица/ελληνικά/português]], normalize([[\\КИРИЛЛИЦА\ΕΛΛΗΝΙΚΆ\PORTUGUÊS]], win_opts));
    end)

    it('allows backslashes on unix-based os', function()
      eq('/home/user/hello\\world', normalize('/home/user/hello\\world', posix_opts))
    end)

    it('preserves / after drive letters', function()
      eq('C:/', normalize([[C:\]], win_opts))
    end)

    it('works with UNC and DOS device paths', function()
      eq('//server/share/foo/bar', normalize([[\\server\\share\\\foo\bar\\\]], win_opts))
      eq('//system07/C$/', normalize([[\\system07\C$\\\\]], win_opts))
      eq('//./C:/foo/bar', normalize([[\\.\\C:\foo\\\\bar]], win_opts))
      eq('//?/C:/foo/bar', normalize([[\\?\C:\\\foo\bar\\\\]], win_opts))
      eq(
        '//?/UNC/server/share/foo/bar',
        normalize([[\\?\UNC\server\\\share\\\\foo\\\bar]], win_opts)
      )
      eq('//./BootPartition/foo/bar', normalize([[\\.\BootPartition\\foo\bar]], win_opts))
      eq(
        '//./Volume{12345678-1234-1234-1234-1234567890AB}/foo/bar',
        normalize([[\\.\Volume{12345678-1234-1234-1234-1234567890AB}\\\foo\bar\\]], win_opts)
      )
    end)

    it('handles invalid UNC and DOS device paths', function()
      eq('//server/share', normalize([[\\server\share]], win_opts))
      eq('//server/', normalize([[\\server\]], win_opts))
      eq('//./UNC/server/share', normalize([[\\.\UNC\server\share]], win_opts))
      eq('//?/UNC/server/', normalize([[\\?\UNC\server\]], win_opts))
      eq('//?/UNC/server/..', normalize([[\\?\UNC\server\..]], win_opts))
      eq('//./', normalize([[\\.\]], win_opts))
      eq('//./foo', normalize([[\\.\foo]], win_opts))
      eq('//./BootPartition', normalize([[\\.\BootPartition]], win_opts))
    end)

    it('converts backward slashes', function()
      eq('C:/Users/jdoe', normalize([[C:\Users\jdoe]], win_opts))
    end)

    describe('. and .. component resolving', function()
      it('works', function()
        -- Windows paths
        eq('C:/Users', normalize([[C:\Users\jdoe\Downloads\.\..\..\]], win_opts))
        eq('C:/Users/jdoe', normalize([[C:\Users\jdoe\Downloads\.\..\.\.\]], win_opts))
        eq('C:/', normalize('C:/Users/jdoe/Downloads/./../../../', win_opts))
        eq('C:foo', normalize([[C:foo\bar\.\..\.]], win_opts))
        -- POSIX paths
        eq('/home', normalize('/home/jdoe/Downloads/./../..', posix_opts))
        eq('/home/jdoe', normalize('/home/jdoe/Downloads/./../././', posix_opts))
        eq('/', normalize('/home/jdoe/Downloads/./../../../', posix_opts))
        -- OS-agnostic relative paths
        eq('foo/bar/baz', normalize('foo/bar/foobar/../baz/./'))
        eq('foo/bar', normalize('foo/bar/foobar/../baz/./../../bar/./.'))
      end)

      it('works when relative path reaches current directory', function()
        eq('C:', normalize('C:foo/bar/../../.', win_opts))

        eq('.', normalize('.'))
        eq('.', normalize('././././'))
        eq('.', normalize('foo/bar/../../.'))
      end)

      it('works when relative path goes outside current directory', function()
        eq('../../foo/bar', normalize('../../foo/bar'))
        eq('../foo', normalize('foo/bar/../../../foo'))

        eq('C:../foo', normalize('C:../foo', win_opts))
        eq('C:../../foo/bar', normalize('C:foo/../../../foo/bar', win_opts))
      end)

      it('.. in root directory resolves to itself', function()
        eq('C:/', normalize('C:/../../', win_opts))
        eq('C:/foo', normalize('C:/foo/../../foo', win_opts))

        eq('//server/share/', normalize([[\\server\share\..\..]], win_opts))
        eq('//server/share/foo', normalize([[\\server\\share\foo\..\..\foo]], win_opts))
        ...
        eq(
          ...
          normalize([[\\?\UNC\server\\share\..\..\foo]], win_opts)
        )

        eq('//?/BootPartition/', normalize([[\\?\BootPartition\..\..]], win_opts))
        eq('//./BootPartition/foo', normalize([[\\.\BootPartition\..\..\foo]], win_opts))

        eq('/', normalize('/../../', posix_opts))
        eq('/foo', normalize('/foo/../../foo', posix_opts))
      end)
    end)
  end)
