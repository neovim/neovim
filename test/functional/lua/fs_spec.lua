...
describe('vim.fs', function()
  ...
  describe('suffix()', function()
    it('works', function()
      -- adapted from CPython's Lib/test/test_pathlib/test_join.py
      eq(nil, vim.fs.suffix(nil))
      eq('', vim.fs.suffix(''))
      eq('', vim.fs.suffix('.'))
      eq('', vim.fs.suffix('..'))
      eq('', vim.fs.suffix('/'))
      eq('', vim.fs.suffix('a/b'))
      eq('', vim.fs.suffix('/a/b'))
      eq('', vim.fs.suffix('/a/b/.'))
      eq('.lua', vim.fs.suffix('a/b.lua'))
      eq('.lua', vim.fs.suffix('/a/b.lua'))
      eq('', vim.fs.suffix('a/.nvimrc'))
      eq('', vim.fs.suffix('/a/.nvimrc'))
      eq('.rc', vim.fs.suffix('a/.hg.rc'))
      eq('.rc', vim.fs.suffix('/a/.hg.rc'))
      eq('.gz', vim.fs.suffix('a/b.tar.gz'))
      eq('.gz', vim.fs.suffix('/a/b.tar.gz'))
      eq('.', vim.fs.suffix('a/trailing.dot.'))
      eq('.', vim.fs.suffix('/a/trailing.dot.'))
      eq('.', vim.fs.suffix('a/..d.o.t..'))
      eq('.dots', vim.fs.suffix('a/inn.er..dots'))
      eq('', vim.fs.suffix('photo'))
      eq('.jpg', vim.fs.suffix('photo.jpg'))
    end)
  end)
end)
