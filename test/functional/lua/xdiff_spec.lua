...
describe('xdiff bindings', function()
  ...
  describe('can diff text', function()
    ...
    it('with error callback', function()
      ...
      eq(
        [[Error executing lua: [string "<nvim>"]:0: error running function on_hunk: [string "<nvim>"]:0: ERROR1]],
    ...
    it('with hunk_lines', function()
      ...
      eq({ { 1, 1, 1, 1 }, { 3, 1, 3, 2 } }, exec_lua([[return vim.diff(a2, b2, {result_type = 'indices'})]]))
    end)
  end)

  it('can handle bad args', function()
    eq(
      [[Error executing lua: [string "<nvim>"]:0: Expected at least 2 arguments]],
      pcall_err(exec_lua, [[vim.diff('a')]])
    )

    eq(
      [[Error executing lua: [string "<nvim>"]:0: bad argument #1 to 'diff' (expected string)]],
      pcall_err(exec_lua, [[vim.diff(1, 2)]])
    )

    eq(
      [[Error executing lua: [string "<nvim>"]:0: bad argument #3 to 'diff' (expected table)]],
      pcall_err(exec_lua, [[vim.diff('a', 'b', true)]])
    )

    eq(
      [[Error executing lua: [string "<nvim>"]:0: unexpected key: bad_key]],
      pcall_err(exec_lua, [[vim.diff('a', 'b', { bad_key = true })]])
    )

    eq(
      [[Error executing lua: [string "<nvim>"]:0: on_hunk is not a function]],
