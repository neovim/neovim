...
describe('luaeval(vim.api.…)', function()
  describe('with channel_id and buffer handle', function()
    describe('nvim_buf_get_lines', function()
      it('works', function()
        ...
        eq({ { _TYPE = {}, _VAL = { 'a\nb' } } }, funcs.luaeval 'vim.api.nvim_buf_get_lines(1, 2, 3, false)')
      end)
    end)
  ...
  describe('with errors', function()
    it('transforms API error from nvim_buf_set_lines into lua error', function()
      funcs.setline(1, { 'abc', 'def', 'a\nb', 'ttt' })
      eq(
        { false, 'String cannot contain newlines' },
    ...
    it('transforms API error from nvim_win_set_cursor into lua error', function()
      ...
      eq({ false, 'Invalid window id: -1' }, funcs.luaeval '{pcall(vim.api.nvim_win_set_cursor, -1, {1, 2, 3})}')
    end)

    it('transforms API error from nvim_win_set_cursor + same array as in first test into lua error', function()
      eq(
        { false, 'Argument "pos" must be a [row, col] array' },
  ...
  it('correctly converts container objects with type_idx to API objects', function()
    ...
    eq(
      { 42 },
      funcs.luaeval 'vim.api.nvim__id({[vim.type_idx]=vim.types.array, [vim.val_idx]=10, [5]=1, foo=2, [1]=42})'
    )
    eq(
      { foo = 2 },
      funcs.luaeval 'vim.api.nvim__id({[vim.type_idx]=vim.types.dictionary, [vim.val_idx]=10, [5]=1, foo=2, [1]=42})'
    )
  ...
  it('correctly converts arrays with type_idx to API objects', function()
    ...
    eq(
      { 42 },
      funcs.luaeval 'vim.api.nvim__id_array({[vim.type_idx]=vim.types.array, [vim.val_idx]=10, [5]=1, foo=2, [1]=42})'
    )
    eq(
      { { foo = 2 } },
      funcs.luaeval 'vim.api.nvim__id_array({{[vim.type_idx]=vim.types.dictionary, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}})'
    )
    eq(
      { 10 },
      funcs.luaeval 'vim.api.nvim__id_array({{[vim.type_idx]=vim.types.float, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}})'
    )
  ...
  it('correctly converts dictionaries with type_idx to API objects', function()
    ...
    eq(
      { v = { 42 } },
      funcs.luaeval 'vim.api.nvim__id_dictionary({v={[vim.type_idx]=vim.types.array, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}})'
    )
    eq(
      { foo = 2 },
      funcs.luaeval 'vim.api.nvim__id_dictionary({[vim.type_idx]=vim.types.dictionary, [vim.val_idx]=10, [5]=1, foo=2, [1]=42})'
    )
    eq(
      { v = 10 },
      funcs.luaeval 'vim.api.nvim__id_dictionary({v={[vim.type_idx]=vim.types.float, [vim.val_idx]=10, [5]=1, foo=2, [1]=42}})'
    )
    eq(
      { v = {} },
      funcs.luaeval 'vim.api.nvim__id_dictionary({v={[vim.type_idx]=vim.types.array, [vim.val_idx]=10, [5]=1, foo=2}})'
    )
  ...
  it('errors out correctly when working with API', function()
    -- Conversion errors
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Cannot convert given lua type',
      exc_exec [[call luaeval("vim.api.nvim__id(vim.api.nvim__id)")]]
    )
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Cannot convert given lua table',
    ...
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Expected lua string',
      exc_exec [[call luaeval("vim.api.nvim_set_var(1, 2)")]]
    )

    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Expected lua number',
      exc_exec [[call luaeval("vim.api.nvim_buf_get_lines(0, 'test', 1, false)")]]
    )
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Number is not integral',
    ...
    eq(
      'Vim(call):E5108: Error executing lua [string "luaeval()"]:1: Unexpected type',
