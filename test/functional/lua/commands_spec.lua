...
describe(':lua command', function()
  ...
  it('throws catchable errors', function()
    ...
    eq([[Vim(lua):E5108: Error executing lua [string ":lua"]:1: TEST]], remove_trace(exc_exec('lua error("TEST")')))
    eq(
...
describe(':luado command', function()
  ...
  it('fails in sandbox when needed', function()
    curbufmeths.set_lines(0, 1, false, { 'ABC', 'def', 'gHi' })
    eq('Vim(luado):E48: Not allowed in sandbox', pcall_err(command, 'sandbox luado runs = (runs or 0) + 1'))
