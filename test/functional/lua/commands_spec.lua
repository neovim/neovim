...
describe(':lua command', function()
  it('works', function()
    eq('', redir_exec 'lua vim.api.nvim_buf_set_lines(1, 1, 2, false, {"TEST"})')
  ...
  it('throws catchable errors', function()
    ...
    eq([[Vim(lua):E5108: Error executing lua [string ":lua"]:1: TEST]], exc_exec 'lua error("TEST")')
    eq(
  ...
  it('works with long strings', function()
    ...
    eq(
      '\nE5107: Error loading lua [string ":lua"]:1: unfinished string near \'<eof>\'',
      redir_exec(('lua vim.api.nvim_buf_set_lines(1, 1, 2, false, {"%s})'):format(s))
    )
...
describe(':luado command', function()
  ...
  it('fails on errors', function()
    eq([[Vim(luado):E5109: Error loading lua: [string ":luado"]:1: unexpected symbol near ')']], exc_exec 'luado ()')
    eq(
      [[Vim(luado):E5111: Error calling lua: [string ":luado"]:1: attempt to perform arithmetic on global 'liness' (a nil value)]],
  ...
  it('works with long strings', function()
    ...
    eq(
      '\nE5109: Error loading lua: [string ":luado"]:1: unfinished string near \'<eof>\'',
      redir_exec(('luado return "%s'):format(s))
    )
    eq({ '' }, curbufmeths.get_lines(0, -1, false))
...
describe(':luafile', function()
  ...
  it('correctly errors out', function()
    write_file(fname, '()')
    eq(
      ("Vim(luafile):E5112: Error while creating lua chunk: %s:1: unexpected symbol near ')'"):format(fname),
  ...
  it('works with NULL errors', function()
    ...
    eq([=[Vim(luafile):E5113: Error while calling lua chunk: [NULL]]=], exc_exec('luafile ' .. fname))
  end)
end)
