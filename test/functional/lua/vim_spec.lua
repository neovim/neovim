...
describe('lua stdlib', function()
  ...
  it('vim.is_truthy', function()
    eq(false, exec_lua[[return vim.is_truthy(false)]])
    eq(false, exec_lua[[return vim.is_truthy(nil)]])
    eq(false, exec_lua[[return vim.is_truthy(0)]])
    eq(false, exec_lua[[return vim.is_truthy("")]])
    eq(false, exec_lua[[return vim.is_truthy({})]])
    eq(false, exec_lua[[return vim.is_truthy(vim.empty_dict())]])
    eq(true, exec_lua[[return vim.is_truthy(true)]])
    eq(true, exec_lua[[return vim.is_truthy(1)]])
    eq(true, exec_lua[[return vim.is_truthy("vim")]])
    eq(true, exec_lua[[return vim.is_truthy({1})]])
    eq(true, exec_lua[[return vim.is_truthy({a="a"})]])
  end)
end)
