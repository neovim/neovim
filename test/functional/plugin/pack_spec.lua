...
describe('vim.pack', function()
  ...
  describe('add()', function()
    ...
    it('loads only the managed directory, even when it is absent from packpath', function()
      local other = vim.fs.joinpath(fn.stdpath('data'), 'site', 'pack', 'other', 'opt', 'basic')
      fn.mkdir(other .. '/plugin', 'p')
      t.write_file(other .. '/plugin/test.lua', 'vim.g.other_package_loaded = true')
      vim_pack_add({ repos_src.basic })
      eq(vim.NIL, exec_lua('return vim.g.other_package_loaded'))
