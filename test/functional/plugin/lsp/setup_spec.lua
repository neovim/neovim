...
describe('Setup and Configuration', function()
  it('should allow you to add clients and commands', function()
    luaeval([[require('lsp.api').server_config.add('vim', 'vim-lsp')]])
    local result_name = luaeval("require('vim.lsp.server_config').get_name('vim')")
    eq('vim', result_name)
  end)

  it('should allow some extra configuration if you want', function()
    luaeval([[require('lsp.api').server_config.add('vim', 'vim-lsp', { name = 'test-server' })]])
    local result_name = luaeval("require('vim.lsp.server_config').get_name('vim')")
