...
describe('incremental treesitter selection with injections', function()
  ...
  it('ignores overlapping injections', function()
    ...
    if get_selected() == 'edit' then
      ...
      exec_lua("require'vim._treeselect'.TEST_SWITCH_PRIORITY=true")
    end
    ...
    exec_lua(
      "require'vim._treeselect'.TEST_SWITCH_PRIORITY=not require'vim._treeselect'.TEST_SWITCH_PRIORITY"
    )
    feed('<esc>gg0lll')
    treeselect('select_node')
    eq('edit', get_selected())
