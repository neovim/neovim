...
describe('completion', function()
  ...
  it('<C-n> mapped to <Down> moves selection without inserting #9905', function()
    command([[inoremap <F2> <Cmd>call complete(1, ['foo', 'foobar'])<CR>]])
    command([[inoremap <expr> <C-n> pumvisible() ? '<Down>' : '<C-n>']])
    feed('i<F2>')
    eq(1, eval('pumvisible()'))
    eq('foo', eval('getline(1)'))
    eq(0, eval("complete_info(['selected']).selected"))
    feed('<C-n>')
    eq('foo', eval('getline(1)'))
    eq(1, eval("complete_info(['selected']).selected"))
    feed('<C-y>')
    eq('foobar', eval('getline(1)'))
  end)
end)
