  ...
  describe('key mappings', function()
    ...
    describe('<Plug>(nvim-closewin) (<M-q>)', function()
      ...
      it('closes a special-purpose last-window buffer without E444', function()
        n.clear({ args_rm = { '--cmd' } })
        local buf = n.api.nvim_create_buf(true, true)
        n.api.nvim_set_current_buf(buf)
        n.feed('<M-q>')
        n.poke_eventloop()
        t.eq(false, n.api.nvim_buf_is_loaded(buf))
        t.neq(buf, n.api.nvim_get_current_buf())
      end)

      it('is a no-op on a normal file in the last window', function()
        n.clear({ args_rm = { '--cmd' } })
        n.insert('change')
        n.feed('<M-q>')
        n.poke_eventloop()
        t.eq('change', n.api.nvim_get_current_line())
        t.eq(true, n.api.nvim_get_option_value('modified', { buf = 0 }))
      end)
    end)
  end)
end)
