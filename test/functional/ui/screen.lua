  ...
  self:_wait(function()
    ...
    if grid ~= nil then
      ...
      if #actual_rows ~= #expected_rows then
        err_msg = 'Expected screen height ' .. #expected_rows .. ' differs from actual height ' .. #actual_rows .. '.'
      end
...
function Screen:_wait(check, flags)
  ...
  local function notification_cb(method, args)
    assert(method == 'redraw', string.format('notification_cb: unexpected method (%s, args=%s)', method, inspect(args)))
...
function Screen:_handle_cmdline_show(content, pos, firstc, prompt, indent, level)
  ...
  self.cmdline[level] = { content = content, pos = pos, firstc = firstc, prompt = prompt, indent = indent }
end
...
function Screen:render(headers, attr_state, preview)
  ...
  for igrid, grid in pairs(self._grids) do
    if headers then
      local suffix = ''
      if igrid > 1 and self.win_position[igrid] == nil and self.float_pos[igrid] == nil and self.msg_grid ~= igrid then
        suffix = ' (hidden)'
      end
