...
describe('float window', function()
  ...
  it("win_execute() call commands that are not allowed when 'hidden' is not set" , function()
    ...
    eq(pcall_err(funcs.win_execute, win, 'close'), 'Vim(close):E37: No write since last change (add ! to ignore)')
    eq(pcall_err(funcs.win_execute, win, 'bdelete'), 'Vim(bdelete):E89: No write since last change for buffer 2 (add ! to ignore)')
    funcs.win_execute(win, 'bwipe!')
  end)
