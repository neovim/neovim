...
describe('float window', function()
  ...
  it('win_execute() should work', function()
    ...
    local win =
      meths.open_win(buf, false, { relative = 'win', width = 16, height = 1, row = 0, col = 10 })
  ...
  it("win_execute() call commands that are not allowed when 'hidden' is not set", function()
    ...
    local win =
      meths.open_win(buf, true, { relative = 'win', width = 16, height = 1, row = 0, col = 10 })
    eq(
      'Vim(close):E37: No write since last change (add ! to override)',
      pcall_err(funcs.win_execute, win, 'close')
    )
    eq(
  ...
  local function with_ext_multigrid(multigrid)
    ...
    it('return their configuration', function()
      ...
      if multigrid then
        ...
        eq(
          { external = true, focusable = true, width = 10, height = 1, relative = '' },
          meths.win_get_config(win)
        )
      end
    end)
    ...
    it('defaults to NormalFloat highlight and inherited options', function()
      ...
      local win =
        meths.open_win(0, false, { relative = 'editor', width = 20, height = 4, row = 4, col = 10 })
      if multigrid then
        screen:expect({
          grid = [[
        ## grid 1
    ...
    it('validates title title_pos', function()
      local buf = meths.create_buf(false, false)
      eq(
        'title requires border to be set',
    ...
    it('can be placed relative window or cursor', function()
      ...
      local win =
        meths.open_win(buf, false, { relative = 'win', width = 20, height = 2, row = 0, col = 10 })
      if multigrid then
        screen:expect({
          grid = [[
        ## grid 1
    ...
    it('can be placed relative text in a window', function()
      ...
      local win =
        meths.open_win(buf, false, { relative = 'win', width = 12, height = 1, bufpos = { 1, 32 } })
      if multigrid then
        screen:expect({
          grid = [[
        ## grid 1
    ...
    it('left drag changes visual selection in float window with winbar', function()
      local buf = meths.create_buf(false, false)
      meths.buf_set_lines(buf, 0, -1, true, { 'foo', 'bar', 'baz' })
      local float_win = meths.open_win(
        buf,
        false,
