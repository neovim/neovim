...
describe('063: Test for ":match", "matchadd()" and related functions', function()
  ...
  it('is working', function()
    ...
    eq({
      { group = 'MyGroup1', pattern = 'TODO', priority = 10, id = 4 },
    ...
    eq(
      'Vim(let):E801: ID already taken: 1',
      pcall_err(
        command,
    ...
    eq(
      'Vim(let):E474: List item 0 is either not a dictionary or an empty one',
      pcall_err(command, 'let rf2 = setmatches([0])')
    )
    eq(
      'Vim(let):E474: List item 0 is missing one of the required keys',
