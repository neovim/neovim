...
describe('API/extmarks', function()
  ...
  it('invalid marks at end of buffer are deleted', function()
    meths.buf_set_text(0, 0, 0, -1, -1, { 'a', 'b' })
    set_extmark(ns, 1, 0, 0, { sign_text = 'S1' })
    set_extmark(ns, 2, 1, 0, { sign_text = 'S2' })
    feed('Gdd')
    eq(true, get_extmark_by_id(ns, 2, { details = true })[3].invalid)
    command('silent undo')
    eq(nil, get_extmark_by_id(ns, 2, { details = true })[3].invalid)
    feed('ggdd')
    eq(nil, get_extmark_by_id(ns, 1, { details = true })[3].invalid)
    eq(nil, get_extmark_by_id(ns, 2, { details = true })[3].invalid)
    feed('dd')
    eq(true, get_extmark_by_id(ns, 1, { details = true })[3].invalid)
    eq(true, get_extmark_by_id(ns, 2, { details = true })[3].invalid)
  end)
end)
