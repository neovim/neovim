...
describe('api/buf', function()
  ...
  describe('nvim_buf_set_mark', function()
    ...
    it('#test allows setting window marks', function()
      api.nvim_buf_set_lines(0, -1, -1, true, { 'a', 'bit of', 'text', 'is always nice' })
      api.nvim_win_set_cursor(0, { 3, 1 })
      local firstWin = api.nvim_get_current_win()
      api.nvim_buf_set_mark(0, '`', 3, 1, { win = firstWin })
      feed('<C-w>v')
      local secondWin = api.nvim_get_current_win()
      t.neq(firstWin, secondWin)
      api.nvim_buf_set_mark(0, '`', 4, 2, { win = secondWin })
      api.nvim_set_current_win(firstWin)
      feed("``")
      eq({ 3, 1 }, api.nvim_win_get_cursor(0))
      api.nvim_set_current_win(secondWin)
      feed("``")
      eq({ 4, 2 }, api.nvim_win_get_cursor(0))
    end)
  end)
