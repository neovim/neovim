...
describe('api/buf', function()
  ...
  describe('nvim_buf_get_lines, nvim_buf_set_lines', function()
    ...
    it('fails correctly when input is not valid', function()
      ...
      eq([[String cannot contain newlines]], pcall_err(bufmeths.set_lines, 1, 1, 2, false, { 'b\na' }))
    end)

    it("fails if 'nomodifiable'", function()
      command('set nomodifiable')
      eq([[Buffer is not 'modifiable']], pcall_err(bufmeths.set_lines, 1, 1, 2, false, { 'a', 'b' }))
    end)
    ...
    it('set_lines on hidden buffer preserves "previous window" #9741', function()
      ...
      feed('<c-w>h')
      eq(2, funcs.winnr())
      meths.buf_set_lines(hiddenbuf, 0, -1, true, { 'hidden buffer line 1', 'line 2' })
      feed('<c-w>p')
      eq(3, funcs.winnr())
    end)
  end)
