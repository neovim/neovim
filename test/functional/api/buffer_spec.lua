...
describe('api/buf', function()
  ...
  describe("nvim_save_buf", function()
    it("save current buffer", function()
      local name = nvim("eval", "resolve(tempname())")
      curbuf("set_name", name)
      curbuf("set_lines", 0, -1, true, { "this is some text" })
      local b = nvim("get_current_buf")
      nvim("buf_save", b, {})
      local text = funcs.readfile(name)
      eq({ "this is some text" }, text)
    end)

    it("ignore readonly", function()
      local name = nvim("eval", "resolve(tempname())")
      curbuf("set_name", name)
      curbuf("set_lines", 0, -1, true, { "this is some text" })
      local b = nvim("get_current_buf")
      command("setlocal readonly")
      nvim("buf_save", b, { ignore_ro = true })
