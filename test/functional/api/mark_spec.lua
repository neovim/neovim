...
describe('api/mark', function()
  ...
  describe('nvim_mark_set', function()
    ...
    it('sets window-local marks in current window', function()
      ...
      eq({ line = 2, col = 5, name = "'" }, api.nvim_mark_get("'", {}))
    end)
    ...
    it('sets window-local marks in specified window', function()
      ...
      eq({ line = 3, col = 1, name = "'" }, api.nvim_mark_get("'", { win = win }))
    end)

    it('deletes window-local marks with row=0', function()
      command("mark '")
      eq(true, api.nvim_mark_set("'", 0, 0, {}))
      eq({ line = 0, col = 0, name = "'" }, api.nvim_mark_get("'", {}))
    end)
    ...
    it('sets buffer-local marks', function()
      ...
      eq({ line = 2, col = 4, name = 'a' }, api.nvim_mark_get('a', {}))
    end)
    ...
    it('sets buffer-local marks in specified buffer', function()
      ...
      eq({ line = 2, col = 1, name = 'z' }, api.nvim_mark_get('z', { buf = buf }))
    end)

    it('sets special buffer marks', function()
      eq(true, api.nvim_mark_set('"', 3, 0, {}))
      eq({ line = 3, col = 0, name = '"' }, api.nvim_mark_get('"', {}))

      eq(true, api.nvim_mark_set('[', 1, 0, {}))
      eq({ line = 1, col = 0, name = '[' }, api.nvim_mark_get('[', {}))

      eq(true, api.nvim_mark_set(']', 2, 0, {}))
      eq({ line = 2, col = 0, name = ']' }, api.nvim_mark_get(']', {}))
    end)

    it('sets visual marks', function()
      eq(true, api.nvim_mark_set('<', 1, 2, {}))
      eq(true, api.nvim_mark_set('>', 3, 4, {}))
      eq({ line = 1, col = 2, name = '<' }, api.nvim_mark_get('<', {}))
      eq({ line = 3, col = 4, name = '>' }, api.nvim_mark_get('>', {}))
    end)
    ...
    it('fails when using win option for non-window-local marks', function()
      eq(
        "cannot use 'win' for non-window-local marks",
        pcall_err(api.nvim_mark_set, 'A', 1, 0, { win = 0 })
      )
    end)
    ...
    it('validates line/col ranges for window-local marks', function()
      matches("Invalid 'line': out of range", pcall_err(api.nvim_mark_set, "'", 999, 0, {}))
      matches("Invalid 'line': out of range", pcall_err(api.nvim_mark_set, "'", -1, 0, {}))
      -- line=0 is legal (clears the mark)
      eq(true, api.nvim_mark_set("'", 0, 0, {}))
    end)
  end)
  ...
  describe('nvim_mark_get', function()
    it('gets window-local marks from current window', function()
      ...
      eq({ line = 2, col = 3, name = "'" }, api.nvim_mark_get("'", {}))
    end)
    ...
    it('gets window-local marks from specified window', function()
      ...
      eq({ line = 2, col = 3, name = "'" }, api.nvim_mark_get("'", { win = win }))
    end)
    ...
    it('returns empty array for unset marks', function()
      eq({ line = 0, col = 0, name = 'z' }, api.nvim_mark_get('z', {}))
    end)
    ...
    it('returns "timestamp"', function()
      ...
      api.nvim_mark_set('a', 2, 3, {})
      local after = os.time()

      local mark = api.nvim_mark_get('a', {})
      eq(nil, mark.timestamp)
      mark = api.nvim_mark_get('a', { timestamp = true })
      eq(2, mark.line)
      eq(3, mark.col)
      eq(true, mark.timestamp >= before and mark.timestamp <= after)

      before = os.time()
      api.nvim_mark_set('A', 2, 3, {})
      mark = api.nvim_mark_get('A', { timestamp = true })
      eq(true, mark.timestamp >= before)

      local buf = api.nvim_create_buf(false, true)
      api.nvim_buf_set_lines(buf, 0, -1, true, { 'line1', 'line2' })
      before = os.time()
      api.nvim_mark_set('B', 1, 0, { buf = buf })
      mark = api.nvim_mark_get('B', { buf = buf, timestamp = true })
      eq(1, mark.line)
      eq(0, mark.col)
      eq(true, mark.timestamp >= before)
      ...
      before = os.time()
      api.nvim_mark_set('z', 2, 1, { buf = buf })
      mark = api.nvim_mark_get('z', { buf = buf, timestamp = true })
      eq(2, mark.line)
      eq(1, mark.col)
      eq(true, mark.timestamp >= before)

      api.nvim_mark_set("'", 2, 3, {})
      mark = api.nvim_mark_get("'", { timestamp = true })
      eq(0, mark.timestamp)
    end)
  end)
  ...
  describe('nvim_mark_del', function()
    ...
    it('deletes global marks', function()
      ...
      eq({ line = 0, col = 0, buf = 0, file = '', name = 'A' }, api.nvim_mark_get('A', {}))
    end)
    ...
    it('deletes numbered global marks', function()
      ...
      eq({ line = 0, col = 0, buf = 0, file = '', name = '0' }, api.nvim_mark_get('0', {}))
    end)
    ...
    it('deletes buffer-local marks', function()
      ...
      command('2mark a')
      eq(true, api.nvim_mark_del('a', {}))
      eq({ line = 0, col = 0, name = 'a' }, api.nvim_mark_get('a', {}))
    end)
    ...
    it('deletes visual marks', function()
      ...
      eq(true, api.nvim_mark_del('>', {}))
      eq({ line = 0, col = 0, name = '<' }, api.nvim_mark_get('<', {}))
      eq({ line = 0, col = n.eval('v:maxcol'), name = '>' }, api.nvim_mark_get('>', {}))
    end)
