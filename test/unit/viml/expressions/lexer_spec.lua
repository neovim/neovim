...
local function eltkn2lua(pstate, tkn)
  ...
  elseif ret.type == 'Number' then
    ...
    ret.data.val = tonumber(tkn.data.num.is_float and tkn.data.num.val.floating or tkn.data.num.val.integer)
...
describe('Expressions lexer', function()
  ...
  local function singl_eltkn_test(typ, str, data)
    ...
    eq({ data = data, len = #str, start = { col = 0, line = 0 }, str = str, type = typ }, next_eltkn(pstate, flags))
    ...
    pstate.pos.col = 1
    eq({ data = data, len = #str, start = { col = 1, line = 0 }, str = str, type = typ }, next_eltkn(pstate, flags))
  ...
  local function stable_tests()
    ...
    simple_test(
      ...
      'Number',
      18,
      { data = { is_float = false, base = 16, val = 81985529216486895 }, str = '0x0123456789ABCDEF' }
    )
    simple_test(
      { { data = '00', size = 2 } },
      'Number',
