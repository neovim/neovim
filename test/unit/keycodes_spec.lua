...
describe('keycodes.c', function()
  describe('find_special_key()', function()
    ...
    itp('case-insensitive', function()
      ...
      srcp[0] = '<C-A>'
      local all_caps_key = keymap.find_special_key(srcp, 5, modp, 0, NULL)
      local all_caps_mod = modp[0]

      srcp[0] = '<C-a>'
      eq(all_caps_key, keymap.find_special_key(srcp, 5, modp, 0, NULL))
    ...
    itp('double-quote in keycode #7411', function()
      ...
      srcp[0] = '<C-">'
      eq(string.byte('"'), keymap.find_special_key(srcp, 5, modp, 0, NULL))
      ...
      eq(string.byte('"'), keymap.find_special_key(srcp, 6, modp, keymap.FSK_IN_STRING, NULL))
    end)
  end)
end)
