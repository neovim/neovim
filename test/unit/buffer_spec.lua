...
describe('buffer functions', function()
  ...
  describe('build_stl_str_hl', function()
    ...
    local function build_stl_str_hl(arg)
      ...
      return buffer.build_stl_str_hl(
        globals.curwin,
        output_buffer,
        buffer_byte_size,
        to_cstr(pat),
    ...
    -- fillchar testing
    statusline_test('should handle `!` as a fillchar', 10, 'abcde%=', 'abcde!!!!!', { fillchar = ('!'):byte() })
    statusline_test('should handle `~` as a fillchar', 10, '%=abcde', '~~~~~abcde', { fillchar = ('~'):byte() })
    statusline_test(
    ...
    statusline_test('should handle zero-fillchar as a space', 10, 'abcde%=', 'abcde     ', { fillchar = 0 })
    statusline_test('should handle multibyte-fillchar as a dash', 10, 'abcde%=', 'abcde-----', { fillchar = 0x80 })
    statusline_test(
    ...
    statusline_test('should copy plain text', 80, 'this is a test', 'this is a test', { expected_cell_count = 14 })

    -- line number testing
    statusline_test('should print the buffer number', 80, '%n', '1', { expected_cell_count = 1 })
    statusline_test('should print the current line number in the buffer', 80, '%l', '0', { expected_cell_count = 1 })
    statusline_test('should print the number of lines in the buffer', 80, '%L', '1', { expected_cell_count = 1 })

    -- truncation testing
    statusline_test('should truncate when standard text pattern is too long', 10, '0123456789abcde', '<6789abcde')
    statusline_test('should truncate when using =', 10, 'abcdef%=ghijkl', 'abcdef<jkl')
    statusline_test('should truncate centered text when using ==', 10, 'abcde%=gone%=fghij', 'abcde<ghij')
    statusline_test('should respect the `<` marker', 10, 'abc%<defghijkl', 'abc<ghijkl')
    statusline_test('should truncate at `<` with one `=`, test 1', 10, 'abc%<def%=ghijklmno', 'abc<jklmno')
    statusline_test('should truncate at `<` with one `=`, test 2', 10, 'abcdef%=ghijkl%<mno', 'abcdefghi>')
    statusline_test('should truncate at `<` with one `=`, test 3', 10, 'abc%<def%=ghijklmno', 'abc<jklmno')
    statusline_test('should truncate at `<` with one `=`, test 4', 10, 'abc%<def%=ghij', 'abcdefghij')
    statusline_test('should truncate at `<` with one `=`, test 4', 10, 'abc%<def%=ghijk', 'abc<fghijk')
    ...
    -- alignment testing
    statusline_test('should right align when using =', 20, 'neo%=vim', 'neo              vim')
    statusline_test(
    ...
    statusline_test('should handle an empty left side when using ==', 20, '%=test%=baz', '      test       baz')
    statusline_test('should handle an empty right side when using ==', 20, 'foobar%=test%=', 'foobar     test     ')
    statusline_test('should handle consecutive empty ==', 20, '%=%=test%=', '          test      ')
    statusline_test('should handle an = alone', 20, '%=', '                    ')
    statusline_test('should right align text when it is alone with =', 20, '%=foo', '                 foo')
    statusline_test('should left align text when it is alone with =', 20, 'foo%=', 'foo                 ')
    ...
    statusline_test('should handle an empty left side when using ==', 21, '%=test%=baz', '       test       baz')
    statusline_test('should handle an empty right side when using ==', 21, 'foobar%=test%=', 'foobar     test      ')
    ...
    -- multi-byte testing
    statusline_test('should handle multibyte characters', 10, 'Ĉ%=x', 'Ĉ        x', { expected_byte_length = 11 })
    statusline_test(
