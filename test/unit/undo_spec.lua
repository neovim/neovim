...
describe('u_write_undo', function()
  ...
  before_each(function()
    ...
    buffer = buffer.buflist_new(c_file, c_file, 1, buffer.BLN_LISTED)
    buffer.b_u_numhead = 1 -- Pretend that the buffer has been changed
  ...
  it('writes an undo file to undodir given a buffer and hash', function()
    u_write_undo(nil, false, buffer, hash)
    correct_name = ffi.string(undo.u_get_undo_file_name(buffer.b_ffname, false))
