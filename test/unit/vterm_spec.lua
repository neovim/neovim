...
describe('vterm', function()
  ...
  itp('25state_input', function()
    ...
    inkey('up', vt)
    expect('output 1b,5b,41')
    inkey('up', vt, { S = true })
    expect('output 1b,5b,31,3b,32,41')
    inkey('up', vt, { C = true })
    expect('output 1b,5b,31,3b,35,41')
    inkey('up', vt, { S = true, C = true })
    expect('output 1b,5b,31,3b,36,41')
    inkey('up', vt, { A = true })
    expect('output 1b,5b,31,3b,33,41')
    inkey('up', vt, { S = true, A = true })
    expect('output 1b,5b,31,3b,34,41')
    inkey('up', vt, { C = true, A = true })
    expect('output 1b,5b,31,3b,37,41')
    inkey('up', vt, { S = true, C = true, A = true })
    expect('output 1b,5b,31,3b,38,41')

    -- Cursor keys in application mode
    push('\x1b[?1h', vt)
    -- Plain "Up" should be SS3 A now
    inkey('up', vt)
    expect('output 1b,4f,41')
    ...
    expect('output 9')
    inkey('tab', vt, { S = true })
    expect('output 1b,5b,5a')
    inkey('tab', vt, { C = true })
    expect('output 1b,5b,39,3b,35,75')
    inkey('tab', vt, { A = true })
    expect('output 1b,9')
    inkey('tab', vt, { C = true, A = true })
    expect('output 1b,5b,39,3b,37,75')
    ...
    inkey('f1', vt)
    expect('output 1b,4f,50')

    -- TODO(dundargoc): fix
    -- Modified F1 is CSI P
    inkey('f1', vt, { S = true })
    expect('output 1b,5b,31,3b,32,50')
    inkey('f1', vt, { A = true })
    expect('output 1b,5b,31,3b,33,50')
    inkey('f1', vt, { C = true })
    expect('output 1b,5b,31,3b,35,50')
    ...
    push('\x1b[?2004h', vt)
    vterm.vterm_keyboard_start_paste(vt)
    expect('output 1b,5b,32,30,30,7e')
    vterm.vterm_keyboard_end_paste(vt)
    expect('output 1b,5b,32,30,31,7e')
    ...
    vterm.vterm_state_focus_in(state)
    expect('output 1b,5b,49')
    vterm.vterm_state_focus_out(state)
    expect('output 1b,5b,4f')
  end)
  ...
  itp('26state_query', function()
    ...
    reset(state, nil)
    push('\x1b[c', vt)
    expect('output 1b,5b,3f,31,3b,32,63')

    -- XTVERSION
    reset(state, nil)
    push('\x1b[>q', vt)
    expect('output 1b,50,3e,7c,6c,69,62,76,74,65,72,6d,28,30,2e,33,29,1b,5c')

    -- DSR
    reset(state, nil)
    push('\x1b[5n', vt)
    expect('output 1b,5b,30,6e')

    -- CPR
    push('\x1b[6n', vt)
    expect('output 1b,5b,31,3b,31,52')
    push('\x1b[10;10H\x1b[6n', vt)
    expect('output 1b,5b,31,30,3b,31,30,52')

    -- DECCPR
    push('\x1b[?6n', vt)
    expect('output 1b,5b,3f,31,30,3b,31,30,52')

    -- DECRQSS on DECSCUSR
    push('\x1b[3 q', vt)
    push('\x1bP$q q\x1b\\', vt)
    expect('output 1b,50,31,24,72,33,20,71,1b,5c')

    -- DECRQSS on SGR
    push('\x1b[1;5;7m', vt)
    push('\x1bP$qm\x1b\\', vt)
    expect('output 1b,50,31,24,72,31,3b,35,3b,37,6d,1b,5c')

    -- DECRQSS on SGR ANSI colours
    push('\x1b[0;31;42m', vt)
    push('\x1bP$qm\x1b\\', vt)
    expect('output 1b,50,31,24,72,33,31,3b,34,32,6d,1b,5c')

    -- DECRQSS on SGR ANSI hi-bright colours
    push('\x1b[0;93;104m', vt)
    push('\x1bP$qm\x1b\\', vt)
    expect('output 1b,50,31,24,72,39,33,3b,31,30,34,6d,1b,5c')

    -- DECRQSS on SGR 256-palette colours
    push('\x1b[0;38:5:56;48:5:78m', vt)
    push('\x1bP$qm\x1b\\', vt)
    expect('output 1b,50,31,24,72,33,38,3a,35,3a,35,36,3b,34,38,3a,35,3a,37,38,6d,1b,5c')

    -- DECRQSS on SGR RGB8 colours
    push('\x1b[0;38:2:24:68:112;48:2:13:57:101m', vt)
    push('\x1bP$qm\x1b\\', vt)
    expect(
      'output 1b,50,31,24,72,33,38,3a,32,3a,32,34,3a,36,38,3a,31,31,32,3b,34,38,3a,32,3a,31,33,3a,35,37,3a,31,30,31,6d,1b,5c'
    )

    -- S8C1T on DSR
    push('\x1b G', vt)
    push('\x1b[5n', vt)
    expect('output 9b,30,6e')
    push('\x1b F', vt)
  end)
