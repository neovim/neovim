...
module.popen_r = function(...)
  return io.popen(module.argss_to_cmd(...), 'r')
end

module.popen_w = function(...)
...
module.eq = function(expected, actual, context)
  return assert.are.same(expected, actual, context)
end
module.neq = function(expected, actual, context)
  return assert.are_not.same(expected, actual, context)
end
module.ok = function(res)
  return assert.is_true(res)
end
module.near = function(actual, expected, tolerance)
  return assert.is.near(actual, expected, tolerance)
end
module.matches = function(pat, actual)
...
module.expect_err = function(pat, ...)
  local fn = select(1, ...)
  local fn_args = {...}
...
module.glob = function(initial_path, re, exc_re)
...
module.filter = function(filter_func, tab)
  local rettab = {}
...
module.check_cores = function(app, force)
...
module.repeated_read_cmd = function(...)
  for _ = 1, 10 do
...
module.shallowcopy = function(orig)
...
module.mergedicts_copy = function(d1, d2)
...
module.dictdiff = function(d1, d2)
  local ret = {}
  local hasdiff = false
...
module.dedent = function(str, leave_indent)
...
module.format_luav = function(v, indent, opts)
  opts = opts or {}
  local linesep = '\n'
...
module.format_string = function(fmt, ...)
  local i = 0
  local args = {...}
...
module.intchar2lua = function(ch)
  ch = tonumber(ch)
...
module.fixtbl = function(tbl)
  return setmetatable(tbl, fixtbl_metatable)
end

module.fixtbl_rec = function(tbl)
...
module.read_file_list = function(filename, start)
...
--
-- filename: path to file
module.read_file = function(filename)
  local file = io.open(filename, 'r')
  if not file then
    return nil
  end
...
module.write_file = function(name, text, no_dedent, append)
