...
function helper.popen_r(...)
  return io.popen(helper.argss_to_cmd(...), 'r')
end

function helper.popen_w(...)
  return io.popen(helper.argss_to_cmd(...), 'w')
end
...
function helper.eq(expected, actual, context)
  return assert.are.same(expected, actual, context)
end
function helper.neq(expected, actual, context)
  return assert.are_not.same(expected, actual, context)
end
function helper.ok(res, msg)
  return assert.is_true(res, msg)
end
function helper.near(actual, expected, tolerance)
  return assert.is.near(actual, expected, tolerance)
end
function helper.matches(pat, actual)
...
function helper.glob(initial_path, re, exc_re)
...
helper.uname = (function()
  ...
  return (function()
    ...
    local status, f = pcall(helper.popen_r, 'uname', '-s')
    if status then
      platform = string.lower(f:read("*l"))
      f:close()
    elseif helper.iswin() then
      platform = 'windows'
    else
...
function helper.is_os(s)
  if not (s == 'win' or s == 'mac' or s == 'unix') then
    error('unknown platform: '..tostring(s))
  end
  return ((s == 'win' and helper.iswin())
    or (s == 'mac' and helper.uname() == 'darwin')
    or (s == 'unix'))
end
...
helper.tmpname = (function()
  ...
  return (function()
    if tmpdir_is_local(tmpdir) then
      ...
      if helper.uname() == 'windows' and fname:sub(1, 2) == '\\s' then
        -- In Windows tmpname() returns a filename starting with
        -- special sequence \s, prepend $TEMP path
        return tmpdir..fname
      elseif fname:match('^/tmp') and helper.uname() == 'darwin' then
...
function helper.filter(filter_func, tab)
  local rettab = {}
...
function helper.check_cores(app, force)
  ...
  local local_tmpdir = (tmpdir_is_local(tmpdir_get())
    ...
    or nil)
  local db_cmd
  if helper.hasenv('NVIM_TEST_CORE_GLOB_DIRECTORY') then
  ...
  local cores = helper.glob(initial_path, re, exc_re)
  local found_cores = 0
  local out = io.stdout
...
function helper.which(exe)
  local pipe = helper.popen_r('which', exe)
...
function helper.repeated_read_cmd(...)
  for _ = 1, 10 do
    local stream = helper.popen_r(...)
  ...
  print('ERROR: Failed to execute ' .. helper.argss_to_cmd(...) .. ': nil return after 10 attempts')
  return nil
end

function helper.shallowcopy(orig)
...
function helper.mergedicts_copy(d1, d2)
  local ret = helper.shallowcopy(d1)
  for k, v in pairs(d2) do
    if d2[k] == helper.REMOVE_THIS then
      ret[k] = nil
    elseif type(d1[k]) == 'table' and type(v) == 'table' then
      ret[k] = helper.mergedicts_copy(d1[k], v)
    else
      ret[k] = v
    end
  end
...
function helper.dictdiff(d1, d2)
  local ret = {}
  local hasdiff = false
  for k, v in pairs(d1) do
    if d2[k] == nil then
      hasdiff = true
      ret[k] = helper.REMOVE_THIS
    elseif type(v) == type(d2[k]) then
      if type(v) == 'table' then
        local subdiff = helper.dictdiff(v, d2[k])
        if subdiff ~= nil then
          hasdiff = true
          ret[k] = subdiff
        end
  ...
  local shallowcopy = helper.shallowcopy
  for k, v in pairs(d2) do
    if d1[k] == nil then
      ret[k] = shallowcopy(v)
      hasdiff = true
...
function helper.dedent(str, leave_indent)
...
-- Commit: 520c0b91a528
function helper.format_luav(v, indent, opts)
  opts = opts or {}
  local linesep = '\n'
  ...
  elseif type(v) == 'table' then
    if v == helper.REMOVE_THIS then
      ret = 'REMOVE_THIS'
    else
      local processed_keys = {}
      ret = '{' .. linesep
      local non_empty = false
      local format_luav = helper.format_luav
...
--
-- Commit: 520c0b91a528
function helper.format_string(fmt, ...)
  local i = 0
  local args = {...}
  ...
  local ret = fmt:gsub('%%[0-9*]*%.?[0-9*]*[cdEefgGiouXxqsr%%]', function(match)
    ...
    if subfmt:sub(-1) == 'r' or subfmt:sub(-1) == 'q' then
      ...
      -- lua leaves as-is.
      arg = helper.format_luav(arg, nil, {dquote_strings = (subfmt:sub(-1) == 'q')})
...
function helper.intchar2lua(ch)
  ch = tonumber(ch)
...
function helper.fixtbl(tbl)
  return setmetatable(tbl, fixtbl_metatable)
end

function helper.fixtbl_rec(tbl)
  local fixtbl_rec = helper.fixtbl_rec
  for _, v in pairs(tbl) do
    if type(v) == 'table' then
      fixtbl_rec(v)
    end
  end
  return helper.fixtbl(tbl)
end

function helper.hexdump(str)
...
function helper.read_file_list(filename, start)
...
--
-- filename: path to file
function helper.read_file(filename)
  local file = io.open(filename, 'r')
  if not file then
    return nil
  end
...
function helper.write_file(name, text, no_dedent, append)
...
function helper.read_nvim_log()
  local logfile = os.getenv('NVIM_LOG_FILE') or '.nvimlog'
  local is_ci = helper.isCI()
  local keep = is_ci and 999 or 10
  local lines = helper.read_file_list(logfile, -keep) or {}
  local log = (('-'):rep(78)..'\n'
...
helper = shared.tbl_extend('error', helper, Paths, shared)
