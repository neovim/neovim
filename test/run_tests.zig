...
pub fn testStep(b: *std.Build, kind: []const u8, nvim_bin: *std.Build.Step.Compile, lua_deps: LazyPath, config_dir: LazyPath) !*std.Build.Step.Run {
...
pub fn test_steps(b: *std.Build, nvim_bin: *std.Build.Step.Compile, depend_on: *std.Build.Step, lua_deps: LazyPath, config_dir: LazyPath, support_unittests: bool) !void {
    ...
    const functional_tests = try testStep(b, "functional", nvim_bin, lua_deps, config_dir);
    ...
    if (support_unittests) {
        const unit_tests = try testStep(b, "unit", nvim_bin, lua_deps, config_dir);
