...
local process_documentation = function(documentation)
...
---@param generate_methods boolean
local function gen_protocol_constants(protocol, generate_methods)
  ---@param output string[]
  ...
  for _, enum in ipairs(protocol.enumerations) do
    if enum.documentation then
      local document =
        vim.split(process_documentation(enum.documentation), '\n?\n', { trimempty = true })
      vim.list_extend(output, document)
    end
    output[#output + 1] = '---@nodoc'
    output[#output + 1] = '---@enum lsp.' .. enum.name
    output[#output + 1] = 'protocol.' .. enum.name .. ' = {'
    for _, value in ipairs(enum.values) do
      output[#output + 1] = indent
        ...
        .. ','
      -- Add inverse key-value pair for reverse lookup.
      if type(value.value) == 'number' then
        output[#output + 1] = indent .. '[' .. value.value .. "] = '" .. value.name .. "',"
      end
    end
    output[#output + 1] = '}'
    output[#output + 1] = ''
...
function M.gen(opt)
  ...
  gen_protocol_constants(protocol, opt.methods)
