...
target_gen='src/nvim/tui/terminfo_defs.h'
target_enum='src/nvim/tui/terminfo_enum.h'
...
wanted_numbers = {"max_colors", "lines", "columns"}
wanted_strings = {
  "bell",
  "carriage_return",
...
db = "/tmp/nvim_terminfo"
if false then
  function sys(cmd) print(cmd) os.execute(cmd) end
  sys("curl -O "..url)
...
function enumify(str) return "kTerm_"..str end
function quote(str)
...
f_enum = io.open(target_enum, 'wb')
f_enum:write("// autogenenerated by src/gen/gen_terminfo.lua\n")
...
f_enum:write("} NeoTerminfoNames;\n")
f_enum:close()

f_defs = io.open(target_gen, 'wb')
...
for _, entry in ipairs(entries) do
  local name, target = unpack(entry)
  local fil = io.popen("infocmp -L -x -1 -A "..db.." "..name):read'*a'
  ...
  for i,line in ipairs(lines) do
    boolmatch = string.match(line, boolpat)
    nummatch, numval = string.match(line, numpat)
    strmatch, strval = string.match(line, strpat)
    if boolmatch then
      dbg("boolean: ".. boolmatch)
      if boolmatch == "back_color_erase" then
        dbg("21 MEGA-JACKPOTS!")
      end
  ...
  f_defs:write("\nstatic const NeoTerminfo NEW__"..target.." = {\n")
