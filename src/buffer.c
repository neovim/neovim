 ...
 */
void close_buffer(win_T *win,   /* if not NULL, set b_last_cursor */
        buf_T *buf, int action, int abort_if_last UNUSED)
{
  int is_curbuf;
  int nwindows;
 ...
 */
static void free_buffer_stuff(buf_T *buf, int free_options /* free options as well */)
{
  if (free_options) {
 ...
 *	OK otherwise.
 */
int setfname(buf_T *buf, char_u *ffname, char_u *sfname, int message /* give message when buffer already exists */)
{
  buf_T       *obuf = NULL;
#ifdef UNIX
  struct stat st;
 ...
 */
char_u  * getaltfname(int errmsg /* give error message */)
{
  char_u      *fname;
  linenr_T dummy;
 ...
 */
void fileinfo(int fullname /* when non-zero print full path */, int shorthelp, int dont_truncate)
{
  char_u      *name;
  int n;
  char_u      *p;
 ...
 * marks.
 */
void wipe_buffer(buf_T *buf, int aucmd UNUSED /* When TRUE trigger autocommands. */)
{
