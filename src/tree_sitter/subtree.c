                                  ...
                                  const ExternalScannerState *b)
{
  return a == b
         || (a->length == b->length
             && !memcmp(ts_external_scanner_state_data(a),
                        ts_external_scanner_state_data(b), a->length));
}
...
static inline bool ts_subtree_can_inline(Length padding,
                                         Length size,
                                         uint32_t lookahead_bytes)
{
  return padding.bytes < TS_MAX_INLINE_TREE_LENGTH && padding.extent.row < 16
...
{
  Subtree result
      = ts_subtree_new_leaf(pool, ts_builtin_sym_error, padding, size,
                            bytes_scanned, parse_state, false, false, language);
...
MutableSubtree ts_subtree_make_mut(SubtreePool *pool, Subtree self)
{
  ...
  if (result->child_count > 0) {
    ...
    memcpy(result->children, self.ptr->children,
           result->child_count * sizeof(Subtree));
                                 ...
                                 MutableSubtreeArray *stack)
{
  ...
  for (unsigned i = 0; i < count; i++) {
    ...
    MutableSubtree grandchild
        = ts_subtree_to_mut_unsafe(child.ptr->children[0]);
    ...
    child.ptr->children[0]
        = grandchild.ptr->children[grandchild.ptr->child_count - 1];
    grandchild.ptr->children[grandchild.ptr->child_count - 1]
  ...
  while (stack->size > initial_stack_size) {
    ...
    MutableSubtree grandchild = ts_subtree_to_mut_unsafe(
        child.ptr->children[child.ptr->child_count - 1]);
    ts_subtree_set_children(grandchild, grandchild.ptr->children,
                            grandchild.ptr->child_count, language);
    ts_subtree_set_children(child, child.ptr->children, child.ptr->child_count,
                            language);
    ts_subtree_set_children(tree, tree.ptr->children, tree.ptr->child_count,
                            language);
  }
}
                        ...
                        const TSLanguage *language)
{
  ...
  while (pool->tree_stack.size > 0) {
    ...
    if (tree.ptr->repeat_depth > 0) {
      ...
      if (ts_subtree_child_count(child1) > 0
...
void ts_subtree_set_children(MutableSubtree self,
                             Subtree *children,
                             ...
                             const TSLanguage *language)
{
  ...
  for (uint32_t i = 0; i < self.ptr->child_count; i++) {
    ...
    if (alias_sequence && alias_sequence[non_extra_index] != 0
        && !ts_subtree_extra(child)) {
      self.ptr->visible_child_count++;
      if (ts_language_symbol_metadata(language, alias_sequence[non_extra_index])
              .named) {
  ...
  if (self.ptr->symbol == ts_builtin_sym_error
    ...
    for (uint32_t i = 0; i < self.ptr->child_count; i++) {
      ...
      if (ts_subtree_visible(child)) {
        ...
        self.ptr->error_cost
            += ERROR_COST_PER_SKIPPED_TREE * child.ptr->visible_child_count;
      }
    }
  }
...
MutableSubtree ts_subtree_new_node(SubtreePool *pool,
                                   TSSymbol symbol,
...
Subtree ts_subtree_new_error_node(SubtreePool *pool,
                                  SubtreeArray *children,
                                  bool extra,
                                  const TSLanguage *language)
{
  MutableSubtree result
      = ts_subtree_new_node(pool, ts_builtin_sym_error, children, 0, language);
...
Subtree ts_subtree_new_missing_leaf(SubtreePool *pool,
                                    ...
                                    const TSLanguage *language)
{
  Subtree result = ts_subtree_new_leaf(pool, symbol, padding, length_zero(), 0,
                        ...
                        SubtreePool *pool)
{
  ...
  while (stack.size) {
    ...
    // If the edit is entirely within the space before this subtree, then shift
    // this subtree over according to the edit without changing its size.
    if (edit.old_end.bytes <= padding.bytes) {
      padding = length_add(edit.new_end, length_sub(padding, edit.old_end));
    }

    // If the edit starts in the space before this subtree and extends into this
    // subtree, shrink the subtree's content to compensate for the change in the
    // space before it.
    ...
    *entry.tree = ts_subtree_from_mut(result);
    ...
    for (uint32_t i = 0, n = ts_subtree_child_count(*entry.tree); i < n; i++) {
      ...
      // If this child starts after the edit, then we're done processing
      // children.
      if (child_left.bytes > edit.old_end.bytes
          || (child_left.bytes == edit.old_end.bytes && child_size.bytes > 0
              && i > 0))
        break;
      ...
      // touches the edit. Subsequent children are only never have any text
      // inserted into them; they are only shrunk to compensate for the edit.
                                          ...
                                          const char *field_name)
{
  ...
  bool is_visible
      = include_all || ts_subtree_missing(self)
        || (alias_symbol ? alias_is_named
  ...
  if (ts_subtree_child_count(self)) {
    ...
    for (uint32_t i = 0; i < self.ptr->child_count; i++) {
      Subtree child = self.ptr->children[i];
      if (ts_subtree_extra(child)) {
        cursor += ts_subtree__write_to_string(child, *writer, limit, language,
                                              include_all, 0, false, NULL);
      } else {
        TSSymbol alias_symbol
        ...
        cursor += ts_subtree__write_to_string(child, *writer, limit, language,
                                              ...
                                              alias_is_named, child_field_name);
                        ...
                        bool include_all)
{
  ...
  size_t size = ts_subtree__write_to_string(self, scratch_string, 0, language,
                                            include_all, 0, false, ROOT_FIELD)
                + 1;
  char *result = malloc(size * sizeof(char));
  ts_subtree__write_to_string(self, result, size, language, include_all, 0,
                              false, ROOT_FIELD);
  return result;
}
                                 ...
                                 FILE *f)
{
  ...
  fprintf(f,
          ...
          "repeat-depth: %u\n"
          "lookahead-bytes: %u",
          start_offset, end_offset, ts_subtree_parse_state(*self),
          ts_subtree_error_cost(*self), ts_subtree_has_changes(*self),
          ts_subtree_repeat_depth(*self), ts_subtree_lookahead_bytes(*self));
...
bool ts_subtree_external_scanner_state_eq(Subtree self, Subtree other)
{
  ...
  if (other.ptr && ts_subtree_has_external_tokens(other)
      && !other.ptr->child_count) {
