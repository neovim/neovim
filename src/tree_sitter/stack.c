...
{
  StackNode *node
      = pool->size > 0 ? array_pop(pool) : ts_malloc(sizeof(StackNode));
...
static bool stack__subtree_is_equivalent(Subtree left, Subtree right)
{
  return left.ptr == right.ptr
         || (left.ptr && right.ptr
             ...
             && ((ts_subtree_error_cost(left) > 0
                  && ts_subtree_error_cost(right) > 0)
                                ...
                                SubtreePool *subtree_pool)
{
  ...
  for (int i = 0; i < self->link_count; i++) {
    ...
    if (stack__subtree_is_equivalent(existing_link->subtree, link.subtree)) {
      // In general, we preserve ambiguities until they are removed from the
      // stack during a pop operation where multiple paths lead to the same
      // node. But in the special case where two links directly connect the same
      // pair of nodes, we can safely remove the ambiguity ahead of time without
      // changing behavior.
...
inline StackSliceArray stack__iter(Stack *self,
...
inline StackAction iterate_callback(void *payload,
                                    const StackIterator *iterator)
{
  StackIterateSession *session = payload;
  session->callback(session->payload, iterator->node->state,
...
inline StackAction pop_count_callback(void *payload,
                                      const StackIterator *iterator)
{
...
inline StackAction pop_pending_callback(void *payload,
                                        const StackIterator *iterator)
{
...
inline StackAction pop_error_callback(void *payload,
                                      const StackIterator *iterator)
{
...
inline StackAction pop_all_callback(void *payload,
                                    const StackIterator *iterator)
{
...
inline StackAction summarize_stack_callback(void *payload,
                                            const StackIterator *iterator)
{
                             ...
                             unsigned max_depth)
{
  SummarizeStackSession session
      = {.summary = ts_malloc(sizeof(StackSummary)), .max_depth = max_depth};
...
bool ts_stack_print_dot_graph(Stack *self, const TSLanguage *language, FILE *f)
{
  ...
  for (uint32_t i = 0; i < self->heads.size; i++) {
    ...
    fprintf(f,
            "label=%u, fontcolor=blue, weight=10000, "
            "labeltooltip=\"node_count: %u\nerror_cost: %u",
            i, ts_stack_node_count_since_error(self, i),
            ts_stack_error_cost(self, i));

    if (head->last_external_token.ptr) {
      const ExternalScannerState *state
          = &head->last_external_token.ptr->external_scanner_state;
  ...
  while (!all_iterators_done) {
    ...
    for (uint32_t i = 0; i < self->iterators.size; i++) {
      ...
      fprintf(f,
              ...
              node->position.extent.row + 1, node->position.extent.column,
              node->node_count, node->error_cost, node->dynamic_precedence);
      ...
      for (int j = 0; j < node->link_count; j++) {
        ...
        if (!link.subtree.ptr) {
          ...
          fprintf(f, "label=\"");
          bool quoted = ts_subtree_visible(link.subtree)
                        && !ts_subtree_named(link.subtree);
          if (quoted)
            fprintf(f, "'");
          const char *name = ts_language_symbol_name(
              language, ts_subtree_symbol(link.subtree));
