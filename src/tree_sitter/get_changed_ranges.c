...
static bool iterator_tree_is_visible(const Iterator *self)
{
  ...
  if (self->cursor.stack.size > 1) {
    ...
    const TSSymbol *alias_sequence
        = ts_language_alias_sequence(self->language, parent.ptr->production_id);
  ...
  for (; i + 1 > 0; i--) {
    ...
    if (i > 0) {
      ...
      const TSSymbol *alias_sequence = ts_language_alias_sequence(
          self->language, parent->ptr->production_id);
      if (alias_sequence) {
...
static inline void iterator_print_state(Iterator *self)
{
  ...
  const char *name = ts_language_symbol_name(self->language,
                                             ts_subtree_symbol(*entry.subtree));
    ...
    TSRange **ranges)
{
  ...
  do {
    ...
    while (included_range_difference_index < included_range_differences->size) {
      const TSRange *range = &included_range_differences
