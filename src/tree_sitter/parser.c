...
#define LOG(...)                                                               \
  if (self->lexer.logger.log || self->dot_graph_file) {                        \
    snprintf(self->lexer.debug_buffer, TREE_SITTER_SERIALIZATION_BUFFER_SIZE,  \
             __VA_ARGS__);                                                     \
    ts_parser__log(self);                                                      \
  }
                                            ...
                                            TableEntry *table_entry)
{
  ...
  // If the token was created in a state with the same set of lookaheads, it is
  // reusable.
                              ...
                              TSStateId parse_state)
{
  ...
  for (;;) {
    ...
    if (valid_external_tokens) {
      LOG("lex_external state:%d, row:%u, column:%u",
          lex_mode.external_lex_state, current_position.extent.row + 1,
          current_position.extent.column);
      ts_lexer_start(&self->lexer);
      ts_parser__restore_external_scanner(self, external_token);
      bool found_token = self->language->external_scanner.scan(
          self->external_scanner_payload, &self->lexer.data,
    ...
    LOG("lex_internal state:%d, row:%u, column:%u", lex_mode.lex_state,
        current_position.extent.row + 1, current_position.extent.column);
    ts_lexer_start(&self->lexer);
    bool found_token
        = self->language->lex_fn(&self->lexer.data, lex_mode.lex_state);
    ts_lexer_finish(&self->lexer, &lookahead_end_byte);
    if (found_token)
      break;
  ...
  if (skipped_error) {
    ...
    result = ts_subtree_new_error(&self->tree_pool, first_error_character,
                                  padding, size, lookahead_bytes, parse_state,
                                  self->language);

    LOG("lexed_lookahead sym:%s, size:%u, character:'%c'",
        SYM_NAME(ts_subtree_symbol(result)),
        ts_subtree_total_size(result).bytes, first_error_character);
  } else {
    ...
    if (found_external_token) {
      ...
      ts_lexer_start(&self->lexer);
      if (self->language->keyword_lex_fn(&self->lexer.data, 0)
    ...
    result = ts_subtree_new_leaf(
        &self->tree_pool, symbol, padding, size, lookahead_bytes, parse_state,
        found_external_token, is_keyword, self->language);
    ...
    if (found_external_token) {
      ...
      ts_external_scanner_state_init(
          ...
          self->lexer.debug_buffer, length);
    }

    LOG("lexed_lookahead sym:%s, size:%u", SYM_NAME(ts_subtree_symbol(result)),
                                     ...
                                     TableEntry *table_entry)
{
  ...
  while ((result = reusable_node_tree(&self->reusable_node)).ptr) {
    ...
    if (!ts_subtree_external_scanner_state_eq(
      ...
      LOG("reusable_node_has_different_external_scanner_state symbol:%s",
          TREE_NAME(result));
      reusable_node_advance(&self->reusable_node);
      continue;
    }
    ...
    if (!ts_parser__can_reuse_first_leaf(self, *state, result, table_entry)) {
      LOG("cant_reuse_node symbol:%s, first_leaf_symbol:%s", TREE_NAME(result),
...
static bool ts_parser__select_tree(TSParser *self, Subtree left, Subtree right)
{
  ...
  if (ts_subtree_error_cost(right) < ts_subtree_error_cost(left)) {
    LOG("select_smaller_error symbol:%s, over_symbol:%s", TREE_NAME(right),
        TREE_NAME(left));
    return true;
  }

  if (ts_subtree_error_cost(left) < ts_subtree_error_cost(right)) {
    LOG("select_smaller_error symbol:%s, over_symbol:%s", TREE_NAME(left),
        TREE_NAME(right));
    return false;
  }
  ...
  if (ts_subtree_dynamic_precedence(right)
    ...
    LOG("select_higher_precedence symbol:%s, prec:%u, over_symbol:%s, "
        "other_prec:%u",
  ...
  switch (comparison) {
    case -1:
      LOG("select_earlier symbol:%s, over_symbol:%s", TREE_NAME(left),
          TREE_NAME(right));
      return false;
      break;
    case 1:
      LOG("select_earlier symbol:%s, over_symbol:%s", TREE_NAME(right),
          TREE_NAME(left));
      return true;
    default:
      LOG("select_existing symbol:%s, over_symbol:%s", TREE_NAME(left),
          TREE_NAME(right));
      return false;
  }
}
...
static bool ts_parser__replace_children(TSParser *self,
                                        MutableSubtree *tree,
                                        SubtreeArray *children)
{
  *self->scratch_tree.ptr = *tree->ptr;
  self->scratch_tree.ptr->child_count = 0;
  ts_subtree_set_children(self->scratch_tree, children->contents,
                          children->size, self->language);
  if (ts_parser__select_tree(self, ts_subtree_from_mut(*tree),
                             ts_subtree_from_mut(self->scratch_tree))) {
                                      ...
                                      bool fragile)
{
  ...
  for (uint32_t i = 0; i < pop.size; i++) {
    ...
    if (i > 0
        && slice_version > MAX_VERSION_COUNT + MAX_VERSION_COUNT_OVERFLOW) {
    ...
    // parent node. They will be re-pushed onto the stack after the parent node
    // is created and pushed.
    SubtreeArray children = slice.subtrees;
    while (children.size > 0
           && ts_subtree_extra(children.contents[children.size - 1])) {
      children.size--;
    }

    MutableSubtree parent = ts_subtree_new_node(
        &self->tree_pool, symbol, &children, production_id, self->language);
    ...
    for (uint32_t j = parent.ptr->child_count; j < slice.subtrees.size; j++) {
      ts_stack_push(self->stack, slice_version, slice.subtrees.contents[j],
                    false, next_state);
    }
    ...
    for (uint32_t j = trees.size - 1; j + 1 > 0; j--) {
      ...
      if (!ts_subtree_extra(child)) {
        ...
        root = ts_subtree_from_mut(ts_subtree_new_node(
            &self->tree_pool, ts_subtree_symbol(child), &trees,
            child.ptr->production_id, self->language));
  ...
  for (unsigned i = 0; true; i++) {
      ...
      for (uint32_t i = 0; i < entry.action_count; i++) {
        ...
        switch (action.type) {
          ...
          case TSParseActionTypeRecover:
            if (!action.params.extra && !action.params.repetition)
    ...
    for (uint32_t i = 0; i < self->reduce_actions.size; i++) {
      ...
      reduction_version = ts_parser__reduce(
          self, version, action.symbol, action.count, action.dynamic_precedence,
          action.production_id, true);
    }
                                    ...
                                    TSSymbol lookahead_symbol)
{
  ...
  // lookahead. After skipping one or more invalid tokens, the parser might find
  ...
  for (StackVersion v = version; v < version_count;) {
    if (!did_insert_missing_token) {
      ...
      for (TSSymbol missing_symbol = 1;
           missing_symbol < self->language->token_count; missing_symbol++) {
        TSStateId state_after_missing_symbol
            = ts_language_next_state(self->language, state, missing_symbol);
        if (state_after_missing_symbol == 0)
          continue;

        if (ts_language_has_reduce_action(
                self->language, state_after_missing_symbol, lookahead_symbol)) {
          // In case the parser is currently outside of any included range, the
          // lexer will snap to the beginning of the next included range. The
          // missing token's padding must be assigned to position it within the
          // next included range.
          ...
          ts_stack_push(self->stack, version_with_missing_tree, missing_tree,
                        false, state_after_missing_symbol);

          if (ts_parser__do_all_potential_reductions(
                  self, version_with_missing_tree, lookahead_symbol)) {
            LOG("recover_with_missing symbol:%s, state:%u",
...
static void ts_parser__halt_parse(TSParser *self)
{
  ...
  Length remaining_length = length_sub(self->lexer.current_position,
                                       ts_stack_position(self->stack, 0));

  Subtree filler_node = ts_subtree_new_error(
      &self->tree_pool, 0, length_zero(), remaining_length,
      remaining_length.bytes, 0, self->language);
  ts_subtree_to_mut_unsafe(filler_node).ptr->visible = false;
  ts_stack_push(self->stack, 0, filler_node, false, 0);

  SubtreeArray children = array_new();
  Subtree root_error = ts_subtree_new_error_node(&self->tree_pool, &children,
                                                 false, self->language);
  ts_stack_push(self->stack, 0, root_error, false, 0);

  Subtree eof
      = ts_subtree_new_leaf(&self->tree_pool, ts_builtin_sym_end, length_zero(),
                            length_zero(), 0, 0, false, false, self->language);
                                        ...
                                        TSStateId goal_state)
{
  ...
  for (unsigned i = 0; i < pop.size; i++) {
    ...
    if (slice.subtrees.size > 0) {
      Subtree error = ts_subtree_new_error_node(
          &self->tree_pool, &slice.subtrees, true, self->language);
                               ...
                               Subtree lookahead)
{
  ...
  // 2. Wrap the lookahead token in an ERROR node, push that ERROR node onto the
  // stack, and
  ...
  if (summary && !ts_subtree_is_error(lookahead)) {
    for (unsigned i = 0; i < summary->size; i++) {
      ...
      // If the current lookahead token is valid in some previous state, recover
      // to that state. Then stop looking for further recoveries.
  ...
  // In the process of attemping to recover, some stack versions may have been
  // created and subsequently halted. Remove those versions.
  ...
  // strategy 2 described above: skip the current lookahead token by wrapping it
  // in an ERROR node.
  ...
  // If the parser is still in the error state at the end of the file, just wrap
  // everything in an ERROR node and terminate.
  if (ts_subtree_is_eof(lookahead)) {
    LOG("recover_eof");
    SubtreeArray children = array_new();
    Subtree parent = ts_subtree_new_error_node(&self->tree_pool, &children,
                                               false, self->language);
  ...
  MutableSubtree error_repeat
      = ts_subtree_new_node(&self->tree_pool, ts_builtin_sym_error_repeat,
                            &children, 0, self->language);

  // If other tokens have already been skipped, so there is already an ERROR at
  // the top of the stack, then pop that ERROR off the stack and wrap the two
  ...
  if (node_count_since_error > 0) {
    ...
    // If multiple stack versions have merged at this point, just pick one of
    // the errors arbitrarily and discard the rest.
    if (pop.size > 1) {
  ...
  ts_stack_push(self->stack, version, ts_subtree_from_mut(error_repeat), false,
                ERROR_STATE);
                               ...
                               bool allow_node_reuse)
{
  ...
  if (!lookahead.ptr) {
    ...
    ts_language_table_entry(self->language, state, ts_subtree_symbol(lookahead),
                            &table_entry);
  }
  ...
  for (;;) {
    ...
    if (ts_subtree_is_keyword(lookahead)
      ...
      ts_language_table_entry(self->language, state,
                              ...
                              &table_entry);
      if (table_entry.action_count > 0) {
        LOG("switch from_keyword:%s, to_word_token:%s", TREE_NAME(lookahead),
...
{
  return (self->lexer.current_position.bytes > 0
...
TSTree *ts_parser_parse(TSParser *self, const TSTree *old_tree, TSInput input)
{
  ...
  if (ts_parser_has_outstanding_parse(self)) {
    ...
    for (unsigned i = 0; i < self->included_range_differences.size; i++) {
      ...
      LOG("different_included_range %u - %u", range->start_byte,
          range->end_byte);
    }
  } else {
    reusable_node_clear(&self->reusable_node);
  ...
  do {
    ...
    while (self->included_range_difference_index
      ...
      TSRange *range = &self->included_range_differences
