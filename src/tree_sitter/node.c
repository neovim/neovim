...
static inline NodeChildIterator ts_node_iterate_children(const TSNode *node)
{
  ...
  if (ts_subtree_child_count(subtree) == 0) {
    return (NodeChildIterator){NULL_SUBTREE, node->tree, length_zero(), 0, 0,
                               NULL};
  }
...
static inline TSNode ts_node__child(TSNode self,
                                    uint32_t child_index,
                                    bool include_anonymous)
{
  TSNode result = self;
...
static inline TSNode ts_node__first_child_for_byte(TSNode self,
                                                   uint32_t goal,
                                                   bool include_anonymous)
{
  TSNode node = self;
...
{
  return ts_node__alias(&self) ? ts_node__alias(&self)
