                                    ...
                                    uint32_t *count)
{
  ...
  ts_range_array_get_changed_ranges(
      ...
      other->included_range_count, &included_range_differences);

  *count = ts_subtree_get_changed_ranges(&self->root, &other->root, &cursor1,
                                         &cursor2, self->language,
...
TSNode ts_tree_get_cached_parent(const TSTree *self, const TSNode *node)
{
  for (uint32_t i = 0; i < self->parent_cache_size; i++) {
    ...
    if (entry->child == node->id) {
      return ts_node_new(self, entry->parent, entry->position,
                         entry->alias_symbol);
    }
  }
...
void ts_tree_set_cached_parent(const TSTree *_self,
                               const TSNode *node,
                               const TSNode *parent)
{
  ...
  if (!self->parent_cache) {
    self->parent_cache
        = ts_calloc(PARENT_CACHE_CAPACITY, sizeof(ParentCacheEntry));
  }

  uint32_t index = (self->parent_cache_start + self->parent_cache_size)
                   % PARENT_CACHE_CAPACITY;
  self->parent_cache[index] = (ParentCacheEntry){
      .child = node->id,
      .parent = (const Subtree *)parent->id,
      .position
      = {parent->context[0], {parent->context[1], parent->context[2]}},
