  ...
  if ((s->c == p_wc && !s->gotesc && KeyTyped) || s->c == p_wcm) {
    if (s->xpc.xp_numfiles > 0) {  // typed p_wc at least twice
      ...
      if (wim_flags[s->wim_index] & WIM_LONGEST) {
        s->res
            = nextwild(&s->xpc, WILD_LONGEST, WILD_NO_BEEP, s->firstc != '@');
      } else if (wim_flags[s->wim_index] & WIM_FULL) {
        s->res = nextwild(&s->xpc, WILD_NEXT, WILD_NO_BEEP, s->firstc != '@');
      } else {
      ...
      if (s->res == OK && s->xpc.xp_numfiles > 1) {
        ...
        if ((wim_flags[s->wim_index] & WIM_LIST)
          ...
          if (wim_flags[s->wim_index] & WIM_LONGEST) {
            ...
            nextwild(&s->xpc, WILD_NEXT, WILD_NO_BEEP, s->firstc != '@');
          }
        } else {
          vim_beep(BO_WILD);
        }
...
static void command_line_next_incsearch(CommandLineState *s, bool next_match)
{
  ...
  int found
      = searchit(curwin, curbuf, &t, NULL, next_match ? FORWARD : BACKWARD, pat,
                 s->count, search_flags, RE_SEARCH, 0, NULL, NULL);
  emsg_off--;
  ui_busy_stop();
  if (found) {
...
static int command_line_changed(CommandLineState *s)
{
  ...
  if (p_is && !cmd_silent && (s->firstc == '/' || s->firstc == '?')) {
    ...
    if (ccline.cmdlen == 0) {
      ...
      i = do_search(NULL, s->firstc, ccline.cmdbuff, s->count, search_flags,
                    &tm, NULL);
      emsg_off--;
