    ...
    for (size_t i = 0; i < kv_size(*acs); i++) {
      ...
      if (ac->pat->group != last_group) {
        ...
        if (ac->pat->group != AUGROUP_DEFAULT) {
          if (last_group_name == NULL) {
            msg_puts_hl_id(get_deleted_augroup(), HLF_E, false);
          } else {
            msg_puts_hl_id(last_group_name, HLF_T, false);
          }
          msg_puts("  ");
        }
        // show the event name
        msg_puts_hl_id(event_nr2name(event), HLF_T, false);
      }
      ...
      if (ac->desc != NULL) {
        ...
        if (ac->exec.type == CALLABLE_CB) {
          msg_puts_hl_id(exec_to_string, HLF_8, false);
