...
static char *pim_info(nfa_pim_T *pim)
{
  ...
  if (pim == NULL || pim->result == NFA_PIM_UNUSED)
    ...
    sprintf(buf, " PIM col %d",
            REG_MULTI ? (int)pim->end.pos.col : (int)(pim->end.ptr - reginput));
  }
  return buf;
}
...
static void clear_sub(regsub_T *sub)
{
  if (REG_MULTI)
    ...
    memset(sub->list.multi, 0xff, sizeof(struct multipos) * nfa_nsubexpr);
  else
 ...
 * positions as "subs".
 */
static int has_state_with_pos(nfa_list_T *l,      /* runtime state list */
 ...
 */
static int state_in_list(nfa_list_T *l,      /* runtime state list */
                         nfa_state_T *state, /* state to update */
                         regsubs_T *subs     /* pointers to subexpressions */
)
{
    ...
    FUNC_ATTR_NONNULL_ARG(1, 2) FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  switch (state->c) {
    ...
    case NFA_BOF:
      ...
      if (reginput > regline && *reginput != NUL
...
)
{
  ...
  if (REG_MULTI) {
    ...
    if (sub->list.multi[subidx].start_lnum == reglnum
      ...
      if (cstrncmp(regline + sub->list.multi[subidx].start_col, reginput, &len)
          == 0) {
        *bytelen = len;
        return TRUE;
      }
...
#define ADD_STATE_IF_MATCH(state)                                              \
  ...
  for (;;) {
      ...
      switch (t->state->c) {
        ...
        case NFA_END_PATTERN:
          ...
          if (nfa_endp != NULL) {
            if (REG_MULTI)
              fprintf(log_fd,
                      "Current lnum: %d, endp lnum: %d; current col: %d, endp "
                      "col: %d\n",
                      (int)reglnum, (int)nfa_endp->se_u.pos.lnum,
        ...
        case NFA_START_PATTERN: {
          ...
          if (result) {
            ...
            if (REG_MULTI) {
              ...
              bytelen
                  = m->norm.list.multi[0].end_col - (int)(reginput - regline);
            } else {
        ...
        case NFA_SKWORD:  //  \K
          result
              = !ascii_isdigit(curc) && vim_iswordp_buf(reginput, rex.reg_buf);
          ADD_STATE_IF_MATCH(t->state);
          break;
        ...
        case NFA_COL_GT:
        case NFA_COL_LT:
          assert(t->state->val >= 0 && reginput >= regline
    ...
    if (!nfa_match
        && ((toplevel && reglnum == 0 && clen != 0
             && (rex.reg_maxcol == 0
                 || (colnr_T)(reginput - regline) < rex.reg_maxcol))
            || (nfa_endp != NULL
                && (REG_MULTI ? (reglnum < nfa_endp->se_u.pos.lnum
                                 || (reglnum == nfa_endp->se_u.pos.lnum
                                     && (int)(reginput - regline)
