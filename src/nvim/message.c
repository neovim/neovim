...
// msg_scrolled_ign  true when msg_scrolled is non-zero and msg_puts_id()
...
bool msg(const char *s, const int id)
  FUNC_ATTR_NONNULL_ARG(1)
{
  return msg_hl_keep(s, id, false, false);
}

/// Similar to msg_outtrans, but support newlines and tabs.
void msg_multiline(const char *s, int id, bool check_int, bool hist, bool *need_clear)
  FUNC_ATTR_NONNULL_ALL
{
  ...
  while (next_spec != NULL) {
    ...
    if (next_spec != NULL) {
      ...
      msg_outtrans_len(s, (int)(next_spec - s), id, hist);

      if (*next_spec != TAB && *need_clear) {
        msg_clr_eos();
        *need_clear = false;
      }
      msg_putchar_hl((uint8_t)(*next_spec), id);
      s = next_spec + 1;
    }
  }
...
void msg_multihl(HlMessage hl_msg, const char *kind, bool history)
{
  ...
  for (uint32_t i = 0; i < kv_size(hl_msg); i++) {
    ...
    msg_multiline(chunk.text.data, chunk.hlid, true, false, &need_clear);
  }
...
bool msg_hl_keep(const char *s, int id, bool keep, bool multiline)
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (multiline) {
    msg_multiline(s, id, false, false, &need_clear);
  } else {
    msg_outtrans(s, id, false);
  }
  if (need_clear) {
    msg_clr_eos();
  }
  ...
  FUNC_ATTR_PRINTF(2, 3)
{
  ...
  va_end(arglist);
  return msg(IObuff, id);
}

int smsg_hl_keep(int id, const char *s, ...)
  FUNC_ATTR_PRINTF(2, 3)
{
  va_list arglist;
  ...
  vim_vsnprintf(IObuff, IOSIZE, s, arglist);
  va_end(arglist);
  return msg_hl_keep(IObuff, id, true, false);
}
...
bool emsg_multiline(const char *s, bool multiline)
{
  ...
  int id = HLF_E + 1;         // set highlight mode for error messages
  if (msg_scrolled != 0) {
  ...
  return msg_hl_keep(s, id, false, multiline);
}
...
char *msg_trunc(char *s, bool force, int id)
{
  // Add message to history before truncating.
  add_msg_hist(s, -1, id, false);
...
static void add_msg_hist(const char *s, int len, int id, bool multiline)
{
  add_msg_hist_multihl(s, len, id, multiline, (HlMessage)KV_INITIAL_VALUE);
}

static void add_msg_hist_multihl(const char *s, int len, int id, bool multiline, HlMessage multihl)
{
  ...
  if (ui_has(kUIMessages)) {
      ...
      if (kv_size(p->multihl) || (p->msg && p->msg[0])) {
        ...
        if (kv_size(p->multihl)) {
          for (uint32_t i = 0; i < kv_size(p->multihl); i++) {
            ...
            ADD(content_entry, INTEGER_OBJ(chunk.hlid ? syn_id2attr(chunk.hlid) : 0));
            ADD(content_entry, STRING_OBJ(copy_string(chunk.text, NULL)));
            ADD(content_entry, INTEGER_OBJ(chunk.hlid));
            ADD(content, ARRAY_OBJ(content_entry));
          }
        } else if (p->msg && p->msg[0]) {
          Array content_entry = ARRAY_DICT_INIT;
          ADD(content_entry, INTEGER_OBJ(p->hlid ? syn_id2attr(p->hlid) : 0));
          ADD(content_entry, CSTR_TO_OBJ(p->msg));
          ADD(content_entry, INTEGER_OBJ(p->hlid));
          ADD(content, ARRAY_OBJ(content_entry));
        }
    ...
    for (; p != NULL && !got_int; p = p->next) {
      if (kv_size(p->multihl)) {
        ...
        msg_hl_keep(p->msg, p->hlid, false, p->multiline);
      }
    }
    msg_hist_off = false;
  }
...
void set_keep_msg(const char *s, int id)
{
  xfree(keep_msg);
...
void msg_putchar_hl(int c, int id)
{
...
static void msg_home_replace_hl(const char *fname, int id)
{
  char *name = home_replace_save(NULL, fname);
  msg_outtrans(name, id, false);
  xfree(name);
}

/// Output "len" characters in "str" (including NULs) with translation
/// if "len" is -1, output up to a NUL character. Use highlight "id".
///
/// @return  the number of characters it takes on the screen.
int msg_outtrans(const char *str, int id, bool hist)
{
  return msg_outtrans_len(str, (int)strlen(str), id, hist);
}
...
const char *msg_outtrans_one(const char *p, int id, bool hist)
{
  int l;

  if ((l = utfc_ptr2len(p)) > 1) {
    msg_outtrans_len(p, l, id, hist);
    return p + l;
  }
  msg_puts_hl(transchar_byte_buf(NULL, (uint8_t)(*p)), id, hist);
  return p + 1;
}

int msg_outtrans_len(const char *msgstr, int len, int id, bool hist)
{
  int retval = 0;
  ...
  while (--len >= 0 && !got_int) {
    ...
    if (mb_l > 1) {
      ...
      if (vim_isprintc(c)) {
        ...
        if (str > plain_start) {
          msg_puts_len(plain_start, str - plain_start, id, hist);
        }
        plain_start = str + mb_l;
        msg_puts_hl(transchar_buf(NULL, c), id == 0 ? HLF_8 + 1 : id, false);
        retval += char2cells(c);
      }
      len -= mb_l - 1;
      str += mb_l;
      ...
      if (s[1] != NUL) {
        ...
        plain_start = str + 1;
        msg_puts_hl(s, id == 0 ? HLF_8 + 1 : id, false);
        retval += (int)strlen(s);
      } else {
        retval++;
      }
...
int msg_outtrans_special(const char *strstart, bool from, int maxlen)
{
  ...
  while (*str != NUL) {
    ...
    msg_puts_hl(text, (len > 1 && utfc_ptr2len(text) <= 1 ? id : 0), false);
    retval += len;
  }
  return retval;
}
...
void msg_outtrans_long(const char *longstr, int id)
{
  ...
  if (len > room && room >= 20) {
    slen = (room - 3) / 2;
    msg_outtrans_len(longstr, slen, id, false);
    msg_puts_hl("...", HLF_8 + 1, false);
  }
  msg_outtrans_len(longstr + len - slen, slen, id, len);
}

/// Basic function for writing a message with highlight id.
void msg_puts_hl(const char *const s, const int id, const bool hist)
{
  msg_puts_len(s, -1, id, hist);
}
...
/// @param[in]  id  Highlight id.
void msg_puts_len(const char *const str, const ptrdiff_t len, int id, bool hist)
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (hist) {
    add_msg_hist(str, (int)len, id, false);
  }
  ...
  if (!msg_use_printf() || (headless_mode && default_grid.chars)) {
    msg_puts_display(str, (int)len, id, false);
  }
...
void msg_printf_hl(const int id, const char *const fmt, ...)
  FUNC_ATTR_NONNULL_ARG(2) FUNC_ATTR_PRINTF(2, 3)
{
  ...
  msg_scroll = true;
  msg_puts_len(msgbuf, (ptrdiff_t)len, id, true);
}
...
static void msg_puts_display(const char *str, int maxlen, int id, int recurse)
{
  const char *s = str;
  const char *sb_str = str;
  int sb_col = msg_col;
  int attr = id ? syn_id2attr(id) : 0;
  ...
  while (true) {
    if (msg_col >= Columns) {
      if (p_more && !recurse) {
        ...
        store_sb_text(&sb_str, s, id, &sb_col, true);
      }
  ...
  if (p_more && !recurse) {
    store_sb_text(&sb_str, s, id, &sb_col, false);
  }
...
static void store_sb_text(const char **sb_str, const char *s, int id, int *sb_col, int finish)
{
  msgchunk_T *mp;
...
static msgchunk_T *disp_sb_line(int row, msgchunk_T *smp)
{
  ...
  while (true) {
    ...
    char *p = mp->sb_text;
    msg_puts_display(p, -1, mp->sb_hlid, true);
  ...
  FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (msg(message, keep_msg_hlid) && msg_scrolled == 0) {
    set_keep_msg(message, keep_msg_hlid);
  }
