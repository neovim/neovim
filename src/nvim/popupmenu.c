...
void pum_display(pumitem_T *array, int size, int selected, bool array_changed, int cmd_startcol)
{
  ...
  do {
    ...
    if (State == MODE_CMDLINE) {
      ...
      cursor_col = (cmdline_win ? cmdline_win->w_config.cmdline_offset : 0) + cmd_startcol;
    ...
    if (pum_win_row + 2 >= below_row - pum_height
      ...
      if (State == MODE_CMDLINE && target_win == NULL) {
        ...
        if (target_win->w_wrow - target_win->w_cline_row >= 2) {
          context_lines = 2;
        } else {
          context_lines = target_win->w_wrow - target_win->w_cline_row;
        }
      }
      ...
      if (State == MODE_CMDLINE && target_win == NULL) {
        ...
        if (target_win->w_cline_row + target_win->w_cline_height - target_win->w_wrow >= 3) {
          context_lines = 3;
        } else {
          context_lines = target_win->w_cline_row + target_win->w_cline_height - target_win->w_wrow;
        }
      }
