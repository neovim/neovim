...
static bool pum_parse_title_pos(WinConfig *fconfig, BorderTextType bt, const char *s, size_t len)
{
  Error err = ERROR_INIT;
  char *data = xmemdupz(s, len);
  if (data == NULL) {
  ...
  bool result = parse_bordertext_pos(str, bt, fconfig, &err);
  api_free_string(str);
  if (ERROR_SET(&err)) {
    emsg(err.msg);
    api_clear_error(&err);
...
bool parse_completepopup(WinConfig *fconfig, int *border_adj)
{
  ...
  while (*p != NUL) {
    ...
    if (strncmp(s, "border:", 7) == 0) {
      ...
      style.data.string.data = xmemdupz(e + 1, len);
      if (!style.data.string.data) {
        return false;
      }
      style.data.string.size = len;
      ...
      if (!pum_parse_title(fconfig, kBorderTextTitle, e + 1, len)) {
        return false;
      }
      fconfig->title = true;
    } else if (strncmp(s, "title_pos:", 10) == 0) {
      if (!pum_parse_title_pos(fconfig, kBorderTextTitle, e + 1, len)) {
        return false;
      }
    } else if (strncmp(s, "footer:", 7) == 0) {
      if (!pum_parse_title(fconfig, kBorderTextFooter, e + 1, len)) {
        return false;
      }
      fconfig->footer = true;
    } else if (strncmp(s, "footer_pos:", 11) == 0) {
      if (!pum_parse_title_pos(fconfig, kBorderTextFooter, e + 1, len)) {
        return false;
      }
