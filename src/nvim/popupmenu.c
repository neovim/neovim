...
static void pum_position_at_mouse(int min_width)
{
  ...
  if (*p_pumborder != NUL) {
    if (strequal(p_pumborder, opt_winborder_values[3])) {
      border_width = 1;
      border_height = 1;  // Shadow only at bottom/right
    } else {
      border_width = 2;
      border_height = 2;  // Full border
    }
  }
...
static void pum_select_mouse_pos(void)
{
  ...
  if (grid == pum_grid.handle) {
    // Non-shadow borders occupy first row/col
    int border_offset = 0;
    if (*p_pumborder != NUL && !strequal(p_pumborder, opt_winborder_values[3])) {
      border_offset = 1;
    }
