...
pub fn addLintcSteps(
    ...
    const lint_sources = try lintSources(b, nvim_sources, nvim_headers);
    const lintc_uncrustify = addLintcUncrustifyStep(b, lint_sources);
    lintc.dependOn(lintc_uncrustify);
    const lintc_clint = addLintcClintStep(b, nvim_exe_install, lint_sources);
    lintc.dependOn(lintc_clint);
    const lintc_clang_tidy = try addLintcClangtidyStep(b, cdb_step, target, lint_sources);
...
fn addLintcUncrustifyStep(
    ...
    const lintc_uncrustify = b.step("lintc-uncrustify", "Check formatting of C source");
...
fn addLintcClangtidyStep(
    ...
    const lintc_clang_tidy = b.step("lintc-clang-tidy", "Run clang-tidy on C source");
    lintc_clang_tidy.dependOn(cdb_step);
...
fn addLintcClintStep(
    ...
    const lintc_clint = b.step("lintc-clint", "Run clint on C source");
    lintc_clint.dependOn(&nvim_exe_install.step);
    ...
    outer: for (lint_sources.items) |file| {
        ...
        run.addArg("--output=vs7"); // TODO: CI vs local - refer to cmake
        run.addFileArg(b.path(file));
        run.expectStdOutEqual("");
