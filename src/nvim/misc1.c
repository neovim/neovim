  ...
  if (lead_len > 0) {
    ...
    for (p = lead_flags; *p && *p != ':'; ++p) {
      ...
      if (*p == COM_START || *p == COM_MIDDLE) {
        ...
        if (lead_len > 0) {
           ...
           */
          if (!isblank(saved_line[lead_len - 1])
              && ((p_extra != NULL
    ...
    if (lead_len > 0) {
      ...
      if (lead_repl != NULL) {
         ...
         * extra space */
        if (lead_len > 0 && isblank(leader[lead_len - 1]))
          extra_space = FALSE;
        leader[lead_len] = NUL;
      }
...
int get_leader_len(char_u *line, char_u **flags, int backward, int include_space)
{
  ...
  result = i = 0;
  while (isblank(line[i]))      /* leading white space is ignored */
    ++i;
