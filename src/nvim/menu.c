...
)
{
  ...
  for (p = arg; *p; ++p)
    if (!VIM_ISDIGIT(*p) && *p != '.')
      break;
  if (isblank(*p)) {
    for (i = 0; i < MENUDEPTH && !isblank(*arg); ++i) {
   ...
   */
  if (STRNCMP(arg, "enable", 6) == 0 && isblank(arg[6])) {
    enable = TRUE;
    arg = skipwhite(arg + 6);
  } else if (STRNCMP(arg, "disable", 7) == 0 && isblank(arg[7])) {
    enable = FALSE;
    arg = skipwhite(arg + 7);
  }
...
char_u *set_context_in_menu_cmd(expand_T *xp, char_u *cmd, char_u *arg, int forceit)
{
  ...
  if (!isblank(*p)) {
    if (STRNCMP(arg, "enable", 6) == 0
        && (arg[6] == NUL ||  isblank(arg[6])))
      p = arg + 6;
    else if (STRNCMP(arg, "disable", 7) == 0
             && (arg[7] == NUL || isblank(arg[7])))
      p = arg + 7;
    else
      p = arg;
  }

  while (*p != NUL && isblank(*p))
    ++p;
