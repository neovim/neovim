...
static void syn_list_one(const int id, const bool syncing, const bool link_only)
{
  ...
  for (int idx = 0;
    ...
    syn_list_flags(namelist1, spp->sp_flags, hl_id);
    ...
    if (spp->sp_next_list != NULL) {
      ...
      syn_list_flags(namelist2, spp->sp_flags, hl_id);
    }
...
static void syn_list_flags(struct name_list *nlist, int flags, int attr)
{
  for (int i = 0; nlist[i].flag != 0; i++) {
    if (flags & nlist[i].flag) {
      msg_puts_hl(nlist[i].name, attr, false);
      msg_putchar(' ');
    }
  }
}
...
static void put_id_list(const char *const name, const int16_t *const list, const int id)
{
  msg_puts_hl(name, id, false);
  msg_putchar('=');
...
static void put_pattern(const char *const s, const int c, const synpat_T *const spp, const int id)
{
  ...
  if (last_matchgroup != spp->sp_syn_match_id) {
    ...
    msg_puts_hl("matchgroup", id, false);
    msg_putchar('=');
