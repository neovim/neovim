...
static char *(hl_name_table[])
    = {"bold",    "standout", "underline",     "undercurl", "italic",
       "reverse", "inverse",  "strikethrough", "nocombine", "NONE"};
static int hl_attr_table[]
    = {HL_BOLD,    HL_STANDOUT, HL_UNDERLINE,     HL_UNDERCURL, HL_ITALIC,
       HL_INVERSE, HL_INVERSE,  HL_STRIKETHROUGH, HL_NOCOMBINE, 0};
...
void syntax_start(win_T *wp, linenr_T lnum)
{
  ...
  while (current_lnum < lnum) {
    ...
    if (current_lnum >= first_stored) {
      ...
      else if (prev == NULL || current_lnum == lnum
 ...
 * is valid.
 */
static bool syn_finish_line(const bool syncing  // called for syncing
)
{
          ...
          if (syn_id != 0) {
            push_current_state(KEYWORD_IDX);
            {
              ...
              if (current_state.ga_len > 1)
                cur_si->si_flags |= CUR_STATE(current_state.ga_len - 2).si_flags
                                    & HL_CONCEAL;
              cur_si->si_id = syn_id;
          ...
          for (int idx = syn_block->b_syn_patterns.ga_len; --idx >= 0;) {
            ...
            if (spp->sp_syncing == syncing
              ...
              if (spp->sp_type == SPTYPE_START
                ...
                startpos = endpos;
                find_endpos(idx, &startpos, &endpos, &hl_endpos, &flags,
                            &eoe_pos, &end_idx, cur_extmatch);
                if (endpos.lnum == 0)
                  continue; /* not found */
              }
              /*
...
)
{
  ...
  lpos_T endpos = {0};
  find_endpos(sip->si_idx, &startpos, &endpos, &hl_endpos, &(sip->si_flags),
              &end_endpos, &end_idx, sip->si_extmatch);
      ...
      if (r && regmatch.startpos[0].col <= best_regmatch.startpos[0].col) {
        ...
        if (pos.col <= matchcol) {
          ...
          for (matchcol = regmatch.endpos[0].col;
               matchcol < line_len && matchcol < pos.col; matchcol++) {
          }
        }
 ...
 */
static void syn_add_end_off(lpos_T *result,         // returned position
 ...
 */
static void syn_add_start_off(lpos_T *result,         // returned position
                              regmmatch_T *regmatch,  // start/end of match
                              synpat_T *spp,
                              int idx,
...
static void syn_cmd_foldlevel(exarg_T *eap, int syncing)
{
  ...
  if (*arg == NUL) {
    switch (curwin->w_s->b_syn_foldlevel) {
      case SYNFLD_START:
        MSG(_("syntax foldlevel start"));
        break;
      case SYNFLD_MINIMUM:
        MSG(_("syntax foldlevel minimum"));
        break;
      default:
        break;
    }
 ...
 */
static void syn_cmd_list(exarg_T *eap,
...
)
{
  ...
  if (ends_excmd(*arg)) {
    ...
    for (int id = 0; id < curwin->w_s->b_syn_clusters.ga_len && !got_int;
         ++id) {
      syn_list_cluster(id);
    }
  } else {
 ...
 */
static void syn_list_one(const int id,
...
)
{
  ...
  static struct name_list namelist1[] = {{HL_DISPLAY, "display"},
                                         ...
                                         {0, NULL}};
  static struct name_list namelist2[] = {{HL_SKIPWHITE, "skipwhite"},
                                         {HL_SKIPNL, "skipnl"},
  ...
  if (!syncing) {
    ...
    did_header
        = syn_list_keywords(id, &curwin->w_s->b_keywtab_ic, did_header, attr);
  }

  // list the patterns for "id"
  for (int idx = 0; idx < curwin->w_s->b_syn_patterns.ga_len && !got_int;
       idx++) {
    ...
    if (spp->sp_flags & (HL_SYNC_HERE | HL_SYNC_THERE)) {
      ...
      if (spp->sp_sync_idx >= 0)
        msg_outtrans(
            HL_TABLE()[SYN_ITEMS(curwin->w_s)[spp->sp_sync_idx].sp_syn.id - 1]
                .sg_name);
      else
        MSG_PUTS("NONE");
      msg_putchar(' ');
...
static void put_pattern(const char *const s,
                        const int c,
                        const synpat_T *const spp,
                        const int attr)
{
                        ...
                        const int conceal_char)
{
  ...
  const char_u *const name_ic
      = (curwin->w_s->b_syn_ic) ? str_foldcase(name, (int)STRLEN(name),
                                               name_folded, sizeof(name_folded))
                                : name;
  ...
  hashtab_T *const ht = (curwin->w_s->b_syn_ic) ? &curwin->w_s->b_keywtab_ic
                                                : &curwin->w_s->b_keywtab;
  hashitem_T *const hi
      = hash_lookup(ht, (const char *)kp->keyword, STRLEN(kp->keyword), hash);
 ...
 */
static char_u *get_group_name(char_u *arg,      /* start of the argument */
 ...
 */
static char_u *get_syn_options(char_u *arg,  // next argument to be checked
    ...
    for (fidx = ARRAY_SIZE(flagtab); --fidx >= 0;) {
      ...
      if (p[i] == NUL
          && (ascii_iswhite(arg[len])
              || (flagtab[fidx].argtype > 0 ? arg[len] == '='
                                            : ends_excmd(arg[len])))) {
        if (opt->keyword
 ...
 */
static void syn_cmd_match(exarg_T *eap,
  ...
  if (rest != NULL) { /* all arguments are valid */
    ...
    if (!ends_excmd(*rest) || eap->skip)
      ...
      if ((syn_id = syn_check_group(arg, (int)(group_name_end - arg))) != 0) {
         ...
         */
        synpat_T *spp
            = GA_APPEND_VIA_PTR(synpat_T, &curwin->w_s->b_syn_patterns);
        *spp = item;
 ...
 */
static void syn_cmd_region(exarg_T *eap,
...
#define ITEM_MATCHGROUP 3
  ...
  if (rest != NULL
      && (pat_ptrs[ITEM_START] == NULL || pat_ptrs[ITEM_END] == NULL)) {
    not_enough = true;
    rest = NULL;
  }
...
static int syn_add_cluster(char_u *name)
{
  ...
  syn_cluster_T *scp
      = GA_APPEND_VIA_PTR(syn_cluster_T, &curwin->w_s->b_syn_clusters);
...
static void syn_cmd_cluster(exarg_T *eap, int syncing)
{
  ...
  if (rest != NULL) {
    ...
    for (;;) {
      ...
      if (scl_id >= 0) {
        syn_combine_list(&SYN_CLSTR(curwin->w_s)[scl_id].scl_list, &clstr_list,
                         list_op);
      } else {
        xfree(clstr_list);
      }
  ...
  while (!ends_excmd(*arg_start)) {
    ...
    if (STRCMP(key, "CCOMMENT") == 0) {
      ...
      if (curwin->w_s->b_syn_linecont_pat != NULL) {
        EMSG(
            _("E403: syntax sync: line continuations pattern specified twice"));
        finished = TRUE;
        break;
      }
...
static struct subcommand subcommands[]
    = {{"case", syn_cmd_case},       {"clear", syn_cmd_clear},
       ...
       {"enable", syn_cmd_enable},   {"foldlevel", syn_cmd_foldlevel},
  ...
  switch (expand_what) {
    ...
    case EXP_SYNC: {
      static char *sync_args[]
          = {"ccomment",  "clear",  "fromstart", "linebreaks=",
             "linecont",  "lines=", "match",     "maxlines=",
             "minlines=", "region", NULL};
...
static void syntime_report(void)
{
  ...
  MSG_PUTS_TITLE(
      _("  TOTAL      COUNT  MATCH   SLOWEST     AVERAGE   NAME               "
        "PATTERN"));
  MSG_PUTS("\n");
...
static const char *highlight_init_light[] = {
    ...
    "DiffDelete   ctermfg=Blue ctermbg=LightCyan gui=bold guifg=Blue "
    "guibg=LightCyan",
    ...
    "TabLine      cterm=underline ctermfg=black ctermbg=LightGrey "
...
static const char *highlight_init_dark[]
    = {"ColorColumn  ctermbg=DarkRed guibg=DarkRed",
       ...
       "DiffDelete   ctermfg=Blue ctermbg=DarkCyan gui=bold guifg=Blue "
       "guibg=DarkCyan",
       ...
       "TabLine      cterm=underline ctermfg=white ctermbg=DarkGrey "
...
void init_highlight(bool both, bool reset)
{
  ...
  const char *const *const pp
      = ((*p_bg == 'l') ? highlight_init_light : highlight_init_dark);
  for (size_t i = 0; pp[i] != NULL; i++) {
    do_highlight(pp[i], reset, true);
  }
...
void do_highlight(const char *line,
                  const bool forceit,
                  const bool init) FUNC_ATTR_NONNULL_ALL
{
  const char *name_end;
  const char *linep;
  ...
  if (dolink) {
    ...
    if (strncmp(to_start, "NONE", 4) == 0) {
      ...
      to_id
          = syn_check_group((const char_u *)to_start, (int)(to_end - to_start));
    }
...
{
  return HL_TABLE()[idx].sg_attr != 0 || HL_TABLE()[idx].sg_cterm_fg != 0
...
static void highlight_list_one(const int id)
{
  ...
  didh = highlight_list_arg(id, didh, LIST_INT, sgp->sg_cterm_fg, NULL,
                            "ctermfg");
  didh = highlight_list_arg(id, didh, LIST_INT, sgp->sg_cterm_bg, NULL,
                            "ctermbg");

  didh = highlight_list_arg(id, didh, LIST_ATTR, sgp->sg_gui, NULL, "gui");
  didh = highlight_list_arg(id, didh, LIST_STRING, 0, sgp->sg_rgb_fg_name,
                            "guifg");
  didh = highlight_list_arg(id, didh, LIST_STRING, 0, sgp->sg_rgb_bg_name,
                            "guibg");
  didh = highlight_list_arg(id, didh, LIST_STRING, 0, sgp->sg_rgb_sp_name,
                            "guisp");

  didh = highlight_list_arg(id, didh, LIST_INT, sgp->sg_blend + 1, NULL,
                            "blend");
                               ...
                               const char *const name)
{
  ...
  if (type == LIST_STRING ? (sarg != NULL) : (iarg != 0)) {
    ...
    (void)syn_list_header(
        didh, (int)(vim_strsize((char_u *)ts) + STRLEN(name) + 1), id, false);
    didh = true;
    if (!got_int) {
      if (*name != NUL) {
...
static void combine_stl_hlt(int id,
                            int id_S,
                            int id_alt,
                            int hlcnt,
                            int i,
                            int hlf,
                            int *table) FUNC_ATTR_NONNULL_ALL
{
...
void highlight_changed(void)
{
  ...
  for (int hlf = 0; hlf < (int)HLF_COUNT; hlf++) {
    ...
    highlight_attr[hlf]
        = hl_get_ui_attr(hlf, final_id, hlf == (int)HLF_INACTIVE);
...
const char *get_highlight_name(expand_T *const xp,
                               int idx) FUNC_ATTR_WARN_UNUSED_RESULT
{
