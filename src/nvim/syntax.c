...
static char *(hl_name_table[])
    = {"bold",   "standout", "underline", "underlineline", "undercurl", "underdot", "underdash",
       "italic", "reverse",  "inverse",   "strikethrough", "nocombine", "NONE"};
static int hl_attr_table[] = {HL_BOLD,
                              HL_STANDOUT,
                              HL_UNDERLINE,
                              HL_UNDERLINELINE,
                              HL_UNDERCURL,
    ...
    if (current_lnum >= first_stored) {
      ...
      if (sp != NULL && sp->sst_lnum == current_lnum && syn_stack_equal(sp)) {
        ...
        load_current_state(prev);
                 ...
                 || current_lnum == lnum || current_lnum >= prev->sst_lnum + dist) {
...
static void syn_update_ends(bool startofline)
{
  ...
  if (startofline) {
    ...
    for (int i = 0; i < current_state.ga_len; i++) {
      cur_si = &CUR_STATE(i);
      if (cur_si->si_idx >= 0 && (SYN_ITEMS(syn_block)[cur_si->si_idx]).sp_type == SPTYPE_MATCH
  ...
  for (; i < current_state.ga_len; i++) {
    cur_si = &CUR_STATE(i);
    if ((cur_si->si_flags & HL_KEEPEND) || (seen_keepend && !startofline)
...
static synstate_T *store_current_state(void)
{
  ...
  for (i = current_state.ga_len - 1; i >= 0; --i) {
    cur_si = &CUR_STATE(i);
    if (cur_si->si_h_startpos.lnum >= current_lnum || cur_si->si_m_endpos.lnum >= current_lnum
    ...
    for (j = 0; j < NSUBEXP; j++) {
      ...
      if (bsx->matches[j] != six->matches[j]) {
        ...
        if (mb_strcmp_ic((SYN_ITEMS(syn_block)[CUR_STATE(i).si_idx]).sp_ic,
                         (const char *)bsx->matches[j], (const char *)six->matches[j])
            != 0) {
          break;
        }
      }
...
int get_syntax_attr(const colnr_T col, bool *const can_spell, const bool keep_state)
{
  ...
  if (can_spell != NULL) {
    ...
    *can_spell = syn_block->b_syn_spell == SYNSPL_DEFAULT ? (syn_block->b_spell_cluster_id == 0)
...
static int syn_current_attr(const bool syncing,
                            ...
                            const bool keep_state)
{
    ...
    if (syn_block->b_syn_containedin || cur_si == NULL || cur_si->si_cont_list != NULL) {
      ...
      if (do_keywords) {
        ...
        if (vim_iswordp_buf(cur_pos, syn_buf)
            && (current_col == 0
                || !vim_iswordp_buf(cur_pos - 1 - utf_head_off(line, cur_pos - 1), syn_buf))) {
          syn_id = check_keyword_id(line, (int)current_col, &endcol, &flags, &next_list, cur_si,
                                    &cchar);
          if (syn_id != 0) {
      ...
      if (syn_id == 0 && syn_block->b_syn_patterns.ga_len) {
        ...
        if (next_match_idx < 0 || next_match_col < (int)current_col) {
          ...
          for (int idx = syn_block->b_syn_patterns.ga_len; --idx >= 0;) {
            ...
            if (spp->sp_syncing == syncing && (displaying || !(spp->sp_flags & HL_DISPLAY))
              ...
              int r = syn_regexec(&regmatch, current_lnum, lc_col, IF_SYN_TIME(&spp->sp_time));
              ...
              if (spp->sp_type == SPTYPE_START && (spp->sp_flags & HL_ONELINE)) {
                ...
                startpos = endpos;
                find_endpos(idx, &startpos, &endpos, &hl_endpos, &flags, &eoe_pos, &end_idx,
                            cur_extmatch);
                if (endpos.lnum == 0) {
                  continue;  // not found
                }
        ...
        if (next_match_idx >= 0 && next_match_col == (int)current_col) {
          ...
          if (next_match_m_endpos.lnum == current_lnum && next_match_m_endpos.col == current_col
  ...
  if (cur_si != NULL) {
    for (int idx = current_state.ga_len - 1; idx >= 0; --idx) {
      ...
      if ((current_lnum > sip->si_h_startpos.lnum
           || (current_lnum == sip->si_h_startpos.lnum && current_col >= sip->si_h_startpos.col))
          && (sip->si_h_endpos.lnum == 0 || current_lnum < sip->si_h_endpos.lnum
  ...
  if (current_next_list != NULL && (line = syn_getcurline())[current_col] != NUL
      && line[current_col + 1] == NUL && !(current_next_flags & (HL_SKIPNL | HL_SKIPEMPTY))) {
    current_next_list = NULL;
  }
...
static bool did_match_already(int idx, garray_T *gap)
{
  for (int i = current_state.ga_len; --i >= 0;) {
    if (CUR_STATE(i).si_m_startcol == (int)current_col
        && CUR_STATE(i).si_m_lnum == (int)current_lnum && CUR_STATE(i).si_idx == idx) {
      return true;
    }
  }
  ...
  for (;;) {
    if (cur_si->si_ends
      ...
      if (cur_si->si_end_idx
         ...
         */
        if (cur_si->si_idx >= 0 && SYN_ITEMS(syn_block)[cur_si->si_idx].sp_type == SPTYPE_START
            && !(cur_si->si_flags & (HL_MATCH | HL_KEEPEND))) {
          update_si_end(cur_si, (int)current_col, true);
          check_keepend();
          if ((current_next_flags & HL_HAS_EOL) && keepend_level < 0
              && syn_getcurline()[current_col] == NUL) {
            break;
          }
        }
...
static void check_keepend(void)
{
  ...
  for (; i < current_state.ga_len; ++i) {
    ...
    if (sip->si_ends && (sip->si_flags & HL_KEEPEND)) {
      if (maxpos.lnum == 0 || maxpos.lnum > sip->si_m_endpos.lnum
          || (maxpos.lnum == sip->si_m_endpos.lnum && maxpos.col > sip->si_m_endpos.col)) {
        maxpos = sip->si_m_endpos;
      }
      if (maxpos_h.lnum == 0 || maxpos_h.lnum > sip->si_h_endpos.lnum
...
static void update_si_end(stateitem_T *sip, int startcol, bool force)
{
  ...
  lpos_T endpos = {0};
  find_endpos(sip->si_idx, &startpos, &endpos, &hl_endpos, &(sip->si_flags), &end_endpos, &end_idx,
              sip->si_extmatch);
...
static int check_keyword_id(char_u *const line,
                            const int startcol,
                            int *const endcolp,
                            long *const flagsp,
  ...
  if (!HASHITEM_EMPTY(hi)) {
    for (keyentry_T *kp = HI2KE(hi); kp != NULL; kp = kp->ke_next) {
      if (current_next_list != 0
              ...
              : (cur_si == NULL ? !(kp->flags & HL_CONTAINED)
                                : in_id_list(cur_si, cur_si->si_cont_list, &kp->k_syn,
...
static void syn_cmd_list(exarg_T *eap, int syncing)
{
  ...
  if (syncing) {
    ...
    msg_puts_title(_("\n--- Syntax sync items ---"));
    if (curwin->w_s->b_syn_sync_minlines > 0 || curwin->w_s->b_syn_sync_maxlines > 0
        || curwin->w_s->b_syn_sync_linebreaks > 0) {
      msg_puts(_("\nsyncing on items"));
      syn_lines_msg();
      syn_match_msg();
...
static void syn_list_one(const int id, const bool syncing, const bool link_only)
{
  ...
  static struct name_list namelist1[] = {{HL_DISPLAY, "display"},
  ...
  static struct name_list namelist2[] = {
      {HL_SKIPWHITE, "skipwhite"}, {HL_SKIPNL, "skipnl"}, {HL_SKIPEMPTY, "skipempty"}, {0, NULL}};
...
static bool syn_list_keywords(const int id,
  ...
  for (const hashitem_T *hi = ht->ht_array; todo > 0 && !got_int; hi++) {
    ...
    for (keyentry_T *kp = HI2KE(hi); kp != NULL && !got_int; kp = kp->ke_next) {
      if (kp->k_syn.id == id) {
        ...
        if (prev_contained != (kp->flags & HL_CONTAINED) || prev_skipnl != (kp->flags & HL_SKIPNL)
            || prev_skipwhite != (kp->flags & HL_SKIPWHITE)
            || prev_skipempty != (kp->flags & HL_SKIPEMPTY)
            || prev_cont_in_list != kp->k_syn.cont_in_list || prev_next_list != kp->next_list) {
          force_newline = true;
        } else {
                        ...
                        const int conceal_char)
{
  ...
  const char_u *const name_ic = (curwin->w_s->b_syn_ic) ? str_foldcase(
                                    name, (int)STRLEN(name), name_folded, sizeof(name_folded))
                                                        : name;
  ...
  hashtab_T *const ht
      = (curwin->w_s->b_syn_ic) ? &curwin->w_s->b_keywtab_ic : &curwin->w_s->b_keywtab;
  hashitem_T *const hi = hash_lookup(ht, (const char *)kp->keyword, STRLEN(kp->keyword), hash);
  ...
  for (;;) {
    ...
    for (fidx = ARRAY_SIZE(flagtab); --fidx >= 0;) {
      ...
      if (p[i] == NUL
        ...
        if (opt->keyword
            && (flagtab[fidx].flags == HL_DISPLAY || flagtab[fidx].flags == HL_FOLD
...
static void syn_cmd_include(exarg_T *eap, int syncing)
{
  ...
  if (source ? do_source((char *)eap->arg, false, DOSO_NONE) == FAIL
      ...
      if (!eap->skip) {
        ...
        for (kw = keyword_copy; --cnt >= 0; kw += STRLEN(kw) + 1) {
          for (p = vim_strchr(kw, '[');;) {
            ...
            add_keyword(kw, syn_id, syn_opt_arg.flags, syn_opt_arg.cont_in_list,
            ...
            if (p[1] == ']') {
              if (p[2] != NUL) {
                semsg(_("E890: trailing char after ']': %s]%s"), kw, &p[2]);
                goto error;
              }
              kw = p + 1;
              break;  // skip over the "]"
...
static void syn_cmd_sync(exarg_T *eap, int syncing)
{
  ...
  while (!ends_excmd(*arg_start)) {
    ...
    if (STRCMP(key, "CCOMMENT") == 0) {
    ...
    } else if (STRNCMP(key, "LINES", 5) == 0 || STRNCMP(key, "MINLINES", 8) == 0
               || STRNCMP(key, "MAXLINES", 8) == 0 || STRNCMP(key, "LINEBREAKS", 10) == 0) {
      if (key[4] == 'S') {
        arg_end = key + 6;
      } else if (key[0] == 'L') {
        arg_end = key + 11;
      ...
      if (!eap->skip) {
        // store the pattern and compiled regexp program
        curwin->w_s->b_syn_linecont_pat = vim_strnsave(next_arg + 1, arg_end - next_arg - 1);
...
static int get_id_list(char_u **const arg, const int keylen, int16_t **const list, const bool skip)
{
  ...
  for (int round = 1; round <= 2; round++) {
    ...
    do {
      ...
      if (STRCMP(name + 1, "ALLBUT") == 0 || STRCMP(name + 1, "ALL") == 0
          || STRCMP(name + 1, "TOP") == 0 || STRCMP(name + 1, "CONTAINED") == 0) {
        ...
        if (count != 0) {
          semsg(_("E408: %s must be first in contains list"), name + 1);
          failed = true;
          xfree(name);
          break;
        }
...
static int in_id_list(stateitem_T *cur_si, int16_t *list, struct sp_syn *ssp, int contained)
{
  ...
  if (cur_si != NULL && ssp->cont_in_list != NULL && !(cur_si->si_flags & HL_MATCH)) {
    ...
    if (cur_si->si_idx >= 0
        && in_id_list(NULL, ssp->cont_in_list, &(SYN_ITEMS(syn_block)[cur_si->si_idx].sp_syn),
...
static struct subcommand subcommands[]
    = {{"case", syn_cmd_case},       {"clear", syn_cmd_clear},
       {"cluster", syn_cmd_cluster}, {"conceal", syn_cmd_conceal},
       {"enable", syn_cmd_on},       {"foldlevel", syn_cmd_foldlevel},
       {"include", syn_cmd_include}, {"iskeyword", syn_cmd_iskeyword},
       {"keyword", syn_cmd_keyword}, {"list", syn_cmd_list},
       {"manual", syn_cmd_manual},   {"match", syn_cmd_match},
       {"on", syn_cmd_on},           {"off", syn_cmd_off},
       {"region", syn_cmd_region},   {"reset", syn_cmd_reset},
       {"spell", syn_cmd_spell},     {"sync", syn_cmd_sync},
       {"", syn_cmd_list},           {NULL, NULL}};
...
{
  return win->w_s->b_syn_patterns.ga_len != 0 || win->w_s->b_syn_clusters.ga_len != 0
         || win->w_s->b_keywtab.ht_used > 0 || win->w_s->b_keywtab_ic.ht_used > 0;
}
...
void set_context_in_syntax_cmd(expand_T *xp, const char *arg)
{
  ...
  if (*arg != NUL) {
    ...
    if (*p != NUL) {  // Past first word.
      ...
      } else if (STRNICMP(arg, "keyword", p - arg) == 0 || STRNICMP(arg, "region", p - arg) == 0
                 || STRNICMP(arg, "match", p - arg) == 0 || STRNICMP(arg, "list", p - arg) == 0) {
        xp->xp_context = EXPAND_HIGHLIGHT;
      } else {
        xp->xp_context = EXPAND_NOTHING;
      }
  ...
  switch (expand_what) {
    ...
    case EXP_SYNC: {
      static char *sync_args[]
          = {"ccomment", "clear",     "fromstart", "linebreaks=", "linecont", "lines=",
             "match",    "maxlines=", "minlines=", "region",      NULL};
...
int syn_get_foldlevel(win_T *wp, long lnum)
{
  ...
  if (wp->w_s->b_syn_folditems != 0 && !wp->w_s->b_syn_error && !wp->w_s->b_syn_slow) {
    syntax_start(wp, lnum);
...
static const char *highlight_init_light[] = {
    ...
    "Comment      term=bold cterm=NONE ctermfg=DarkBlue ctermbg=NONE gui=NONE guifg=Blue "
    "guibg=NONE",
    "Constant     term=underline cterm=NONE ctermfg=DarkRed ctermbg=NONE gui=NONE guifg=Magenta "
    "guibg=NONE",
    "Special      term=bold cterm=NONE ctermfg=DarkMagenta ctermbg=NONE gui=NONE guifg=#6a5acd "
    "guibg=NONE",
    "Identifier   term=underline cterm=NONE ctermfg=DarkCyan ctermbg=NONE gui=NONE guifg=DarkCyan "
    "guibg=NONE",
    "Statement    term=bold cterm=NONE ctermfg=Brown ctermbg=NONE gui=bold guifg=Brown guibg=NONE",
    "PreProc      term=underline cterm=NONE ctermfg=DarkMagenta ctermbg=NONE gui=NONE "
    "guifg=#6a0dad guibg=NONE",
    "Type         term=underline cterm=NONE ctermfg=DarkGreen ctermbg=NONE gui=bold guifg=SeaGreen "
    "guibg=NONE",
...
static const char *highlight_init_dark[] = {
    ...
    "Constant     term=underline cterm=NONE ctermfg=Magenta ctermbg=NONE gui=NONE guifg=#ffa0a0 "
    "guibg=NONE",
    "Special      term=bold cterm=NONE ctermfg=LightRed ctermbg=NONE gui=NONE guifg=Orange "
    "guibg=NONE",
    "Identifier   term=underline cterm=bold ctermfg=Cyan ctermbg=NONE gui=NONE guifg=#40ffff "
    "guibg=NONE",
    "Statement    term=bold cterm=NONE ctermfg=Yellow ctermbg=NONE gui=bold guifg=#ffff60 "
    "guibg=NONE",
    "PreProc      term=underline cterm=NONE ctermfg=LightBlue ctermbg=NONE gui=NONE guifg=#ff80ff "
    "guibg=NONE",
    "Type         term=underline cterm=NONE ctermfg=LightGreen ctermbg=NONE gui=bold guifg=#60ff60 "
    "guibg=NONE",
...
void init_highlight(bool both, bool reset)
{
  ...
  const char *const *const pp = ((*p_bg == 'l') ? highlight_init_light : highlight_init_dark);
  for (size_t i = 0; pp[i] != NULL; i++) {
    do_highlight(pp[i], reset, true);
  }
  ...
  if (t_colors > 8) {
    do_highlight((*p_bg == 'l' ? "Visual cterm=NONE ctermbg=LightGrey"
...
static int color_numbers_256[28] = {0,  4,  2,  6,   1,  5,   130, 3,   248, 248, 7,  7,   242, 242,
...
void set_hl_group(int id, HlAttrs attrs, Dict(highlight) * dict, int link_id)
{
  ...
  for (int j = 0; cattrs[j].dest; j++) {
    ...
    if (!*cattrs[j].dest || STRCMP(*cattrs[j].dest, name) != 0) {
      xfree(*cattrs[j].dest);
...
void do_highlight(const char *line, const bool forceit, const bool init) FUNC_ATTR_NONNULL_ALL
{
  ...
  if (dolink) {
    ...
    if (ends_excmd((uint8_t)(*from_start)) || ends_excmd((uint8_t)(*to_start))) {
    ...
    if (from_id > 0 && (!init || hlgroup->sg_set == 0)) {
      ...
      } else if (hlgroup->sg_link != to_id || hlgroup->sg_script_ctx.sc_sid != current_sctx.sc_sid
                 || hlgroup->sg_cleared) {
        if (!init) {
  ...
  if (!doclear) {
    while (!ends_excmd((uint8_t)(*linep))) {
      ...
      xfree(key);
      key = (char *)vim_strnsave_up((const char_u *)key_start, linep - key_start);
            ...
            if (is_normal_group) {
              ...
              if (!ui_rgb_attached()) {
                if (color >= 0) {
                  ...
                  // wrong.
                  if (dark != -1 && dark != (*p_bg == 'd') && !option_was_set("bg")) {
...
static int hl_has_settings(int idx, bool check_link)
{
  return HL_TABLE()[idx].sg_cleared == 0
         && (HL_TABLE()[idx].sg_attr != 0 || HL_TABLE()[idx].sg_cterm_fg != 0
             || HL_TABLE()[idx].sg_cterm_bg != 0 || HL_TABLE()[idx].sg_rgb_fg_name != NULL
             || HL_TABLE()[idx].sg_rgb_bg_name != NULL || HL_TABLE()[idx].sg_rgb_sp_name != NULL
...
static bool highlight_list_arg(const int id,
                               bool didh,
                               const int type,
                               int iarg,
                               char *const sarg,
                               const char *const name)
{
  ...
  if (type == LIST_STRING ? (sarg != NULL) : (iarg != 0)) {
    ...
    (void)syn_list_header(didh, (int)(vim_strsize((char_u *)ts) + STRLEN(name) + 1), id, false);
    didh = true;
    if (!got_int) {
      if (*name != NUL) {
...
const char *highlight_has_attr(const int id,
                               const int flag,
                               const int modec) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_PURE
{
  int attr;
...
const char *highlight_color(const int id,
                            ...
                            const int modec) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ALL
{
  static char name[20];
  int n;
  bool fg = false;
...
static bool syn_list_header(const bool did_header,
                            const int outlen,
                            const int id,
                            bool force_newline)
{
...
void highlight_changed(void)
{
  ...
  for (int hlf = 0; hlf < HLF_COUNT; hlf++) {
    ...
    highlight_attr[hlf] = hl_get_ui_attr(hlf, final_id, hlf == HLF_INACTIVE);

    if (highlight_attr[hlf] != highlight_attr_last[hlf]) {
      if (hlf == HLF_MSG) {
        clear_cmdline = true;
      }
      ui_call_hl_group_set(cstr_as_string((char *)hlf_names[hlf]), highlight_attr[hlf]);
...
const char *get_highlight_name_ext(expand_T *xp,
                                   int idx,
                                   bool skip_cleared) FUNC_ATTR_WARN_UNUSED_RESULT
{
  if (idx < 0) {
    return NULL;
  }
...
{
  if (name[0] == '#' && isxdigit(name[1]) && isxdigit(name[2]) && isxdigit(name[3])
      && isxdigit(name[4]) && isxdigit(name[5]) && isxdigit(name[6]) && name[7] == NUL) {
    // rgb hex string
...
int name_to_ctermcolor(const char *name)
{
  ...
  for (i = ARRAY_SIZE(color_names); --i >= 0;) {
    if (off == color_names[i][0] && STRICMP(name + 1, color_names[i] + 1) == 0) {
      break;
    }
  }
  if (i < 0) {
