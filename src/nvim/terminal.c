...
static void emit_termrequest(void **argv)
{
  ...
  term->refcount++;
  apply_autocmds_group(EVENT_TERMREQUEST, NULL, NULL, true, AUGROUP_ALL, buf, curwin->handle, NULL,
                       &DICT_OBJ(data), false);
  term->refcount--;
  xfree(sequence);
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (buf && !is_autocmd_blocked()) {
    ...
    // curwin is NULL when close_buffer() is reached from free_all_mem().
    apply_autocmds_group(EVENT_TERMCLOSE, NULL, NULL, status >= 0, AUGROUP_ALL,
                         buf, curwin == NULL ? 0 : curwin->handle, NULL, &DICT_OBJ(data), false);
