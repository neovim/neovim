...
void terminal_init(void)
{
  ...
  for (int color_index = 0; color_index < 256; color_index++) {
    ...
    map_put(int, int)(color_indexes,
        NVIM_RGB(color.red, color.green, color.blue), color_index + 1);
  }

  VTermColor fg, bg;
  vterm_state_get_default_colors(state, &fg, &bg);
  default_vt_fg = NVIM_RGB(fg.red, fg.green, fg.blue);
  default_vt_bg = NVIM_RGB(bg.red, bg.green, bg.blue);
...
Terminal *terminal_open(TerminalOptions opts)
{
  ...
  for (int i = 0; i < (int)ARRAY_SIZE(rv->colors); i++) {
    ...
    if (color_val == -1) {
      ...
      rv->colors[i] = NVIM_RGB(color.red, color.green, color.blue);
    }
  }
    ...
    int *term_attrs)
{
  ...
  for (int col = 0; col < width; col++) {
    ...
    int vt_fg = NVIM_RGB(cell.fg.red, cell.fg.green, cell.fg.blue);
    int vt_bg = NVIM_RGB(cell.bg.red, cell.bg.green, cell.bg.blue);
