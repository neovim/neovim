      ...
      for (i = 0; i < DB_COUNT; ++i) {
        if ((tp->tp_diffbuf[i] != NULL) && (i != idx)) {
          if (dprev == NULL) {
            ...
            dnext->df_lnum[i] = line1 + (dprev->df_lnum[i] + dprev->df_count[i])
...
static void diff_check_unchanged(tabpage_T *tp, diff_T *dp)
{
  ...
  for (;;) {
    ...
    while (dp->df_count[i_org] > 0) {
      ...
      char_u *line_org
          = vim_strsave(ml_get_buf(tp->tp_diffbuf[i_org], dp->df_lnum[i_org] + off_org, false));
      ...
      for (i_new = i_org + 1; i_new < DB_COUNT; ++i_new) {
        ...
        if (diff_cmp(line_org,
                     ml_get_buf(tp->tp_diffbuf[i_new], dp->df_lnum[i_new] + off_new, false))
            != 0) {
          break;
        }
      }
...
static int diff_write(buf_T *buf, diffin_T *din)
{
  ...
  int r = buf_write(buf, din->din_fname, NULL, (linenr_T)1, buf->b_ml.ml_line_count, NULL, false,
                    false, false, true);
...
static void diff_try_update(diffio_T *dio, int idx_orig, exarg_T *eap)
{
  ...
  if (dio->dio_internal) {
    ...
    if (dio->dio_orig.din_fname == NULL || dio->dio_new.din_fname == NULL
...
static int diff_file_internal(diffio_T *diffio)
{
  ...
  if (xdl_diff(&diffio->dio_orig.din_mmfile, &diffio->dio_new.din_mmfile, &param, &emit_cfg,
               &emit_cb)
      < 0) {
...
static int diff_file(diffio_T *dio)
{
  ...
  if (dio->dio_internal) {
    ...
    const size_t len = (strlen(tmp_orig) + strlen(tmp_new) + strlen(tmp_diff) + STRLEN(p_srr) + 27);
    ...
    vim_snprintf(
        cmd, len, "diff %s%s%s%s%s%s%s%s %s", diff_a_works == kFalse ? "" : "-a ", "",
        (diff_flags & DIFF_IWHITE) ? "-b " : "", (diff_flags & DIFF_IWHITEALL) ? "-w " : "",
        (diff_flags & DIFF_IWHITEEOL) ? "-Z " : "", (diff_flags & DIFF_IBLANK) ? "-B " : "",
        (diff_flags & DIFF_ICASE) ? "-i " : "", tmp_orig, tmp_new);
    append_redir(cmd, len, (char *)p_srr, tmp_diff);
    block_autocmds();  // Avoid ShellCmdPost stuff
    (void)call_shell((char_u *)cmd, kShellOptFilter | kShellOptSilent | kShellOptDoOut, NULL);
    unblock_autocmds();
    xfree(cmd);
    return OK;
  }
...
void ex_diffpatch(exarg_T *eap)
{
  ...
  if (buf_write(curbuf, tmp_orig, NULL, (linenr_T)1, curbuf->b_ml.ml_line_count, NULL, false, false,
                false, true)
      == FAIL) {
    goto theend;
  }
...
#ifdef UNIX
    eval_patch((char *)tmp_orig, (fullname != NULL ? fullname : (char *)eap->arg), (char *)tmp_new);
#else
...
void diff_win_options(win_T *wp, int addbuf)
{
  ...
  set_string_option_direct("fdm", -1, (char_u *)"diff", OPT_LOCAL | OPT_FREE, 0);
  curwin = old_curwin;
  curbuf = curwin->w_buffer;
...
static void diff_read(int idx_orig, int idx_new, diffio_T *dio)
{
  ...
  for (;;) {
    ...
    if ((dp != NULL) && (hunk->lnum_orig <= dp->df_lnum[idx_orig] + dp->df_count[idx_orig])
      ...
      if (off > 0) {
        ...
        dp->df_count[idx_new] += hunk->count_new - hunk->count_orig + dpl->df_lnum[idx_orig]
...
static bool diff_equal_entry(diff_T *dp,
                             int idx1,
                             int idx2) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
    FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  for (int i = 0; i < dp->df_count[idx1]; i++) {
    char_u *line = vim_strsave(ml_get_buf(curtab->tp_diffbuf[idx1], dp->df_lnum[idx1] + i, false));

    int cmp = diff_cmp(line, ml_get_buf(curtab->tp_diffbuf[idx2], dp->df_lnum[idx2] + i, false));
    xfree(line);
...
{
  if ((diff_flags & DIFF_IBLANK) && (*skipwhite(s1) == NUL || *skipwhite(s2) == NUL)) {
    return 0;
  }
  ...
  while (*p1 != NUL && *p2 != NUL) {
    if (((diff_flags & DIFF_IWHITE) && ascii_iswhite(*p1) && ascii_iswhite(*p2))
        || ((diff_flags & DIFF_IWHITEALL) && (ascii_iswhite(*p1) || ascii_iswhite(*p2)))) {
      p1 = skipwhite(p1);
      p2 = skipwhite(p2);
    } else {
      int l;
...
bool diffopt_horizontal(void) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
...
bool diffopt_closeoff(void) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  return (diff_flags & DIFF_CLOSE_OFF) != 0;
}

// Return true if 'diffopt' contains "filler".
bool diffopt_filler(void) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  for (i = 0; i < DB_COUNT; ++i) {
    if ((curtab->tp_diffbuf[i] != NULL) && (i != idx)) {
      ...
      while (line_org[si_org] != NUL) {
        if (((diff_flags & DIFF_IWHITE) && ascii_iswhite(line_org[si_org])
             && ascii_iswhite(line_new[si_new]))
            || ((diff_flags & DIFF_IWHITEALL)
                && (ascii_iswhite(line_org[si_org]) || ascii_iswhite(line_new[si_new])))) {
      ...
      if ((line_org[si_org] != NUL) || (line_new[si_new] != NUL)) {
        ...
        while (ei_org >= *startp && ei_new >= si_new && ei_org >= 0 && ei_new >= 0) {
          if (((diff_flags & DIFF_IWHITE) && ascii_iswhite(line_org[ei_org])
               && ascii_iswhite(line_new[ei_new]))
              || ((diff_flags & DIFF_IWHITEALL)
                  && (ascii_iswhite(line_org[ei_org]) || ascii_iswhite(line_new[ei_new])))) {
...
bool diff_infold(win_T *wp, linenr_T lnum) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
    FUNC_ATTR_NONNULL_ARG(1)
{
  bool other = false;
  diff_T *dp;
...
void ex_diffgetput(exarg_T *eap)
{
  ...
  if (*eap->arg == NUL) {
    ...
    for (idx_other = 0; idx_other < DB_COUNT; ++idx_other) {
      if ((curtab->tp_diffbuf[idx_other] != curbuf) && (curtab->tp_diffbuf[idx_other] != NULL)) {
        if ((eap->cmdidx != CMD_diffput) || MODIFIABLE(curtab->tp_diffbuf[idx_other])) {
          break;
        }
        found_not_ma = true;
      }
    ...
    for (i = idx_other + 1; i < DB_COUNT; ++i) {
      if ((curtab->tp_diffbuf[i] != curbuf) && (curtab->tp_diffbuf[i] != NULL)
          && ((eap->cmdidx != CMD_diffput) || MODIFIABLE(curtab->tp_diffbuf[i]))) {
        emsg(
            _("E101: More than two buffers in diff mode, don't know "
              "which one to use"));
        return;
      }
    }
  ...
  if (eap->addr_count == 0) {
    ...
    if ((eap->cmdidx == CMD_diffget) && (eap->line1 == curbuf->b_ml.ml_line_count)
  ...
  for (dp = curtab->tp_first_diff; dp != NULL;) {
    ...
    if ((dp->df_lnum[idx_cur] + dp->df_count[idx_cur] > eap->line1 + off)
      ...
      if (added != 0) {
        mark_adjust(lnum, lnum + count - 1, (long)MAXLNUM, (long)added, kExtmarkUndo);
...
static linenr_T diff_get_corresponding_line_int(buf_T *buf1, linenr_T lnum1)
{
  ...
  for (dp = curtab->tp_first_diff; dp != NULL; dp = dp->df_next) {
    ...
    if ((dp->df_lnum[idx1] == lnum1) && (dp->df_count[idx1] == 0)
        && (dp->df_lnum[idx2] <= curwin->w_cursor.lnum)
        && ((dp->df_lnum[idx2] + dp->df_count[idx2]) > curwin->w_cursor.lnum)) {
