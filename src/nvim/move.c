...
void 
curs_columns (
    bool may_scroll                 /* when true, may scroll horizontally */
)
{
  int width = 0;
  colnr_T startcol;
  ...
  int extra = curwin_col_off(); /* offset for first screen line */
   ...
   * If scrolling is off, curwin->w_leftcol is assumed to be 0 */
    ...
    int off_right = endcol - curwin->w_leftcol - curwin->w_width + (int)p_siso + 1;
    int new_leftcol;
...
scrolldown (
    long line_count,
    bool byfold              /* true: count a closed fold as one line */
)
{
...
{
  bool can_fill = (curwin->w_topfill
...
void scroll_cursor_top(int min_scroll, bool always)
{
  ...
  int i = diff_check_fill(curwin, curwin->w_cursor.lnum);
  used += i;
  extra += i;
...
void scroll_cursor_bot(int min_scroll, bool set_topbot)
{
  ...
  loff.fill = 0;
  boff.fill = 0;
  int fill_below_window = diff_check_fill(curwin, curwin->w_botline)
...
///
void scroll_cursor_halfway(bool atend)
{
  int above = 0;
  linenr_T topline;
  int topfill = 0;
