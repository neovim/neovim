...
void curs_columns(win_T *wp, int may_scroll)
{
  ...
  if (width1 <= 0) {
    ...
    if (wp->w_p_cole > 0) {
      int coff = extconceal_off_before(wp, wp->w_cursor.lnum, wp->w_cursor.col, NULL, NULL);
                       ...
                       bool local)
{
  ...
  if ((local || visible_row) && existing_row) {
    ...
    if (is_folded) {
      ...
      // so screenpos() reflects the reflowed (displayed) position, not the pre-conceal one. Columns
      // before w_skipcol are not visible and adjust_plines_for_skipcol() above already counted them
      // in rows, so discount only hidden cells after it: the ones before would be subtracted twice.
      colnr_T coff = (colnr_T)extconceal_off_before(wp, pos->lnum, pos->col, NULL, NULL);
      if (coff > 0 && lnum == wp->w_topline && wp->w_skipcol > 0) {
        colnr_T const skipcol = vcol2col(wp, pos->lnum, wp->w_skipcol, NULL);
        coff -= MIN(coff, (colnr_T)extconceal_off_before(wp, pos->lnum, skipcol, NULL, NULL));
      }
      if (coff > 0) {
        scol -= MIN(scol, coff);
        ccol -= MIN(ccol, coff);
