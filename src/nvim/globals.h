...
EXTERN bool cmdline_star INIT(= false);    ///< cmdline is crypted
EXTERN bool exec_from_reg INIT(= false);   ///< executing register
EXTERN bool screen_cleared INIT(= false);  ///< screen has been cleared
...
EXTERN bool compl_interrupted INIT(= false);
...
EXTERN int msg_col;
EXTERN int msg_row;
EXTERN int msg_scrolled;  // Number of screen lines that windows have scrolled
                          // because of printing messages
...
EXTERN bool msg_scroll INIT(= false);        ///< msg_start() will scroll
...
EXTERN bool emsg_off INIT(= false);          ///< don't display errors for now,
                                             ///< unless 'debug' is set
...
EXTERN bool need_clr_eos INIT(= false);      ///< need to clear text bef
                                             ///< displaying a message.
EXTERN bool emsg_skip INIT(= false);         ///< don't display errors for
                                             ///< expression that is skipped
EXTERN bool emsg_severe INIT(= false);       ///< use message of next of seve
...
EXTERN bool did_emsg_syntax;                 ///< did_emsg set because of a
                                             ///< syntax error
EXTERN bool called_emsg;                     ///< always set by emsg()
...
EXTERN bool did_wait_return INIT(= false);   ///< wait_return() was used
                                             ///< nothing written since then
EXTERN bool need_maketitle INIT(= true);     ///< call maketitle() soon
EXTERN bool quit_more INIT(= false);         ///< 'q' hit at "--more--" msg
#if defined(UNIX) || defined(MACOS_X)
EXTERN bool newline_on_exit INIT(= false);   ///< did msg in altern. screen
EXTERN int intr_char INIT(= 0);              ///< extra interrupt character
#endif
EXTERN bool ex_keep_indent INIT(= false);    ///< getexmodeline(): keep indent
...
EXTERN bool msg_no_more INIT(= false);       ///< don't use more prompt,
                                             ///< truncate messages
...
EXTERN bool no_smartcase INIT(= false);           ///< don't use 'smartcase'
                                                  ///< once
...
EXTERN bool did_check_timestamps INIT(= false);   ///< did check timestamps
                                                  ///< recently
EXTERN bool no_check_timestamps INIT(= false);    ///< Don't check timestamps
...
EXTERN bool autocmd_no_enter INIT(= false);  ///< *Enter autocmds disabled
EXTERN bool autocmd_no_leave INIT(= false);  ///< *Leave autocmds disabled
EXTERN bool modified_was_set;                ///< did ":set modified"
EXTERN bool did_filetype INIT(= false);      ///< FileType event found
EXTERN bool keep_filetype INIT(= false);     ///< value for did_filetype when
                                             ///< starting to execute
                                             ///< autocommands
...
EXTERN bool aucmd_win_used INIT(= false);  ///< aucmd_win is being used
...
EXTERN bool mf_dont_release INIT(= false);  ///< don't release blocks

// List of files being edited (global argument list). curwin->w_alist points
// to this when the window is using the global argument list.
EXTERN alist_T global_alist;             ///< global argument list
EXTERN int max_alist_id INIT(= 0);       ///< the previous argument list id
EXTERN bool arg_had_last INIT(= false);  ///< accessed last file in global_alist
...
// true when planning to exit Vim. Might still keep on running if there is a
// changed buffer. volatile because it is used in signal handler deathtrap().
EXTERN volatile bool full_screen INIT(= false);
// true when doing full-screen output otherwise only writing some messages

EXTERN bool restricted INIT(= false);
// true when started in restricted mode (-Z)
EXTERN int secure INIT(= 0);
// true when only "safe" commands are allowed, e.g. when sourcing config file
// in current directory

EXTERN bool textlock INIT(= false);
// true when changing text and jumping
// to another window or buffer is not allowed

EXTERN bool curbuf_lock INIT(= false);
// true when the current buffer can't be
// changed. Used for FileChangedRO
EXTERN bool allbuf_lock INIT(= false);
// true when no buffer name can be changed, no buffer can be deleted and
// current directory can't be changed. Used for SwapExists et al
EXTERN bool sandbox INIT(= false);
// true when evaluating an expression in a "sandbox"
...
EXTERN bool VIsual_active INIT(= false);  ///< whether Visual mode is active
EXTERN bool VIsual_select INIT(= false);  ///< whether Select mode is active
EXTERN bool VIsual_reselect;  ///< whether to restart the selection
                              ///< after a Select mode mapping or menu

EXTERN int VIsual_mode INIT(= 'v');
/* type of Visual mode */

EXTERN bool redo_VIsual_busy INIT(= false);  ///< true when redoing Visual
...
EXTERN bool enc_latin1like INIT(= true);  ///< 'encoding' is latin1 comp.
EXTERN bool has_mbyte INIT(= false);      ///< any multi-byte encoding
...
EXTERN bool Recording INIT(= false);        ///< true when recording into a reg.
EXTERN bool Exec_reg INIT(= false);         ///< true when executing a register
EXTERN bool no_mapping INIT(= false);       ///< currently no mapping allowed
EXTERN bool no_zero_mapping INIT(= false);  ///< mapping zero not allowed
EXTERN bool allow_keys INIT(= false);       ///< allow key codes when no_mapping
                                            ///< is set
EXTERN bool no_u_sync INIT(= false);        ///< Don't call u_sync()
EXTERN int u_sync_once INIT(= 0);           ///< Call u_sync() once when
...
EXTERN bool no_abbr INIT(= true);        ///< when no abbreviations loaded
...
EXTERN bool readonlymode INIT(= false);  ///< "view"
EXTERN bool recoverymode INIT(= false);  ///< "-r" option

// typeahead buffer
EXTERN typebuf_T typebuf INIT(= { NULL, NULL, 0, 0, 0, 0, 0, 0, 0 });

EXTERN bool ex_normal_busy INIT(= false);    ///< recursiveness of ex_normal()
EXTERN bool ex_normal_lock INIT(= false);    ///< forbid use of ex_normal()
EXTERN bool ignore_script INIT(= false);     ///< ignore script input
EXTERN bool stop_insert_mode;                ///< for ":stopinsert" and
                                             ///< 'insertmode'
EXTERN bool KeyTyped;                        ///< if user typed current char
EXTERN bool KeyStuffed;                      ///< if current char from stuffbuf
...
EXTERN volatile bool got_int INIT(= false);  ///< set when interrupt
                                             ///< signal occurred
EXTERN bool bangredo INIT(= false);          ///< set to bool with ! command
...
EXTERN bool did_swapwrite_msg INIT(= false);  ///< set after swap write errmsg
EXTERN bool undo_off INIT(= false);           ///< undo switched off for now
EXTERN bool global_busy INIT(= false);        ///< set when :global is executing
...
EXTERN bool need_cursor_line_redraw INIT(= false);
