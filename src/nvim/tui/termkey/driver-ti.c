...
#define KDEF(x) kTermKey_##x, #x
  ...
  { KDEF(down),      TERMKEY_TYPE_KEYSYM, TERMKEY_SYM_DOWN,      0 },
  ...
  { KDEF(next),      TERMKEY_TYPE_KEYSYM, TERMKEY_SYM_PAGEDOWN,  0 },  // Not quite, but it's the best we can do
  ...
  { KDEF(previous),  TERMKEY_TYPE_KEYSYM, TERMKEY_SYM_PAGEUP,    0 },  // Not quite, but it's the best we can do
  ...
  { KDEF(up),        TERMKEY_TYPE_KEYSYM, TERMKEY_SYM_UP,        0 },
  { 0, NULL,         0,                   0,                     0 },
};
...
static bool try_load_terminfo_key(TermKeyTI *ti, int key, bool shift, const char *name, struct keyinfo *info)
{
  const char *value = NULL;

  if (ti->ti) {
    if (key < kTermKeyCount){
      value = ti->ti->keys[key][shift ? 1 : 0];
    } else {
      // TODO: NOT LIKE THIS
      assert(!shift);
      value = ti->ti->f_keys[key-kTermCount];
    }
  }
...
static int load_terminfo(TermKeyTI *ti)
{
  ...
  for (i = 0; funcs[i].funcname; i++) {
    ...
    if (!try_load_terminfo_key(ti, (int)funcs[i].ti_key, false, name, &(struct keyinfo){
      .type = funcs[i].type,
      .sym = funcs[i].sym,
    ...
    try_load_terminfo_key(ti, (int)funcs[i].ti_key, true, name, &(struct keyinfo){
      .type = funcs[i].type,
      .sym = funcs[i].sym,
  ...
  for (i = 1; i <= kTerminfoFuncKeyMax; i++) {
    ...
    if (!try_load_terminfo_key(ti, kTermKeyCount + (i-1), false, name, &(struct keyinfo){
