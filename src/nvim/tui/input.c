...
static void tinput_wait_enqueue(void **argv)
{
  ...
  if (rbuffer_size(input->key_buffer) == 0 && input->paste == 3) {
    ...
    multiqueue_put(main_loop.events, tinput_paste_event, 3, copy.data, copy.size,
...
{
  if (rbuffer_size(input->key_buffer) > rbuffer_capacity(input->key_buffer) - 0xff) {
...
static void tk_getkeys(TermInput *input, bool force)
{
  ...
  if (input->ttimeout && input->ttimeoutlen >= 0) {
    ...
    time_watcher_stop(&input->timer_handle);
    time_watcher_start(&input->timer_handle, tinput_timer_cb, (uint64_t)input->ttimeoutlen, 0);
  } else {
  ...
  if (buf_size > 5
    ...
    return kComplete;
             ...
             && (!rbuffer_cmp(input->read_stream.buffer, START_PASTE, buf_size)
                 || !rbuffer_cmp(input->read_stream.buffer, END_PASTE, buf_size))) {
...
static bool handle_forced_escape(TermInput *input)
{
  if (rbuffer_size(input->read_stream.buffer) > 1
    ...
    size_t rcnt;
    termkey_push_bytes(input->tk, rbuffer_read_ptr(input->read_stream.buffer, &rcnt), 1);
...
static void set_bg_deferred(void **argv)
{
  ...
  if (!option_was_set("bg") && !strequal((char *)p_bg, bgvalue)) {
    ...
    if (starting) {
      ...
      do_cmdline_cmd((bgvalue[0] == 'l') ? "autocmd VimEnter * ++once ++nested set bg=light"
...
static HandleState handle_background_color(TermInput *input)
{
  ...
  bool done = false;
  bool bad = false;
  if (buf_size >= 9 && !rbuffer_cmp(input->read_stream.buffer, "\x1b]11;rgb:", 9)) {
    header_size = 9;
    num_components = 3;
  } else if (buf_size >= 10 && !rbuffer_cmp(input->read_stream.buffer, "\x1b]11;rgba:", 10)) {
    header_size = 10;
    num_components = 4;
  } else if (buf_size < 10 && !rbuffer_cmp(input->read_stream.buffer, "\x1b]11;rgba", buf_size)) {
  ...
  if (done && !bad && rgb_max[0] && rgb_max[1] && rgb_max[2]) {
    ...
    loop_schedule_deferred(&main_loop, event_create(set_bg_deferred, 1, bgvalue));
    input->waiting_for_bg_response = 0;
  } else if (!done && !bad) {
    // An incomplete sequence was found, waiting for the next input.
    return kIncomplete;
...
static void tinput_read_cb(Stream *stream, RBuffer *buf, size_t count_, void *data, bool eof)
{
  ...
  if (rbuffer_size(input->read_stream.buffer)) {
    ...
    // the next input.
    long ms = input->ttimeout ? (input->ttimeoutlen >= 0 ? input->ttimeoutlen : 0) : 0;
    // Stop the current timer if already running
    time_watcher_stop(&input->timer_handle);
