                          ...
                          VTermStateFields *statefields)
{
  ...
  while (old_row >= 0) {
    ...
    if (new_row_start < 0    // we'd fall off the top
      ...
      memmove(&new_buffer[downwards * new_cols], &new_buffer[0],
              (unsigned long)rowcount * new_cols * sizeof(ScreenCell));
      memmove(&new_lineinfo[downwards],          &new_lineinfo[0],
              rowcount * sizeof(new_lineinfo[0]));
  ...
  if (new_row >= 0) {
    ...
    memmove(&new_buffer[0], &new_buffer[(new_row + 1) * new_cols],
            (unsigned long)moverows * new_cols * sizeof(ScreenCell));
...
void vterm_screen_set_default_colors(VTermScreen *screen, const VTermColor *default_fg,
