...
void conceal_check_cursor_line(void)
{
  ...
  // the lines below it, so redraw the whole window (see extconceal_line_changes_height());
  // whole-line conceal ('conceal_lines') is covered by decor_conceal_line().
  if (decor_conceal_line(curwin, curwin->w_cursor.lnum - 1, true)
      || extconceal_line_changes_height(curwin, curwin->w_cursor.lnum)) {
