...
for _, cmd in ipairs(defs) do
  if bit.band(cmd.flags, flags.RANGE) == flags.RANGE then
    assert(cmd.addr_type ~= 'ADDR_NONE', string.format('ex_cmds.lua:%s: Using RANGE with ADDR_NONE\n', cmd.command))
  else
...
for i = byte_a, byte_z do
  ...
  for j = byte_a, byte_z do
    ...
    cmdidxs2_out = cmdidxs2_out
      .. ((cmdidxs2[c1] and cmdidxs2[c1][c2]) and string.format('%3d', cmdidxs2[c1][c2] - cmdidxs1[c1]) or '  0')
      .. ','
  end
  cmdidxs2_out = cmdidxs2_out .. ' },\n'
end
