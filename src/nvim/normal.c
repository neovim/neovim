...
static bool normal_handle_special_visual_command(NormalState *s)
{
  ...
  if (km_stopsel && (nv_cmds[s->idx].cmd_flags & NV_STS)
...
static bool normal_need_additional_char(NormalState *s)
{
  ...
  return flags & NV_NCH
         && (
             ...
             // 'q' without a pending operator, recording or executing a
             // register, needs to be followed by a second char, examples:
             // - qc => record using register c
             // - q: => open command-line window
             || (cmdchar == 'q' && !pending_op && reg_recording == 0
                 && reg_executing == 0)
             // 'a' or 'i' after an operator is a text object, examples:
             // - ciw => change inside word
             // - da( => delete parenthesis and everything inside.
             // Also, don't do anything when these keys are received in visual
             // mode so just get another char.
             //
...
static bool normal_need_redraw_mode_message(NormalState *s)
{
  return (
      ...
      && s->oa.regname == 0 && !(s->ca.retval & CA_COMMAND_BUSY)
      && stuff_empty() && typebuf_typed() && emsg_silent == 0
      && !did_wait_return && s->oa.op_type == OP_NOP);
}
  ...
  if (cp != NULL) {
    ...
    *cp = plain_vgetc();
    ...
    if (!lit) {
      ...
      if (*cp == Ctrl_K
          && ((nv_cmds[s->idx].cmd_flags & NV_LANG) || cp == &s->ca.extra_char)
...
normal_end:
  ...
  if (s->oa.op_type == OP_NOP
      ...
      && !(s->ca.retval & CA_COMMAND_BUSY) && stuff_empty()
      && s->oa.regname == 0) {
...
static int normal_execute(VimState *state, int key)
{
  ...
  if (restart_edit == 0) {
    s->old_mapped_len = 0;
  } else if (s->old_mapped_len
             || (VIsual_active && s->mapped_len == 0 && typebuf_maplen() > 0)) {
    s->old_mapped_len = typebuf_maplen();
  }
  ...
  if (VIsual_active && VIsual_select
      && (vim_isprintc(s->c) || s->c == NL || s->c == CAR
          || s->c == K_KENTER)) {
...
void do_pending_operator(cmdarg_T *cap, int old_col, bool gui_yank)
{
  ...
  if ((finish_op || VIsual_active) && oap->op_type != OP_NOP) {
    ...
    if ((redo_yank || oap->op_type != OP_YANK)
        ...
        && cap->cmdchar != 'D' && oap->op_type != OP_FOLD
        && oap->op_type != OP_FOLDOPEN && oap->op_type != OP_FOLDOPENREC
        && oap->op_type != OP_FOLDCLOSE && oap->op_type != OP_FOLDCLOSEREC
        && oap->op_type != OP_FOLDDEL && oap->op_type != OP_FOLDDELREC) {
    ...
    if (VIsual_active || redo_VIsual_busy) {
      ...
      if ((redo_yank || oap->op_type != OP_YANK) && oap->op_type != OP_COLON
          ...
          && oap->op_type != OP_FOLDDELREC && oap->motion_force == NUL) {
    ...
    if (oap->is_VIsual
        && (oap->empty || !MODIFIABLE(curbuf) || oap->op_type == OP_FOLD)) {
      curwin->w_p_lbr = lbr_saved;
      redraw_curbuf_later(INVERTED);
    }
     ...
     */
    if (oap->motion_type == kMTCharWise && oap->inclusive == false
        && !(cap->retval & CA_NO_ADJ_OP_END) && oap->end.col == 0
        && (!oap->is_VIsual || *p_sel == 'o') && oap->line_count > 1) {
    ...
    switch (oap->op_type) {
      ...
      case OP_JOIN:
        ...
        if (curwin->w_cursor.lnum + oap->line_count - 1
          ...
          do_join((size_t)oap->line_count, oap->op_type == OP_JOIN, true, true,
                  true);
          auto_format(false, true);
        }
        break;
      ...
      case OP_COLON:
        ...
        if (oap->op_type == OP_INDENT && *get_equalprg() == NUL) {
          ...
          op_reindent(
              oap, *curbuf->b_p_inde != NUL ? get_expr_indent : get_c_indent);
          break;
        }
...
static void op_function(const oparg_T *oap) FUNC_ATTR_NONNULL_ALL
{
  ...
  if (*p_opfunc == NUL)
    ...
    argv[0].vval.v_string = (char_u *)(((const char *const[]){
        [kMTBlockWise] = "block",
        [kMTLineWise] = "line",
        [kMTCharWise] = "char",
    })[oap->motion_type]);
 ...
 */
bool do_mouse(oparg_T *oap, /* operator argument, can be NULL */
...
)
{
          ...
          typval_T argv[] = {
              {
                  ...
                  .v_type = VAR_NUMBER,
                  .vval = {.v_number
                           = (varnumber_T)tab_page_click_defs[mouse_col].tabnr},
              },
              {
                  .v_lock = VAR_FIXED,
                  .v_type = VAR_NUMBER,
              ...
              },
              {
                  ...
                  .v_type = VAR_STRING,
                  .vval = {.v_string
                           = (char_u *)(which_button == MOUSE_LEFT     ? "l"
                                        : which_button == MOUSE_RIGHT  ? "r"
              ...
              },
              {
                  ...
                  .vval
                  = {.v_string
                     = (char_u[]){(char_u)(mod_mask & MOD_MASK_SHIFT ? 's'
                                                                     : ' '),
   ...
   * JUMP!
   */
  jump_flags = jump_to_mouse(jump_flags, oap == NULL ? NULL : &(oap->inclusive),
                             which_button);
 ...
 */
static void prep_redo(int regname,
                      long num,
                      int cmd1,
                      int cmd2,
                      int cmd3,
...
bool add_to_showcmd(int c)
{
  char_u *p;
  int i;
  static int ignore[] = {K_IGNORE,        K_LEFTMOUSE,
                         K_LEFTDRAG,      K_LEFTRELEASE,
...
)
{
   ...
   * and "~" causes trouble. */
  assert(len <= INT_MAX);
  sprintf((char *)pat, vim_iswordp(ptr) ? "\\V\\<%.*s\\>" : "\\V%.*s", (int)len,
          ptr);
  ...
  switch (nchar) {
    ...
    case 'D':
      if (foldManualAllowed(false)) {
        if (VIsual_active) {
          ...
          deleteFold(curwin, curwin->w_cursor.lnum, curwin->w_cursor.lnum,
                     nchar == 'D', false);
        }
      }
      break;
    ...
    case 'E':
      if (foldmethodIsManual(curwin)) {
        ...
        deleteFold(curwin, (linenr_T)1, curbuf->b_ml.ml_line_count, true,
                   false);
      } else {
  ...
  if (cap->cmdchar == 'L') {
    ...
    if (cap->cmdchar == 'M') {
      ...
      for (n = 0; curwin->w_topline + n < curbuf->b_ml.ml_line_count; n++) {
        ...
        if (n > 0
            && used + diff_check_fill(curwin, curwin->w_topline + n) / 2
                   >= half) {
          --n;
          break;
        }
 ...
 */
static int normal_search(cmdarg_T *cap,
                         int dir,
                         char_u *pat,
...
static void nv_brackets(cmdarg_T *cap)
{
   ...
   */
    ...
    (void)do_mouse(cap->oap, cap->nchar,
                   (cap->cmdchar == ']') ? FORWARD : BACKWARD, cap->count1,
                   PUT_FIXINDENT);
  }
  /*
   ...
   */
    ...
    if (foldMoveTo(false, cap->cmdchar == ']' ? FORWARD : BACKWARD, cap->count1)
        == false)
      clearopbeep(cap->oap);
  }
  /*
    ...
    if (cap->count0 > 0 && resel_VIsual_mode != NUL) {
      ...
      if (VIsual_mode == 'v') {
        if (resel_VIsual_line_count <= 1) {
          ...
          curwin->w_curswant
              = (curwin->w_virtcol + resel_VIsual_vcol * (int)cap->count0 - 1);
        } else
 ...
 */
static void invoke_edit(cmdarg_T *cap,
...
static void nv_put_opt(cmdarg_T *cap, bool fix_indent)
{
  ...
  if (cap->oap->op_type != OP_NOP) {
    ...
    if (VIsual_active) {
      ...
      bool clipoverwrite
          = (regname == '+' || regname == '*') && (cb_flags & CB_UNNAMEDMASK);
