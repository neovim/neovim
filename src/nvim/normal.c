  ...
  /* Handle double clicks, unless on status line */
    ...
    if ((mod_mask & MOD_MASK_MULTI_CLICK) == MOD_MASK_2CLICK) {
      ...
      if (is_click) {
         ...
         * {}, [], #if/#endif, etc. block. */
        end_visual = curwin->w_cursor;
        while (gc = gchar_pos(&end_visual), isblank(gc))
          inc(&end_visual);
        if (oap != NULL)
          oap->motion_type = MCHAR;
        if (oap != NULL
...
int find_ident_at_pos(win_T *wp, linenr_T lnum, colnr_T startcol, char_u **string, int find_type)
{
  ...
  for (i = (find_type & FIND_IDENT) ? 0 : 1; i < 2; ++i) {
    ...
    if (has_mbyte) {
      ...
      while (ptr[col] != NUL
             && (i == 0 ? !vim_iswordc(ptr[col]) : isblank(ptr[col]))
             )
        ++col;
  ...
  case '_':
    ...
    if (cursor_down(cap->count1 - 1,
      ...
      while (curwin->w_cursor.col > 0
             && isblank(ptr[curwin->w_cursor.col]))
        --curwin->w_cursor.col;
      curwin->w_set_curswant = true;
      adjust_for_sel(cap);
    }
