...
static colnr_T nv_scol_at_cursor(void)
{
  ...
  if (!extconceal_line_has_scol_shift(curwin, curwin->w_cursor.lnum)) {
    return curwin->w_virtcol - curwin->w_cursor.coladd;
  }
  colnr_T vcol;
  extconceal_off_before(curwin, curwin->w_cursor.lnum, curwin->w_cursor.col, &vcol, NULL);
...
void nv_g_home_m_cmd(cmdarg_T *cap)
{
  ...
  if (use_scol && wrapped) {
    target = scol2vcol(curwin, curwin->w_cursor.lnum, (colnr_T)i);
  }
...
static void nv_g_dollar_cmd(cmdarg_T *cap)
{
  ...
  if (curwin->w_p_wrap && curwin->w_view_width != 0) {
    ...
    if (cap->count1 == 1) {
      ...
      coladvance(curwin, use_scol ? scol2vcol(curwin, curwin->w_cursor.lnum, (colnr_T)i)
                                  : (colnr_T)i);
