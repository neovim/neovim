...
typedef int (*cfunc_T)(int argcount,
                       typval_T *argvars,
                       typval_T *rettv,
                       void *state);  // NOLINT
...
{
  ListLog *ret
      = xmalloc(offsetof(ListLog, entries) + size * sizeof(ret->entries[0]));
  ret->size = 0;
  ret->capacity = size;
  ret->next = NULL;
...
static inline void tv_list_set_ret(typval_T *const tv,
                                   list_T *const l) REAL_FATTR_ALWAYS_INLINE
...
static inline void tv_list_set_copyid(list_T *const l,
                                      const int copyid) FUNC_ATTR_NONNULL_ALL
{
  l->lv_copyID = copyid;
}
...
static inline void tv_dict_set_ret(typval_T *const tv,
                                   dict_T *const d) REAL_FATTR_ALWAYS_INLINE
    REAL_FATTR_NONNULL_ARG(1);
...
#define _TV_LIST_ITER_MOD(modifier, l, li, code)                               \
  do {                                                                         \
    ...
    if (l_ != NULL) {                                                          \
      for (modifier listitem_T *li = l_->lv_first; li != NULL;                 \
           li = li->li_next) {                                                 \
        code                                                                   \
      }                                                                        \
    }                                                                          \
