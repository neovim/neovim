...
{
  ListLog *ret = xmalloc(offsetof(ListLog, entries) + size * sizeof(ret->entries[0]));
  ret->size = 0;
  ret->capacity = size;
  ret->next = NULL;
...
static inline void list_log(const list_T *const l,
                            ...
                            const char *const action) REAL_FATTR_ALWAYS_INLINE;
...
static inline void tv_list_ref(list_T *const l) REAL_FATTR_ALWAYS_INLINE;
...
static inline void tv_list_set_ret(typval_T *const tv, list_T *const l) REAL_FATTR_ALWAYS_INLINE
...
static inline int tv_list_len(const list_T *const l) REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT;
...
static inline int tv_list_uidx(const list_T *const l,
                               int n) REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT;
...
static inline void tv_dict_set_ret(typval_T *const tv, dict_T *const d) REAL_FATTR_ALWAYS_INLINE
    REAL_FATTR_NONNULL_ARG(1);
...
static inline long tv_dict_len(const dict_T *const d) REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT;
...
static inline void tv_blob_set_ret(typval_T *const tv, blob_T *const b) REAL_FATTR_ALWAYS_INLINE
    REAL_FATTR_NONNULL_ARG(1);
...
static inline int tv_blob_len(const blob_T *const b) REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT;
...
static inline void tv_blob_set(blob_T *const b,
                               int idx,
                               char_u c) REAL_FATTR_ALWAYS_INLINE REAL_FATTR_NONNULL_ALL;
...
#define _TV_LIST_ITER_MOD(modifier, l, li, code)                                                   \
  do {                                                                                             \
    ...
    if (l_ != NULL) {                                                                              \
      for (modifier listitem_T *li = l_->lv_first; li != NULL; li = li->li_next) {                 \
        code                                                                                       \
      }                                                                                            \
    }                                                                                              \
  } while (0)
...
static inline DictWatcher *tv_dict_watcher_node_data(QUEUE *q) REAL_FATTR_NONNULL_ALL
    REAL_FATTR_NONNULL_RET REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT REAL_FATTR_ALWAYS_INLINE;
