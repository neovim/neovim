...
static inline size_t tv_strlen(const typval_T *const tv)
    REAL_FATTR_ALWAYS_INLINE REAL_FATTR_PURE REAL_FATTR_WARN_UNUSED_RESULT REAL_FATTR_NONNULL_ALL;
...
#define _TYPVAL_ENCODE_DO_CHECK_SELF_REFERENCE(val, copyID_attr, copyID, conv_type)                \
  do {                                                                                             \
    const int te_csr_ret = _TYPVAL_ENCODE_CHECK_SELF_REFERENCE(                                    \
        TYPVAL_ENCODE_FIRST_ARG_NAME, (val), &(val)->copyID_attr, mpstack, copyID, conv_type,      \
        objname);                                                                                  \
