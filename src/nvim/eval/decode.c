...
static inline void create_special_dict(typval_T *const rettv,
                                       ...
                                       typval_T val) FUNC_ATTR_NONNULL_ALL
{
...
static inline int json_decoder_pop(ValuesStackItem obj,
                                   ...
                                   bool *const didcomma,
                                   bool *const didcolon) FUNC_ATTR_NONNULL_ALL
{
  ...
  if (last_container.container.v_type == VAR_LIST) {
    if (tv_list_len(last_container.container.vval.v_list) != 0 && !obj.didcomma) {
    ...
    if (!obj.didcolon) {
      semsg(_("E474: Expected colon before dictionary value: %s"), val_location);
      tv_clear(&obj.val);
      return FAIL;
    }
    ...
    if (last_container.special_val == NULL
        && (obj.is_special_string || obj.val.vval.v_string == NULL || *obj.val.vval.v_string == NUL
...
#define POP(obj_tv, is_sp_string)                                                                  \
  do {                                                                                             \
    if (json_decoder_pop(OBJ(obj_tv, is_sp_string, *didcomma, *didcolon), stack, container_stack,  \
...
list_T *decode_create_map_special_dict(typval_T *const ret_tv,
                                       const ptrdiff_t len) FUNC_ATTR_NONNULL_ALL
{
  list_T *const list = tv_list_alloc(len);
  tv_list_ref(list);
  create_special_dict(ret_tv, kMPMap,
                      ((typval_T){
                          .v_type = VAR_LIST,
                          .v_lock = VAR_UNLOCKED,
                          .vval = {.v_list = list},
...
typval_T decode_string(const char *const s,
                       ...
                       const bool binary,
                       const bool s_allocated) FUNC_ATTR_WARN_UNUSED_RESULT
{
  assert(s != NULL || len == 0);
  const bool really_hasnul
      = (hasnul == kNone ? ((s != NULL) && (memchr(s, NUL, len) != NULL)) : (bool)hasnul);
  if (really_hasnul) {
    typval_T tv;
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_ALWAYS_INLINE
{
  ...
  while (p < e && *p != '"') {
    if (*p == '\\') {
      p++;
      if (p == e) {
        semsg(_("E474: Unfinished escape sequence: %.*s"), (int)buf_len, buf);
        goto parse_json_string_fail;
      }
      switch (*p) {
        case 'u':
          if (p + 4 >= e) {
            semsg(_("E474: Unfinished unicode escape sequence: %.*s"), (int)buf_len, buf);
            goto parse_json_string_fail;
          } else if (!ascii_isxdigit(p[1]) || !ascii_isxdigit(p[2]) || !ascii_isxdigit(p[3])
                     || !ascii_isxdigit(p[4])) {
            semsg(_("E474: Expected four hex digits after \\u: %.*s"), LENP(p - 1, e));
            goto parse_json_string_fail;
          }
      ...
      if (ch >= 0x80 && p_byte == ch && !(ch == 0xC3 && p + 1 < e && (uint8_t)p[1] == 0x83)) {
        ...
        semsg(_("E474: Only UTF-8 code points up to U+10FFFF "
                "are allowed to appear unescaped: %.*s"),
              LENP(p, e));
        goto parse_json_string_fail;
      }
    ...
    if (*t == '\\') {
      ...
      switch (*t) {
        case 'u': {
          ...
          t += 4;
          uvarnumber_T ch;
          vim_str2nr((char_u *)ubuf, NULL, NULL, STR2NR_HEX | STR2NR_FORCE, NULL, &ch, 4, true);
          if (ch == 0) {
            hasnul = true;
          }
          if (SURROGATE_HI_START <= ch && ch <= SURROGATE_HI_END) {
            ...
            const int full_char
                = ((int)(ch - SURROGATE_LO_START) + ((fst_in_pair - SURROGATE_HI_START) << 10)
...
parse_json_number_check:
  ...
  if (json_decoder_pop(OBJ(tv, false, *didcomma, *didcolon), stack, container_stack, &p,
...
int json_decode_string(const char *const buf,
                       const size_t buf_len,
                       typval_T *const rettv) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  const char *p = buf;
  const char *const e = buf + buf_len;
    ...
    switch (*p) {
      ...
      case ']': {
        ...
        if (*p == '}' && last_container.container.v_type != VAR_DICT) {
          ...
          semsg(_("E474: Closing dictionary with square bracket: %.*s"), LENP(p, e));
          goto json_decode_string_fail;
        } else if (didcomma) {
        ...
        if (kv_size(stack) == 1) {
          ...
          if (json_decoder_pop(kv_pop(stack), &stack, &container_stack, &p, &next_map_special,
                               &didcomma, &didcolon)
              == FAIL) {
            goto json_decode_string_fail;
          }
        ...
        if (didcomma) {
          ...
          goto json_decode_string_fail;
                       ...
                       ? (last_container.container.v_type == VAR_DICT
                              ? (DICT_LEN(last_container.container.vval.v_dict) == 0)
                              : (tv_list_len(last_container.container.vval.v_list) == 0))
      ...
      case 'f':
        ...
        break;
      case '"':
        if (parse_json_string(buf, buf_len, &p, &stack, &container_stack, &next_map_special,
                              &didcomma, &didcolon)
            == FAIL) {
      ...
      case '8':
      case '9':
        if (parse_json_number(buf, buf_len, &p, &stack, &container_stack, &next_map_special,
                              &didcomma, &didcolon)
            == FAIL) {
          // Error message was already given
          goto json_decode_string_fail;
  ...
  switch (mobj.type) {
    ...
    case MSGPACK_OBJECT_BOOLEAN:
      *rettv = (typval_T){
          .v_type = VAR_BOOL,
          .v_lock = VAR_UNLOCKED,
          .vval = {.v_bool = mobj.via.boolean ? kBoolVarTrue : kBoolVarFalse},
      };
      break;
    case MSGPACK_OBJECT_POSITIVE_INTEGER:
      if (mobj.via.u64 <= VARNUMBER_MAX) {
        *rettv = (typval_T){
        ...
        tv_list_ref(list);
        create_special_dict(rettv, kMPInteger,
                            ((typval_T){
                                .v_type = VAR_LIST,
                                .v_lock = VAR_UNLOCKED,
                                .vval = {.v_list = list},
    ...
    case MSGPACK_OBJECT_ARRAY: {
      ...
      *rettv = (typval_T){
      ...
      for (size_t i = 0; i < mobj.via.array.size; i++) {
        ...
        if (msgpack_to_vim(mobj.via.array.ptr[i], TV_LIST_ITEM_TV(tv_list_last(list))) == FAIL) {
          return FAIL;
        }
      }
      break;
    ...
    case MSGPACK_OBJECT_MAP: {
      ...
      *rettv = (typval_T){
      ...
      for (size_t i = 0; i < mobj.via.map.size; i++) {
        dictitem_T *const di
            = xmallocz(offsetof(dictitem_T, di_key) + mobj.via.map.ptr[i].key.via.str.size);
    ...
    case MSGPACK_OBJECT_EXT: {
      ...
      create_special_dict(rettv, kMPExt,
                          ((typval_T){
                              .v_type = VAR_LIST,
                              .v_lock = VAR_UNLOCKED,
                              .vval = {.v_list = list},
