                                   ...
                                   bool *const didcolon) FUNC_ATTR_NONNULL_ALL
{
  ...
  if (last_container.container.v_type == VAR_LIST) {
    ...
    if (last_container.special_val == NULL
        && (obj.is_special_string || obj.val.vval.v_string == NULL
...
#define POP(obj_tv, is_sp_string)                                              \
  do {                                                                         \
    if (json_decoder_pop(OBJ(obj_tv, is_sp_string, *didcomma, *didcolon),      \
                         stack, container_stack, &p, next_map_special,         \
                         didcomma, didcolon)                                   \
        == FAIL) {                                                             \
                       ...
                       const bool s_allocated) FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  const bool really_hasnul
      = (hasnul == kNone ? ((s != NULL) && (memchr(s, NUL, len) != NULL))
                         : (bool)hasnul);
  if (really_hasnul) {
    ...
    return (typval_T){
        ...
        .v_lock = VAR_UNLOCKED,
        .vval = {.v_string
                 = (char_u *)((s == NULL || s_allocated) ? (char *)s
                                                         : xmemdupz(s, len))},
    };
  }
}
        ...
        case 'u': {
          if (p + 4 >= e) {
            emsgf(_("E474: Unfinished unicode escape sequence: %.*s"),
                  (int)buf_len, buf);
            goto parse_json_string_fail;
          } else if (!ascii_isxdigit(p[1]) || !ascii_isxdigit(p[2])
                     || !ascii_isxdigit(p[3]) || !ascii_isxdigit(p[4])) {
    ...
    if (*t == '\\') {
      ...
      switch (*t) {
        case 'u': {
          ...
          t += 4;
          uvarnumber_T ch;
          vim_str2nr((char_u *)ubuf, NULL, NULL, STR2NR_HEX | STR2NR_FORCE,
                     NULL, &ch, 4);
          if (ch == 0) {
            hasnul = true;
          }
...
#undef PUT_FST_IN_PAIR
  *str_end = NUL;
  typval_T obj = decode_string(str, (size_t)(str_end - str),
                               hasnul ? kTrue : kFalse, false, true);
...
parse_json_number_check:
  ...
  if (json_decoder_pop(OBJ(tv, false, *didcomma, *didcolon), stack,
                       container_stack, &p, next_map_special, didcomma,
                       didcolon)
      == FAIL) {
    goto parse_json_number_fail;
  }
        ...
        if (didcomma) {
          ...
          goto json_decode_string_fail;
                       ...
                       ? (last_container.container.v_type == VAR_DICT
                              ? (DICT_LEN(last_container.container.vval.v_dict)
                                 == 0)
                              : (tv_list_len(
                                     last_container.container.vval.v_list)
                                 == 0))
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  switch (mobj.type) {
    ...
    msgpack_to_vim_generic_map : {
      ...
      list_T *const list
          = decode_create_map_special_dict(rettv, (ptrdiff_t)mobj.via.map.size);
      for (size_t i = 0; i < mobj.via.map.size; i++) {
        list_T *const kv_pair = tv_list_alloc(2);
