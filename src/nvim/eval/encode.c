    ...
    FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (li != NULL) {
    ...
    if (line_end != buf) {
      ...
      TV_LIST_ITEM_TV(li)->vval.v_string
          = xrealloc(str, li_len + line_length + 1);
    ...
    switch (v.type) {
      case kMPConvDict: {
        typval_T key_tv = {
            .v_type = VAR_STRING,
            .vval = {.v_string
                     = (v.data.d.hi == NULL ? v.data.d.dict->dv_hashtab.ht_array
                                            : (v.data.d.hi - 1))
                           ->hi_key},
        };
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  while (p < buf_end) {
    ...
    for (size_t i = state->offset; i < state->li_length && p < buf_end; i++) {
      ...
      const char ch
          = (char)(TV_LIST_ITEM_TV(state->li)->vval.v_string[state->offset++]);
