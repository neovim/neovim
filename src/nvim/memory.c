...
void *xmalloc(size_t size) FUNC_ATTR_MALLOC
    FUNC_ATTR_ALLOC_SIZE(1) FUNC_ATTR_NONNULL_RET
{
  void *ret = try_malloc(size);
  if (!ret) {
...
size_t xstrlcpy(char *restrict dst,
                ...
                size_t dsize) FUNC_ATTR_NONNULL_ALL
{
...
size_t xstrlcat(char *const dst,
                ...
                const size_t dsize) FUNC_ATTR_NONNULL_ALL
{
  assert(dsize > 0);
...
/// @param str 0-terminated string that will be copied
/// @return pointer to a copy of the string
char *xstrndup(const char *str, size_t len) FUNC_ATTR_MALLOC
    FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_RET FUNC_ATTR_NONNULL_ALL
{
...
bool strequal(const char *a,
              const char *b) FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT
{
