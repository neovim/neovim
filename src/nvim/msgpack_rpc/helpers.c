      ...
      case MSGPACK_OBJECT_ARRAY: {
        ...
        if (cur.container) {
          ...
          *cur.aobj = ARRAY_OBJ(((Array){
              .size = size,
              .capacity = size,
              .items = (size > 0 ? xcalloc(size, sizeof(*cur.aobj->data.array.items)) : NULL),
          }));
          cur.container = true;
          kv_last(stack) = cur;
        }
      ...
      case MSGPACK_OBJECT_MAP: {
        ...
        if (cur.container) {
          if (cur.idx >= size) {
            ...
            switch (key->type) {
#define ID(x) x
              STR_CASE(MSGPACK_OBJECT_STR, str, key, cur.aobj->data.dictionary.items[idx].key, ID)
              STR_CASE(MSGPACK_OBJECT_BIN, bin, key, cur.aobj->data.dictionary.items[idx].key, ID)
#undef ID
              case MSGPACK_OBJECT_NIL:
              case MSGPACK_OBJECT_BOOLEAN:
              case MSGPACK_OBJECT_POSITIVE_INTEGER:
          ...
          *cur.aobj = DICTIONARY_OBJ(((Dictionary){
              .size = size,
              .capacity = size,
              .items = (size > 0 ? xcalloc(size, sizeof(*cur.aobj->data.dictionary.items)) : NULL),
          }));
          cur.container = true;
          kv_last(stack) = cur;
        }
      ...
      case MSGPACK_OBJECT_EXT:
        if (0 <= cur.mobj->via.ext.type && cur.mobj->via.ext.type <= EXT_OBJECT_TYPE_MAX) {
          ...
          msgpack_unpack_return status
              = msgpack_unpack(cur.mobj->via.ext.ptr, cur.mobj->via.ext.size, NULL, &zone, &data);
                                  ...
                                  String *const arg) FUNC_ATTR_NONNULL_ALL
{
  if (obj->type == MSGPACK_OBJECT_BIN || obj->type == MSGPACK_OBJECT_STR) {
    arg->data = obj->via.bin.ptr != NULL ? xmemdupz(obj->via.bin.ptr, obj->via.bin.size) : NULL;
    ...
    FUNC_ATTR_NONNULL_ARG(2)
{
  ...
  while (kv_size(stack)) {
    ...
    STATIC_ASSERT(
        kObjectTypeWindow == kObjectTypeBuffer + 1 && kObjectTypeTabpage == kObjectTypeWindow + 1,
...
void msgpack_rpc_serialize_response(uint32_t response_id,
                                    Error *err,
                                    Object arg,
                                    msgpack_packer *pac) FUNC_ATTR_NONNULL_ARG(2, 4)
{
