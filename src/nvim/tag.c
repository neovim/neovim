  ...
  for (;;) {
    ...
    if (num_matches <= 0) {
      ...
      if (type != DT_TAG && type != DT_SELECT && type != DT_JUMP
          && type != DT_CSCOPE && (num_matches > 1 || ic) && !skip_msg) {
...
//
static void print_tag_list(int new_tag,
                           int use_tagstack,
                           int num_matches,
                           char_u **matches)
{
    ...
    for (first_file = true;
      ...
      for (;;) {
         ...
         */
          ...
          /* skip empty and blank lines */
          do {
            eof = use_cscope ? cs_fgets(lbuf, lbuf_size)
...
#define ML_EXTRA 3
            ...
            *tagp.tagname_end = NUL;
            ...
            snprintf((char *)p + len + 1 + ML_EXTRA, 10, "%06d",
                     help_heuristic(tagp.tagname, match_re ? matchoff : 0,
                                    !match_no_ic)
                         + help_pri);
          ...
          if (mfp != NULL) {
            ...
            hi = hash_lookup(&ht_match[mtt], (const char *)mfp, STRLEN(mfp),
                             hash);
...
static char_u *expand_tag_fname(char_u *fname,
                                char_u *const tag_fname,
                                const bool expand)
{
  char_u *p;
                            ...
                            char_u *buf_ffname)
{
  ...
  if (buf_ffname != NULL) { /* if the buffer has a name */
    ...
    retval
        = (path_full_compare(fullname, buf_ffname, true, true) & kEqualFiles);
    xfree(fullname);
    *fname_end = c;
  }
    ...
    for (i = 0; i < num_matches; ++i) {
      ...
      if (add_tag_field(dict, "name", tp.tagname, tp.tagname_end) == FAIL
          ...
          || add_tag_field(dict, "kind", tp.tagkind,
                           tp.tagkind ? tp.tagkind_end : NULL)
                 == FAIL
...
static void tagstack_push_items(win_T *wp, list_T *l)
{
  ...
  for (li = tv_list_first(l); li != NULL; li = TV_LIST_ITEM_NEXT(l, li)) {
    ...
    tagstack_push_item(
        wp, tagname, (int)tv_dict_get_number(itemdict, "bufnr"),
