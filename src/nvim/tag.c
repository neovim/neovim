...
bool do_tag(char_u *tag, int type, int count, int forceit, int verbose)
{
  ...
  if (!p_tgst && *tag != NUL) {  // -V522
    ...
    if (*tag != NUL
        && (type == DT_TAG || type == DT_SELECT || type == DT_JUMP || type == DT_LTAG
            || type == DT_CSCOPE)) {
      if (g_do_tagpreview != 0) {
  ...
  for (;;) {
    ...
    if (new_tag || (cur_match >= num_matches && max_num_matches != MAXCOL) || other_name) {
      ...
      if (find_tags(name, &new_num_matches, &new_matches, flags, max_num_matches, buf_ffname) == OK
    ...
    if (num_matches <= 0) {
      ...
      if (use_tagstack) {
        ...
        if (use_tfu && parse_match(matches[cur_match], &tagp2) == OK && tagp2.user_data) {
          XFREE_CLEAR(tagstack[tagstackidx].user_data);
          tagstack[tagstackidx].user_data
              = vim_strnsave(tagp2.user_data, tagp2.user_data_end - tagp2.user_data);
        }
      ...
      if (type != DT_TAG && type != DT_SELECT && type != DT_JUMP && type != DT_CSCOPE
        ...
        snprintf((char *)IObuff, sizeof(IObuff), _("tag %d of %d%s"), cur_match + 1, num_matches,
...
static int add_llist_tags(char_u *tag, int num_matches, char_u **matches)
{
  ...
  for (i = 0; i < num_matches; i++) {
    ...
    if (isdigit(*tagp.command)) {
      ...
      snprintf((char *)cmd + len, CMDBUFFSIZE + 1 - len, "%.*s", cmd_len, cmd_start);
      len += cmd_len;
  ...
  if (has_re) {
    ...
    if (pats->head == pats->pat) {
      ...
      for (pats->headlen = 0; pats->head[pats->headlen] != NUL; ++pats->headlen) {
        if (vim_strchr((char_u *)(p_magic ? ".[~*\\$" : "\\$"), pats->head[pats->headlen])
            != NULL) {
          break;
        }
      }
...
static int find_tagfunc_tags(char_u *pat,
                             garray_T *ga,
                             int *match_count,
                             int flags,
                             char_u *buf_ffname)
{
  ...
  vim_snprintf((char *)flagString, sizeof(flagString), "%s%s%s", g_tag_at_cursor ? "c" : "",
               flags & TAG_INS_COMP ? "i" : "", flags & TAG_REGEXP ? "r" : "");
              ...
              char_u *buf_ffname)
{
  ...
  for (round = 1; round <= 2; ++round) {
    ...
    for (first_file = true; use_cscope || get_tagfname(&tn, first_file, tag_fname) == OK;
         first_file = false) {
      ...
      for (;;) {
         ...
         */
          ...
          // skip empty and blank lines
          do {
            eof = use_cscope ? cs_fgets(lbuf, lbuf_size) : vim_fgets(lbuf, lbuf_size, fp);
          } while (!eof && vim_isblankline(lbuf));
  ...
  for (;;) {
    if (tnp->tn_did_filefind_init) {
      ...
      *filename++ = NUL;
      ...
      tnp->tn_search_ctx = vim_findfile_init(buf, filename, r_ptr, 100,
  ...
  *pbuf_end = NUL;
  ...
  if (GETFILE_SUCCESS(getfile_result)) {  // got to the right file
    ...
    if (str > pbuf_end - 1) {  // search command with nothing following
      ...
      if (do_search(NULL, pbuf[0], pbuf[0], pbuf + 1, (long)1, search_options, NULL)) {
        retval = OK;
      } else {
        int found = 1;
        int cc;
        ...
        if (!do_search(NULL, pbuf[0], pbuf[0], pbuf + 1, (long)1, search_options, NULL)) {
          ...
          *tagp.tagname_end = NUL;
          ...
          if (!do_search(NULL, '/', '/', pbuf, (long)1, search_options, NULL)) {
            ...
            snprintf((char *)pbuf, LSIZE, "^\\[#a-zA-Z_]\\.\\*\\<%s\\s\\*(", tagp.tagname);
            if (!do_search(NULL, '/', '/', pbuf, (long)1, search_options, NULL)) {
              found = 0;
            }
          }
...
static char_u *expand_tag_fname(char_u *fname, char_u *const tag_fname, const bool expand)
{
  ...
  if (expand && path_has_wildcard(fname)) {
    ...
    expanded_fname
        = ExpandOne(&xpc, fname, NULL, WILD_LIST_NOTFOUND | WILD_SILENT, WILD_EXPAND_FREE);
...
int expand_tags(int tagnames, char_u *pat, int *num_file, char_u ***file)
{
  ...
  if (pat[0] == '/') {
    ret = find_tags(pat + 1, num_file, file, TAG_REGEXP | extra_flag | TAG_VERBOSE | TAG_NO_TAGFUNC,
                    TAG_MANY, curbuf->b_ffname);
  } else {
    ret = find_tags(pat, num_file, file,
                    TAG_REGEXP | extra_flag | TAG_VERBOSE | TAG_NO_TAGFUNC | TAG_NOIC, TAG_MANY,
                    curbuf->b_ffname);
  }
  if (ret == OK && !tagnames) {
    ...
    for (i = 0; i < *num_file; i++) {
      ...
      name_buf[len++] = 0;
      name_buf[len++] = (t_p.tagkind != NULL && *t_p.tagkind) ? *t_p.tagkind : 'f';
      name_buf[len++] = 0;
...
static int add_tag_field(dict_T *dict,
                         ...
                         const char_u *start,
                         const char_u *end) FUNC_ATTR_NONNULL_ARG(1, 2)
{
  int len = 0;
  int retval;
...
int get_tags(list_T *list, char_u *pat, char_u *buf_fname)
{
  ...
  ret = find_tags(pat, &num_matches, &matches, TAG_REGEXP | TAG_NOIC, MAXCOL, buf_fname);
  if (ret == OK && num_matches > 0) {
    for (i = 0; i < num_matches; ++i) {
      ...
      if (tp.command_end != NULL) {
        for (char_u *p = tp.command_end + 3; *p != NUL && *p != '\n' && *p != '\r'; MB_PTR_ADV(p)) {
...
static void get_tag_details(taggy_T *tag, dict_T *retdict)
{
  ...
  fmark = &tag->fmark;
  tv_list_append_number(pos, (varnumber_T)(fmark->fnum != -1 ? fmark->fnum : 0));
...
static void tagstack_push_item(win_T *wp,
                               char_u *tagname,
                               int cur_fnum,
                               int cur_match,
                               pos_T mark,
...
static void tagstack_push_items(win_T *wp, list_T *l)
{
  ...
  for (li = tv_list_first(l); li != NULL; li = TV_LIST_ITEM_NEXT(l, li)) {
    if (TV_LIST_ITEM_TV(li)->v_type != VAR_DICT || TV_LIST_ITEM_TV(li)->vval.v_dict == NULL) {
    ...
    if ((tagname = (char_u *)tv_dict_get_string(itemdict, "tagname", true)) == NULL) {
      continue;
    }
    ...
    tagstack_push_item(wp, tagname, (int)tv_dict_get_number(itemdict, "bufnr"),
                       (int)tv_dict_get_number(itemdict, "matchnr") - 1, mark, fnum,
