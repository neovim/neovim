                           ...
                           long xtra)
{
  ...
  FOR_ALL_TAB_WINDOWS(tp, wp)
  {
    if (wp->w_buffer == curbuf) {
      ...
      // change.
      if (wp->w_p_rnu || (wp->w_p_cul && lnum <= wp->w_last_cursorline)) {
              ...
              int second_line_indent)
{
    ...
    if (lead_len > 0) {
      ...
      if (lead_repl != NULL) {
        ...
        if (c == COM_RIGHT) {  // right adjusted leader
          ...
          for (p = leader + lead_len - 1; p > leader && ascii_iswhite(*p);
               p--) {
          }
          p++;
  ...
  if (dir == FORWARD) {
    if (trunc_line || (State & INSERT)) {
      ...
      if (new_len < curwin->w_cursor.col) {
        extmark_splice_cols(curbuf, (int)curwin->w_cursor.lnum, new_len,
      ...
      if (did_append) {
        ...
        extmark_splice(curbuf, (int)lnum - 1, mincol - 1, 0, less_cols_off,
                       less_cols_off, 1, cols_added, 1 + cols_added,
                       kExtmarkUndo);
      } else {
  ...
  if (did_append) {
    ...
    extmark_splice(curbuf, (int)curwin->w_cursor.lnum - 1, 0, 0, 0, 0, 1, 0,
                   1 + extra, kExtmarkUndo);
  }
  curbuf_splice_pending--;
  ...
  if (!p_paste && (curbuf->b_p_cin || *curbuf->b_p_inde != NUL)
