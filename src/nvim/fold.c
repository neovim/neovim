...
)
{
  ...
  if (last_lnum > 0) {
    ...
    buf_updates_send_changes(curbuf, first_lnum, num_changed, num_changed,
                             true);
  }
}
...
static void deleteFoldEntry(garray_T *const gap,
                            const int idx,
 ...
 */
static void deleteFoldMarkers(fold_T *fp,
                              int recursive,
 ...
 * "bot"!
 */
static void foldSplit(garray_T *const gap,
                      const int i,
                      const linenr_T top,
                      const linenr_T bot)
{
