...
#define ISCTRL(schar) (schar < ' ')
      ...
      ret.data.err.msg = _("E15: Invalid control character present in input: %.*s");
      break;
#undef ISCTRL
    ...
    case '9': {
      ...
      if (flags & kELFlagAllowFloat) {
        ...
        if (pline.size > ret.len + 1 && pline.data[ret.len] == '.'
          ...
          for (; ret.len < pline.size && ascii_isdigit(pline.data[ret.len]); ret.len++) {
          ...
          if (pline.size > ret.len + 1 && (pline.data[ret.len] == 'e' || pline.data[ret.len] == 'E')
              && ((pline.size > ret.len + 2
                   && (pline.data[ret.len + 1] == '+' || pline.data[ret.len + 1] == '-')
                   && ascii_isdigit(pline.data[ret.len + 2]))
                  || ascii_isdigit(pline.data[ret.len + 1]))) {
            ret.len++;
            if (pline.data[ret.len] == '+' || (exp_negative = (pline.data[ret.len] == '-'))) {
              ret.len++;
            }
            exp_start = ret.len;
            CHARREG(kExprLexNumber, ascii_isdigit);
        ...
        if (pline.size > ret.len
            && (pline.data[ret.len] == '.' || ASCII_ISALPHA(pline.data[ret.len]))) {
          ret = non_float_ret;
        }
      }
      ...
      if (ret.data.num.is_float) {
        ...
        ret.data.num.val.floating = scale_number(significand_part, 10, exp_part, exp_negative);
      } else {
        int len;
        int prep;
        vim_str2nr(pline.data, &prep, &len, STR2NR_ALL, NULL, &ret.data.num.val.integer,
                   (int)pline.size, false);
        ret.len = (size_t)len;
        const uint8_t bases[] = {
    ...
    case '_':
      ...
      if (!(flags & kELFlagIsNotCmp)
        ...
        // Scope: `s:`, etc.
                 ...
                 && memchr(EXPR_VAR_SCOPE_LIST, schar, sizeof(EXPR_VAR_SCOPE_LIST)) != NULL
                 && pline.data[ret.len] == ':' && !(flags & kELFlagForbidScope)) {
        ret.len++;
        ret.data.var.scope = (ExprVarScope)schar;
        CHARREG(kExprLexPlainIdentifier, ISWORD_OR_AUTOLOAD);
        ret.data.var.autoload = (memchr(pline.data + 2, AUTOLOAD_CHAR, ret.len - 2) != NULL);
  ...
  do {                                                                                             \
    ...
    ret.data.err.msg = _("E112: Option name missing: %.*s");                                       \
      ...
      if (pline.size > 2 && pline.data[2] == ':'
          && memchr(EXPR_OPT_SCOPE_LIST, pline.data[1], sizeof(EXPR_OPT_SCOPE_LIST)) != NULL) {
        ret.len += 2;
...
#undef OPTNAMEMISS
    ...
    case '<': {
      ...
      ret.data.cmp.inv = (schar == '<');
      ret.data.cmp.type
          = ((ret.data.cmp.inv ^ haseqsign) ? kExprCmpGreaterOrEqual : kExprCmpGreater);
      break;
    }
...
#undef CHAR_OR_ASSIGN
    ...
    default:
      ret.len = (size_t)utfc_ptr2len_len((const char_u *)pline.data, (int)pline.size);
      ret.type = kExprLexInvalid;
...
const char *const eltkn_cmp_type_tab[] = {
    [kExprCmpEqual] = "Equal",         [kExprCmpMatches] = "Matches",
    [kExprCmpGreater] = "Greater",     [kExprCmpGreaterOrEqual] = "GreaterOrEqual",
...
#define ADDSTR(...)                                                                                \
  ...
  ADDSTR("%zu:%zu:%s", token.start.line, token.start.col, eltkn_type_tab[token.type]);
  switch (token.type) {
#define TKNARGS(tkn_type, ...)                                                                     \
  case tkn_type: {                                                                                 \
    ...
    TKNARGS(kExprLexComparison, "(type=%s,ccs=%s,inv=%i)", eltkn_cmp_type_tab[token.data.cmp.type],
            ccs_tab[token.data.cmp.ccs], (int)token.data.cmp.inv)
    TKNARGS(kExprLexMultiplication, "(type=%s)", eltkn_mul_type_tab[token.data.mul.type])
    TKNARGS(kExprLexAssignment, "(type=%s)", expr_asgn_type_tab[token.data.ass.type])
    TKNARGS(kExprLexRegister, "(name=%s)", intchar2str(token.data.reg.name))
    case kExprLexDoubleQuotedString:
      TKNARGS(kExprLexSingleQuotedString, "(closed=%i)", (int)token.data.str.closed)
      TKNARGS(kExprLexOption, "(scope=%s,name=%.*s)", eltkn_opt_scope_tab[token.data.opt.scope],
              (int)token.data.opt.len, token.data.opt.name)
      TKNARGS(kExprLexPlainIdentifier, "(scope=%s,autoload=%i)",
              intchar2str((int)token.data.var.scope), (int)token.data.var.autoload)
      TKNARGS(kExprLexNumber, "(is_float=%i,base=%i,val=%lg)", (int)token.data.num.is_float,
              (int)token.data.num.base,
              (double)(token.data.num.is_float ? (double)token.data.num.val.floating
...
#undef TKNARGS
  ...
  if (pstate == NULL) {
    ...
    *p++ = ':';
    memmove(p, &pstate->reader.lines.items[token.start.line].data[token.start.col], token.len);
    p += token.len;
    *p = NUL;
  }
#undef ADDSTR
                                                   ...
                                                   const char *const prefix)
{
  if (*eastnode_p == NULL) {
    ...
    fprintf(stderr, "%s %p : %p : %s : %zu:%zu:%zu\n", prefix, (void *)eastnode_p,
            (void *)(*eastnode_p), east_node_type_tab[(*eastnode_p)->type],
            (*eastnode_p)->start.line, (*eastnode_p)->start.col, (*eastnode_p)->len);
  }
}
...
static inline void viml_pexpr_debug_print_token(const ParserState *const pstate,
                                                const LexExprToken token) FUNC_ATTR_ALWAYS_INLINE
{
...
#define PNODE_P(eastnode_p, msg)                                                                   \
  viml_pexpr_debug_print_ast_node((const ExprASTNode *const *)eastnode_p, (#msg))
...
static bool viml_pexpr_handle_bop(const ParserState *const pstate,
                                  ...
                                  ExprASTError *const ast_err) FUNC_ATTR_NONNULL_ALL
{
  bool ret = true;
  ...
  const ExprOpLvl bop_node_lvl
      = ((bop_node->type == kExprNodeCall || bop_node->type == kExprNodeSubscript)
             ? kEOpLvlSubscript
             : node_lvl(*bop_node));
#ifndef NDEBUG
  ...
  if (top_node_ass == kEOpAssLeft || top_node_lvl != bop_node_lvl) {
    ...
    *top_node_p = bop_node;
    ...
    if (bop_node->type == kExprNodeComparison) {
      east_set_error(pstate, ast_err, _("E15: Operator is not associative: %.*s"), bop_node->start);
      ret = false;
    }
  }
...
#define SELECT_FIGURE_BRACE_TYPE(node, new_type, hl)                                               \
  do {                                                                                             \
    ...
    assert(node_->type == kExprNodeUnknownFigure || node_->type == kExprNode##new_type);           \
...
#undef SINGLE_CHAR_ESC
          ...
          case '<': {
            const size_t special_len
                = (trans_special((const char_u **)&p, (size_t)(e - p), (char_u *)v_p, true, true));
            if (special_len != 0) {
              v_p += special_len;
            } else {
              is_unknown = true;
  ...
  if (pstate->colors) {
    ...
    const char *const body_str = (is_double ? HL(DoubleQuotedBody) : HL(SingleQuotedBody));
    const char *const esc_str = (is_double ? HL(DoubleQuotedEscape) : HL(SingleQuotedQuote));
    const char *const ukn_esc_str
        = (is_double ? HL(DoubleQuotedUnknownEscape) : HL(SingleQuotedUnknownEscape));
    for (size_t i = 0; i < kv_size(shifts); i++) {
      const StringShift cur_shift = kv_A(shifts, i);
      if (cur_shift.start > next_col) {
        viml_parser_highlight(pstate, recol_pos(token.start, next_col), cur_shift.start - next_col,
                              body_str);
      }
      viml_parser_highlight(pstate, recol_pos(token.start, cur_shift.start), cur_shift.orig_len,
  ...
  if (token.data.str.closed) {
    if (is_double) {
      viml_parser_highlight(pstate, shifted_pos(token.start, token.len - 1), 1, HL(DoubleQuote));
    } else {
      viml_parser_highlight(pstate, shifted_pos(token.start, token.len - 1), 1, HL(SingleQuote));
    }
  }
  kvi_destroy(shifts);
}
...
ExprAST viml_pexpr_parse(ParserState *const pstate,
                         const int flags) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ALL
{
  ExprAST ast = {
    .err = {
      .msg = NULL,
  ...
  do {
    ...
    const int lexer_additional_flags
        = (kELFlagPeek | ((flags & kExprFlagsDisallowEOC) ? kELFlagForbidEOC : 0)
  ...
  viml_pexpr_parse_process_token:
    ...
    } else if (is_invalid && prev_token.type == kExprLexSpacing && !highlighted_prev_spacing) {
      viml_parser_highlight(pstate, prev_token.start, prev_token.len, HL(Spacing));
      is_invalid = false;
    ...
    for (size_t i = 0; i + 1 < kv_size(ast_stack); i++) {
      ...
      assert((&(*kv_A(ast_stack, i))->children == kv_A(ast_stack, i + 1)
             ...
             || ((&(*kv_A(ast_stack, i))->children->next == kv_A(ast_stack, i + 1))
                 && (item_null ? (*kv_A(ast_stack, i))->children->next == NULL
    ...
    switch (kv_last(pt_stack)) {
      ...
      case kEPTLambdaArguments:
        if ((want_node == kENodeOperator && tok_type != kExprLexComma && tok_type != kExprLexArrow)
            || (want_node == kENodeValue
                && !(cur_token.type == kExprLexPlainIdentifier
          ...
          if (lambda_node->children != NULL && lambda_node->children->type == kExprNodeComma) {
      ...
      case kEPTAssignment:
        if (want_node == kENodeValue && tok_type != kExprLexBracket
            ...
            && !(node_is_key && tok_type == kExprLexNumber) && tok_type != kExprLexEnv
            && tok_type != kExprLexOption && tok_type != kExprLexRegister) {
          ERROR_FROM_TOKEN_AND_MSG(cur_token,
...
#undef MUL_OP
      ...
      case kExprLexOption: {
        ...
        if (cur_token.type == kExprLexInvalid) {
          assert(cur_token.len == 1
                 || (cur_token.len == 3 && pline.data[cur_token.start.col + 2] == ':'));
          cur_node->data.opt.ident = (pline.data + cur_token.start.col + cur_token.len);
          cur_node->data.opt.ident_len = 0;
          cur_node->data.opt.scope
              = (cur_token.len == 3 ? (ExprOptScope)pline.data[cur_token.start.col + 1]
        ...
        *top_node_p = cur_node;
        ...
        const size_t scope_shift = (cur_token.data.opt.scope == kExprOptScopeUnspecified ? 0 : 2);
        if (scope_shift) {
          viml_parser_highlight(pstate, shifted_pos(cur_token.start, 1), 1, HL(OptionScope));
      ...
      case kExprLexEnv:
        ...
        if (cur_node->data.env.ident_len == 0) {
          ERROR_FROM_TOKEN_AND_MSG(cur_token, _("E15: Environment variable name missing"));
        }
        *top_node_p = cur_node;
        want_node = kENodeOperator;
        viml_parser_highlight(pstate, cur_token.start, 1, HL(EnvironmentSigil));
        viml_parser_highlight(pstate, shifted_pos(cur_token.start, 1), cur_token.len - 1,
                              HL(EnvironmentName));
        break;
      case kExprLexNot:
        if (want_node == kENodeOperator) {
      ...
      case kExprLexComparison:
        ...
        if (cur_token.data.cmp.ccs != kCCStrategyUseOption) {
          viml_parser_highlight(pstate, cur_token.start, cur_token.len - 1, HL(Comparison));
      ...
      case kExprLexComma:
        ...
        for (size_t i = 1; i < kv_size(ast_stack); i++) {
          ExprASTNode *const *const eastnode_p = (ExprASTNode *const *)kv_Z(ast_stack, i);
          ...
          if (eastnode_type == kExprNodeLambda) {
            assert(cur_pt == kEPTLambdaArguments && want_node == kENodeOperator);
            break;
          } else if (eastnode_type == kExprNodeDictLiteral || eastnode_type == kExprNodeListLiteral
                     || eastnode_type == kExprNodeCall) {
            break;
          } else if (eastnode_type == kExprNodeComma || eastnode_type == kExprNodeColon
...
#define EXP_VAL_COLON "E15: Expected value, got colon: %.*s"
      case kExprLexColon: {
        ...
        for (size_t i = 1; i < kv_size(ast_stack); i++) {
          ...
          const ExprASTNodeType eastnode_type = (*eastnode_p)->type;
          const ExprOpLvl eastnode_lvl = node_lvl(**eastnode_p);
          STATIC_ASSERT(kEOpLvlTernary > kEOpLvlComma, "Unexpected operator priorities");
        ...
        if (is_subscript) {
          ...
          // part like a[:2].
          if (want_node == kENodeValue && (*kv_Z(ast_stack, 1))->type == kExprNodeSubscript) {
            NEW_NODE_WITH_CUR_POS(*top_node_p, kExprNodeMissing);
            (*top_node_p)->len = 0;
            want_node = kENodeOperator;
          } else {
...
#undef EXP_VAL_COLON
      ...
      case kExprLexFigureBrace:
        if (cur_token.data.brc.closing) {
            ...
            new_top_node_p = kv_pop(ast_stack);
                   ...
                   && (new_top_node_p == NULL
                       || ((*new_top_node_p)->type != kExprNodeUnknownFigure
                           ...
                           && ((*new_top_node_p)->type != kExprNodeCurlyBracesIdentifier)
          ...
          switch (new_top_node->type) {
            case kExprNodeUnknownFigure:
              if (new_top_node->children == NULL) {
                ...
                SELECT_FIGURE_BRACE_TYPE(new_top_node, CurlyBracesIdentifier, Curly);
                HL_CUR_TOKEN(Curly);
              } else {
                ...
                if (pstate->colors) {
                  ...
                  kv_A(*pstate->colors, new_top_node->data.fig.opening_hl_idx).group
                      = (HL(FigureBrace));
                }
          ...
          if (pt_is_assignment(cur_pt) && !pt_is_assignment(kv_last(pt_stack))) {
            assert(want_node == kENodeValue);  // Subtract 1 for NULL at top.
            asgn_level = kv_size(ast_stack) - 1;
          }
        }
      ...
      case kExprLexPlainIdentifier: {
        const ExprVarScope scope
            = (cur_token.type == kExprLexInvalid ? kExprVarScopeMissing : cur_token.data.var.scope);
        if (want_node == kENodeValue) {
          want_node = kENodeOperator;
          ...
          cur_node->data.var.ident = (pline.data + cur_token.start.col + scope_shift);
          cur_node->data.var.ident_len = cur_token.len - scope_shift;
          *top_node_p = cur_node;
          if (scope_shift) {
            assert(!node_is_key);
            viml_parser_highlight(pstate, cur_token.start, 1, HL(IdentifierScope));
            viml_parser_highlight(pstate, shifted_pos(cur_token.start, 1), 1,
                                  HL(IdentifierScopeDelimiter));
          }
          viml_parser_highlight(pstate, shifted_pos(cur_token.start, scope_shift),
      ...
      case kExprLexNumber:
        ...
        if (node_is_key) {
          ...
          const uint8_t prefix_length = base_to_prefix_length[cur_token.data.num.base];
          viml_parser_highlight(pstate, cur_token.start, prefix_length, HL(NumberPrefix));
      ...
      case kExprLexSingleQuotedString: {
        ...
        if (!cur_token.data.str.closed) {
          ...
          // "E115: Missing quote".
          ERROR_FROM_TOKEN_AND_MSG(cur_token, (is_double ? _("E114: Missing double quote: %.*s")
        ...
        NEW_NODE_WITH_CUR_POS(
            cur_node, (is_double ? kExprNodeDoubleQuotedString : kExprNodeSingleQuotedString));
        *top_node_p = cur_node;
...
viml_pexpr_parse_end:
  ...
  if (want_node == kENodeValue
    ...
    east_set_error(pstate, &ast.err, _("E15: Expected value, got EOC: %.*s"), pstate->pos);
    ...
    while (ast.err.msg == NULL && kv_size(ast_stack)) {
      ...
      switch (cur_node->type) {
        ...
        case kExprNodeListLiteral:
          ...
          east_set_error(pstate, &ast.err, _("E697: Missing end of List ']': %.*s"),
                         cur_node->start);
          break;
        case kExprNodeDictLiteral:
          // Same problem like with list literal with E722 (missing comma) vs
          // E723, but additionally just "{" yields only E15.
          east_set_error(pstate, &ast.err, _("E723: Missing end of Dictionary '}': %.*s"),
                         cur_node->start);
          break;
        case kExprNodeUnknownFigure:
          east_set_error(pstate, &ast.err, _("E15: Missing closing figure brace: %.*s"),
                         cur_node->start);
          break;
        case kExprNodeLambda:
          east_set_error(pstate, &ast.err, _("E15: Missing closing figure brace for lambda: %.*s"),
                         cur_node->start);
          break;
        case kExprNodeCurlyBracesIdentifier:
