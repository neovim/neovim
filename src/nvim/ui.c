...
#define UI_LOG(funname)                                                                            \
  do {                                                                                             \
    if (entered_free_all_mem) {                                                                    \
      ...
      if (uilog_seen > 0) {                                                                        \
        logmsg(DEBUG_LOG_LEVEL, "UI: ", NULL, -1, true, "%s (+%zu times...)", uilog_last_event,    \
               uilog_seen);                                                                        \
      }                                                                                            \
...
void ui_refresh(void)
{
  ...
  for (UIExtension i = 0; (int)i < kUIExtCount; i++) {
    ...
    if (i < kUIGlobalCount) {
      ui_call_option_set(cstr_as_string((char *)ui_ext_names[i]), BOOLEAN_OBJ(ext_widgets[i]));
    }
  }
...
int ui_pum_get_height(void)
{
  ...
  for (size_t i = 1; i < ui_count; i++) {
    ...
    if (ui_pum_height) {
      pum_height = pum_height != 0 ? MIN(pum_height, ui_pum_height) : ui_pum_height;
    }
  }
  return pum_height;
}
...
{
  ui_call_default_colors_set(normal_fg, normal_bg, normal_sp, cterm_normal_fg_color,
...
void ui_set_ext_option(UI *ui, UIExtension ext, bool active)
{
  ...
  if (ui->option_set && (ui_ext_names[ext][0] != '_' || active)) {
    ui->option_set(ui, cstr_as_string((char *)ui_ext_names[ext]), BOOLEAN_OBJ(active));
  }
             ...
             bool wrap)
{
  ...
  ui_call_raw_line(grid->handle, row, startcol, endcol, clearcol, clearattr, flags,
                   (const schar_T *)grid->chars + off, (const sattr_T *)grid->attrs + off);
  ...
  if (p_wd && !(rdb_flags & RDB_COMPOSITOR)) {
    ...
    ui_call_grid_cursor_goto(grid->handle, row, MIN(clearcol, (int)grid->Columns - 1));
    ui_call_flush();
