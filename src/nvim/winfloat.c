...
/// Adjust a preview floating window position to fit the screen.
///
/// @param wp pointer of floating preview window
...
/// Parse 'previewpopup' ("height:N,width:N") into `config`.
...
bool win_float_parse_option(WinConfig *config)
{
  ...
  for (char *p = p_pvp; *p != NUL;) {
    ...
    if (!ascii_isdigit((uint8_t)(*p))) {
      return false;
    }
    char *end = NULL;
    long val = strtol(p, &end, 10);
    if (val < 1 || val > INT_MAX || (*end != ',' && *end != NUL)) {
      return false;
    }
    *target = (int)val;
    p = (*end == ',') ? end + 1 : end;
