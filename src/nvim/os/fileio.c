...
int file_open(FileDescriptor *const ret_fp,
              ...
              const int flags,
              const int mode) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  int os_open_flags = 0;
  TriState wr = kNone;
  // -V:FLAG:501
...
int file_open_fd(FileDescriptor *const ret_fp,
                 const int fd,
                 const int flags) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ret_fp->wr
      = !!(flags & (kFileCreate | kFileCreateOnly | kFileTruncate | kFileAppend | kFileWriteOnly));
...
static void file_rb_write_full_cb(RBuffer *const rv, FileDescriptor *const fp) FUNC_ATTR_NONNULL_ALL
{
  ...
  const ptrdiff_t wres = os_write(fp->fd, writebuf, read_bytes, fp->non_blocking);
  ...
  while (read_remaining) {
    ...
    if (read_remaining) {
      ...
      struct iovec iov[] = {
          ...
          {.iov_base = rbuffer_write_ptr(rv, &write_count), .iov_len = kRWBufferSize},
      };
      assert(write_count == kRWBufferSize);
      const ptrdiff_t r_ret = os_readv(fp->fd, &fp->eof, iov, ARRAY_SIZE(iov), fp->non_blocking);
      if (r_ret > 0) {
      ...
      if (read_remaining >= kRWBufferSize) {
        ...
        const ptrdiff_t r_ret = os_read(fp->fd, &fp->eof, buf, read_remaining, fp->non_blocking);
        if (r_ret >= 0) {
          read_remaining -= (size_t)r_ret;
        ...
        size_t write_count;
        const ptrdiff_t r_ret = os_read(fp->fd, &fp->eof, rbuffer_write_ptr(rv, &write_count),
...
ptrdiff_t file_write(FileDescriptor *const fp,
                     ...
                     const size_t size) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ARG(1)
{
  assert(fp->wr);
...
int msgpack_file_write(void *data,
                       const char *buf,
                       size_t len) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
