...
TriState os_dyn_conpty_init(void)
{
  ...
  static struct {
    ...
    FARPROC *ptr;
                      ...
                      {NULL, NULL}};
  for (int i = 0; conpty_entry[i].name != NULL && conpty_entry[i].ptr != NULL;
       i++) {
                         ...
                         uint16_t height)
{
  ...
  const DWORD mode = PIPE_ACCESS_INBOUND | PIPE_ACCESS_OUTBOUND
                     | FILE_FLAG_FIRST_PIPE_INSTANCE;

  sa.nLength = sizeof(sa);
  snprintf(buf, sizeof(buf), "\\\\.\\pipe\\nvim-term-in-%d-%d", os_get_pid(),
           count);
  *in_name = xstrdup(buf);
  if ((in_read = CreateNamedPipeA(
           *in_name, mode, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1,
           0, 0, 30000, &sa))
      == INVALID_HANDLE_VALUE) {
    emsg = "create input pipe failed";
    goto failed;
  }
  snprintf(buf, sizeof(buf), "\\\\.\\pipe\\nvim-term-out-%d-%d", os_get_pid(),
           count);
  *out_name = xstrdup(buf);
  if ((out_write = CreateNamedPipeA(
           *out_name, mode, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1,
           0, 0, 30000, &sa))
      == INVALID_HANDLE_VALUE) {
  ...
  if (!UpdateProcThreadAttribute(conpty_object->si_ex.lpAttributeList, 0,
                                 ...
                                 conpty_object->pty, sizeof(conpty_object->pty),
                                 NULL, NULL)) {
...
failed:
  ELOG("os_conpty_init:%s : error code: %d", emsg,
                     ...
                     wchar_t *env)
{
  ...
  if (!CreateProcessW(name, cmd_line, NULL, NULL, false,
                      ...
                      env, cwd, &conpty_object->si_ex.StartupInfo, &pi)) {
    return false;
  }
