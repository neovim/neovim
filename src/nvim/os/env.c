...
char *vim_getenv(const char *name)
{
  ...
  if (vim_path == NULL) {
    ...
    char abs_runtime_path[MAXPATHL];
    if (vim_path == NULL) {
      if (GetModuleFileName(NULL, exe_name, MAXPATHL) != 0) {
        // Remove trailing bin/nvim.exe
        char *path_end = (char *)path_tail_with_sep((char_u *)exe_name);
        *path_end = '\0';
        path_end = (char *)path_tail((char_u *)exe_name);
        *path_end = '\0';
        if (append_path(exe_name, "share" _PATHSEPSTR "nvim" _PATHSEPSTR "runtime" _PATHSEPSTR, MAXPATHL) == OK) {
          vim_path = exe_name;
        }
      }
    }
