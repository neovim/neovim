...
{
#ifndef WIN32
  // Ensure a clean slate by unblocking all signals. For example, when
  // SIGCHLD is blocked, libuv may hang after spawning a subprocess on
  // Linux, see #5230.
  sigset_t mask;
  sigemptyset(&mask);
  if (pthread_sigmask(SIG_SETMASK, &mask, NULL) != 0) {
    ELOG("Could not unblock signals, Neovim might behave strangely.");
  }
#endif
