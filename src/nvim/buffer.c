...
void handle_swap_exists(bufref_T *old_curbuf)
{
  ...
  if (swap_exists_action == SEA_QUIT) {
    ...
    swap_exists_did_quit = true;
    close_buffer(curwin, curbuf, DOBUF_DEL, false);
...
void buflist_list(exarg_T *eap)
{
  ...
  for (; buf != NULL && !got_int;
       buf = buflist_data ? (++p < buflist_data + buflist.ga_len ? *p : NULL)
                          : buf->b_next) {
  ...
  *p++ = '"';
  ...
  vim_snprintf_add(
      ...
      (curbuf->b_flags & BF_NOTEDITED) && !bt_dontwrite(curbuf)
          ? _("[Not edited]")
          : "",
      (curbuf->b_flags & BF_NEW) && !bt_dontwrite(curbuf) ? _("[New file]")
                                                          : "",
  ...
  if (maxwidth > 0 && width > maxwidth) {
    ...
    if (num_separators) {
      ...
      for (int i = 0; i < num_separators; i++) {
        ...
        for (int item_idx = separator_locations[i] + 1; item_idx < itemcnt;
             item_idx++) {
