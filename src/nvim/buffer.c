  ...
  if (win_valid && win->w_buffer == buf && buf->b_nwindows == 1) {
    ...
    if (!unload_buf) {
      ...
      if (apply_autocmds(EVENT_BUFHIDDEN, buf->b_fname, buf->b_fname, false,
                         buf, win_valid ? win : curwin) && !bufref_valid(&bufref)) {
        // Autocommands deleted the buffer.
        emsg(_(e_auabort));
        return false;
      }
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if ((flags & BFA_DEL)
      && buf->b_p_bl
      && apply_autocmds(EVENT_BUFDELETE, buf->b_fname, buf->b_fname, false, buf, curwin)
      && !bufref_valid(&bufref)) {
...
void set_curbuf(buf_T *buf, int action, bool update_jumplist)
{
  ...
  if (!apply_autocmds(EVENT_BUFLEAVE, NULL, NULL, false, curbuf, curwin)
...
buf_T *buflist_new(char *ffname_arg, char *sfname_arg, linenr_T lnum, int flags)
{
  ...
  if (ffname != NULL && !(flags & (BLN_DUMMY | BLN_NEW))
    ...
    if ((flags & BLN_LISTED) && !buf->b_p_bl) {
      ...
      set_bufref(&bufref, buf);
      if (!(flags & BLN_DUMMY)) {
        if (apply_autocmds(EVENT_BUFADD, NULL, NULL, false, buf, curwin)
            && !bufref_valid(&bufref)) {
          return NULL;
        }
      }
  ...
  if (!(flags & BLN_DUMMY)) {
    ...
    bufref_T bufref;
    set_bufref(&bufref, buf);
    if (apply_autocmds(EVENT_BUFNEW, NULL, NULL, false, buf, curwin)
        && !bufref_valid(&bufref)) {
      return NULL;
    }
    if ((flags & BLN_LISTED)
        && apply_autocmds(EVENT_BUFADD, NULL, NULL, false, buf, curwin)
        && !bufref_valid(&bufref)) {
      return NULL;
    }
    if (aborting()) {
...
void set_buflisted(int on)
{
  ...
  curbuf->b_p_bl = on;
  if (on) {
    apply_autocmds(EVENT_BUFADD, NULL, NULL, false, curbuf, curwin);
  } else {
    apply_autocmds(EVENT_BUFDELETE, NULL, NULL, false, curbuf, curwin);
  }
}
