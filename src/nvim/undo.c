...
/// Restores undo history on external change if 'backup' and 'undosync' are enabled.
static void u_handle_extern_change(const uint8_t *read_hash, linenr_T line_count)
{
  ...
  char *dirp = p_bdir;
  while (*dirp) {
    backup = buf_get_backup_name(fname, &dirp, false);
  ...
  if (temp_fname == NULL) {
    give_warning(_("Cannot create temp file, cannot use undo info"), true);
    return;
  }

  msg_silent++;
  int retval = buf_write(curbuf, temp_fname, NULL, 1, curbuf->b_ml.ml_line_count,
                         NULL, false, true, false, false);
  msg_silent--;
  if (retval == FAIL) {
    give_warning(_("Cannot write to temp file, cannot use undo info"), true);
  ...
  if (memcmp(new_hash, read_hash, UNDO_HASH_SIZE) != 0
      || line_count != curbuf->b_ml.ml_line_count) {
    give_warning(_("Backup is out of sync, cannot use undo info"), true);
    mismatch = true;
    goto finish;
  }
...
finish:
  // Swap buffer back in
  buf_clear();
  if (readfile(temp_fname,
               NULL,
               0,
               0,
               MAXLNUM,
               NULL,
               READ_NEW + READ_KEEP_UNDO,
               true) != OK) {
    give_warning(_("Couldn't read temp file to restore buffer"), true);
  }
  if (mismatch) {
    u_clearall(curbuf);
  }
