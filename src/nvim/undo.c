...
int u_savecommon(buf_T *buf, linenr_T top, linenr_T bot, linenr_T newbot, bool reload)
{
  ...
  if (size > 0) {
    ...
    for (i = 0, lnum = top + 1; i < size; i++) {
      ...
      u_save_line_buf(buf, &uep->ue_array[i], lnum++);
    }
  } else {
    uep->ue_array = NULL;
  }
...
static u_entry_T *unserialize_uep(bufinfo_T *bi, bool *error, const char *file_name)
{
  ...
  if (uep->ue_size > 0) {
    if ((size_t)uep->ue_size < SIZE_MAX / sizeof(undoline_T)) {
  ...
  for (size_t i = 0; i < (size_t)uep->ue_size; i++) {
    ...
    array[i].ul_len = (size_t)line_len + 1;
  }
  return uep;
}
  ...
  FUNC_ATTR_NONNULL_ARG(2)
{
  ...
  if (str_len > 0) {
    ...
    line_ptr.ul_len = (size_t)str_len + 1;
  }
    ...
    if (top < newlnum) {
      ...
      if (lnum >= top && lnum <= top + newsize + 1) {
        ...
        for (i = 0; i < newsize && i < oldsize; i++) {
          ...
          if (curbuf->b_ml.ml_line_len != (colnr_T)uep->ue_array[i].ul_len
    ...
    if (newsize) {
      ...
      for (lnum = top, i = 0; i < newsize; i++, lnum++) {
        ...
        if (empty_buffer && lnum == 0) {
          ml_replace_len((linenr_T)1, uep->ue_array[i].ul_line, uep->ue_array[i].ul_len - 1,  true);
        } else {
...
void u_find_first_changed(void)
{
  ...
  for (lnum = 1; lnum < curbuf->b_ml.ml_line_count
    ...
    if ((colnr_T)uep->ue_array[lnum - 1].ul_len != curbuf->b_ml.ml_line_len
...
void u_undoline(void)
{
  ...
  ml_replace_len(curbuf->b_u_line_lnum, curbuf->b_u_line_ptr.ul_line,
                 curbuf->b_u_line_ptr.ul_len - 1,
                 false);
  extmark_splice_cols(curbuf, (int)curbuf->b_u_line_lnum - 1, 0, (colnr_T)oldp.ul_len,
                      (colnr_T)curbuf->b_u_line_ptr.ul_len, kExtmarkUndo);
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  if (buf->b_ml.ml_line_len == 0) {
    ...
    ul->ul_len = (size_t)buf->b_ml.ml_line_len;
    ul->ul_line = xmemdup(line, ul->ul_len);
  }
}
