...
{ /* XXX */
  ...
  while (curwin->w_cursor.lnum > 1) {
    ...
    if (cin_isterminated(line, TRUE, FALSE) || cin_isscopedecl(line)
...
{
  return STRNCMP(s, "default", 7) == 0 && *(s = cin_skipcomment(s + 7)) == ':'
         && s[1] != ':';
}
 ...
 */
static int get_indent_nolabel(/* XXX */
                              linenr_T lnum)
{
  char_u *l;
  pos_T fp;
 ...
 */
static char_u cin_isterminated(
    char_u *s,
 ...
 */
static int cin_iswhileofdo(/* XXX */
                           char_u *p,
                           linenr_T lnum)
{
  pos_T cursor_save;
    ...
    if (trypos != NULL) {
      ...
      if (theline[0] == ')' && curbuf->b_ind_paren_prev) {
        ...
        for (lnum = cur_curpos.lnum - 1; lnum > our_paren_pos.lnum; --lnum) {
          ...
          /* XXX */
          if ((trypos = find_match_paren(corr_ind_maxparen(&cur_curpos)))
                  != NULL
      ...
      if (amount == -1) {
        ...
        if (*look == '(') {
          ...
          if ((trypos = findmatchlimit(NULL, ')', 0, curbuf->b_ind_maxparen))
                  != NULL
              && trypos->lnum == our_paren_pos.lnum
              && trypos->col < our_paren_pos.col)
        ...
        if (theline[0] == ')'
            || (curbuf->b_ind_unclosed == 0 && is_if_for_while == 0)
      ...
      if (theline[0] == '}') {
        ...
        for (;;) {
          ...
          if (lookfor == LOOKFOR_COMMA) {
            if (tryposBrace != NULL
                && tryposBrace->lnum >= curwin->w_cursor.lnum) {
              break;
            }
            if (terminated == ',') {
          ...
          if (terminated == 0
              || (lookfor != LOOKFOR_UNTERM && terminated == ',')) {
            if (lookfor != LOOKFOR_ENUM_OR_INIT
             ...
             */
              ...
              if (lookfor == LOOKFOR_ENUM_OR_INIT) {
                ...
                if (lookfor == LOOKFOR_INITIAL && terminated == ',') {
                  ...
                  if (lookfor == LOOKFOR_INITIAL && *l != NUL
                      && l[STRLEN(l) - 1] == '\\') {
                    // XXX
           ...
           */
            ...
            term_again:
               ...
               * If whilelevel != 0 continue looking for a "do {".
               */
              if (lookfor == LOOKFOR_TERM && *l != '}' && cin_iselse(l)
                  && whilelevel == 0) {
                if ((trypos = find_start_brace()) == NULL
                    || find_match(LOOKFOR_IF, trypos->lnum) == FAIL)
                  break;
