...
#define HEADER_SIZE                                                            \
  (sizeof(DATA_BL) - INDEX_SIZE) /* size of data block header */
...
void ml_open_file(buf_T *buf)
{
  ...
  if (mfp->mf_fname == NULL) { /* Failed! */
    ...
    ++no_wait_return;
    (void)EMSG2(
        _("E303: Unable to open swap file for \"%s\", recovery impossible"),
...
void ml_recover(bool checkext)
{
    ...
    if ((hp = mf_get(mfp, bnum, page_count)) == NULL) {
      ...
      if (pp->pb_id == PTR_ID) { /* it is a pointer block */
        ...
        if (idx == 0 && line_count != 0) {
          ...
          if (line_count != 0) {
            ++error;
            ml_append(lnum++, (char_u *)_("???LINE COUNT WRONG"), (colnr_T)0,
                      true);
          }
        }
        ...
        if (pp->pb_count == 0) {
          ...
          if (pp->pb_pointer[idx].pe_bnum < 0) {
            ...
            if (cannot_open) {
              ++error;
              ml_append(lnum++, (char_u *)_("???LINES MISSING"), (colnr_T)0,
                        true);
            }
  ...
  if (got_int)
    ...
    MSG(">>>>>>>>>>>>>");
    EMSG(
        _("E312: Errors detected while recovering; look for lines starting "
          "with ???"));
    --no_wait_return;
  ...
  while (*dirp) {
    ...
    if (dir_name[0] == '.' && dir_name[1] == NUL) { /* check current dir */
      ...
      if (fname == NULL) {
        ...
        p = dir_name + len;
        if (after_pathsep((char *)dir_name, (char *)p) && len > 1
            && p[-1] == p[-2]) {
...
static int recov_file_names(char_u **names,
                            char_u *path,
                            int prepend_dot) FUNC_ATTR_NONNULL_ALL
{
  int num_names = 0;
...
)
{
   ...
   */
  if ((hp = ml_find_line(buf, lnum == 0 ? (linenr_T)1 : lnum, ML_INSERT))
      == NULL)
    return FAIL;
  ...
  if ((int)dp->db_free >= space_needed) { /* enough room in data block */
    ...
    /*
     * If there is not enough room we have to create a new data block and copy
     * some lines into it. Then we have to insert an entry in the pointer block.
    ...
    if (db_idx < 0) { /* left block is new, right block is existing */
      ...
      if (lines_moved == 0)
        ...
        data_moved
            = ((dp->db_index[db_idx]) & DB_INDEX_MASK) - dp->db_txt_start;
    ...
    if (lines_moved) {
      ...
      memmove((char *)dp_right + dp_right->db_txt_start,
              (char *)dp_left + dp_left->db_txt_start, (size_t)data_moved);
       ...
       */
      for (to = line_count_right, from = db_idx + 1; from < line_count_left;
           ++from, ++to)
...
static int ml_delete_int(buf_T *buf, linenr_T lnum, bool message)
{
  ...
  count
      = (long)(buf->b_ml.ml_locked_high) - (long)(buf->b_ml.ml_locked_low) + 2;
                     ...
                     char_u *dir_name)
{
  ...
  if (after_pathsep((char *)dir_name, (char *)s) && len > 1
    ...
    r = NULL;
    if ((s = (char_u *)make_percent_swname((char *)dir_name, (char *)fname))
        != NULL) {
...
static char *findswapname(buf_T *buf,
                          char **dirp,
                          char *old_fname,
                          bool *found_existing_dir) FUNC_ATTR_WARN_UNUSED_RESULT
    FUNC_ATTR_NONNULL_ARG(1, 2, 4)
{
  ...
  for (;;) {
    ...
    if (fname[n - 2] == 'w' && fname[n - 1] == 'p') {  // first try
      ...
      if (!recoverymode && buf_fname != NULL && !buf->b_help
        ...
        if (fd >= 0) {
          if (read_eintr(fd, &b0, sizeof(b0)) == sizeof(b0)) {
            ...
            if (b0.b0_flags & B0_SAME_DIR) {
              if (fnamecmp(path_tail(buf->b_ffname), path_tail(b0.b0_fname))
                      != 0
        ...
        if (differ == false && !(curbuf->b_flags & BF_RECOVERED)
          ...
          if (swap_exists_action != SEA_NONE && choice == 0) {
            ...
            choice = do_dialog(
                VIM_WARNING, (char_u *)_("VIM - ATTENTION"), (char_u *)name,
...
static void ml_updatechunk(buf_T *buf, linenr_T line, long len, int updtype)
{
  ...
  if (updtype == ML_CHNK_ADDLINE) {
    ...
    if (buf->b_ml.ml_usedchunks + 1 >= buf->b_ml.ml_numchunks) {
      ...
      buf->b_ml.ml_chunksize = (chunksize_T *)xrealloc(
          buf->b_ml.ml_chunksize, sizeof(chunksize_T) * buf->b_ml.ml_numchunks);
    }
...
long ml_find_line_or_offset(buf_T *buf, linenr_T lnum, long *offp, bool no_ff)
{
  ...
  if (buf->b_ml.ml_usedchunks == -1 || buf->b_ml.ml_chunksize == NULL
      || lnum < 0)
    return -1;
  ...
  while ((lnum != 0 && curline < lnum) || (offset != 0 && size < offset)) {
    if (curline > buf->b_ml.ml_line_count
      ...
      return -1;
    dp = hp->bh_data;
    count = (long)(buf->b_ml.ml_locked_high) - (long)(buf->b_ml.ml_locked_low)
            + 1;
