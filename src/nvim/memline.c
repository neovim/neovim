...
void ml_recover(bool checkext)
{
  ...
  if ((hp = mf_get(mfp, 0, 1)) == NULL) {
    msg_start();
    msg_puts_hl(_("Unable to read block 0 from "), id, true);
    msg_outtrans(mfp->mf_fname, id, true);
    msg_puts_hl(_("\nMaybe no changes were made or Vim did not update the swap file."), id, true);
    msg_end();
    goto theend;
  }
  ZeroBlock *b0p = hp->bh_data;
  if (strncmp(b0p->b0_version, "VIM 3.0", 7) == 0) {
    ...
    msg_puts_hl(_(" cannot be used with this version of Vim.\n"), 0, true);
  ...
  if (b0_magic_wrong(b0p)) {
    ...
    msg_puts_hl(_("The file was created on "), id, true);
    // avoid going past the end of a corrupted hostname
    b0p->b0_fname[0] = NUL;
    msg_puts_hl(b0p->b0_hname, id, true);
    msg_puts_hl(_(",\nor the file has been damaged."), id, true);
    msg_end();
    goto theend;
  }
