...
void pum_redraw(void)
{
  ...
  for (i = 0; i < pum_height; ++i) {
    ...
    for (round = 1; round <= 3; ++round) {
      ...
      if (pum_rl) {
        grid_fill(&pum_grid, row, row + 1, col_off - pum_base_width - n + 1, grid_col + 1, ' ', ' ',
                  attr);
        grid_col = col_off - pum_base_width - n + 1;
      } else {
        grid_fill(&pum_grid, row, row + 1, grid_col, col_off + pum_base_width + n, ' ', ' ', attr);
    ...
    if (pum_rl) {
      grid_fill(&pum_grid, row, row + 1, col_off - pum_width + 1, grid_col + 1, ' ', ' ', attr);
    } else {
      grid_fill(&pum_grid, row, row + 1, grid_col, col_off + pum_width, ' ', ' ', attr);
    }
