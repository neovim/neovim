...
static void ins_compl_longest_match(compl_T *match)
{
  if (compl_leader.data == NULL) {
    ...
    ins_bytes(compl_leader.data + get_compl_len());
    ins_redraw(false);
...
static int ins_compl_build_pum(void)
{
  ...
  while (comp != NULL) {
    if (comp->cp_text[CPT_ABBR] != NULL) {
      ...
      compl_match_array[i].pum_text = comp->cp_str.data;
    }
  ...
  if (c == Ctrl_E) {
    ...
    if (p != NULL) {
      ...
      if ((int)plen > compl_len) {
        ins_bytes_len(p + compl_len, plen - (size_t)compl_len);
      }
    }
...
void ins_compl_insert(bool in_compl_func)
{
  ...
  if (compl_len < (int)compl_shown_match->cp_str.size) {
    ins_bytes(compl_shown_match->cp_str.data + compl_len);
  }
                          ...
                          bool in_compl_func)
{
  ...
  if (compl_no_insert && !started) {
    ins_bytes(compl_orig_text.data + get_compl_len());
    compl_used_match = false;
    restore_orig_extmarks();
