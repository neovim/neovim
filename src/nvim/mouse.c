...
static bool handle_mouse_cb(bool is_click, bool is_drag, bool is_scroll, int button)
{
  ...
  win_T *wp = NULL;
  if (!is_click && !is_scroll && button != MOUSE_RELEASE && last_click_win > 0) {
    wp = win_id2wp(last_click_win);
  }
  if (wp == NULL) {
    wp = mouse_find_win(&grid, &row, &col);
    if (is_click) {
      // TODO(zeertzjq): should this also be done when dragging the mouse
      // and the last-clicked window cannot be found?
