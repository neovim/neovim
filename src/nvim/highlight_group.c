...
bool hl_ns_get_attrs(int ns_id, int hl_id, HlAttrs *attrs)
{
  ...
  if (ns_id == 0) {
    ...
    *attrs = syn_attr2entry(sgp->sg_attr);
    return true;
  }

  // Non-global namespace
  NS ns = (NS)ns_id;
  int attr_id = ns_get_hl(&ns, hl_id, false, sgp->sg_set);

  if (attr_id == 0) {
    return false;
  }

  *attrs = syn_attr2entry(attr_id);
  return true;
}
