...
#ifdef WIN32
#define DFLT_EFM                                                                                   \
  "%f(%l) \\=: %t%*\\D%n: %m,%*[^\"]\"%f\"%*\\D%l: %m,%f(%l) \\=: %m,%*[^ ] %f %l: "               \
  "%m,%f:%l:%c:%m,%f(%l):%m,%f:%l:%m,%f|%l| %m"
#else
#define DFLT_EFM                                                                                   \
  "%*[^\"]\"%f\"%*\\D%l: %m,\"%f\"%*\\D%l: %m,%-G%f:%l: (Each undeclared identifier is reported "  \
  "only once,%-G%f:%l: for each function it appears in.),%-GIn file included from "                \
  "%f:%l:%c:,%-GIn file included from %f:%l:%c\\,,%-GIn file included from %f:%l:%c,%-GIn file "   \
  "included from %f:%l,%-G%*[ ]from %f:%l:%c,%-G%*[ ]from %f:%l:,%-G%*[ ]from %f:%l\\,,%-G%*[ "    \
...
#define STL_ALL                                                                                    \
  ((char_u[]){                                                                                     \
      STL_FILEPATH,    STL_FULLPATH,                                                               \
      STL_FILENAME,    STL_COLUMN,                                                                 \
      STL_VIRTCOL,     STL_VIRTCOL_ALT,                                                            \
      STL_LINE,        STL_NUMLINES,                                                               \
      STL_BUFNO,       STL_KEYMAP,                                                                 \
      STL_OFFSET,      STL_OFFSET_X,                                                               \
      STL_BYTEVAL,     STL_BYTEVAL_X,                                                              \
      STL_ROFLAG,      STL_ROFLAG_ALT,                                                             \
      STL_HELPFLAG,    STL_HELPFLAG_ALT,                                                           \
      STL_FILETYPE,    STL_FILETYPE_ALT,                                                           \
      STL_PREVIEWFLAG, STL_PREVIEWFLAG_ALT,                                                        \
      STL_MODIFIED,    STL_MODIFIED_ALT,                                                           \
      STL_QUICKFIX,    STL_PERCENTAGE,                                                             \
      STL_ALTPERCENT,  STL_ARGLISTSTAT,                                                            \
      STL_PAGENUM,     STL_VIM_EXPR,                                                               \
      STL_SEPARATE,    STL_TRUNCMARK,                                                              \
      STL_USER_HL,     STL_HIGHLIGHT,                                                              \
...
#define LISPWORD_VALUE                                                                             \
  "defun,define,defmacro,set!,lambda,if,case,let,flet,let*,letrec,do,do*,define-syntax,let-"       \
  "syntax,letrec-syntax,destructuring-bind,defpackage,defparameter,defstruct,deftype,defvar,do-"   \
  "all-symbols,do-external-symbols,do-symbols,dolist,dotimes,ecase,etypecase,eval-when,labels,"    \
  "macrolet,multiple-value-bind,multiple-value-call,multiple-value-prog1,multiple-value-setq,"     \
  "prog1,progv,typecase,unless,unwind-protect,when,with-input-from-string,with-open-file,with-"    \
  "open-stream,with-output-to-string,with-package-iterator,define-condition,handler-bind,handler-" \
  "case,restart-bind,restart-case,with-simple-restart,store-value,use-value,muffle-warning,abort," \
  "continue,with-slots,with-slots*,with-accessors,with-accessors*,defclass,defmethod,print-"       \
  "unreadable-object"
...
static char *(p_bo_values[])
    = {"all",      "backspace", "cursor", "complete",   "copy",     "ctrlg", "error",
...
static char *(p_ssop_values[])
    = {"buffers",  "winpos",  "resize", "winsize", "localoptions", "options", "help",   "blank",
       "globals",  "slash",   "unix",   "sesdir",  "curdir",       "folds",   "cursor", "tabpages",
       "terminal", "skiprtp", NULL};
#endif
