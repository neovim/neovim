...
/// Return the list of the user-defined commands in the specified buffer,
/// if no buffer is specified, then return list of global(non-buffer) the
/// commands
///
/// @param[in]  buf     The specified buffer, if NULL then
///                     global commands required
///
...
ArrayOf(Dictionary) commands_array(buf_T *buf)
{
  ...
  for (int i = 0; i < gap->ga_len; i++) {
    ...
    // Name
    PUT(dic, "name", STRING_OBJ(cstr_to_string((const char *)cmd->uc_name)));
    ...
    PUT(dic, "nargs", STRING_OBJ(cstr_to_string((const char *)arg)));

    // Definition
    PUT(dic, "definition",
        STRING_OBJ(cstr_to_string((const char *)cmd->uc_rep)));

    // Complete
    char *cmd_compl = get_command_complete(cmd->uc_compl);
    if (cmd_compl != NULL) {
      PUT(dic, "complete",
          STRING_OBJ(cstr_to_string(cmd_compl)));
    }

    // Complete Arg
    if (cmd->uc_compl_arg != NULL) {
      PUT(dic, "complete_arg",
          STRING_OBJ(cstr_to_string((const char *)cmd->uc_compl_arg)));
    }
    ...
    if (cmd->uc_argt & (RANGE|COUNT)) {
      ...
      PUT(dic, "range",
          STRING_OBJ(cstr_to_string((const char *)Range)));
    }
    ...
    for (int j = 0; addr_type_complete[j].expand != -1; j++) {
      if (addr_type_complete[j].expand != ADDR_LINES
          && addr_type_complete[j].expand == cmd->uc_addr_type) {
        PUT(dic, "addr",
            STRING_OBJ(cstr_to_string(addr_type_complete[j].name)));
        break;
      }
    }
