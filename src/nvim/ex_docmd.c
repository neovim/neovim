...
static char *find_command_after_range(exarg_T *eap)
{
  char *cmd = eap->cmd;
...
static void set_forceit_flag(exarg_T *eap, char **p)
{
...
// Parse register designation and count values for the command.
static bool parse_register_and_count(exarg_T *eap, const char **errormsg, bool for_execution)
{
  parse_register(eap);
  return parse_count(eap, errormsg, for_execution) != FAIL;
}
...
bool parse_cmdline(char **cmdline, exarg_T *eap, CmdParseInfo *cmdinfo, const char **errormsg)
{
  ...
  *eap = (exarg_T){
  ...
  // Find command name after any range specification
  char *p = find_command_after_range(eap);
  if (p == NULL) {
  ...
  // Parse register designation and count values
  if (!parse_register_and_count(eap, errormsg, false)) {
    goto end;
  }
                        ...
                        void *cookie)
{
  ...
  char *p = find_command_after_range(&ea);
  ...
  // Parse register and count specifications
  if (!parse_register_and_count(&ea, &errormsg, true)) {
    goto doend;
  }
