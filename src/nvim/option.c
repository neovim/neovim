...
)
{
  int opt_idx;
  char_u      *errmsg;
  char_u errbuf[ERROR_BUFFER_LENGTH];
  char_u      *startarg;
...
char_u *check_stl_option(char_u *s)
{
  int itemcnt = 0;
  int groupdepth = 0;
  static char_u errbuf[ERROR_BUFFER_LENGTH];
  ...
  while (*s && itemcnt < STL_MAX_ITEM) {
    ...
    if (vim_strchr(STL_ALL, *s) == NULL) {
      return illegal_char(errbuf, ERROR_BUFFER_LENGTH, *s);
    }
    if (*s == '{') {
      s++;
      while (*s != '}' && *s)
