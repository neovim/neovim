...
#define MAP_HASH(mode, c1)                                                     \
  (((mode) & (NORMAL + VISUAL + SELECTMODE + OP_PENDING + TERM_FOCUS))         \
       ? (c1)                                                                  \
       : ((c1) ^ 0x80))
...
)
{
  ...
  for (const buffblock_T *bp = buffer->bh_first.b_next; bp != NULL;
       bp = bp->b_next) {
...
void AppendToRedobuffLit(const char_u *str, int len)
{
  ...
  while (len < 0 ? *s != NUL : s - (const char *)str < len) {
    ...
    const int c
        = (has_mbyte ? mb_cptr2char_adv((const char_u **)&s) : (uint8_t)(*s++));
...
void del_typebuf(int len, int offset)
{
   ...
   */
    ...
    if (typebuf.tb_off > MAXMAPLEN) {
      memmove(typebuf.tb_buf + MAXMAPLEN, typebuf.tb_buf + typebuf.tb_off,
              (size_t)offset);
    ...
    int bytes = typebuf.tb_len - offset + 1;
    assert(bytes > 0);
    memmove(typebuf.tb_buf + typebuf.tb_off + offset, typebuf.tb_buf + i + len,
            (size_t)bytes);
...
)
{
  ...
  if ((scriptin[curscript]
       = file_open_new(&error, (char *)NameBuff, kFileReadOnly, 0))
      == NULL) {
...
void vungetc(/* unget one character (can only be done once!) */
             int c)
{
  old_char = c;
      ...
      for (;;) {
        ...
        if (got_int) {
          ...
          if (no_mapping == 0 && maphash_valid
            ...
            mp_match = NULL;
            mp_match_len = 0;
            for (; mp != NULL; mp->m_next == NULL ? (mp = mp2, mp2 = NULL)
                                                  : (mp = mp->m_next)) {
              /*
                        ...
                        MapArguments *mapargs)
{
  ...
  char_u *replaced = replace_termcodes(orig_lhs, orig_lhs_len, &lhs_buf, true,
                                       true, true, cpo_flags);
  ...
  if (STRICMP(orig_rhs, "<nop>") == 0) {        // "<Nop>" means nothing
    ...
    replaced = replace_termcodes(orig_rhs, orig_rhs_len, &rhs_buf, false, true,
                                 true, cpo_flags);
...
int str_to_mapargs(const char_u *strargs, bool is_unmap, MapArguments *mapargs)
{
  ...
  set_maparg_lhs_rhs(lhs_to_replace, orig_lhs_len, rhs_start, orig_rhs_len,
...
int check_abbr(int c, char_u *ptr, int col, int mincol)
{
  ...
  if (scol < col) { /* there is a word in front of the cursor */
    ...
    for (; mp;
      ...
      match = (mp->m_mode & State) && qlen == len
 ...
 * special characters.
 */
static char_u *eval_map_expr(char_u *str,
    ...
    for (hash = 0; hash < 256; ++hash) {
      ...
      for (; mp != NULL; mp = mp->m_next) {
        ...
        if ((mp->m_mode & mode) && (!exact || mp->m_keylen == len)) {
          ...
          if (ign_mod && keylen >= 3 && s[0] == K_SPECIAL
              && s[1] == KS_MODIFIER) {
            s += 3;
            keylen -= 3;
          }
