 ...
 */
static long xdl_split(unsigned long const *ha1,
                      long off1,
                      long lim1,
  ...
  for (ec = 1;; ec++) {
    ...
    if (got_snake && ec > xenv->heur_min) {
      for (best = 0, d = fmax; d >= fmin; d -= 2) {
        ...
        if (v > XDL_K_HEUR * ec && v > best && off1 + xenv->snake_cnt <= i1
            && i1 < lim1 && off2 + xenv->snake_cnt <= i2 && i2 < lim2) {
                ...
                xdfenv_t *xe)
{
  ...
  if (xdl_recs_cmp(&dd1, 0, dd1.nrec, &dd2, 0, dd2.nrec, kvdf, kvdb,
                   (xpp->flags & XDF_NEED_MINIMAL) != 0, &xenv)
      < 0) {
    xdl_free(kvd);
    xdl_free_env(xe);
    return -1;
                            ...
                            struct split_score *s)
{
  ...
  if (indent == -1) {
    ...
    if (m->post_indent != -1 && m->post_indent > indent) {
       ...
       */
      s->penalty += any_blanks ? RELATIVE_OUTDENT_WITH_BLANK_PENALTY
                               : RELATIVE_OUTDENT_PENALTY;
    } else {
      /*
       * That was probably the end of a block.
       */
      s->penalty += any_blanks ? RELATIVE_DEDENT_WITH_BLANK_PENALTY
...
static int xdl_call_hunk_func(xdfenv_t *xe UNUSED,
                              xdchange_t *xscr,
                              xdemitcb_t *ecb,
                              xdemitconf_t const *xecfg)
{
  ...
  for (xch = xscr; xch; xch = xche->next) {
    ...
    if (!xch)
      break;
    if (xecfg->hunk_func(xch->i1, xche->i1 + xche->chg1 - xch->i1, xch->i2,
                         xche->i2 + xche->chg2 - xch->i2, ecb->priv)
        < 0)
      return -1;
  }
  return 0;
