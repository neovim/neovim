...
static buf_T *do_sub(exarg_T *eap, proftime_T timeout, bool do_buf_event)
{
  long i = 0;
...
#define PUSH_PREVIEW_LINES()                                                   \
  ...
  if (preview && !aborting()) {
    if (got_quit || profile_passed_limit(timeout)) {  // Too slow, disable.
      ...
      preview_buf
          = show_sub(eap, old_cursor, &preview_lines, pre_hl_id, pre_src_id);
      if (subsize > 0) {
...
static buf_T *show_sub(exarg_T *eap,
                       ...
                       int hl_id,
                       int src_id) FUNC_ATTR_NONNULL_ALL
{
