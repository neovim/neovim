...
int do_write(exarg_T *eap)
{
  ...
  if (check_overwrite(eap, curbuf, fname, ffname, other) == OK) {
    if (eap->cmdidx == CMD_saveas && alt_buf != NULL) {
      ...
      if (!alt_buf->b_p_bl) {
        alt_buf->b_p_bl = true;
        apply_autocmds(EVENT_BUFADD, NULL, NULL, false, alt_buf, curwin);
      }
      if (curbuf != was_curbuf || aborting()) {
            ...
            win_T *oldwin)
{
  ...
  if (!auto_buf) {
    ...
    if (!oldbuf) {                          // need to read the file
      ...
      apply_autocmds_retval(EVENT_BUFENTER, NULL, NULL, false, curbuf, curwin,
                            &retval);
      if ((flags & ECMD_NOWINENTER) == 0) {
        apply_autocmds_retval(EVENT_BUFWINENTER, NULL, NULL, false, curbuf, curwin,
                              &retval);
      }
    }
    check_arg_idx(curwin);
