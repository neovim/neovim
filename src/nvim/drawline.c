...
static int draw_virt_text_item(buf_T *buf, int col, VirtText vt, HlMode hl_mode, int max_col,
                               int vcol, int skip_cells, int flags)
{
  const char *virt_str = "";
  int virt_attr = 0;
  size_t virt_pos = 0;
  ...
  while (col < max_col) {
    if (*virt_str == NUL && (flags & kVLEolHl) && virt_pos == kv_size(vt)) {
      virt_str = " ";
    }
  ...
  while (true) {
    ...
    if (mb_schar == NUL) {
      ...
      if (kv_size(fold_vt) > 0) {
        draw_virt_text_item(buf, win_col_offset, fold_vt, kHlModeCombine, view_width, 0, 0, 0);
      }
      draw_virt_text(wp, buf, win_col_offset, &wlv.col, wlv.row);
    ...
    if (wlv.col >= view_width && (!has_foldtext || wlv.filler_todo > 0)
      ...
      if (has_virt_line) {
        ...
        draw_virt_text_item(buf,
                            ...
                            view_width,
                            0,
                            virt_line_skip_cells, virt_line_flags);
      } else if (wlv.filler_todo <= 0) {
