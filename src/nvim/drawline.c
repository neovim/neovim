...
#define vcol_hlc(wlv) ((wlv).vcol - (wlv).vcol_off_co)
  ...
  TriState virt_line_leftcol = kFalse;
    ...
    if (draw_cols) {
      ...
      if (wlv.filler_todo > 0) {
        ...
        if (index > 0) {
          ...
          virt_line_leftcol = kv_A(virt_lines, virt_line_index).left_col;
        }
      }

      if (virt_line_index >= 0 && virt_line_leftcol == kTrue) {
        // skip columns
    ...
    if (wlv.col >= grid->cols && (!has_foldtext || virt_line_index >= 0)
      ...
      if (virt_line_index >= 0) {
        draw_virt_text_item(buf,
                            virt_line_leftcol == kTrue ? 0 : win_col_offset,
                            kv_A(virt_lines, virt_line_index).line,
                            kHlModeReplace,
                            grid->cols,
                            0,
                            virt_line_leftcol == kNone ? wp->w_leftcol : 0);
      } else if (wlv.filler_todo <= 0) {
