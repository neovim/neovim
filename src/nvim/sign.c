...
static void sign_sort_by_prio_on_line(buf_T *buf,
                                      signlist_T *sign) FUNC_ATTR_NONNULL_ALL
{
...
static void sign_unplace_cmd(buf_T *buf,
                             linenr_T lnum,
                             char_u *sign_name,
                             int id,
                             char_u *group)
...
static void sign_jump_cmd(buf_T *buf,
                          linenr_T lnum,
                          char_u *sign_name,
                          int id,
                          char_u *group)
...
static void sign_list_defined(sign_T *sp)
{
  ...
  if (sp->sn_line_hl > 0) {
    msg_puts(" linehl=");
    const char *const p
        = get_highlight_name_ext(NULL, sp->sn_line_hl - 1, false);
    if (p == NULL) {
      msg_puts("NONE");
    } else {
      msg_puts(p);
  ...
  if (sp->sn_text_hl > 0) {
    msg_puts(" texthl=");
    const char *const p
        = get_highlight_name_ext(NULL, sp->sn_text_hl - 1, false);
    if (p == NULL) {
      msg_puts("NONE");
    } else {
      msg_puts(p);
  ...
  if (sp->sn_num_hl > 0) {
    msg_puts(" numhl=");
    const char *const p
        = get_highlight_name_ext(NULL, sp->sn_num_hl - 1, false);
    if (p == NULL) {
      msg_puts("NONE");
    } else {
      msg_puts(p);
...
char_u *get_sign_name(expand_T *xp, int idx)
{
  switch (expand_what) {
    ...
    case EXP_DEFINE: {
      char *define_arg[]
          = {"icon=", "linehl=", "text=", "texthl=", "numhl=", NULL};
