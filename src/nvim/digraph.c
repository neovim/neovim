...
char_u *get_digraph_for_char(int val_arg)
{
  ...
  for (int use_defaults = 0; use_defaults <= 1; use_defaults++) {
    ...
    for (int i = 0; use_defaults ? dp->char1 != NUL : i < user_digraphs.ga_len;
         i++) {
      if (dp->result == val) {
        r[0] = dp->char1;
        r[1] = dp->char2;
    ...
    FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (dp->result != 0) {
    ...
    *p = NUL;
    ...
    assert(p >= buf);
    vim_snprintf((char *)p, sizeof(buf) - (size_t)(p - buf), " %3d",
                 dp->result);
    msg_outtrans(buf);
  }
}
...
#define KMAP_LLEN 200  // max length of "to" and "from" together
  ...
  for (;;) {
    ...
    if ((*p != '"') && (*p != NUL)) {
      ...
      if ((STRLEN(kp->from) + STRLEN(kp->to) >= KMAP_LLEN) || (*kp->from == NUL)
          || (*kp->to == NUL)) {
        if (*kp->to == NUL) {
