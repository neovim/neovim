                      ...
                      bool on_top)
{
  ...
  if (grid->comp_index != 0) {
    ...
    if (ui_comp_should_draw()) {
      ...
      compose_area(grid->comp_row, row, grid->comp_col,
    ...
    bool cmd_completion
        = (grid == &pum_grid && (State & CMDLINE) && (wop_flags & WOP_PUM));
...
static void ui_comp_raise_grid(ScreenGrid *grid, size_t new_index)
{
  ...
  for (size_t i = old_index; i < new_index; i++) {
    ...
    int endcol
        = MIN(grid->comp_col + grid->Columns, grid2->comp_col + grid2->Columns);
    compose_area(
        MAX(grid->comp_row, grid2->comp_row),
        MIN(grid->comp_row + grid->Rows, grid2->comp_row + grid2->Rows),
        startcol, endcol);
  }
}

static void ui_comp_grid_cursor_goto(UI *ui,
                                     Integer grid_handle,
                                     Integer r,
                                     Integer c)
{
...
ScreenGrid *ui_comp_mouse_focus(int row, int col)
{
  for (ssize_t i = (ssize_t)kv_size(layers) - 1; i > 0; i--) {
    ...
    if (grid->focusable && row >= grid->comp_row
        && row < grid->comp_row + grid->Rows && col >= grid->comp_col
        && col < grid->comp_col + grid->Columns) {
      return grid;
    }
  }
  return NULL;
...
/// the downstream UI.)
static void compose_line(Integer row,
                         Integer startcol,
                         Integer endcol,
                         LineFlags flags)
{
  ...
  ScreenGrid *grid = NULL;
  schar_T *bg_line
      = &default_grid.chars[default_grid.line_offset[row] + (size_t)startcol];
  sattr_T *bg_attrs
      = &default_grid.attrs[default_grid.line_offset[row] + (size_t)startcol];
  ...
  while (col < endcol) {
    ...
    if (row == msg_sep_row && grid->comp_index <= msg_grid.comp_index) {
      ...
      memcpy(linebuf + (col - startcol), grid->chars + off,
             n * sizeof(*linebuf));
      memcpy(attrbuf + (col - startcol), grid->attrs + off,
             n * sizeof(*attrbuf));
    ...
    if (grid->blending) {
      ...
      for (int i = col - (int)startcol; i < until - startcol; i += width) {
        ...
        if (width == 2) {
          attrbuf[i + 1]
              = (sattr_T)hl_blend_attrs(bg_attrs[i + 1], attrbuf[i + 1], &thru);
        }
        if (thru) {
  ...
  ui_composed_call_raw_line(1, row, startcol + skipstart, endcol - skipend,
                            endcol - skipend, 0, flags,
                            (const schar_T *)linebuf + skipstart,
                            (const sattr_T *)attrbuf + skipstart);
}

static void compose_debug(Integer startrow,
                          Integer endrow,
                          Integer startcol,
                          Integer endcol,
                          int syn_id,
...
static void compose_area(Integer startrow,
                         Integer endrow,
                         Integer startcol,
                         Integer endcol)
{
...
static void ui_comp_raw_line(UI *ui,
                             Integer grid,
                             Integer row,
                             Integer startcol,
                             Integer endcol,
...
static void ui_comp_msg_set_pos(UI *ui,
                                Integer grid,
                                Integer row,
                                Boolean scrolled,
                                String sep_char)
...
static void ui_comp_grid_scroll(UI *ui,
                                Integer grid,
                                Integer top,
                                Integer bot,
                                Integer left,
                                ...
                                Integer cols)
{
  ...
  if (covered || curgrid->blending) {
    ...
    for (int r = (int)(top + MAX(-rows, 0)); r < bot - MAX(rows, 0); r++) {
      ...
      // the invalid space.
      if (curgrid->attrs[curgrid->line_offset[r - curgrid->comp_row] + left
                         - curgrid->comp_col]
          >= 0) {
...
static void ui_comp_grid_resize(UI *ui,
                                Integer grid,
                                Integer width,
                                Integer height)
{
