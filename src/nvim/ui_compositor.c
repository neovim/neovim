                      ...
                      bool on_top)
{
  ...
  if (grid->comp_index != 0) {
    ...
    if (ui_comp_should_draw()) {
      ...
      compose_area(grid->comp_row, row, grid->comp_col, grid->comp_col + grid->Columns);
...
static void ui_comp_raise_grid(ScreenGrid *grid, size_t new_index)
{
  ...
  for (size_t i = old_index; i < new_index; i++) {
    ...
    int endcol = MIN(grid->comp_col + grid->Columns, grid2->comp_col + grid2->Columns);
...
static void compose_line(Integer row, Integer startcol, Integer endcol, LineFlags flags)
{
  ...
  while (col < endcol) {
    ...
    if (row == msg_sep_row && grid->comp_index <= msg_grid.comp_index) {
      ...
      if (grid->comp_col + grid->Columns > until && grid->chars[off + n][0] == NUL) {
                                ...
                                String sep_char)
{
  ...
  } else if (row < msg_current_row && ui_comp_should_draw() && msg_current_row < Rows) {
                                ...
                                Integer cols)
{
  ...
  if (covered || curgrid->blending) {
    ...
    for (int r = (int)(top + MAX(-rows, 0)); r < bot - MAX(rows, 0); r++) {
      ...
      // the invalid space.
      if (curgrid->attrs[curgrid->line_offset[r - curgrid->comp_row] + left - curgrid->comp_col]
          >= 0) {
