...
typedef struct qf_list_S {
  ...
  char_u      *qf_qftf;         ///< setting for this list
                           ...
                           char_u *dirname, bool first_bufline)
{
  ...
  if (qftf != NULL && *qftf != NUL) {
    ...
    tv_dict_add_nr(dict, S_LEN("quickfix"), (long)IS_QF_LIST(qfl));
    tv_dict_add_nr(dict, S_LEN("id"), (long)qfl->qf_id);
    tv_dict_add_nr(dict, S_LEN("idx"), (long)(lnum + 1));
    dict->dv_refcount++;
