...
expand:
    ...
    if (flags & DIP_START) {
      // pack/*/start/*/ patterns
      snprintf(buf, buf_len, "pack/*/start/*/%s%s%s%s", *dirnames[i] ? dirnames[i] : "",
               *dirnames[i] ? "/" : "", pat, expand_dirs ? "*" : "*.{vim,lua}");
      globpath(p_pp, buf, gap, glob_flags, expand_dirs);

      // start/*/ patterns
      snprintf(buf, buf_len, "start/*/%s%s%s%s", *dirnames[i] ? dirnames[i] : "",
               *dirnames[i] ? "/" : "", pat, expand_dirs ? "*" : "*.{vim,lua}");
      globpath(p_pp, buf, gap, glob_flags, expand_dirs);
    }
    ...
    if (flags & DIP_OPT) {
      // pack/*/opt/*/ patterns
      snprintf(buf, buf_len, "pack/*/opt/*/%s%s%s%s", *dirnames[i] ? dirnames[i] : "",
               *dirnames[i] ? "/" : "", pat, expand_dirs ? "*" : "*.{vim,lua}");
      globpath(p_pp, buf, gap, glob_flags, expand_dirs);

      // opt/*/ patterns
      snprintf(buf, buf_len, "opt/*/%s%s%s%s", *dirnames[i] ? dirnames[i] : "",
               *dirnames[i] ? "/" : "", pat, expand_dirs ? "*" : "*.{vim,lua}");
      globpath(p_pp, buf, gap, glob_flags, expand_dirs);
    }
