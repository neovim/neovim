...
/// Get highlight attributes for a namespace highlight
///
/// @param ns_id Namespace ID
/// @param hl_id Highlight group ID
/// @param[out] attrs Pointer to store the attributes
/// @return true if namespace highlight exists and has valid attributes
bool hl_ns_get_attrs(int ns_id, int hl_id, HlAttrs *attrs)
{
  ...
  if (it.attr_id > 0 && it.attr_id < (int)set_size(&attr_entries)) {
    *attrs = attr_entry(it.attr_id).attr;
    return true;
  }
