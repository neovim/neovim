...
#define _RINGBUF_LENGTH(rb)                                                                        \
  ((rb)->first == NULL                                                                             \
       ? 0                                                                                         \
       : ((rb)->next == (rb)->first)                                                               \
             ? (size_t)((rb)->buf_end - (rb)->buf) + 1                                             \
             : ((rb)->next > (rb)->first)                                                          \
                   ? (size_t)((rb)->next - (rb)->first)                                            \
...
#define RINGBUF_FORALL(rb, RBType, varname)                                                        \
  ...
  for (RBType *varname = ((rb)->first == NULL ? (rb)->next : (rb)->first); varname##_length_fa_;   \
       (varname = _RINGBUF_NEXT(rb, varname)), varname##_length_fa_--)
...
#define RINGBUF_ITER_BACK(rb, RBType, varname)                                                     \
  ...
  for (varname = ((rb)->next == (rb)->buf ? (rb)->buf_end : (rb)->next - 1); varname##_length_ib_; \
       (varname = _RINGBUF_PREV(rb, varname)), varname##_length_ib_--)
...
#define RINGBUF_INIT(TypeName, funcprefix, RBType, rbfree)                                         \
  ...
  static inline void funcprefix##_rb_free(TypeName##RingBuffer *const rb) REAL_FATTR_UNUSED;       \
  ...
  static inline void funcprefix##_rb_dealloc(TypeName##RingBuffer *const rb) REAL_FATTR_UNUSED;    \
  static inline void funcprefix##_rb_dealloc(TypeName##RingBuffer *const rb)                       \
  {                                                                                                \
    XFREE_CLEAR(rb->buf);                                                                          \
  }                                                                                                \
  static inline void funcprefix##_rb_push(TypeName##RingBuffer *const rb, RBType item)             \
  ...
  static inline RBType funcprefix##_rb_idx(const TypeName##RingBuffer *const rb, const size_t idx) \
      REAL_FATTR_NONNULL_ALL REAL_FATTR_PURE REAL_FATTR_UNUSED;                                    \
  static inline RBType funcprefix##_rb_idx(const TypeName##RingBuffer *const rb, const size_t idx) \
  {                                                                                                \
    return *funcprefix##_rb_idx_p(rb, idx);                                                        \
  }                                                                                                \
  static inline void funcprefix##_rb_insert(TypeName##RingBuffer *const rb, const size_t idx,      \
                                            RBType item) REAL_FATTR_NONNULL_ARG(1)                 \
      REAL_FATTR_UNUSED;                                                                           \
  static inline void funcprefix##_rb_insert(TypeName##RingBuffer *const rb, const size_t idx,      \
                                            RBType item)                                           \
  {                                                                                                \
    ...
    if (insertpos < rb->next) {                                                                    \
      memmove(insertpos + 1, insertpos, (size_t)((uintptr_t)rb->next - (uintptr_t)insertpos));     \
    } else {                                                                                       \
      assert(insertpos > rb->first);                                                               \
      assert(rb->next <= rb->first);                                                               \
      memmove(rb->buf + 1, rb->buf, (size_t)((uintptr_t)rb->next - (uintptr_t)rb->buf));           \
      *rb->buf = *rb->buf_end;                                                                     \
  ...
  static inline void funcprefix##_rb_remove(TypeName##RingBuffer *const rb, const size_t idx)      \
  ...
  {                                                                                                \
    ...
    if (rmpos == rb->next - 1) {                                                                   \
      ...
      memmove(rb->first + 1, rb->first, (size_t)((uintptr_t)rmpos - (uintptr_t)rb->first));        \
      rb->first = _RINGBUF_NEXT(rb, rb->first);                                                    \
    } else if (rmpos < rb->next) {                                                                 \
      memmove(rmpos, rmpos + 1, (size_t)((uintptr_t)rb->next - (uintptr_t)rmpos));                 \
