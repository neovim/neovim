...
# endif
  ...
  if (!read_stdin && !read_fifo && (!read_buffer || sfname != NULL)) {
    ...
    if (filtering) {
      ...
      bool did_bufrp = apply_autocmds_exarg(EVENT_BUFREADPOST, NULL, sfname, false, curbuf,
                                            curwin, eap);
...
int set_rw_fname(char *fname, char *sfname)
{
  ...
  apply_autocmds(EVENT_BUFWIPEOUT, NULL, NULL, false, curbuf, curwin);
  if (aborting()) {         // autocmds may abort script processing
    return FAIL;
  }
  if (curbuf != buf) {
  ...
  apply_autocmds(EVENT_BUFNEW, NULL, NULL, false, curbuf, curwin);
  if (curbuf->b_p_bl) {
