...
/// Parse the 'guicursor' option, or clears `shape_table` if 'guicursor'
/// is empty.
///
...
char_u *parse_shape_opt(int what)
{
  ...
  for (round = 1; round <= 2; round++) {
    if (round == 2 || *p_guicursor == NUL) {
      // Set all entry to default(block, blinkon0, default color).
      // This will default to anything that is not set.
      clear_shape_table();
      if (*p_guicursor == NUL) {
        ui_mode_info_set();
        return NULL;
...
/// Clear all entries in shape_table to block, blinkon0, and the default color.
