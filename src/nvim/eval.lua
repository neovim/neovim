  ...
  jobstart = {
    ...
    desc = [=[
      ...
      {opts} is a dictionary with these keys:
        ...
        stdin:      (string) What the job's stdin (fd 0) is connected
      	      to. One of "pipe" (default), "null", or "fd".
      	        - "pipe": a channel; write to it with |chansend()|.
      	          But for a "pty"/"term" job stdin IS the terminal, so
      	          |chansend()| sends keys (as if typed), not a separate
      	          stream.
      	        - "null": disconnected.
      	        - "fd" ("pty"/"term" only): a separate pipe, even
      	          though it is a pty. The pty stays the controlling
      	          terminal (output, and /dev/tty for prompts) while
      	          |chansend()|/|chanclose()| feed fd 0. Because stdin is
      	          split from the tty, the terminal stays interactive
      	          while data is piped: you can still type (e.g. a
      	          password), so e.g. `:w !sudo tee` works. (Feeding a
      	          pty's own stdin can't do both at once — this is why
      	          Vim's `:[range]terminal` disables typing when a range
      	          is piped; Nvim's "fd" has no such restriction.) For a
      	          non-pty job, where stdin is already a pipe, "fd" is the
      	          same as "pipe".
