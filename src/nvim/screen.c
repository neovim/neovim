...
int update_screen(int type)
{
  ...
  if (msg_scrolled || msg_grid_invalid) {
    ...
    if (msg_use_msgsep()) {
      ...
      if (type == NOT_VALID && !ui_has(kUIMultigrid) && msg_scrolled) {
        ui_comp_set_screen_valid(false);
        for (int i = valid; i < g_rows-p_ch; i++) {
    ...
    } else if (msg_scrolled > g_rows - 5) {  // clearing is faster
      type = CLEAR;
    } else if (type != CLEAR) {
      check_for_delay(false);
      grid_ins_lines(&default_grid, 0, msg_scrolled, g_rows, 0, g_columns);
      FOR_ALL_WINDOWS_IN_TAB(wp, curtab) {
        if (wp->w_floating) {
          continue;
        }
  ...
  if (type == NOT_VALID && (msg_use_grid() || msg_grid.chars)) {
    grid_fill(&default_grid, g_rows-p_ch, g_rows, 0, g_columns, ' ', ' ', 0);
  }
...
#define DID_FOLD 3      // updated a folded line
  ...
  if (wp->w_grid.g_rows == 0) {
    wp->w_redr_type = 0;
    return;
  }

  // Window is zero-width: Only need to draw the separator.
  if (wp->w_grid.g_columns == 0) {
  ...
  if ((type == VALID || type == SOME_VALID
    ...
    if (mod_top != 0
      ...
      if (j < wp->w_grid.g_rows - 2) {               // not too far off
        i = plines_m_win(wp, wp->w_topline, wp->w_lines[0].wl_lnum - 1);
        // insert extra lines for previously invisible filler lines
        if (wp->w_lines[0].wl_lnum != wp->w_topline) {
          i += win_get_fill(wp, wp->w_lines[0].wl_lnum) - wp->w_old_topfill;
        }
        if (i != 0 && i < wp->w_grid.g_rows - 2) {  // less than a screen off
          ...
          if (wp->w_lines_valid != 0) {
            ...
            if ((wp->w_lines_valid += j) > wp->w_grid.g_rows) {
              wp->w_lines_valid = wp->w_grid.g_rows;
            }
      ...
      if (j == -1) {
        ...
        if (row > 0) {
          ...
          bot_start = wp->w_grid.g_rows - row;
        }
        if ((row == 0 || bot_start < 999) && wp->w_lines_valid != 0) {
          ...
          for (;; ) {
            ...
            if (row > 0 && bot_start + row
                + (int)wp->w_lines[j].wl_size > wp->w_grid.g_rows) {
  ...
  for (;; ) {
    ...
    if (row < top_end
      ...
      if (lnum == mod_top
        ...
        if (i >= wp->w_lines_valid) {
          ...
          for (l = lnum; l < mod_bot; l++) {
            ...
            j++;
            if (new_rows > wp->w_grid.g_rows - row - 2) {
          ...
          if (xtra_rows < 0) {
            ...
            if (row - xtra_rows >= wp->w_grid.g_rows - 2) {
              mod_bot = MAXLNUM;
            } else {
              win_scroll_lines(wp, row, xtra_rows);
              bot_start = wp->w_grid.g_rows + xtra_rows;
            }
          } else if (xtra_rows > 0) {
          ...
          if (mod_bot != MAXLNUM && i != j) {
            if (j < i) {
              ...
              j -= i;
              wp->w_lines_valid += j;
              if (wp->w_lines_valid > wp->w_grid.g_rows) {
                wp->w_lines_valid = wp->w_grid.g_rows;
              }
      ...
      if (foldinfo.fi_lines == 0
          ...
          && srow + wp->w_lines[idx].wl_size > wp->w_grid.g_rows
          && win_get_fill(wp, lnum) == 0) {
        // This line is not going to fit.  Don't draw anything here,
        // will draw "@  " lines below.
        row = wp->w_grid.g_rows + 1;
      } else {
        ...
        row = win_line(wp, lnum, srow,
                       foldinfo.fi_lines ? srow : wp->w_grid.g_rows,
      ...
      if (row > wp->w_grid.g_rows) {         // past end of grid
      ...
      if (wp->w_p_rnu) {
        ...
        (void)win_line(wp, lnum, srow, wp->w_grid.g_rows, true, true,
                       info, &line_providers);
      }

      // This line does not need to be drawn, advance to the next one.
      row += wp->w_lines[idx++].wl_size;
      if (row > wp->w_grid.g_rows) {  // past end of screen
        break;
      }
  ...
  if (!eof && !didline) {
    ...
    if (lnum == wp->w_topline) {
       ...
       */
      wp->w_botline = lnum + 1;
    } else if (win_get_fill(wp, lnum) >= wp->w_grid.g_rows  - srow) {
      // Window ends in filler lines.
      wp->w_botline = lnum;
      wp->w_filler_rows = wp->w_grid.g_rows - srow;
    } else if (dy_flags & DY_TRUNCATE) {      // 'display' has "truncate"
      int scr_row = wp->w_grid.g_rows - 1;

      // Last line isn't finished: Display "@@@" in the last screen line.
      grid_puts_len(&wp->w_grid, (char_u *)"@@", 2, scr_row, 0, at_attr);

      grid_fill(&wp->w_grid, scr_row, scr_row + 1, 2, wp->w_grid.g_columns,
                '@', ' ', at_attr);
      set_empty_rows(wp, srow);
      wp->w_botline = lnum;
    } else if (dy_flags & DY_LASTLINE) {      // 'display' has "lastline"
      // Last line isn't finished: Display "@@@" at the end.
      grid_fill(&wp->w_grid, wp->w_grid.g_rows - 1, wp->w_grid.g_rows,
                wp->w_grid.g_columns - 3, wp->w_grid.g_columns, '@', '@', at_attr);
      set_empty_rows(wp, srow);
      wp->w_botline = lnum;
    } else {
      win_draw_end(wp, '@', ' ', true, srow, wp->w_grid.g_rows, HLF_AT);
      wp->w_botline = lnum;
    }
  } else {
    if (eof) {  // we hit the end of the file
      ...
      if (j > 0 && !wp->w_botfill) {
        ...
        row = win_line(wp, wp->w_botline, row, wp->w_grid.g_rows,
    ...
    win_draw_end(wp, wp->w_p_fcs_chars.eob, ' ', false, row, wp->w_grid.g_rows,
                 HLF_EOB);
  }
                        ...
                        int attr)
{
  ...
  if (nn > wp->w_grid.g_columns) {
    nn = wp->w_grid.g_columns;
  }
...
static void win_draw_end(win_T *wp, int c1, int c2, bool draw_margin, int row, int endrow, hlf_T hl)
{
  ...
  if (wp->w_p_rl) {
    ...
    grid_fill(&wp->w_grid, row, endrow, n, wp->w_grid.g_columns, c1, c2, attr);
  }
...
static int compute_foldcolumn(win_T *wp, int col)
{
  ...
  int wwidth = wp->w_grid.g_columns;
...
#define FIX_FOR_BOGUSCOLS \
  ...
  for (;; ) {
    ...
    if (((dollar_vcol >= 0
        ...
        && filler_todo <= 0) {
      draw_virt_text(buf, win_col_offset, &col, grid->g_columns);
      grid_put_linebuf(grid, row, 0, col, -grid->g_columns, wp->w_p_rl, wp,
    ...
    if (draw_state == WL_LINE
      ...
      n_extra = wp->w_p_rl ? (col + 1) : (grid->g_columns - col);
    }
    ...
    if (n_extra > 0) {
      if (c_extra != NUL || (n_extra == 1 && c_final != NUL)) {
        ...
        if ((wp->w_p_rl ? (col <= 0) : (col >= grid->g_columns - 1))
      ...
      if (extra_check) {
        ...
        if (wp->w_p_lbr && c0 == c && vim_isbreak(c)
          ...
          if (c == TAB && n_extra + col > grid->g_columns) {
            ...
            c = ' ';
                   ...
                   && (wp->w_p_list
                       || ((fromcol >= 0 || fromcol_prev >= 0)
                           && tocol > vcol
                           && VIsual_mode != Ctrl_V
                           && (wp->w_p_rl ? (col >= 0) : (col < grid->g_columns))
                           && !(noinvcur
                                && lnum == wp->w_cursor.lnum
                                && (colnr_T)vcol == wp->w_virtcol)))
                   && lcs_eol_one > 0) {
    ...
    if (!did_wcol && draw_state == WL_LINE
      ...
      if (wp->w_p_rl) {
        wp->w_wcol = grid->g_columns - col + boguscols - 1;
      } else {
    ...
    if (c == NUL && eol_hl_off == 0) {
      ...
      if (((wp->w_p_cuc
            ...
            && (int)wp->w_virtcol <
            grid->g_columns * (row - startrow + 1) + v
            && lnum != wp->w_cursor.lnum)
        ...
        while (wp->w_p_rl ? col >= 0 : col < grid->g_columns) {
          schar_from_ascii(linebuf_char[off], ' ');
          col += col_stride;
          if (draw_color_col) {
            draw_color_col = advance_color_col(VCOL_HLC, &color_cols);
      ...
      if (wp->w_buffer->terminal) {
        ...
        // logical line
        int n = wp->w_p_rl ? -1 : 1;
        while (col >= 0 && col < grid->g_columns) {
          schar_from_ascii(linebuf_char[off], ' ');
    ...
    if (wp->w_p_lcs_chars.ext != NUL
        ...
        && !wp->w_p_wrap
        && filler_todo <= 0
        && (wp->w_p_rl ? col == 0 : col == grid->g_columns - 1)
        && !has_fold
        && (*ptr != NUL
            || lcs_eol_one > 0
     ...
     */
    if ((wp->w_p_rl ? (col < 0) : (col >= grid->g_columns))
        && foldinfo.fi_lines == 0
        && (*ptr != NUL
            || filler_todo > 0
      ...
      bool wrap = wp->w_p_wrap       // Wrapping enabled.
                  ...
                  && (grid->g_columns == g_columns  // Window spans the width of the screen,
      ...
      if (filler_todo > 0) {
        ...
        if (index > 0) {
          ...
          draw_virt_text_item(buf, offset, kv_A(buf->b_virt_lines, fpos),
                              kHlModeReplace, grid->g_columns, offset);
        }
      } else {
        draw_virt_text(buf, win_col_offset, &draw_col, grid->g_columns);
      }

      grid_put_linebuf(grid, row, 0, draw_col, grid->g_columns, wp->w_p_rl,
      ...
      if (draw_state != WL_LINE && filler_todo <= 0) {
        win_draw_end(wp, '@', ' ', true, row, wp->w_grid.g_rows, HLF_AT);
        row = endrow;
      }
      ...
      off = 0;
      if (wp->w_p_rl) {
        col = grid->g_columns - 1;  // col is not used if breaking!
        off += col;
      }
                             ...
                             int rlflag, win_T *wp, int bg_attr, bool wrap)
{
  ...
  if (endcol > grid->g_columns) {
    endcol = grid->g_columns;
  }

  screen_adjust_grid(&grid, &row, &coloff);

  // Safety check. Avoids clang warnings down the call stack.
  if (grid->chars == NULL || row >= grid->g_rows || coloff >= grid->g_columns) {
  ...
  max_off_to = grid->line_offset[row] + grid->g_columns;
  ...
  if (clear_width > 0 || wp->w_width != grid->g_columns) {
...
#define L_MATCH(m) (showtail ? sm_gettail(matches[m], false) : matches[m])
  ...
  buf = xmalloc(g_columns * MB_MAXBYTES + 1);
  ...
  i = first_match;
  while ((long)(clen + status_match_len(xp, L_MATCH(i)) + 2) < g_columns) {
    if (i == match) {
      selstart = buf + len;
      selstart_col = clen;
    }
  ...
  if (row >= 0) {
    ...
    grid_fill(grid, row, row + 1, clen, g_columns,
...
static void win_redr_status(win_T *wp)
{
  ...
  if (wp->w_status_height == 0) {
    ...
    this_ru_col = ru_col - (g_columns - wp->w_width);
...
static void win_redr_custom(win_T *wp, bool draw_ruler)
{
  ...
  if (wp == NULL) {
    ...
    if (draw_ruler) {
      ...
      col = ru_col - (g_columns - wp->w_width);
...
bool grid_lefthalve(ScreenGrid *grid, int row, int col)
{
  ...
  return grid_off2cells(grid, grid->line_offset[row] + col,
                        grid->line_offset[row] + grid->g_columns) > 1;
}
...
void grid_getbytes(ScreenGrid *grid, int row, int col, char_u *bytes, int *attrp)
{
  ...
  // safety check
  if (grid->chars != NULL && row < grid->g_rows && col < grid->g_columns) {
...
void grid_puts_len(ScreenGrid *grid, char_u *text, int textlen, int row, int col, int attr)
{
  ...
  if (grid->chars == NULL
      || row >= grid->g_rows || row < 0
      || col >= grid->g_columns || col < 0) {
    return;
  }
  ...
  max_off = grid->line_offset[row] + grid->g_columns;
  while (col < grid->g_columns
    ...
    if (col + mbyte_cells > grid->g_columns) {
  ...
  if (put_dirty_first < put_dirty_last) {
    if (set_cursor) {
      ui_grid_cursor_goto(put_dirty_grid->handle, put_dirty_row,
                          MIN(put_dirty_last, put_dirty_grid->g_columns-1));
    }
               ...
               int c2, int attr)
{
  ...
  // safety check
  if (end_row > grid->g_rows) {
    end_row = grid->g_rows;
  }
  if (end_col > grid->g_columns) {
    end_col = grid->g_columns;
  }
...
void win_grid_alloc(win_T *wp)
{
  ...
  if (want_allocation && (!has_allocation
                          || grid_allocated->g_rows != total_rows
                          || grid_allocated->g_columns != total_cols)) {
  ...
  if ((send_grid_resize || was_resized) && want_allocation) {
    ui_call_grid_resize(grid_allocated->handle,
                        grid_allocated->g_columns, grid_allocated->g_rows);
  }
}
...
/// Resize the screen to g_rows and g_columns.
///
/// Allocate default_grid.chars[] and other grid arrays.
///
/// There may be some time between setting g_rows and g_columns and (re)allocating
/// default_grid arrays.  This happens when starting up and when
/// (manually) changing the shell size.  Always use default_grid.g_rows and
/// default_grid.g_columns to access items in default_grid.chars[].  Use g_rows
/// and g_columns for positioning text etc. where the final size of the shell is
/// needed.
...
retry:
  ...
  // when g_rows and g_columns have been set and we have started doing full
  // screen stuff.
  if ((default_grid.chars != NULL
       && g_rows == default_grid.g_rows
       && g_columns == default_grid.g_columns
       )
      || g_rows == 0
      || g_columns == 0
  ...
  grid_alloc(&default_grid, g_rows, g_columns, true, true);
  StlClickDefinition *new_tab_page_click_defs =
    xcalloc((size_t)g_columns, sizeof(*new_tab_page_click_defs));

  clear_tab_page_click_defs(tab_page_click_defs, tab_page_click_defs_size);
  xfree(tab_page_click_defs);

  tab_page_click_defs = new_tab_page_click_defs;
  tab_page_click_defs_size = g_columns;

  default_grid.comp_height = g_rows;
  default_grid.comp_width = g_columns;
   ...
   * in case applying autocommands always changes g_rows or g_columns.
   */
  if (starting == 0 && ++retry_count <= 3) {
    apply_autocmds(EVENT_VIMRESIZED, NULL, NULL, false, curbuf);
    // In rare cases, autocommands may have altered g_rows or g_columns,
...
void grid_alloc(ScreenGrid *grid, int rows, int columns, bool copy, bool valid)
{
  ...
  new.g_rows = rows;
  new.g_columns = columns;

  for (new_row = 0; new_row < new.g_rows; new_row++) {
    new.line_offset[new_row] = new_row * new.g_columns;
    ...
    if (copy) {
      ...
      if (new_row < grid->g_rows && grid->chars != NULL) {
        int len = MIN(grid->g_columns, new.g_columns);
...
void screenclear(void)
{
  ...
  for (i = 0; i < default_grid.g_rows; i++) {
    grid_clear_line(&default_grid, default_grid.line_offset[i],
                    default_grid.g_columns, true);
...
{
  (void)memset(grid->attrs, -1, grid->g_rows * grid->g_columns * sizeof(sattr_T));
}
...
void win_scroll_lines(win_T *wp, int row, int line_count)
{
  ...
  if (row + abs(line_count) >= wp->w_grid.g_rows) {
    return;
  }

  if (line_count < 0) {
    grid_del_lines(&wp->w_grid, row, -line_count,
                   wp->w_grid.g_rows, 0, wp->w_grid.g_columns);
  } else {
...
/// 'end' is the line after the scrolled part. Normally it is g_rows.
...
void grid_ins_lines(ScreenGrid *grid, int row, int line_count, int end, int col, int width)
{
  ...
  for (i = 0; i < line_count; i++) {
    if (width != grid->g_columns) {
      ...
      grid_clear_line(grid, temp, grid->g_columns, false);
    }
  }
...
int showmode(void)
{
  ...
  if (do_mode || reg_recording != 0) {
    ...
    if (clear_cmdline && cmdline_row < g_rows - 1) {
    ...
    if (do_mode) {
      ...
      if (edit_submode != NULL && !shortmess(SHM_COMPLETIONMENU)) {
        ...
        if (ui_has(kUIMessages)) {
          ...
          length = (g_rows - msg_row) * g_columns - 3;
        }
...
void draw_tabline(void)
{
  ...
  assert(g_columns == tab_page_click_defs_size);
  clear_tab_page_click_defs(tab_page_click_defs, tab_page_click_defs_size);
  ...
  if (*p_tal != NUL) {
    ...
    if (tabcount > 0) {
      tabwidth = (g_columns - 1 + tabcount / 2) / tabcount;
    }
    ...
    grid_fill(&default_grid, 0, 1, col, g_columns, c, c, attr_fill);

    // Put an "X" for closing the current tab if there are several.
    if (first_tabpage->tp_next != NULL) {
      grid_putchar(&default_grid, 'X', 0, g_columns - 1, attr_nosel);
      tab_page_click_defs[g_columns - 1] = (StlClickDefinition) {
...
#define RULER_BUF_LEN 70
    ...
    int this_ru_col = ru_col - (g_columns - width);
    if (this_ru_col < 0) {
      this_ru_col = 0;
    }
...
void screen_resize(int width, int height)
{
  ...
  g_columns = width;
  check_shellsize();
  int max_p_ch = g_rows - min_rows() + 1;
  ...
  if (starting != NO_SCREEN) {
     ...
     * sure g_rows/g_columns and the size of the screen is correct!
     */
    if (State == ASKMORE || State == EXTERNCMD || State == CONFIRM
        || exmode_active) {
      screenalloc();
...
// Limit g_rows and g_columns to avoid an overflow in g_rows * g_columns.
void limit_screen_size(void)
{
  if (g_columns < MIN_COLUMNS) {
    g_columns = MIN_COLUMNS;
  } else if (g_columns > 10000) {
    g_columns = 10000;
  }
...
void win_new_shellsize(void)
{
  ...
  if (old_columns != g_columns) {
    old_columns = g_columns;
