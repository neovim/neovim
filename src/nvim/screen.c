...
static bool check_can_spell(win_T* wp) {
  return (wp->w_p_spell
      && *wp->w_s->b_p_spl != NUL
...
static bool should_conceal_line(win_T* wp, int lnum, bool in_visual_area) {
  return (wp->w_p_cole > 0
    ...
    && !(in_visual_area && vim_strchr(wp->w_p_cocu, 'v') == NULL));
}

static bool check_column_limit(int col, win_T* wp) {
  return (wp->w_p_rl ? (col >= 0) : (col < wp->w_width));
}

static int check_column_diff(int col, win_T* wp) {
  return (wp->w_p_rl ? col : wp->w_width - 1 - col);
}

static bool should_correct_column(win_T* wp, int lnum, int col) {
  return (wp == curwin
      && lnum == wp->w_cursor.lnum
      && conceal_cursor_line(wp) && (int)wp->w_virtcol <= col);
}

static int get_search_attr(win_T* wp, long if_col) {
...
static int get_search_attr_cur(win_T* wp) {
  int attr = search_hl.attr_cur;
  bool shl_flag = false;
  matchitem_T *cur = wp->w_match_head;
  match_T     *shl;
  while (cur != NULL || shl_flag == false) {
...
static void write_char (int c, int mb_c, bool mb_utf8, int attr, int *u8cc, unsigned off) {
...
static int win_line (win_T *wp, linenr_T lnum, int startrow, int endrow, bool nochange) {
  ...
  static char_u *at_end_str = (char_u *)""; // used for p_extra when displaying lcs_eol at end-of-line
  static linenr_T checked_lnum = 0;     // line number for "checked_col"
  static int checked_col = 0;           // column in "checked_lnum" up to which there are no spell errors
  ...
  bool p_extra_needs_free = false;      // p_extra needs to be freed

  int c_extra = NUL;                    // extra chars, all the same
  int extra_attr = 0;                   // attributes when n_extra != 0

  bool did_print_precedes = false;
  bool did_print_extends = false;
  bool did_print_eol = false;
  ...
  int main_attr = 0;                    // attributes for area highlighting
  ...
  hlf_T diff_hlf = 0;                   // type of diff highlighting
  ...
  int fdc_width = foldcolumn_width(wp, 0);
  int nu_width = number_width(wp);
...
# define FIX_FOR_BOGUSCOLS \
  ...
  if (VIsual_active && is_current_buffer) {
    ...
    if (!highlight_match && is_current_window && is_cursor_line)
      noinvcur = TRUE;
  ...
  if (sign_type != 0)
    line_attr = sign_get_attr((int)sign_type, TRUE);
  ...
  if (wp->w_p_cul && is_cursor_line && !(is_current_window && VIsual_active)) {
  ...
  for (;; ) {
    ...
    if (draw_state != WL_LINE) {
      if (draw_state == WL_CMDLINE - 1 && n_extra == 0) {
        ...
        if (cmdwin_type != 0 && is_current_window) { // Draw the cmdline character.
          n_extra = 1;
          c_extra = cmdwin_type;
      ...
      if (draw_state == WL_SIGN - 1 && n_extra == 0) {
        ...
        if (draw_signcolumn(wp)) {
          c_extra = ' '; /* Draw two cells with the sign value or blank. */
          n_extra = 2;
          ...
          if (row == startrow + filler_lines && filler_todo <= 0) {
            ...
            if (text_sign != 0) {
                ...
                if (p_extra != NULL)
                  { c_extra = NUL; n_extra = (int)STRLEN(p_extra); }
      ...
      if (draw_state == WL_NR - 1 && n_extra == 0) {
        ...
        if ((wp->w_p_nu || wp->w_p_rnu) && (row == startrow + filler_lines && !has_numcol)) {
          ...
          if (row == startrow + filler_lines ) {
            ...
            if (wp->w_p_nu && !wp->w_p_rnu) { /* 'number' + 'norelativenumber' */
              num = (long)lnum;
            } else { /* 'relativenumber', don't use negative numbers */
              num = labs((long)get_cursor_rel_lnum(wp, lnum));
            ...
            sprintf((char *)extra, fmt, nu_width, num);
          ...
          if ((wp->w_p_cul || wp->w_p_rnu) && is_cursor_line)
            char_attr = hl_attr(HLF_CLN);
        }
      }

      if (wp->w_p_brisbr && draw_state == WL_BRI - 1 && n_extra == 0 && *p_sbr != NUL) {
        draw_state = WL_BRI;        // draw indent after showbreak value
      } else if (wp->w_p_brisbr && draw_state == WL_SBR && n_extra == 0) {
        draw_state = WL_BRI - 1;    // after the showbreak, draw the breakindent
      }
      ...
      if (draw_state == WL_LINE - 1 && n_extra == 0) {
        draw_state = WL_LINE;
        char_attr = 0;
        if (saved_n_extra) { /* Continue item from end of wrapped line. */
    ...
    if (dollar_vcol >= 0 && is_current_window && is_cursor_line
    ...
    if (draw_state == WL_LINE && area_highlighting) {
      ...
      if (!n_extra) {
        ...
        search_attr = get_search_attr_cur(wp);
      }
      ...
      attr_has_priority = true; // Decide which of the highlight attributes to use.
    ...
    if (n_extra > 0) {
      if (c_extra != NUL) {
        ...
        if (!character_fits_at_col(mb_c, col, wp)) {
          ...
          n_extra++; // put the pointer back to output the double-width
          p_extra--; // character at the start of the next line.
        } else {
          n_extra -= mb_l - 1;
          p_extra += mb_l - 1;
        }
      ...
      if (p_extra_needs_free) {
        p_extra_needs_free = false;
        xfree(p_extra);
      }
      ...
      if ((mb_l == 1 && c >= 0x80) || (mb_l >= 1 && mb_c == 0) || (mb_l > 1 && (!vim_isprintc(mb_c)))) {
        ...
        mb_l = 1; // at the NUL at end-of-line
      ...
      /* If a double-width char doesn't fit display a '>' in the
        ...
        * next line. */
      if (!character_fits_at_col(mb_c, col, wp)) {
        --ptr; // Put pointer back; char will be displayed at the start of the next line.
        SET_CHAR('>')
        multi_attr = hl_attr(HLF_AT);
      } else if (*ptr != NUL) {
        ptr += mb_l - 1;
        ...
        if (c == TAB && (!wp->w_p_list || lcs_tab1)) {
          ...
          if (wp->w_p_list) {
            ...
            SET_CHAR(' ')
              ...
              && !(noinvcur && is_cursor_line && (colnr_T)vcol == wp->w_virtcol))) && !did_print_eol) {
          ...
          extra_attr = hl_attr(HLF_8);
          saved_attr2 = char_attr;  // save current attr
        } else if (VIsual_active && virtual_active() && !(VIsual_mode == 'V') && tocol != MAXCOL && vcol < tocol && check_column_limit(col, wp)) {
          c = ' ';
          ptr--;  // put it back at the NUL
        } else if ((diff_hlf != (hlf_T)0 || line_attr != 0) && check_column_limit((wp->w_p_rl ? col : col-boguscols), wp)) {
          c = ' '; // Highlight until the right side of the window
          ptr--;  // put it back at the NUL
      ...
      if (should_conceal_line(wp, lnum, lnum_in_visual_area) && ((syntax_flags & HL_CONCEAL) != 0 || has_match_conc)) {
    ...
    if (draw_state == WL_LINE && !did_wcol && should_correct_column(wp, lnum, vcol+n_skip)) {
      did_wcol = true;
     ...
     */
    if (c == NUL) {
      if (eol_hl_off > 0 && vcol - eol_hl_off == (long)wp->w_virtcol && is_cursor_line) {
        col--; off--; vcol--; // highlight last char after line
      }
      ...
      /* Update w_cline_height and w_cline_folded if the cursor line was
       * updated (saves a call to plines() later). */
      if (is_current_window && is_cursor_line) {
    ...
    if (lcs_ext && !wp->w_p_wrap && filler_todo <= 0 && check_column_diff(col, wp) == 0
    ...
    if (draw_state == WL_LINE && !lnum_in_visual_area) {
      if (wp->w_p_cuc && VCOL_HLC == (long)wp->w_virtcol && !is_cursor_line) {
    ...
    if (check_column_diff(col, wp) < 0 && (*ptr != NUL || filler_todo > 0
          || (wp->w_p_list && lcs_eol != NUL && p_extra != at_end_str) || (n_extra != 0)) ) {
      //&& (c_extra != NUL || *p_extra != NUL)
      SCREEN_LINE(screen_row, wp->w_wincol, col - boguscols, wp->w_width, wp->w_p_rl);
      boguscols = 0;
      ++row;
      ++screen_row;
      ...
      if ((!wp->w_p_wrap && filler_todo <= 0) || did_print_eol)
        break;
      ...
      if (ui_current_row() == screen_row - 1
         ...
         */
        if (!(((*mb_off2cells)(LineOffset[screen_row], LineOffset[screen_row] + screen_Columns) == 2
          || (*mb_off2cells)(LineOffset[screen_row - 1] + (int)Columns - 2, LineOffset[screen_row] + screen_Columns) == 2))) {
      ...
      n_extra = 0;
      did_print_precedes = false;
      did_print_extends = false;
      did_print_eol = false;
