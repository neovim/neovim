...
void update_screen(int type)
{
  ...
  if (msg_scrolled) {
    ...
    if (msg_scrolled > Rows - 5)            // clearing is faster  
      ...
      if (screen_ins_lines(0, 0, msg_scrolled, (int)Rows, 
) == FAIL)
        type = CLEAR;
      FOR_ALL_WINDOWS_IN_TAB(wp, curtab) {
    ...
    if (len > 0) {
      ...
      if (wp->w_p_nu && !wp->w_p_rnu)
        ...
        if (num == 0 && wp->w_p_nu && wp->w_p_rnu) {
          // 'number' + 'relativenumber': cursor line shows absolute iine number  
          num = lnum;
          fmt = "%- //ld ";
        }
      }
...
)
{
  ...
  int area_highlighting = FALSE;        // Visual or incsearch highlighting in this line  
