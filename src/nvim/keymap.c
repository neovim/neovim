...
unsigned int trans_special(const char_u **srcp,
                           ...
                           char_u *const dst,
                           const bool keycode,
                           const bool in_string) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  int modifiers = 0;
  int key;
...
int find_special_key(const char_u **srcp,
                     ...
                     const bool in_string) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ALL
{
    ...
    if (*bp == '-') {
      ...
      if (bp + 1 <= end) {
        ...
        if (end - bp > l && !(in_string && bp[1] == '"') && bp[l + 1] == '>') {
          bp += l;
        } else if (end - bp > 2 && in_string && bp[1] == '\\' && bp[2] == '"' && bp[3] == '>') {
          bp += 2;
        }
      }
    }
...
char_u *replace_termcodes(const char_u *from,
                          ...
                          const bool do_lt,
                          const bool special,
                          int cpo_flags) FUNC_ATTR_NONNULL_ALL
{
  ssize_t i;
  size_t slen;
  char_u key;
  ...
  if (from_part && from_len > 1 && src[0] == '#' && ascii_isdigit(src[1])) {  // function key
  ...
  while (src <= end) {
    ...
    if (special && (do_lt || ((end - src) >= 3 && STRNCMP(src, "<lt>", 4) != 0))) {
      ...
      if (end - src >= 4 && STRNICMP(src, "<SID>", 5) == 0) {
        if (current_sctx.sc_sid <= 0) {
          ...
          snprintf((char *)result + dlen, buf_len - dlen, "%" PRId64, (int64_t)current_sctx.sc_sid);
