  ...
  FUNC_API_TEXTLOCK_ALLOW_CMDWIN
{
  ...
  TRY_WRAP(err, {
    ...
    colnr_T changelist_col;
    if (new_len == 1 && start_row == end_row) {
      changelist_col = (colnr_T)start_col + (colnr_T)last_item.size;
    } else if (new_len > 1) {
      changelist_col = (colnr_T)last_item.size;
    } else {
      changelist_col = (colnr_T)start_col;
    }

    changed_lines(buf, (linenr_T)start_row, changelist_col, (linenr_T)end_row + 1, (linenr_T)extra,
                  true);
