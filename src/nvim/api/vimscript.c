...
/// @param args Optional arguments to pass to Vimscript via v:args.
  ...
  FUNC_API_SINCE(11) FUNC_API_RET_ALLOC
{
  ...
  typval_T save_args;
  typval_T save_result;
  if (recursive++) {
    prepare_vimvar(VV_ARGS, &save_args);
    prepare_vimvar(VV_RESULT, &save_result);
  }

  tv_clear(get_vim_var_tv(VV_ARGS));
  tv_clear(get_vim_var_tv(VV_RESULT));
  ...
  if (args.size > 0) {
    ...
    set_vim_var_list(VV_ARGS, args_list);
  }
  ...
  if (!ERROR_SET(err)) {
    ...
    typval_T *res_val = get_vim_var_tv(VV_RESULT);
...
theend:
  ...
  if (--recursive) {
    restore_vimvar(VV_ARGS, &save_args);
    restore_vimvar(VV_RESULT, &save_result);
  }
