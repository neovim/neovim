...
#define FOREACH_KEY_VALUE(d, k, v, code) \
  for (size_t __foreach_i = 0; __foreach_i < d.size; __foreach_i++) { \
    String k = d.items[__foreach_i].key; \
    Object *v = &d.items[__foreach_i].value; \
    code; \
  }
...
void nvim_define_autocmd(DictionaryOf(LuaRef) opts, Error *err)
  FUNC_API_SINCE(7)
{
  char_u *pattern = NULL;
  ...
  FOREACH_KEY_VALUE(opts, k, v, {
    if (strequal("callback", k.data)) {
      api_set_error(err, kErrorTypeException, "Not implemented");
...
void nvim_define_augroup(Dictionary opts, Error *err)
  FUNC_API_SINCE(7)
{
  char_u *name = NULL;
  ...
  FOREACH_KEY_VALUE(opts, k, v, {
    if (strequal("name", k.data)) {
      if (v->type != kObjectTypeString) {
        api_set_error(err, kErrorTypeValidation, "'name' must be a string");
        return;
      }

      name = (char_u *)v->data.string.data;
    } else if (strequal("clear", k.data)) {
  ...
  if (name == NULL) {
    api_set_error(err, kErrorTypeValidation, "name is required key");
    return;
  }

  int augroup = au_new_group(name);
  ...
  if (clear_autocmds) {
    FOR_ALL_AUEVENTS(event) {
      if (autocmd_delete_event(augroup, event, &pat) == FAIL) {
...
void nvim_do_autocmd(Dictionary opts, Error *err)
  FUNC_API_SINCE(7)
{
  int au_group = AUGROUP_ALL;
  buf_T *buf = curbuf;
  ...
  FOREACH_KEY_VALUE(opts, k, v, {
    if (strequal("group", k.data)) {
      ...
      if (v->type != kObjectTypeInteger && v->type != kObjectTypeBuffer) {
        api_set_error(err, kErrorTypeException, "invalid bufnr: %d", v->type);
        goto cleanup;
      }
  ...
  // TODO(autocmd): Probably should not let you pass BOTH fname and bufnr.
  //                Also, what to do when fname is a bufnr? I'm not sure.

  // TODO(autocmd): Should figure out how to do `nomodeline` here as well.
  //                I can't even figure out where that code is coming from.
