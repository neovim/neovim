...
#define HAS_KEY_X(d, key) HAS_KEY(d, win_config, key)
  ...
  if (HAS_KEY_X(config, scrollbar)) {
    if ((config->scrollbar && !fconfig->border)
        || (schar_from_str(fconfig->border_chars[3]) == NUL)) {
      api_set_error(err, kErrorTypeException, "scollbar requires border and right-side set");
      goto fail;
    }
