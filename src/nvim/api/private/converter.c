...
#define TYPVAL_ENCODE_CONV_BLOB(tv, blob, len)                                                     \
  do {                                                                                             \
    ...
    kvi_push(edata->stack,                                                                         \
             STRING_OBJ(                                                                           \
                 ((String){.data = len_ != 0 ? xmemdupz(blob_->bv_ga.ga_data, len_) : xstrdup(""), \
                           .size = len_})));                                                       \
  } while (0)
...
#define TYPVAL_ENCODE_CONV_EMPTY_DICT(tv, dict)                                                    \
  kvi_push(edata->stack, DICTIONARY_OBJ(((Dictionary){.capacity = 0, .size = 0})))
    ...
    FUNC_ATTR_ALWAYS_INLINE FUNC_ATTR_NONNULL_ALL
{
  ...
  if (key.type == kObjectTypeString) {
    dict->data.dictionary.items[dict->data.dictionary.size].key = key.data.string;
  } else {
    api_free_object(key);
