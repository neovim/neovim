  ...
  FUNC_API_SINCE(1)
{
  ...
  if (win) {
    ADD(rv, INTEGER_OBJ(win->w_cursors[0].w_cursor.lnum));
    ADD(rv, INTEGER_OBJ(win->w_cursors[0].w_cursor.col));
  }
  ...
  FUNC_API_SINCE(1)
{
  ...
  win->w_cursors[0].w_cursor.lnum = (linenr_T)row;
  win->w_cursors[0].w_cursor.col = (colnr_T)col;
