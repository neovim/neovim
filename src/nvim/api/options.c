...
static buf_T *do_ft_buf(char *filetype, aco_save_T *aco, Error *err)
{
  ...
  TRY_WRAP(err, {
    set_option_value("bufhidden", STATIC_CSTR_AS_OPTVAL("hide"), OPT_LOCAL, 0, false);
    set_option_value("buftype", STATIC_CSTR_AS_OPTVAL("nofile"), OPT_LOCAL, 0, false);
    set_option_value("swapfile", BOOLEAN_OPTVAL(false), OPT_LOCAL, 0, false);
    set_option_value("modeline", BOOLEAN_OPTVAL(false), OPT_LOCAL, 0, false);  // 'nomodeline'
  ...
  FUNC_API_SINCE(9)
{
  ...
  WITH_SCRIPT_CONTEXT(channel_id, {
    set_option_value_for(name.data, optval, scope, 0, false, req_scope, to, err);
  });
}
