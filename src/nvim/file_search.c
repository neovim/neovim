                        ...
                        char_u *rel_fname)
{
  ...
  if (free_visited == TRUE) {
    ...
    search_ctx->ffsc_visited_list
        = ff_get_visited_list(filename, &search_ctx->ffsc_visited_lists_list);
    if (search_ctx->ffsc_visited_list == NULL) {
      goto error_return;
    }
    search_ctx->ffsc_dir_visited_list
        = ff_get_visited_list(filename, &search_ctx->ffsc_dir_visited_lists_list);
  ...
  if (path[0] == '.' && (vim_ispathsep(path[1]) || path[1] == NUL)
      && (!tagfile || vim_strchr(p_cpo, CPO_DOTTAG) == NULL) && rel_fname != NULL) {
    ...
    if (*path != NUL && path[1] == ':') {
      ...
      drive[1] = ':';
      drive[2] = NUL;
      if (vim_FullName((const char *)drive, (char *)ff_expand_buffer, MAXPATHL, true) == FAIL) {
        goto error_return;
      }
      path += 2;
    } else
  ...
  if (stopdirs != NULL) {
    ...
    do {
      ...
      helper = walker;
      ptr = xrealloc(search_ctx->ffsc_stopdirs_v, (dircount + 1) * sizeof(char_u *));
      search_ctx->ffsc_stopdirs_v = ptr;
      walker = vim_strchr(walker, ';');
      if (walker) {
        assert(walker - helper >= 0);
        search_ctx->ffsc_stopdirs_v[dircount - 1] = vim_strnsave(helper, (size_t)(walker - helper));
        walker++;
      } else {
    ...
    while (*wc_part != NUL) {
      ...
      if (STRNCMP(wc_part, "**", 2) == 0) {
        ...
        if (*wc_part != NUL && !vim_ispathsep(*wc_part)) {
          semsg(_("E343: Invalid path: '**[number]' must be at the end of the path or be followed "
                  "by '%s'."),
                PATHSEPSTR);
          goto error_return;
        }
  ...
  add_pathsep((char *)ff_expand_buffer);
  {
    ...
    if (os_isdir(buf)) {
      ...
      if (search_ctx->ffsc_wc_path != NULL) {
        ...
        temp = xmalloc(STRLEN(search_ctx->ffsc_wc_path) + STRLEN(search_ctx->ffsc_fix_path + len)
                       + 1);
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (stackp->ffs_filearray == NULL) {
        ...
        if (path_with_url((char *)dirptrs[0])) {
          ...
          expand_wildcards((dirptrs[1] == NULL) ? 1 : 2, dirptrs, &stackp->ffs_filearray_size,
                           &stackp->ffs_filearray, EW_DIR | EW_ADDSLASH | EW_SILENT | EW_NOTWILD);
        }
      ...
      if (stackp->ffs_stage == 0) {
        ...
        if (*rest_of_wildcards == NUL) {
                   ...
                   || (os_path_exists(file_path)
                       && (search_ctx->ffsc_find_what == FINDFILE_BOTH
                           || ((search_ctx->ffsc_find_what == FINDFILE_DIR)
                               == os_isdir(file_path)))))
#ifndef FF_VERBOSE
                  && (ff_check_visited(&search_ctx->ffsc_visited_list->ffvl_visited_list, file_path,
                                       (char_u *)"")
                      == OK)
#endif
              ) {
#ifdef FF_VERBOSE
                if (ff_check_visited(&search_ctx->ffsc_visited_list->ffvl_visited_list, file_path,
                                     (char_u *)"")
                    == FAIL) {
                  if (p_verbose >= 5) {
                    verbose_enter_scroll();
          ...
          for (int i = stackp->ffs_filearray_cur; i < stackp->ffs_filearray_size; ++i) {
            ...
            ff_push(search_ctx, ff_create_stack_element(stackp->ffs_filearray[i], rest_of_wildcards,
      ...
      if (STRNCMP(stackp->ffs_wc_path, "**", 2) == 0) {
        for (int i = stackp->ffs_filearray_cur; i < stackp->ffs_filearray_size; ++i) {
          ...
          ff_push(search_ctx, ff_create_stack_element(stackp->ffs_filearray[i], stackp->ffs_wc_path,
    ...
    if (search_ctx->ffsc_start_dir && search_ctx->ffsc_stopdirs_v != NULL && !got_int) {
      ...
      *path_end = 0;
      ...
      sptr
          = ff_create_stack_element(file_path, search_ctx->ffsc_wc_path, search_ctx->ffsc_level, 0);
      ff_push(search_ctx, sptr);
    } else {
      break;
    }
...
static int ff_check_visited(ff_visited_T **visited_list, char_u *fname, char_u *wc_path)
{
  ...
  for (vp = *visited_list; vp != NULL; vp = vp->ffv_next) {
    if ((url && fnamecmp(vp->ffv_fname, ff_expand_buffer) == 0)
        || (!url && vp->file_id_valid && os_fileid_equal(&(vp->file_id), &file_id))) {
 ...
 */
static ff_stack_T *ff_create_stack_element(char_u *fix_part,
                                           char_u *wc_part,
                                           int level,
                                           int star_star_empty)
{
...
{
  return find_file_in_path_option(ptr, len, options, TRUE, p_cdpath, FINDFILE_DIR, rel_fname,
                                  (char_u *)"");
}
                                 ...
                                 char_u *suffixes)
{
  ...
  rel_to_curdir = (ff_file_to_find[0] == '.'
                   && (ff_file_to_find[1] == NUL || vim_ispathsep(ff_file_to_find[1])
                       || (ff_file_to_find[1] == '.'
                           && (ff_file_to_find[2] == NUL || vim_ispathsep(ff_file_to_find[2])))));
  if (vim_isAbsName(ff_file_to_find)
    ...
    if (first == TRUE) {
      ...
      for (int run = 1; run <= 2; ++run) {
        ...
        if (run == 1 && rel_to_curdir && (options & FNAME_REL) && rel_fname != NULL
    ...
    for (;;) {
      if (did_findfile_init) {
        ...
        fdip_search_ctx = vim_findfile_init(buf, ff_file_to_find, r_ptr, 100, FALSE, find_what,
  ...
  if (file_name == NULL && (options & FNAME_MESS)) {
    if (first == TRUE) {
      if (find_what == FINDFILE_DIR) {
        ...
        semsg(_("E345: Can't find file \"%s\" in path"), ff_file_to_find);
      }
    } else {
      if (find_what == FINDFILE_DIR) {
        semsg(_("E346: No more directory \"%s\" found in cdpath"), ff_file_to_find);
      } else {
        semsg(_("E347: No more file \"%s\" found in path"), ff_file_to_find);
      }
    }
  }
...
{
  char *dir_name
      = (char *)find_directory_in_path(new_dir, STRLEN(new_dir), FNAME_MESS, curbuf->b_ffname);
  if (dir_name == NULL) {
    return -1;
  }
