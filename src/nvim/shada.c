...
enum SRNIFlags {
  ...
  kSDReadUndisableableData = ((1 << kSDItemSearchPattern) | (1 << kSDItemSubString)
                              | (1 << kSDItemJump)),  ///< Data reading which cannot be disabled by
                                                      ///< &shada or other options except for
                                                      ///< disabling reading ShaDa as a whole.
...
typedef int (*ShaDaFileSkipper)(struct sd_read_def *const sd_reader, const size_t offset)
    REAL_FATTR_NONNULL_ALL REAL_FATTR_WARN_UNUSED_RESULT;
...
#define DEF_SDE(name, attr, ...)                                                                   \
  [kSDItem##name] = {.timestamp = 0, .type = kSDItem##name, .data = {.attr = {__VA_ARGS__}}}
#define DEFAULT_POS                                                                                \
  {                                                                                                \
    1, 0, 0                                                                                        \
  }
...
static const ShadaEntry sd_default_values[] = {
    ...
    DEF_SDE(Jump,
            ...
            .fname = NULL,
            .additional_data = NULL),
    DEF_SDE(BufferList, buffer_list, .size = 0, .buffers = NULL),
    DEF_SDE(LocalMark,
            filemark,
            .name = '"',
            .mark = DEFAULT_POS,
...
#define HMLL_FORALL(hmll, cur_entry, code)                                                         \
  for (HMLListEntry *cur_entry = (hmll)->first; cur_entry != NULL; cur_entry = cur_entry->next) {  \
    code                                                                                           \
  }
...
static inline void hmll_remove(HMLList *const hmll,
                               HMLListEntry *const hmll_entry) FUNC_ATTR_NONNULL_ALL
{
  ...
  const khiter_t k
      = kh_get(hmll_entries, &hmll->contained_entries, hmll_entry->data.data.history_item.string);
...
static inline void hmll_insert(HMLList *const hmll,
                               ...
                               const bool can_free_entry) FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (hmll->free_entry == NULL) {
    assert((size_t)(hmll->last_free_entry - hmll->entries) == hmll->num_entries);
    target_entry = hmll->last_free_entry++;
  } else {
    assert((size_t)(hmll->last_free_entry - hmll->entries) - 1 == hmll->num_entries);
  ...
  int kh_ret;
  const khiter_t k
      = kh_put(hmll_entries, &hmll->contained_entries, data.data.history_item.string, &kh_ret);
  if (kh_ret > 0) {
...
static ptrdiff_t read_file(ShaDaReadDef *const sd_reader,
                           void *const dest,
                           const size_t size) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
    ...
    FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ALL
{
  if (sd_reader->skip(sd_reader, offset) != OK) {
    if (sd_reader->error != NULL) {
      semsg(_(SERR "System error while skipping in ShaDa file: %s"), sd_reader->error);
...
static void close_file(void *cookie)
{
  ...
  if (error != 0) {
    semsg(_(SERR "System error while closing ShaDa file: %s"), os_strerror(error));
  }
}
...
static inline bool in_bufset(const khash_t(bufset) *const set, const buf_T *buf) FUNC_ATTR_PURE
{
...
///
/// @return true or false.
static inline bool in_strset(const khash_t(strset) *const set, char *str) FUNC_ATTR_PURE
{
...
static int msgpack_sd_writer_write(void *data, const char *buf, size_t len)
{
  ...
  if (written_bytes == -1) {
    semsg(_(SERR "System error while writing ShaDa file: %s"), sd_writer->error);
    return -1;
  }
  return 0;
}
...
static bool shada_disabled(void) FUNC_ATTR_PURE
{
...
static int shada_read_file(const char *const file, const int flags) FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  if (p_verbose > 1) {
    verbose_enter();
    smsg(_("Reading ShaDa file \"%s\"%s%s%s%s"), fname, (flags & kShaDaWantInfo) ? _(" info") : "",
         (flags & kShaDaWantMarks) ? _(" marks") : "",
         (flags & kShaDaGetOldfiles) ? _(" oldfiles") : "", of_ret != 0 ? _(" FAILED") : "");
    verbose_leave();
  }

  if (of_ret != 0) {
    if (of_ret != UV_ENOENT || (flags & kShaDaMissingError)) {
      semsg(_(SERR "System error while opening ShaDa file %s for reading: %s"), fname,
            os_strerror(of_ret));
    }
    xfree(fname);
    return FAIL;
...
static void hms_insert(HistoryMergerState *const hms_p,
                       ...
                       const bool do_iter,
                       const bool can_free_entry) FUNC_ATTR_NONNULL_ALL
{
  if (do_iter) {
  ...
  const khiter_t k
      = kh_get(hmll_entries, &hms_p->hmll.contained_entries, entry.data.history_item.string);
...
static inline void hms_init(HistoryMergerState *const hms_p,
                            ...
                            const bool do_merge,
                            const bool reading) FUNC_ATTR_NONNULL_ALL
{
  hmll_init(&hms_p->hmll, num_elements);
  hms_p->do_merge = do_merge;
  hms_p->reading = reading;
  hms_p->iter = shada_hist_iter(NULL, history_type, hms_p->reading, &hms_p->last_hist_entry);
...
static inline void hms_to_he_array(const HistoryMergerState *const hms_p,
                                   ...
                                   int *const new_hisnum) FUNC_ATTR_NONNULL_ALL
{
  ...
  HMLL_FORALL(&hms_p->hmll, cur_entry, {
    ...
    hist->additional_elements = cur_entry->data.data.history_item.additional_elements;
    hist++;
  })
...
#define MERGE_JUMPS(jumps_size, jumps, jumps_type, timestamp_attr, mark_attr, entry, fname_cond,   \
                    free_func, fin_func, idxadj_func, afterfree_func)                              \
  do {                                                                                             \
    ...
    for (i = jl_len; i > 0; i--) {                                                                 \
      ...
      if (jl_entry.timestamp_attr <= entry.timestamp) {                                            \
        if (marks_equal(jl_entry.mark_attr, entry.data.filemark.mark) && fname_cond) {             \
          i = -1;                                                                                  \
        }                                                                                          \
        break;                                                                                     \
      }                                                                                            \
    ...
    if (i > 0) {                                                                                   \
      if (jl_len == JUMPLISTSIZE) {                                                                \
        ...
        memmove(&jumps[i + 1], &jumps[i], sizeof(jumps[0]) * (size_t)(jl_len - i));                \
      }                                                                                            \
    } else if (i == 0) {                                                                           \
      if (jl_len == JUMPLISTSIZE) {                                                                \
        i = -1;                                                                                    \
...
static void shada_read(ShaDaReadDef *const sd_reader, const int flags) FUNC_ATTR_NONNULL_ALL
{
  ...
  const bool get_old_files
      = (flags & (kShaDaGetOldfiles | kShaDaForceit) && (force || tv_list_len(oldfiles_list) == 0));
  const bool want_marks = flags & kShaDaWantMarks;
  const unsigned srni_flags
      = (unsigned)((flags & kShaDaWantInfo
                        ? (kSDReadUndisableableData | kSDReadRegisters | kSDReadGlobalMarks
                           | (p_hi ? kSDReadHistory : 0)
                           | (find_shada_parameter('!') != NULL ? kSDReadVariables : 0)
                           | (find_shada_parameter('%') != NULL && ARGCOUNT == 0 ? kSDReadBufferList
                                                                                 : 0))
                        : 0)
                   | (want_marks && get_shada_parameter('\'') > 0
                          ? kSDReadLocalMarks | kSDReadChanges
                          : 0)
                   | (get_old_files ? kSDReadLocalMarks : 0));
  if (srni_flags == 0) {
    // Nothing to do.
    return;
  }
  ...
  while ((srni_ret = shada_read_next_item(sd_reader, &cur_entry, srni_flags, 0))
    ...
    switch (cur_entry.type) {
      ...
      case kSDItemSearchPattern:
        if (!force) {
          SearchPattern pat;
          (cur_entry.data.search_pattern.is_substitute_pattern ? &get_substitute_pattern
      ...
      case kSDItemHistoryEntry:
        ...
        hms_insert(hms + cur_entry.data.history_item.histtype, cur_entry, true, true);
        // Do not free shada entry: its allocated memory was saved above.
        break;
      case kSDItemRegister:
        if (cur_entry.data.reg.type != kMTCharWise && cur_entry.data.reg.type != kMTLineWise
        ...
        if (!op_reg_set(cur_entry.data.reg.name,
                        (yankreg_T){
...
#define DUMMY_AFTERFREE(entry)
          MERGE_JUMPS(
              curwin->w_jumplistlen, curwin->w_jumplist, xfmark_T, fmark.timestamp, fmark.mark,
              cur_entry,
              (buf == NULL ? (jl_entry.fname != NULL && STRCMP(fm.fname, jl_entry.fname) == 0)
...
#undef DUMMY_AFTERFREE
      ...
      case kSDItemBufferList:
        for (size_t i = 0; i < cur_entry.data.buffer_list.size; i++) {
          char *const sfname = (char *)path_try_shorten_fname(
              (char_u *)cur_entry.data.buffer_list.buffers[i].fname);
          buf_T *const buf = buflist_new((char_u *)cur_entry.data.buffer_list.buffers[i].fname,
                                         (char_u *)sfname, 0, BLN_LISTED);
          if (buf != NULL) {
            RESET_FMARK(&buf->b_last_cursor, cur_entry.data.buffer_list.buffers[i].pos, 0);
            buflist_setfpos(buf, curwin, buf->b_last_cursor.mark.lnum, buf->b_last_cursor.mark.col,
                            false);
      ...
      case kSDItemLocalMark: {
        if (get_old_files && !in_strset(&oldfiles_set, cur_entry.data.filemark.fname)) {
...
#define DUMMY_IDX_ADJ(i)
          MERGE_JUMPS(buf->b_changelistlen, buf->b_changelist, fmark_T, timestamp, mark, cur_entry,
                      true, free_fmark, SDE_TO_FMARK, DUMMY_IDX_ADJ, AFTERFREE);
#undef SDE_TO_FMARK
#undef AFTERFREE
#undef DUMMY_IDX_ADJ
        }
...
shada_read_main_cycle_end:
  ...
  const char *key;
  kh_foreach_key(&fname_bufs, key, { xfree((void *)key); }) kh_dealloc(fnamebufs, &fname_bufs);
...
static ShaDaWriteResult shada_pack_entry(msgpack_packer *const packer,
                                         ShadaEntry entry,
                                         const size_t max_kbyte) FUNC_ATTR_NONNULL_ALL
{
    ...
    if (d != NULL) {                                                                               \
      ...
      for (const hashitem_T *hi = d->dv_hashtab.ht_array; todo; hi++) {                            \
        if (!HASHITEM_EMPTY(hi)) {                                                                 \
          ...
          if (encode_vim_to_msgpack(spacker, &di->di_tv, _("additional data of ShaDa " what))      \
              == FAIL) {                                                                           \
            goto shada_pack_entry_error;                                                           \
          }                                                                                        \
        }                                                                                          \
  ...
  switch (entry.type) {
    ...
    case kSDItemHistoryEntry: {
      ...
      const size_t arr_size = 2 + (size_t)is_hist_search
                              + (size_t)(tv_list_len(entry.data.history_item.additional_elements));
      ...
      DUMP_ADDITIONAL_ELEMENTS(entry.data.history_item.additional_elements, "history entry item");
      break;
    }
    case kSDItemVariable: {
      ...
      memcpy(&vardesc[sizeof("variable g:") - 1], varname.data, varname.size + 1);
      if (encode_vim_to_msgpack(spacker, &entry.data.global_var.value, vardesc) == FAIL) {
        ret = kSDWriteIgnError;
        semsg(_(WERR "Failed to write variable %s"), entry.data.global_var.name);
        goto shada_pack_entry_error;
      }
      DUMP_ADDITIONAL_ELEMENTS(entry.data.global_var.additional_elements, "variable item");
      break;
    }
    case kSDItemSubString: {
      const size_t arr_size = 1 + (size_t)(tv_list_len(entry.data.sub_string.additional_elements));
      msgpack_pack_array(spacker, arr_size);
      PACK_BIN(cstr_as_string(entry.data.sub_string.sub));
      DUMP_ADDITIONAL_ELEMENTS(entry.data.sub_string.additional_elements, "sub string item");
      break;
    }
    case kSDItemSearchPattern: {
...
#undef PACK_BOOL
      DUMP_ADDITIONAL_DATA(entry.data.search_pattern.additional_data, "search pattern item");
      break;
    }
    case kSDItemChange:
    case kSDItemGlobalMark:
    ...
    case kSDItemRegister: {
      const size_t map_size
          = (size_t)(2  // Register contents and name
                     + ONE_IF_NOT_DEFAULT(entry, reg.type) + ONE_IF_NOT_DEFAULT(entry, reg.width)
  ...
  if (!max_kbyte || sbuf.size <= max_kbyte * 1024) {
    ...
    if (sbuf.size > 0) {
      if ((msgpack_pack_uint64(packer, (uint64_t)sbuf.size) == -1)
          || (packer->callback(packer->data, sbuf.data, (unsigned)sbuf.size) == -1)) {
        goto shada_pack_entry_error;
      }
    }
  }
...
static int compare_file_marks(const void *a,
                              const void *b) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_PURE
{
...
static inline ShaDaReadResult shada_parse_msgpack(ShaDaReadDef *const sd_reader,
                                                  ...
                                                  char **const ret_buf) FUNC_ATTR_WARN_UNUSED_RESULT
...
shada_parse_msgpack_read_next : {
  ...
  const msgpack_unpack_return result = msgpack_unpack_next(&unpacked, buf, length, &off);
...
#define FORMAT_MARK_ENTRY(entry_name, name_fmt, name_fmt_arg)                                      \
  do {                                                                                             \
    typval_T ad_tv = {.v_type = VAR_DICT, .vval.v_dict = entry.data.filemark.additional_data};     \
    size_t ad_len;                                                                                 \
    ...
    vim_snprintf_add(S_LEN(ret),                                                                   \
                     entry_name " {" name_fmt                                                      \
                                ...
                                "}, "                                                              \
                                "ad={%p:[%zu]%.64s} }",                                            \
                     name_fmt_arg, strlen(entry.data.filemark.fname), entry.data.filemark.fname,   \
                     entry.data.filemark.mark.lnum, entry.data.filemark.mark.col,                  \
                     entry.data.filemark.mark.coladd, (void *)entry.data.filemark.additional_data, \
                     ad_len, ad);                                                                  \
  } while (0)
    case kSDItemGlobalMark:
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  ShadaEntry entry;
  ShaDaReadResult srni_ret;
  while ((srni_ret = shada_read_next_item(sd_reader, &entry, srni_flags, max_kbyte))
         != kSDReadStatusFinished) {
    switch (srni_ret) {
      case kSDReadStatusSuccess:
        break;
  ...
  do {                                                                                             \
    ...
    *wms_entry = pfs_entry;                                                                        \
    ...
    switch (entry.type) {
      ...
      case kSDItemSearchPattern:
        COMPARE_WITH_ENTRY(
            (entry.data.search_pattern.is_substitute_pattern ? &wms->sub_search_pattern
                                                             : &wms->search_pattern),
            entry);
        break;
      case kSDItemSubString:
      ...
      case kSDItemHistoryEntry:
        ...
        if (wms->hms[entry.data.history_item.histtype].hmll.size != 0) {
          hms_insert(&wms->hms[entry.data.history_item.histtype], entry, true, true);
        } else {
          shada_free_shada_entry(&entry);
        }
        break;
        ...
        if (ascii_isdigit(entry.data.filemark.name)) {
          ...
          for (size_t i = ARRAY_SIZE(wms->numbered_marks); i > 0; i--) {
            ...
            if (wms_entry.timestamp == entry.timestamp
                ...
                && marks_equal(wms_entry.data.filemark.mark, entry.data.filemark.mark)
                && strcmp(wms_entry.data.filemark.fname, entry.data.filemark.fname) == 0) {
              shada_free_shada_entry(&entry);
              processed_mark = true;
              break;
            }
        ...
        if (entry.type == kSDItemLocalMark) {
          ...
          if (idx < 0) {
            filemarks->additional_marks = xrealloc(
                ...
                (++filemarks->additional_marks_size * sizeof(filemarks->additional_marks[0])));
            ...
            if (wms_entry->data.type != kSDItemMissing) {
              ...
              if (wms_entry->can_free_entry) {
                if (kh_key(&wms->file_marks, k) == wms_entry->data.data.filemark.fname) {
...
#define DUMMY_IDX_ADJ(i)
          MERGE_JUMPS(filemarks->changes_size, filemarks->changes, PossiblyFreedShadaEntry,
                      data.timestamp, data.data.filemark.mark, entry, true,
                      FREE_POSSIBLY_FREED_SHADA_ENTRY, SDE_TO_PFSDE, DUMMY_IDX_ADJ,
                      AFTERFREE_DUMMY);
        }
        break;
      }
      case kSDItemJump:
        MERGE_JUMPS(wms->jumps_size, wms->jumps, PossiblyFreedShadaEntry, data.timestamp,
                    data.data.filemark.mark, entry,
                    strcmp(jl_entry.data.data.filemark.fname, entry.data.filemark.fname) == 0,
                    FREE_POSSIBLY_FREED_SHADA_ENTRY, SDE_TO_PFSDE, DUMMY_IDX_ADJ, AFTERFREE_DUMMY);
...
{
  return (buf->b_ffname == NULL || !buf->b_p_bl || bt_quickfix(buf) || bt_terminal(buf)
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_ALWAYS_INLINE
{
  ...
  FOR_ALL_BUFFERS(buf)
  {
    if (!ignore_buf(buf, removable_bufs) && (max_bufs < 0 || buf_count < (size_t)max_bufs)) {
      buf_count++;
    }
  }
...
static inline void add_search_pattern(PossiblyFreedShadaEntry *const ret_pse,
                                      ...
                                      const bool search_highlighted) FUNC_ATTR_ALWAYS_INLINE
{
    ...
    *ret_pse = (PossiblyFreedShadaEntry){
        ...
        = {.type = kSDItemSearchPattern,
           ...
           = {.search_pattern = {
                  ...
                  .has_line_offset
                  = (is_substitute_pattern ? defaults.data.search_pattern.has_line_offset
                                           : pat.off.line),
                  .place_cursor_at_end
                  = (is_substitute_pattern ? defaults.data.search_pattern.place_cursor_at_end
                                           : pat.off.end),
                  .offset
                  = (is_substitute_pattern ? defaults.data.search_pattern.offset : pat.off.off),
                  .is_last_used = (is_substitute_pattern ^ search_last_used),
                  .is_substitute_pattern = is_substitute_pattern,
                  .highlighted = ((is_substitute_pattern ^ search_last_used) && search_highlighted),
                  .pat = (char *)pat.pat,
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_ALWAYS_INLINE
{
  ...
  memmove(wms->numbered_marks + idx + 1, wms->numbered_marks + idx,
          sizeof(wms->numbered_marks[0]) * (ARRAY_SIZE(wms->numbered_marks) - 1 - idx));
    ...
    FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  const unsigned srni_flags
      = (unsigned)(kSDReadUndisableableData | kSDReadUnknown | (dump_history ? kSDReadHistory : 0)
  ...
  msgpack_packer *const packer = msgpack_packer_new(sd_writer, &msgpack_sd_writer_write);
    ...
    do {
      ...
      switch (vartv.v_type) {
        ...
        case VAR_DICT: {
          ...
          if (!set_ref_in_ht(&di->dv_hashtab, copyID, NULL) && copyID == di->dv_copyID) {
            tv_clear(&vartv);
            continue;
          }
          break;
        ...
        case VAR_LIST: {
          ...
          int copyID = get_copyID();
          if (!set_ref_in_list(l, copyID, NULL) && copyID == l->lv_copyID) {
            tv_clear(&vartv);
            continue;
          }
          break;
  ...
  const bool search_highlighted = !(no_hlsearch || find_shada_parameter('h') != NULL);
  const bool search_last_used = search_was_last_used();

  // Initialize search pattern
  add_search_pattern(&wms->search_pattern, &get_search_pattern, false, search_last_used,
                     search_highlighted);

  // Initialize substitute search pattern
  add_search_pattern(&wms->sub_search_pattern, &get_substitute_pattern, true, search_last_used,
                     search_highlighted);
  ...
  if (dump_global_marks) {
    ...
    do {
      ...
      if (fm.fmark.fnum == 0) {
        ...
        if (buf == NULL || buf->b_ffname == NULL || in_bufset(&removable_bufs, buf)) {
          continue;
        }
    ...
    FOR_ALL_BUFFERS(buf)
    {
      ...
      do {
        ...
        filemarks->marks[mark_local_index(name)]
            = (PossiblyFreedShadaEntry){.can_free_entry = false,
                                        .data = {.type = kSDItemLocalMark,
...
#define PACK_WMS_ARRAY(wms_array)                                                                  \
  ...
  for (size_t i = 0; i < wms->jumps_size; i++) {
    if (shada_pack_pfreed_entry(packer, wms->jumps[i], max_kbyte) == kSDWriteFailed) {
      ret = kSDWriteFailed;
      goto shada_write_exit;
    }
  }
...
#undef PACK_WMS_ENTRY
  ...
  FileMarks **const all_file_markss = xmalloc(file_markss_size * sizeof(*all_file_markss));
  FileMarks **cur_file_marks = all_file_markss;
  for (khint_t i = kh_begin(&wms->file_marks); i != kh_end(&wms->file_marks); i++) {
    if (kh_exist(&wms->file_marks, i)) {
      *cur_file_marks++ = &kh_val(&wms->file_marks, i);
    }
  }
  qsort((void *)all_file_markss, file_markss_size, sizeof(*all_file_markss), &compare_file_marks);
  ...
  for (size_t i = 0; i < file_markss_to_dump; i++) {
    ...
    for (size_t j = 0; j < all_file_markss[i]->changes_size; j++) {
      if (shada_pack_pfreed_entry(packer, all_file_markss[i]->changes[j], max_kbyte)
          == kSDWriteFailed) {
        ret = kSDWriteFailed;
        goto shada_write_exit;
      }
    ...
    for (size_t j = 0; j < all_file_markss[i]->additional_marks_size; j++) {
      if (shada_pack_entry(packer, all_file_markss[i]->additional_marks[j], 0) == kSDWriteFailed) {
...
int shada_write_file(const char *const file, bool nomerge)
{
  ...
  shada_write_file_nomerge : {
    ...
    int error;
    sd_writer.cookie = file_open_new(&error, fname, kFileCreate | kFileTruncate, 0600);
    if (sd_writer.cookie == NULL) {
      semsg(_(SERR "System error while opening ShaDa file %s for writing: %s"), fname,
            os_strerror(error));
    }
  }
  ...
  if (!nomerge) {
    ...
    if (sw_ret == kSDWriteSuccessfull) {
      ...
      if (os_fileinfo(fname, &old_info)) {
        if (getuid() == ROOT_UID) {
          if (old_info.stat.st_uid != ROOT_UID || old_info.stat.st_gid != getgid()) {
            const uv_uid_t old_uid = (uv_uid_t)old_info.stat.st_uid;
            const uv_gid_t old_gid = (uv_gid_t)old_info.stat.st_gid;
            const int fchown_ret = os_fchown(file_fd(sd_writer.cookie), old_uid, old_gid);
            if (fchown_ret != 0) {
              semsg(_(RNERR "Failed setting uid and gid for file %s: %s"), tempname,
              ...
              goto shada_write_file_did_not_remove;
                         ...
                         : (old_info.stat.st_gid == getgid() ? (old_info.stat.st_mode & 0020)
      ...
      if (sw_ret == kSDWriteReadNotShada) {
        semsg(_(RNERR "Did not rename %s because %s "
                      "does not look like a ShaDa file"),
              tempname, fname);
      } else {
...
#endif
      semsg(_(RNERR "Do not forget to remove %s or rename it manually to %s."), tempname, fname);
    }
    xfree(tempname);
  }
...
{
  return shada_read_file(fname, kShaDaWantInfo | kShaDaWantMarks | kShaDaGetOldfiles
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  if (read_bytes != (ptrdiff_t)length) {
    if (sd_reader->error != NULL) {
      semsg(_(SERR "System error while reading ShaDa file: %s"), sd_reader->error);
      return kSDReadStatusReadError;
    } else {
      semsg(_(RCERR "Error while reading ShaDa file: "
                    "last entry specified that it occupies %" PRIu64 " bytes, "
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  if (first_char == EOF) {
    if (sd_reader->error) {
      semsg(_(SERR "System error while reading integer from ShaDa file: %s"), sd_reader->error);
      return kSDReadStatusReadError;
    } else if (sd_reader->eof) {
      semsg(_(RCERR "Error while reading ShaDa file: "
                    "expected positive integer at position %" PRIu64 ", but got nothing"),
            (uint64_t)fpos);
      return kSDReadStatusNotShaDa;
    }
  }
...
#define READERR(entry_name, error_desc)                                                            \
  RERR "Error while reading ShaDa file: " entry_name " entry at position %" PRIu64 " " error_desc
...
#define CHECK_KEY_IS_STR(un, entry_name)                                                           \
  if (un.data.via.map.ptr[i].key.type != MSGPACK_OBJECT_STR) {                                     \
    semsg(_(READERR(entry_name, "has key which is not a string")), initial_fpos);                  \
    CLEAR_GA_AND_ERROR_OUT(ad_ga);                                                                 \
...
#define TYPED_KEY(un, entry_name, name, type_name, tgt, objtype, attr, proc)                       \
  CHECKED_KEY(un, entry_name, name, "which is not " type_name, tgt,                                \
              un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_##objtype, attr, proc)
...
#define CONVERTED_STRING_KEY(un, entry_name, name, tgt)                                            \
  TYPED_KEY(un, entry_name, name, "a binary", tgt, BIN, bin, BIN_CONVERTED)
#define INT_KEY(un, entry_name, name, tgt, proc)                                                   \
  CHECKED_KEY(un, entry_name, name, "which is not an integer", tgt,                                \
              ((un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_POSITIVE_INTEGER)                \
               || (un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_NEGATIVE_INTEGER)),           \
              i64, proc)
...
#define ADDITIONAL_KEY(un)                                                                         \
  ...
  { /* NOLINT(readability/braces) */                                                               \
    ga_grow(&ad_ga, 1);                                                                            \
    memcpy(((char *)ad_ga.ga_data) + ((size_t)ad_ga.ga_len * sizeof(*un.data.via.map.ptr)),        \
           un.data.via.map.ptr + i, sizeof(*un.data.via.map.ptr));                                 \
    ad_ga.ga_len++;                                                                                \
  }
...
#define SET_ADDITIONAL_DATA(tgt, name)                                                             \
  do {                                                                                             \
    if (ad_ga.ga_len) {                                                                            \
      msgpack_object obj = {.type = MSGPACK_OBJECT_MAP,                                            \
                            .via = {.map = {                                                       \
                                        .size = (uint32_t)ad_ga.ga_len,                            \
                                        .ptr = ad_ga.ga_data,                                      \
                                    }}};                                                           \
      typval_T adtv;                                                                               \
      if (msgpack_to_vim(obj, &adtv) == FAIL || adtv.v_type != VAR_DICT) {                         \
        semsg(_(READERR(name, "cannot be converted to a VimL dictionary")), initial_fpos);         \
        ga_clear(&ad_ga);                                                                          \
        tv_clear(&adtv);                                                                           \
        goto shada_read_next_item_error;                                                           \
      }                                                                                            \
...
#define SET_ADDITIONAL_ELEMENTS(src, src_maxsize, tgt, name)                                       \
  do {                                                                                             \
    if ((src).size > (size_t)(src_maxsize)) {                                                      \
      msgpack_object obj = {.type = MSGPACK_OBJECT_ARRAY,                                          \
                            .via = {.array = {                                                     \
      ...
      if (msgpack_to_vim(obj, &aetv) == FAIL) {                                                    \
        semsg(_(READERR(name, "cannot be converted to a VimL list")), initial_fpos);               \
        tv_clear(&aetv);                                                                           \
        goto shada_read_next_item_error;                                                           \
      }                                                                                            \
...
shada_read_next_item_start:
  ...
  if (((mru_ret = msgpack_read_uint64(sd_reader, first_char, &type_u64)) != kSDReadStatusSuccess)
      || ((mru_ret = msgpack_read_uint64(sd_reader, read_char(sd_reader), &timestamp_u64))
          != kSDReadStatusSuccess)
      || ((mru_ret = msgpack_read_uint64(sd_reader, read_char(sd_reader), &length_u64))
          != kSDReadStatusSuccess)) {
    return mru_ret;
  }
  ...
  if ((type_u64 > SHADA_LAST_ENTRY ? !(flags & kSDReadUnknown)
    ...
    if (initial_fpos == 0 && (type_u64 == '\n' || type_u64 > SHADA_LAST_ENTRY)) {
      const ShaDaReadResult spm_ret = shada_parse_msgpack(sd_reader, length, NULL, NULL);
  ...
  if (type_u64 > SHADA_LAST_ENTRY) {
    ...
    if (initial_fpos == 0) {
      const ShaDaReadResult spm_ret
          = shada_parse_msgpack(sd_reader, length, NULL, &entry->data.unknown_item.contents);
      if (spm_ret != kSDReadStatusSuccess) {
  ...
  const ShaDaReadResult spm_ret = shada_parse_msgpack(sd_reader, length, &unpacked, &buf);
  if (spm_ret != kSDReadStatusSuccess) {
    ret = spm_ret;
    goto shada_read_next_item_error;
  }
  ...
  switch ((ShadaEntryType)type_u64) {
    ...
    case kSDItemSearchPattern: {
      if (unpacked.data.type != MSGPACK_OBJECT_MAP) {
        semsg(_(READERR("search pattern", "is not a dictionary")), initial_fpos);
        goto shada_read_next_item_error;
      }
      garray_T ad_ga;
      ...
      for (size_t i = 0; i < unpacked.data.via.map.size; i++) {
        ...
        BOOLEAN_KEY(unpacked, "search pattern", SEARCH_KEY_MAGIC, entry->data.search_pattern.magic)
        ...
        BOOLEAN_KEY(unpacked, "search pattern", SEARCH_KEY_IS_SUBSTITUTE_PATTERN,
      ...
      SET_ADDITIONAL_DATA(entry->data.search_pattern.additional_data, "search pattern");
      break;
    }
    case kSDItemChange:
    case kSDItemJump:
      ...
      for (size_t i = 0; i < unpacked.data.via.map.size; i++) {
        ...
        if (CHECK_KEY(unpacked.data.via.map.ptr[i].key, KEY_NAME_CHAR)) {
          if (type_u64 == kSDItemJump || type_u64 == kSDItemChange) {
            semsg(_(READERR("mark",
                            "has n key which is only valid for "
                            "local and global mark entries")),
                  initial_fpos);
            CLEAR_GA_AND_ERROR_OUT(ad_ga);
          }
          CHECKED_ENTRY((unpacked.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_POSITIVE_INTEGER),
                        "has n key value which is not an unsigned integer", "mark",
                        unpacked.data.via.map.ptr[i].val, entry->data.filemark.name, u64, TOCHAR);
        }
    ...
    case kSDItemRegister: {
      ...
      for (size_t i = 0; i < unpacked.data.via.map.size; i++) {
        ...
        if (CHECK_KEY(unpacked.data.via.map.ptr[i].key, REG_KEY_CONTENTS)) {
          if (unpacked.data.via.map.ptr[i].val.type != MSGPACK_OBJECT_ARRAY) {
            semsg(_(READERR("register", "has " REG_KEY_CONTENTS " key with non-array value")),
                  initial_fpos);
            CLEAR_GA_AND_ERROR_OUT(ad_ga);
          }
          if (unpacked.data.via.map.ptr[i].val.via.array.size == 0) {
            semsg(_(READERR("register", "has " REG_KEY_CONTENTS " key with empty array")),
                  initial_fpos);
            CLEAR_GA_AND_ERROR_OUT(ad_ga);
          }
          const msgpack_object_array arr = unpacked.data.via.map.ptr[i].val.via.array;
        ...
        BOOLEAN_KEY(unpacked, "register", REG_KEY_UNNAMED, entry->data.reg.is_unnamed)
        TYPED_KEY(unpacked, "register", REG_KEY_TYPE, "an unsigned integer", entry->data.reg.type,
                  POSITIVE_INTEGER, u64, TOU8)
        TYPED_KEY(unpacked, "register", KEY_NAME_CHAR, "an unsigned integer", entry->data.reg.name,
                  POSITIVE_INTEGER, u64, TOCHAR)
        TYPED_KEY(unpacked, "register", REG_KEY_WIDTH, "an unsigned integer", entry->data.reg.width,
                  POSITIVE_INTEGER, u64, TOSIZE)
        ADDITIONAL_KEY(unpacked)
      }
      if (entry->data.reg.contents == NULL) {
        semsg(_(READERR("register", "has missing " REG_KEY_CONTENTS " array")), initial_fpos);
        CLEAR_GA_AND_ERROR_OUT(ad_ga);
      }
    ...
    case kSDItemHistoryEntry: {
      ...
      if (unpacked.data.via.array.size < 2) {
        semsg(_(READERR("history", "does not have enough elements")), initial_fpos);
        goto shada_read_next_item_error;
      }
      if (unpacked.data.via.array.ptr[0].type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
      ...
      entry->data.history_item.histtype = (uint8_t)unpacked.data.via.array.ptr[0].via.u64;
      const bool is_hist_search = entry->data.history_item.histtype == HIST_SEARCH;
      if (is_hist_search) {
        ...
        if (unpacked.data.via.array.ptr[2].type != MSGPACK_OBJECT_POSITIVE_INTEGER) {
          semsg(_(READERR("search history", "has wrong history separator type")), initial_fpos);
          goto shada_read_next_item_error;
        }
        entry->data.history_item.sep = (char)unpacked.data.via.array.ptr[2].via.u64;
      }
      size_t strsize;
      strsize = (unpacked.data.via.array.ptr[1].via.bin.size + 1  // Zero byte
                 + 1);                                            // Separator character
      entry->data.history_item.string = xmalloc(strsize);
      memcpy(entry->data.history_item.string, unpacked.data.via.array.ptr[1].via.bin.ptr,
             unpacked.data.via.array.ptr[1].via.bin.size);
      ...
      SET_ADDITIONAL_ELEMENTS(unpacked.data.via.array, (2 + is_hist_search),
                              entry->data.history_item.additional_elements, "history");
      break;
    }
    case kSDItemVariable: {
      if (unpacked.data.type != MSGPACK_OBJECT_ARRAY) {
      ...
      if (unpacked.data.via.array.size < 2) {
        semsg(_(READERR("variable", "does not have enough elements")), initial_fpos);
        goto shada_read_next_item_error;
      }
      ...
      entry->data.global_var.name = xmemdupz(unpacked.data.via.array.ptr[0].via.bin.ptr,
                                             unpacked.data.via.array.ptr[0].via.bin.size);
      SET_ADDITIONAL_ELEMENTS(unpacked.data.via.array, 2,
                              entry->data.global_var.additional_elements, "variable");
      bool is_blob = false;
      ...
      if (unpacked.data.via.array.ptr[1].type == MSGPACK_OBJECT_BIN) {
        const listitem_T *type_item = tv_list_first(entry->data.global_var.additional_elements);
        if (type_item != NULL) {
          const typval_T *type_tv = TV_LIST_ITEM_TV(type_item);
          if (type_tv->v_type != VAR_NUMBER || type_tv->vval.v_number != VAR_TYPE_BLOB) {
      ...
      if (is_blob) {
        const msgpack_object_bin *const bin = &unpacked.data.via.array.ptr[1].via.bin;
        blob_T *const blob = tv_blob_alloc();
        ga_concat_len(&blob->bv_ga, bin->ptr, (size_t)bin->size);
        tv_blob_set_ret(&entry->data.global_var.value, blob);
      } else if (msgpack_to_vim(unpacked.data.via.array.ptr[1], &(entry->data.global_var.value))
                 == FAIL) {
        semsg(_(READERR("variable",
                        "has value that cannot "
    ...
    case kSDItemSubString:
      ...
      if (unpacked.data.via.array.size < 1) {
        semsg(_(READERR("sub string", "does not have enough elements")), initial_fpos);
        goto shada_read_next_item_error;
      }
      if (unpacked.data.via.array.ptr[0].type != MSGPACK_OBJECT_BIN) {
      ...
      entry->data.sub_string.sub = BIN_CONVERTED(unpacked.data.via.array.ptr[0].via.bin);
      SET_ADDITIONAL_ELEMENTS(unpacked.data.via.array, 1,
                              entry->data.sub_string.additional_elements, "sub string");
      break;
    case kSDItemBufferList:
      if (unpacked.data.type != MSGPACK_OBJECT_ARRAY) {
      ...
      entry->data.buffer_list.buffers
          = xcalloc(unpacked.data.via.array.size, sizeof(*entry->data.buffer_list.buffers));
...
static bool shada_removable(const char *name) FUNC_ATTR_WARN_UNUSED_RESULT
{
  char *p;
                                      ...
                                      khash_t(bufset) *const removable_bufs)
{
  ...
  do {
    ...
    if (fm.fmark.mark.lnum == 0) {
      siemsg("ShaDa: mark lnum zero (ji:%p, js:%p, len:%i)", (void *)jump_iter,
             (void *)&curwin->w_jumplist[0], curwin->w_jumplistlen);
      continue;
    }
    const buf_T *const buf = (fm.fmark.fnum == 0 ? NULL : buflist_findnr(fm.fmark.fnum));
    if (buf != NULL ? in_bufset(removable_bufs, buf) : fm.fmark.fnum != 0) {
      continue;
    }
    const char *const fname = (char *)(fm.fmark.fnum == 0 ? (fm.fname == NULL ? NULL : fm.fname)
...
void shada_encode_regs(msgpack_sbuffer *const sbuf) FUNC_ATTR_NONNULL_ALL
{
  ...
  for (size_t i = 0; i < ARRAY_SIZE(wms->registers); i++) {
    if (wms->registers[i].data.type == kSDItemRegister) {
      if (kSDWriteFailed == shada_pack_pfreed_entry(&packer, wms->registers[i], 0)) {
        abort();
      }
    }
  }
...
static ptrdiff_t read_sbuf(ShaDaReadDef *const sd_reader,
                           void *const dest,
                           const size_t size) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
...
static void open_shada_sbuf_for_reading(const msgpack_sbuffer *const sbuf,
                                        ShaDaReadDef *sd_reader) FUNC_ATTR_NONNULL_ALL
{
  *sd_reader = (ShaDaReadDef){
      .read = &read_sbuf,
      .close = NULL,
