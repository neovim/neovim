...
enum SRNIFlags {
  ...
  = ((1 << kSDItemSearchPattern) | (1 << kSDItemSubString)
...
#define DEF_SDE(name, attr, ...)                                               \
  [kSDItem##name] = {                                                          \
      .timestamp = 0, .type = kSDItem##name, .data = {.attr = {__VA_ARGS__}}}
#define DEFAULT_POS                                                            \
  {                                                                            \
    1, 0, 0                                                                    \
  }
...
static inline void hmll_init(HMLList *const hmll,
                             const size_t size) FUNC_ATTR_NONNULL_ALL
{
  *hmll = (HMLList){
...
static inline bool in_bufset(const khash_t(bufset) *const set,
                             const buf_T *buf) FUNC_ATTR_PURE
{
...
///
/// @return true or false.
static inline bool in_strset(const khash_t(strset) *const set,
                             char *str) FUNC_ATTR_PURE
{
...
static int shada_read_file(const char *const file,
                           const int flags) FUNC_ATTR_WARN_UNUSED_RESULT
{
  if (shada_disabled()) {
    return FAIL;
  }
  ...
  if (p_verbose > 0) {
    verbose_enter();
    smsg(_("Reading ShaDa file \"%s\"%s%s%s%s"), fname,
...
static inline void hms_insert_whole_neovim_history(
    HistoryMergerState *const hms_p) FUNC_ATTR_NONNULL_ALL
{
  while (hms_p->last_hist_entry.type != kSDItemMissing) {
    hms_insert(hms_p, hms_p->last_hist_entry, false, hms_p->reading);
    if (hms_p->iter == NULL) {
...
#define MERGE_JUMPS(jumps_size, jumps, jumps_type, timestamp_attr, mark_attr,  \
                    entry, fname_cond, free_func, fin_func, idxadj_func,       \
                    afterfree_func)                                            \
  do {                                                                         \
...
static void shada_read(ShaDaReadDef *const sd_reader,
                       const int flags) FUNC_ATTR_NONNULL_ALL
{
  ...
  const unsigned srni_flags
      = (unsigned)((flags & kShaDaWantInfo ? (
                        kSDReadUndisableableData | kSDReadRegisters
                        | kSDReadGlobalMarks | (p_hi ? kSDReadHistory : 0)
                        | (find_shada_parameter('!') != NULL ? kSDReadVariables
                                                             : 0)
...
#undef DUMMY_AFTERFREE
      ...
      case kSDItemBufferList: {
        for (size_t i = 0; i < cur_entry.data.buffer_list.size; i++) {
          ...
          buf_T *const buf
              = buflist_new(cur_entry.data.buffer_list.buffers[i].fname, sfname,
                            0, BLN_LISTED);
          if (buf != NULL) {
...
#define DUMMY_IDX_ADJ(i)
          MERGE_JUMPS(buf->b_changelistlen, buf->b_changelist, fmark_T,
                      timestamp, mark, cur_entry, true, free_fmark,
                      SDE_TO_FMARK, DUMMY_IDX_ADJ, AFTERFREE);
#undef SDE_TO_FMARK
#undef AFTERFREE
#undef DUMMY_IDX_ADJ
        }
  ...
  switch (entry.type) {
    ...
    case kSDItemVariable: {
      const size_t arr_size
          = 2
            + (size_t)(tv_list_len(entry.data.global_var.additional_elements));
      msgpack_pack_array(spacker, arr_size);
    ...
    case kSDItemSubString: {
      const size_t arr_size
          = 1
            + (size_t)(tv_list_len(entry.data.sub_string.additional_elements));
      msgpack_pack_array(spacker, arr_size);
    ...
    case kSDItemSearchPattern: {
      const size_t map_size = (size_t)(
          ...
          + (size_t)(entry.data.search_pattern.additional_data
                         ? entry.data.search_pattern.additional_data->dv_hashtab
                               .ht_used
                         : 0));
      ...
      for (size_t i = 0; i < entry.data.buffer_list.size; i++) {
        const size_t map_size = (size_t)(
            ...
            + (size_t)(entry.data.buffer_list.buffers[i].additional_data == NULL
                           ? 0
                           : (entry.data.buffer_list.buffers[i]
                                  .additional_data->dv_hashtab.ht_used)));
        msgpack_pack_map(spacker, map_size);
        PACK_STATIC_STR(KEY_FILE);
...
static inline ShaDaReadResult shada_parse_msgpack(
    ...
    char **const ret_buf) FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ARG(1)
{
...
#define FORMAT_MARK_ENTRY(entry_name, name_fmt, name_fmt_arg)                  \
  do {                                                                         \
    typval_T ad_tv = {.v_type = VAR_DICT,                                      \
    ...
    vim_snprintf_add(                                                          \
        ...
        entry_name " {" name_fmt                                               \
                   ...
                   "}, "                                                       \
                   "ad={%p:[%zu]%.64s} }",                                     \
        name_fmt_arg, strlen(entry.data.filemark.fname),                       \
        entry.data.filemark.fname, entry.data.filemark.mark.lnum,              \
        entry.data.filemark.mark.col, entry.data.filemark.mark.coladd,         \
        (void *)entry.data.filemark.additional_data, ad_len, ad);              \
  } while (0)
    case kSDItemGlobalMark: {
    ...
    FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT
{
  ...
  ShaDaReadResult srni_ret;
  while ((srni_ret
          = shada_read_next_item(sd_reader, &entry, srni_flags, max_kbyte))
         != kSDReadStatusFinished) {
    switch (srni_ret) {
      case kSDReadStatusSuccess: {
        break;
...
#define DUMMY_IDX_ADJ(i)
      ...
      case kSDItemJump: {
        MERGE_JUMPS(
            wms->jumps_size, wms->jumps, PossiblyFreedShadaEntry,
            data.timestamp, data.data.filemark.mark, entry,
            strcmp(jl_entry.data.data.filemark.fname, entry.data.filemark.fname)
                == 0,
        ...
        = {.type = kSDItemSearchPattern,
           ...
           = {.search_pattern = {
                  ...
                  = (is_substitute_pattern
                         ...
                         : pat.off.end),
                  .offset
                  = (is_substitute_pattern ? defaults.data.search_pattern.offset
                                           : pat.off.off),
                  ...
                  .search_backward
                  = (!is_substitute_pattern && pat.off.dir == '?'),
              }}}};
  }
}
    ...
    wms->registers[op_reg_index(name)] = (PossiblyFreedShadaEntry){
        ...
        .data = {.type = kSDItemRegister,
                 ...
                 .data = {.reg = {
                              ...
                              .type = reg.y_type,
                              .width = (size_t)(
                                  reg.y_type == kMTBlockWise ? reg.y_width : 0),
    ...
    FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  const unsigned srni_flags
      = (unsigned)(kSDReadUndisableableData | kSDReadUnknown
                   ...
                   | (num_marked_files ? kSDReadLocalMarks | kSDReadChanges
                                       : 0));
...
#undef PACK_WMS_ENTRY
  ...
  for (size_t i = 0; i < file_markss_to_dump; i++) {
    ...
    for (size_t j = 0; j < all_file_markss[i]->additional_marks_size; j++) {
      if (shada_pack_entry(packer, all_file_markss[i]->additional_marks[j], 0)
          == kSDWriteFailed) {
...
int shada_write_file(const char *const file, bool nomerge)
{
  ...
  shada_write_file_open : {
    ...
    sd_writer.cookie = file_open_new(&error, tempname,
                                     kFileCreateOnly | kFileNoSymlink, perm);
  ...
  shada_write_file_nomerge : {
    ...
    int error;
    sd_writer.cookie
        = file_open_new(&error, fname, kFileCreate | kFileTruncate, 0600);
    if (sd_writer.cookie == NULL) {
  ...
  if (!nomerge) {
    ...
    if (sw_ret == kSDWriteSuccessfull) {
        ...
        if (getuid() == ROOT_UID) {
          if (old_info.stat.st_uid != ROOT_UID
            ...
            const int fchown_ret
                = os_fchown(file_fd(sd_writer.cookie), old_uid, old_gid);
            if (fchown_ret != 0) {
      ...
      if (vim_rename(tempname, fname) == -1) {
        EMSG3(_(RNERR "Can't rename ShaDa file from %s to %s!"), tempname,
              fname);
      } else {
        did_remove = true;
        os_remove(tempname);
...
{
  return shada_read_file(fname, kShaDaWantInfo | kShaDaWantMarks
                                    | kShaDaGetOldfiles
...
#define READERR(entry_name, error_desc)                                        \
  RERR "Error while reading ShaDa file: " entry_name                           \
       " entry at position %" PRIu64 " " error_desc
...
#define ADDITIONAL_KEY(un)                                                     \
  ...
  { /* NOLINT(readability/braces) */                                           \
    ...
    memcpy(((char *)ad_ga.ga_data)                                             \
               + ((size_t)ad_ga.ga_len * sizeof(*un.data.via.map.ptr)),        \
  ...
  do {                                                                         \
    if (ad_ga.ga_len) {                                                        \
      ...
      if (msgpack_to_vim(obj, &adtv) == FAIL || adtv.v_type != VAR_DICT) {     \
        emsgf(_(READERR(name, "cannot be converted to a VimL dictionary")),    \
              initial_fpos);                                                   \
        ga_clear(&ad_ga);                                                      \
        tv_clear(&adtv);                                                       \
        goto shada_read_next_item_error;                                       \
...
shada_read_next_item_start:
  ...
  const ShaDaReadResult spm_ret
      = shada_parse_msgpack(sd_reader, length, &unpacked, &buf);
  if (spm_ret != kSDReadStatusSuccess) {
    ret = spm_ret;
    goto shada_read_next_item_error;
  }
      ...
      for (size_t i = 0; i < unpacked.data.via.map.size; i++) {
        ...
        if (CHECK_KEY(unpacked.data.via.map.ptr[i].key, REG_KEY_CONTENTS)) {
          if (unpacked.data.via.map.ptr[i].val.type != MSGPACK_OBJECT_ARRAY) {
            emsgf(_(READERR("register", "has " REG_KEY_CONTENTS
    ...
    case kSDItemHistoryEntry: {
      ...
      if (is_hist_search) {
        if (unpacked.data.via.array.size < 3) {
          emsgf(
              _(READERR("search history", "does not have separator character")),
              initial_fpos);
          goto shada_read_next_item_error;
        }
        if (unpacked.data.via.array.ptr[2].type
            != MSGPACK_OBJECT_POSITIVE_INTEGER) {
          emsgf(
              _(READERR("search history", "has wrong history separator type")),
              initial_fpos);
          goto shada_read_next_item_error;
        }
                                      ...
                                      khash_t(bufset) *const removable_bufs)
{
  ...
  do {
    ...
    if (fm.fmark.mark.lnum == 0) {
      iemsgf("ShaDa: mark lnum zero (ji:%p, js:%p, len:%i)", (void *)jump_iter,
             (void *)&curwin->w_jumplist[0], curwin->w_jumplistlen);
      continue;
    }
    const buf_T *const buf
        = (fm.fmark.fnum == 0 ? NULL : buflist_findnr(fm.fmark.fnum));
    if (buf != NULL ? in_bufset(removable_bufs, buf) : fm.fmark.fnum != 0) {
      continue;
    }
    const char *const fname
        = (char *)(fm.fmark.fnum == 0 ? (fm.fname == NULL ? NULL : fm.fname)
                   : buf              ? buf->b_ffname
                                      : NULL);
    if (fname == NULL) {
      continue;
