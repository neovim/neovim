...
static bool log_path_init(void)
{
  ...
  expand_env((char_u *)"$" LOG_FILE_ENV, (char_u *)log_file_path, (int)size - 1);
  if (strequal("$" LOG_FILE_ENV, log_file_path) || log_file_path[0] == '\0'
    ...
    if (log_dir_failure) {
      WLOG("Failed to create directory %s for writing logs: %s", failed_dir,
           os_strerror(log_dir_failure));
    }
            ...
            ...) FUNC_ATTR_UNUSED FUNC_ATTR_PRINTF(6, 7)
{
  ...
  va_list args;
  va_start(args, fmt);
  ret = v_do_log_to_file(log_file, log_level, context, func_name, line_num, eol, fmt, args);
  va_end(args);
...
FILE *open_log_file(void)
{
  ...
  do_log_to_file(stderr, ERROR_LOG_LEVEL, NULL, __func__, __LINE__, true,
                 "Logging to stderr, failed to open $" LOG_FILE_ENV ": %s", log_file_path);
  return stderr;
}
...
void log_callstack_to_file(FILE *log_file, const char *const func_name, const int line_num)
{
  ...
  do_log_to_file(log_file, DEBUG_LOG_LEVEL, NULL, func_name, line_num, true, "trace:");
                           ...
                           ...) FUNC_ATTR_PRINTF(7, 8)
{
  va_list args;
  va_start(args, fmt);
  bool ret = v_do_log_to_file(log_file, log_level, context, func_name, line_num, eol, fmt, args);
  va_end(args);
                             ...
                             va_list args)
{
  ...
  int rv = (line_num == -1 || func_name == NULL)
               ? fprintf(log_file, "%s %s.%03d %-5" PRId64 " %s", log_levels[log_level], date_time,
                         millis, pid, (context == NULL ? "?:" : context))
               : fprintf(log_file, "%s %s.%03d %-5" PRId64 " %s%s:%d: ", log_levels[log_level],
                         date_time, millis, pid, (context == NULL ? "" : context), func_name,
                         line_num);
  if (rv < 0) {
    return false;
  }
