  ...
  FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_ALWAYS_INLINE
{
  ...
  SPELL_READ_BYTES(buf, VIMSPELLMAGICL, fd,; );  // NOLINT
...
static int read_region_section(FILE *fd, slang_T *lp, int len)
{
  ...
  SPELL_READ_NONNUL_BYTES((char *)lp->sl_regions, (size_t)len, fd,; );  // NOLINT
...
static int read_prefcond_section(FILE *fd, slang_T *lp)
{
  ...
  for (int i = 0; i < cnt; i++) {
    ...
    if (n > 0) {
      ...
      SPELL_READ_NONNUL_BYTES(buf + 1, (size_t)n, fd,; );  // NOLINT
      buf[n + 1] = NUL;
