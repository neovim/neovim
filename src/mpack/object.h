...
#define MPACK_PARSER_STRUCT_SIZE(c) (sizeof(mpack_node_t) * c + sizeof(mpack_one_parser_t))
...
MPACK_API int mpack_parse_tok(mpack_parser_t *walker,
                              ...
                              mpack_walk_cb exit_cb) FUNUSED FNONULL_ARG((1, 3, 4));
MPACK_API int mpack_unparse_tok(mpack_parser_t *walker,
                                mpack_token_t *tok,
                                mpack_walk_cb enter_cb,
                                mpack_walk_cb exit_cb) FUNUSED FNONULL_ARG((1, 2, 3, 4));

MPACK_API int mpack_parse(mpack_parser_t *parser,
                          const char **b,
                          size_t *bl,
                          mpack_walk_cb enter_cb,
                          mpack_walk_cb exit_cb) FUNUSED FNONULL_ARG((1, 2, 3, 4, 5));
