...
#define MPACK_RPC_SESSION_STRUCT_SIZE(c)                                                           \
  (sizeof(struct mpack_rpc_slot_s) * (c - 1) + sizeof(mpack_rpc_one_session_t))
...
MPACK_API int mpack_rpc_reply_tok(mpack_rpc_session_t *s,
                                  mpack_token_t *t,
