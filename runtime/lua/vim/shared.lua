...
---   - "lower": returns the index of the first element
---     that is not less than the given value.
---   - "upper": returns the index of the first element
...
--- not less than the {val} by default (i.e., lower bound).
--- Use {bound} to specify the search variant.
---
--- Example:
--- ```lua
---
...
---@return integer index serves as either the lower bound or the upper bound.
function vim.list.bisect(t, val, opts)
  ...
  if opts.bound == 'upper' then
    return upper_bound(t, val, lo, hi, opts.key)
  else
    return lower_bound(t, val, lo, hi, opts.key)
  end
end
