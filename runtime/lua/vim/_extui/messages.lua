...
local function msg_to_full(src, height)
  if will_full then
    return
  end
  ...
  vim.schedule(function()
    ...
    -- key press, or append to pager in case that is already open (not hidden) or
    -- if the message will not fit in the cmdline.
    local hidden = api.nvim_win_get_config(ext.wins.pager).hide
    local tar = hidden and height < math.ceil(vim.o.lines * 0.5) and 'cmd' or 'pager'
    if tar == 'pager' or ext.cfg.msg.target == 'msg' then
...
function M.set_pos(type)
  local function win_set_pos(win)
    ...
    if type == 'cmd' then
      ...
      local save_config = api.nvim_win_get_config(win)
