...
--- @class (private) vim._extui.messages.Float
local MsgFloat = {
...
local M = {
  ...
  float = MsgFloat, -- Floating message window.
}
...
function MsgFloat:start_timer(buf, len, restart)
...
---@param self vim._extui.messages.Float?
---@param win integer
---@param buf integer
---@param content MsgContent
---@param replace_last boolean
function M.show_msg(self, win, buf, content, replace_last)
  local msg, dupe_str = '', ''
  if self then
    for _, chunk in ipairs(content) do
      msg = msg .. chunk[2]
    end
    self.dupe = (msg == self.prev_msg and self.dupe + 1 or 0)
    dupe_str = self.dupe > 0 and ' (' .. self.dupe .. ')' or ''
  end

  local col = 0
  local row = api.nvim_buf_line_count(buf)
    - ((replace_last or (self and (self.count == 0 or self.dupe > 0))) and 1 or 0)
  local start_row = row
  ...
  for _, chunk in ipairs(content) do
    ...
    api.nvim_buf_set_extmark(buf, ext.ns, srow, scol, {
      end_row = row,
      end_col = col,
      hl_group = chunk[3],
      undo_restore = false,
  ...
  if self then
    ...
    for line = start_row, row do
      ...
      self.width = math.max(self.width, vim.fn.virtcol('$', false, win) - 1)
    end
    api.nvim_win_set_width(win, self.width)

    local restart = self.count > 0 and replace_last or self.dupe > 0
    self.count = self.count + (restart and 0 or 1)
    -- Set message window position and start timer to remove the message unless
    -- messages were moved to more window.
    M.set_pos(vim.o.cmdheight, 0, ext.wins[ext.tab].msg)
    if self.count ~= 0 then
      self.prev_msg = msg
      self:start_timer(ext.bufs.msg, row - start_row + 1, restart)
    end
  elseif buf == ext.bufs.cmd then
...
M.msg_show = function(kind, content, replace_last)
  ...
  elseif kind == 'return_prompt' then
    ...
    M.show_msg(MsgFloat, ext.wins[ext.tab].msg, ext.bufs.msg, content, replace_last)
  end
end
...
M.msg_showmode = function(content)
  ...
  elseif not ext.cmd.active and #content > 0 then
    M.show_msg(nil, ext.wins[ext.tab].cmd, ext.bufs.cmd, content, true)
    ext.cmd.shown = false
  end
end
...
M.msg_ruler = function(content)
  if ext.cmd.active and #content > 0 then
    return
  end
...
M.msg_history_show = function(entries)
  ...
  for i, entry in ipairs(entries) do
    M.show_msg(nil, ext.wins[ext.tab].more, ext.bufs.more, entry[2], i == 1)
  end
...
function M.set_pos(cmdheight, closedid, win)
  local function win_set_pos(w)
    ...
    if tomore then
      MsgFloat.count, MsgFloat.width, MsgFloat.prev_msg = 0, 1, ''
    ...
    if more and api.nvim_get_current_win() ~= w then
      ...
      vim.wo[w].statusline = 'Messages'
    end
    -- Position the window.
    api.nvim_win_set_config(w, {
      hide = false,
      relative = 'editor',
      border = more and 'none' or nil,
