...
function M.show_msg(target, content, replace_last, showmode)
  ...
  -- Determine start row after last line in the target buffer, unless target
  -- buffer is the cmdline, or in case of a repeated message or "replace_last".
  ...
  -- Add highlighted message or repeated message indicator to buffer.
  if M.dupe == 0 or target ~= ext.cfg.messages.pos then
    local overflow = target == 'cmd' and row + 1 - ext.cmdheight or 0
    -- Append [x] indicator for lines that overflow 'cmdheight'.
    if overflow > 0 then
      lines[#lines - 1] = lines[#lines - 1] .. (' [+%d]'):format(overflow)
    end
...
function M.set_pos(type)
  local function win_set_pos(w)
    ...
    api.nvim_win_set_config(w, {
      hide = false,
      relative = 'laststatus',
      height = tomore and math.ceil(vim.o.lines / 3) or type and h or nil,
