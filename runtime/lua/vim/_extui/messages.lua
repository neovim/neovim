...
local function set_virttext(type, content, col)
  ...
  elseif #content > 0 and not (type == 'showcmd' and content[1][2]:sub(0):match(':/?')) then
    local row, scol = 0, 0 -- luacheck: no unused
    ...
    if col then
      ...
      local wincol = 0
      if tar == 'box' then -- Subtract wincol + border from screenpos().
      ...
      -- TODO: virt_text_pos/virt_text_win_row that covers this use case?
