...
local Cmd = {
  lines = 0, -- Number of lines in 'cmdheight' area (including overflow).
...
function Box:start_timer(buf, len)
  self.timer = vim.defer_fn(function()
    if self.count == 0 or buf ~= ext.bufs.box or not api.nvim_buf_is_valid(buf) then
...
function M.show_msg(target, content, replace_last, showmode)
  ...
  local row = (showmode or (Box.count == 0 and Cmd.lines == 0)) and 0
  ...
  for i, chunk in ipairs(M.dupe > 0 and target == ext.cfg.messages.pos and {} or content) do
    ...
    if chunk[3] > 0 then
      ...
      marks[#marks][3] = {
        ...
        invalidate = true,
        undo_restore = false,
        end_right_gravity = (i == #content), -- extend highlight for dupe/overflow indicator.
      }
    end
  end

  if M.dupe == 0 or target ~= ext.cfg.messages.pos then
    ...
    -- Append or replace [+x] indicator for lines that overflow 'cmdheight'.
    if not showmode and target == 'cmd' then
      if Cmd.lines == 0 then
        vim.schedule(function()
          Cmd.lines = 0
        end)
      end
      local replace = Cmd.lines - ext.cmdheight
      Cmd.lines = Cmd.lines + row - start_row + 1
      if Cmd.lines - ext.cmdheight > 0 then
        local str = (' [+%d]'):format(Cmd.lines - ext.cmdheight)
        local srow = math.min(ext.cmdheight, Cmd.lines) - 1
        local scol = replace > 0 and (-5 - #tostring(replace)) or -1
        api.nvim_buf_set_text(ext.bufs.cmd, srow, scol, srow, -1, { str })
      end
    end
  else
    -- Append or replace duplicate (x) indicator to previous message.
    local str = (' (%d)'):format(M.dupe)
    local scol = M.dupe == 1 and -1 or (-4 - #tostring(M.dupe - 1))
    api.nvim_buf_set_text(ext.bufs[target], row, scol, row, -1, { str })
    if ext.cfg.messages.pos == 'box' then
      local line = api.nvim_buf_get_lines(ext.bufs[target], -2, -1, false)[1]
...
local function set_extmark(content, id)
  ...
  elseif #content > 0 and not (id == Cmd.showcmd_id and content[1][2]:sub(0):match(':/?')) then
    local wincol = id == Cmd.showcmd_id and (Cmd.rucol or vim.o.columns) - showcmd_cols
      or vim.o.columns - #content[1][2] - 1
    crop_cmdline(wincol)
    return api.nvim_buf_set_extmark(ext.bufs.cmd, ext.ns, 0, 0, {
      virt_text = { { content[1][2]:sub(id == Cmd.showcmd_id and -10 or 0), content[1][3] } },
...
M.msg_showcmd = function(content)
  Cmd.showcmd_id = set_extmark(content, Cmd.showcmd_id)
end
...
M.msg_ruler = function(content)
  if not ext.cmd.active or #content == 0 then
    Cmd.ruler_id, Cmd.rucol = set_extmark(content, Cmd.ruler_id)
  end
end
