...
local M = {
  ...
  wins = {}, ---@type { ['cmd'|'msg'|'more'|'prompt']: integer }[]
  bufs = { cmd = -1, msg = -1, more = -1, prompt = -1 },
...
M.tab_check_wins = function()
  ...
  if not M.wins[M.tab] then
    M.wins[M.tab] = { cmd = -1, msg = -1, more = -1, prompt = -1 }
  end

  for _, type in ipairs({ 'cmd', 'msg', 'more', 'prompt' }) do
    ...
    if not api.nvim_win_is_valid(M.wins[M.tab][type]) then
      ...
      local cfg = vim.tbl_deep_extend('force', M.wincfg, {
        relative = type ~= 'cmd' and 'laststatus' or nil,
        anchor = type ~= 'cmd' and 'SE' or nil,
        hide = type ~= 'cmd' or M.cmdheight == 0 or nil,
        title = type == 'more' and 'Messages' or nil,
        border = type == 'msg' and not o.termguicolors and 'single' or border or nil,
      })
