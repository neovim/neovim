...
local M = {
  ...
  prompt = false, -- Whether a prompt is currently active; in which case 'list_cmd' messages are
                  -- placed in the 'list' buffer/window.
}
...
M.cmdline_show = function(content, pos, firstc, prompt, indent)
  ...
  if ext.cmdheight ~= 0 and vim.o.cmdheight ~= height then
  ...
  if ext.cmdheight == 0 and api.nvim_win_get_config(ext.wins[ext.tab].cmd).hide == true then
    ...
    ext.msg.set_pos(height - (ext.cmdheight == 0 and 1 or 0), 0)
...
M.cmdline_hide = function(_, abort)
  ...
  if abort or M.prompt then
    api.nvim_buf_set_lines(ext.bufs.cmd, 0, -1, false, {})
  end
  if M.prompt then
    api.nvim_win_set_config(ext.wins[ext.tab].list, { hide = true })
  end
  ...
  if ext.cmdheight == 0 then
    ...
    ext.msg.set_pos(0, 0)
  elseif vim.o.cmdheight ~= ext.cmdheight then
