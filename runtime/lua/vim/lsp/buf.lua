...
--- Selects the {count}th range after the currently selected range.
--- Ranges should be saved in `b:lsp_selection_ranges` first.
--- Index of the currently selected range is saved in `b:lsp_selection_index`.
---
---@param winid integer
---@param count integer
---@return boolean success if there are saved ranges
local function select_range(winid, count)
  ...
  index = math.min(math.max(0, index), #ranges)
  if index == 0 then
...
---@class vim.lsp.selection_range.Opts
---@inlinedoc
---
--- Window ID
...
--- Negative values shrink the selection.
--- (default: 1)
---@field count? integer

--- Expand or shrink a selection.
---@param opts? vim.lsp.selection_range.Opts
function M.selection_range(opts)
  opts = opts or {}
  local winid = opts.winid or api.nvim_get_current_win()
  local pos = opts.pos or api.nvim_win_get_cursor(winid)
  local count = opts.count or 1
  if select_range(winid, count) then
    return
  end
      ...
      for i = 1, #selections do
        ...
        for range in parent(selection) do
          ...
          if not (start_row == end_row and start_col == end_col) then
            ...
            -- TODO(ofseed): sort ranges for multi-client support?
