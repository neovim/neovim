...
function Client:request(method, params, handler, bufnr)
  ...
  end, function(message_id)
    -- Called when the server sends a response to the request (including cancelled acknowledgment).
    if request_registered then
      self:_process_request(message_id, 'complete')
    end
    local token = self._partial_result_tokens[message_id]
    if token then
      self._partial_result_tokens[message_id] = nil
