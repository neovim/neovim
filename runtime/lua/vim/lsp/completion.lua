...
--- @field request_ids table<integer, integer> Table tracking ongoing requests
...
--- @return integer The time (in milliseconds) before the next request is made
function CompletionResolver:next_debounce_time()
  if not self.last_request_time then
    return self.doc_rtt_ms
  end
  local ms_since_request = (vim.uv.hrtime() - self.last_request_time) * ns_to_ms
  return math.max((ms_since_request - self.doc_rtt_ms) * -1, 0)
end
...
function CompletionResolver:cancel_pending_requests()
  for client_id, request_id in pairs(self.request_ids) do
...
function CompletionResolver:request(bufnr, param, selected_word)
  ...
  local debounce_time = self:next_debounce_time()
  ...
  self.timer:start(
    ...
    vim.schedule_wrap(function()
      ...
      local ok, request_id = client:request('completionItem/resolve', param, function(err, result)
...
local function trigger(bufnr, clients, ctx)
  ...
  local cancel_request = request(clients, bufnr, win, ctx, function(responses)
    ...
    if #matches > 0 and vim.regex('\\<popup\\>'):match_str(vim.o.completeopt) then
