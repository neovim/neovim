...
local function make_comment_check(parts)
  ...
  local regex = table.concat({ '^%s-', vim.trim(l_esc), '.*', vim.trim(r_esc), '%s-$' })
...
local function get_lines_info(lines, parts)
  ...
  for _, l in ipairs(lines) do
    ...
    if indent_width_cur < #l then
      ...
      is_commented = is_commented and comment_check(l)
    end
  end

  return indent, is_commented
end
...
local function make_comment_function(parts, indent)
  ...
  local blank_comment = table.concat({ indent, vim.trim(parts.left), vim.trim(parts.right) })

  return function(line)
    if is_blank(line) then
      return blank_comment
    end
    return table.concat({ prefix, line:sub(nonindent_start), suffix })
  end
end
...
local function make_uncomment_function(parts)
  local l_esc, r_esc = vim.pesc(parts.left), vim.pesc(parts.right)
  local pattern = table.concat({ '^(%s*)', l_esc, '(.*)', r_esc, '(%s-)$' })
  local pattern_trimmed =
    table.concat({ '^(%s*)', vim.trim(l_esc), '(.*)', vim.trim(r_esc), '(%s-)$' })

  --- @param line string
  return function(line)
    -- Try regex with exact comment parts first, fall back to trimmed parts
    local indent, new_line, trail = line:match(pattern)
    if not new_line then
      indent, new_line, trail = line:match(pattern_trimmed)
    end
    ...
    return table.concat({ indent, new_line, trail })
  end
end
