...
---Currently supports PNG images, displayed via UI events when an attached UI
...
---Retrieve the active backend: UI protocol events when an attached UI
...
function M.set(data_or_id, opts)
  ...
  if type(data_or_id) == 'string' then
    ...
    local entry = {
      ...
      handle = bk.set(data_or_id),
      backend = bk,
    }

    local id = require('vim.ui.img._util').generate_id()
    state[id] = entry
