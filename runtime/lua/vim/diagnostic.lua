...
--- ```lua
--- function(diagnostic)
---   -- Use only the first line of the message.
...
M.handlers.virtual_lines = {
  show = function(namespace, bufnr, diagnostics, opts)
    ...
    api.nvim_clear_autocmds({ group = virtual_lines_augroup })

    if opts.virtual_lines.format then
      diagnostics = reformat_diagnostics(opts.virtual_lines.format, diagnostics)
    end

    if opts.virtual_lines.current_line ~= false then
      api.nvim_create_autocmd('CursorMoved', {
        buffer = bufnr,
        group = virtual_lines_augroup,
        callback = function()
          render_current_line(diagnostics, ns.user_data.virt_lines_ns, bufnr, opts.virtual_lines)
        end,
      })
      -- Also show diagnostics for the current line before the first CursorMoved event.
      render_current_line(diagnostics, ns.user_data.virt_lines_ns, bufnr, opts.virtual_lines)
    else
      render_virtual_lines(ns.user_data.virt_lines_ns, bufnr, diagnostics, opts.virtual_lines)
    end
  end,
  hide = function(namespace, bufnr)
    local ns = M.get_namespace(namespace)
