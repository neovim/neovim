...
local function get_location_extmark(diagnostic)
  local ns = M.get_namespace(diagnostic.namespace)

  return api.nvim_buf_get_extmark_by_id(
    diagnostic.bufnr,
...
local function diagnostic_lines(diagnostics)
  ...
  for _, diagnostic in ipairs(diagnostics) do
    local lnum = get_location_extmark(diagnostic)[1]
...
local function next_diagnostic(search_forward, opts)
  ...
  local function col(diagnostic)
    return get_location_extmark(diagnostic)[2]
  end
...
local function goto_diagnostic(diagnostic, opts)
  ...
  local extmark = get_location_extmark(diagnostic)
