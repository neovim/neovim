...
local function diagnostic_lines(diagnostics)
  ...
  for _, diagnostic in ipairs(diagnostics) do
    local lnum = get_logical_location(diagnostic)
...
local function next_diagnostic(search_forward, opts)
  ...
  local function col(diagnostic)
    return select(2, get_logical_location(diagnostic))
  end
...
local function goto_diagnostic(diagnostic, opts)
  ...
  local lnum, col = get_logical_location(diagnostic)
...
function M.open_float(opts, ...)
  ...
  if scope == 'line' then
    ...
    diagnostics = vim.tbl_filter(function(d)
      local d_lnum, _, d_end_lnum, d_end_col = get_logical_location(d)
  ...
  elseif scope == 'cursor' then
    ...
    --- @param d vim.Diagnostic
    diagnostics = vim.tbl_filter(function(d)
      local d_lnum, d_col, d_end_lnum, d_end_col = get_logical_location(d)
