...
function Tabstop.new(index, bufnr, placement, range, choices)
  ...
  local self = setmetatable(
    { extmark_id = extmark_id, bufnr = bufnr, index = index, placement = placement, choices = choices },
    { __index = Tabstop }
  )
...
function Session.new(bufnr, snippet_extmark, tabstop_data)
  ...
  for index, ranges in pairs(tabstop_data) do
    for _, data in ipairs(ranges) do
      ...
      table.insert(self.tabstops[index], Tabstop.new(index, self.bufnr, data.placement, data.range, data.choices))
  ...
  for i, tabstops in pairs(self.tabstops) do
    if i ~= index then
      for _, tabstop in ipairs(tabstops) do
        ...
        while vim.list_contains(all_tabstop_placements, placement)
          and not vim.list_contains(dest_tabstop_placements, placement) do
  ...
  for index, child in ipairs(snippet.data.children) do
    ...
    elseif type == G.NodeType.Variable then
      ...
      if not value then
        ...
        local tabstop_index = math.max(unpack((#tabstop_indexes == 0 and { 0 }) or tabstop_indexes)) + 1
