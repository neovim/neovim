...
--- Examples:
---
--- Note: these examples use |vim.async.spawn()| to run the work in
--- detached coroutines. Without it the channel operations would still
--- work, but they would block the main loop (|send| would error on a
--- full buffer, |recv| would poll with |vim.wait()| instead of
--- yielding). Use |spawn()| whenever channel calls might block.
---
--- ```lua
---     -- Send messages from a timer-based producer:
---     local function counter(chan)
