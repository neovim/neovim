...
local function compute_folds_levels(bufnr, info, srow, erow, parse_injections, callback)
  srow = srow or 0
  erow = erow or api.nvim_buf_line_count(bufnr)

  local parser = assert(ts.get_parser(bufnr, nil, { error = false }))

  parser:parse(parse_injections and { srow, erow } or nil, function(trees)
    if not trees then
      return
    end
...
local function on_changedtree(bufnr, foldinfo, tree_changes)
  schedule_if_loaded(bufnr, function()
    ...
    for _, change in ipairs(tree_changes) do
      ...
      compute_folds_levels(bufnr, foldinfo, srow, erow, nil, function()
        completed_iterations = completed_iterations + 1
        if completed_iterations == #tree_changes then
...
api.nvim_create_autocmd('OptionSet', {
  ...
  callback = function()
    for bufnr, _ in pairs(foldinfos) do
      foldinfos[bufnr] = FoldInfo.new()
      compute_folds_levels(bufnr, foldinfos[bufnr], nil, nil, nil, function()
