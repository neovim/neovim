...
---@field private _parse_regions table<integer, vim.treesitter.languagetree.ParseRegion>
...
function LanguageTree.new(source, lang, opts)
  ...
  self._parse_regions[id] = { valid = false, value = max_region, root = true }
...
function LanguageTree:invalidate(reload)
  ...
  if reload then
    ...
    for _, mark in ipairs(marks) do
      local mark_region = self._parse_regions[mark[1]]
...
function LanguageTree:is_valid(exclude_children, range)
  ...
  for _, mark in ipairs(marks) do
    local region = self._parse_regions[mark[1]]
  ...
  for lang, injection_regions in pairs(injections_by_lang) do
    ...
    if has_lang then
      ...
      for _, region in ipairs(injection_regions) do
        ...
        child._parse_regions[id] = { valid = false, value = region }
      end
...
function LanguageTree:for_each_tree(fn)
  local marks = self:_get_marks()
  for _, mark in ipairs(marks) do
    local tree = self._parse_regions[mark[1]].value
...
function LanguageTree:destroy()
  ...
  if self._has_scratch_buf then
    ...
    -- TODO: Else, clear extmarks and _parse_regions
  end
  for _, child in pairs(self._children) do
    child:destroy()
  end
...
function LanguageTree:tree_for_range(range, opts)
  ...
  for _, mark in ipairs(marks) do
    local id = mark[1]
    local tree = self._parse_regions[id].value
    if type(tree) == 'userdata' and tree_contains(tree, range) then
      return tree
    end
  end
