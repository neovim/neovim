...
---@alias MarkInfo { begin_line: integer, begin_col: integer, opts: vim.api.keyset.set_extmark }
...
local function add_mark(m, buf, line, next_marks)
  local cur_begin_l = m.begin_line
  local cur_begin_c = m.begin_col
  if cur_begin_l < line then
    cur_begin_l = line
    cur_begin_c = 0
  end
  ...
  if cur_opts.end_line >= line + 1 then
    cur_opts = vim.tbl_extend('error', {}, cur_opts)
    cur_opts.end_line = line + 1
    cur_opts.end_col = 0
    table.insert(next_marks, m)
  end

  local empty = cur_opts.end_line < cur_begin_l
    or (cur_opts.end_line == cur_begin_l and cur_opts.end_col <= cur_begin_c)
  if cur_begin_l <= line and not empty then
    api.nvim_buf_set_extmark(buf, ns, cur_begin_l, cur_begin_c, cur_opts)
  end
end
