...
local function visual_select(node)
  local rows, cols, rowe, cole = Range.unpack4(node_range(node))
...
function M.select_node()
  ...
  local node, alternative_nodes = get_node(range)

  if node == false then
    visual_select(assert(alternative_nodes[1]))
    return
  end
...
function M.select_parent()
  ...
  if node == false then
    visual_select(assert(parent_chain[1]))
    return
  end
...
function M.select_child()
  ...
  if node == false then
    visual_select(assert(alternative_child_nodes[1]))
    return
  end
...
--- @param prev boolean
local function select_sibling(prev)
  local range = get_selection()
  local node, parent_chain = get_node(range)
  if not node then
    return
