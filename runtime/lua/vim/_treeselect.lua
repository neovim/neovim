...
---@type {[integer]:vim.treeselect.node,[any]:any}
local history = {
  ---@type integer?
  bufnr = nil,
...
---@class vim.treeselect.node
---@field node TSNode
---@field top vim.treeselect.node.top

---@class vim.treeselect.node.top: vim.treeselect.node
---@field ltree vim.treesitter.LanguageTree
---@field region Range4

local M = {}

---@param node vim.treeselect.node
---@return string
...
---@param node1 vim.treeselect.node
---@param node2 vim.treeselect.node
---@return boolean
...
---@return vim.treeselect.node.top
local function create_top_node(tree, region, ltree)
  ---@type vim.treeselect.node.top
  local self = {
    node = tree:root(),
    top = {} --[[@as any]],
    ltree = ltree,
...
---@param node1 vim.treeselect.node.top
---@param node2 vim.treeselect.node.top
---@return boolean
...
---@param range Range4
---@param top_node vim.treeselect.node.top?
---@param parent_chain vim.treeselect.node[]?
---@return vim.treeselect.node|false|nil
---@return vim.treeselect.node[]
---@return vim.treeselect.node.top?
local function get_node(range, top_node, parent_chain)
  ...
  if not top_node then
    ...
    if not Range.contains(node_range(top_node), range) then
      return false, { top_node }
    end
  end

  assert(Range.contains(node_range(top_node), range))

  ---@param node vim.treeselect.node|vim.treeselect.node.top
  ---@return vim.treeselect.node|vim.treeselect.node.top
      ...
      for _, child_tree in ipairs(child:trees()) do
        for _, child_region in ipairs(tree_get_ranges(child_tree)) do
          ...
          if
            ...
            and (
              not node.ltree
              or top_node_is_higher_priority(node --[[@as vim.treeselect.node.top]], child_top_node)
            )
          then
            ...
            if
              ...
              or (
                ...
                and top_node_is_higher_priority(
                  node --[[@as vim.treeselect.node.top]],
                  child_top_node
                )
              )
            then
  ...
  if node.ltree then
    ...
    ---@cast node vim.treeselect.node.top
    return get_node(range, node, parent_chain)
  end
  ---@cast node vim.treeselect.node
...
---@param nodes vim.treeselect.node[]
---@param node vim.treeselect.node.top
    ...
    if Range.intercepts(node_range(nodes[n]), node_range(node)) then
      if
        not nodes
          [n] --[[@as any]]
          .ltree
        or top_node_is_higher_priority(nodes[n] --[[@as vim.treeselect.node.top]], node)
      then
