...
local function normalize_spec(spec)
  ...
  if not spec.src:match('^https?://') and not spec.src:match('^git@') and not spec.src:match('^file://') then
  ...
  --   function M.new_progress_report(title)
    ...
    --     return vim.schedule_wrap(function(kind, msg, percent)
      ...
      local function resolve_version(p)
        ...
        -- Handle wildcard version specs like "*", "v1.*", "0.*"
        if type(version) == "string" and version:find("%*") then
          local tags = git_get_tags(p.path)
          local pattern = "^" .. version:gsub("%.", "%%."):gsub("%*", ".*") .. "$"  -- Convert wildcard to Lua pattern
          local matched_tags = {}
          ...
          for _, tag in ipairs(tags) do
            if tag:match(pattern) then
              ...
              if ver_tag then
                matched_tags[#matched_tags + 1] = {tag = tag, ver = ver_tag}
              end
            end
          end

          if #matched_tags == 0 then
            local err = ("No versions matching pattern `%s` found for plugin %s. Available tags: %s"):format(
              version, p.spec.name, table.concat(tags, ", ")
            )
            error(err)
          end

          -- Sort matched tags by version descending to pick the newest
          table.sort(matched_tags, function(a, b) return a.ver > b.ver end)
          p.info.version_str = matched_tags[1].tag
          p.info.version_ref = matched_tags[1].tag
          return
        end
