...
local function current_enc()
  local enc = (M.config.encoding or 'utf-8'):lower()
  if enc == 'iso-8859-1' or enc == 'iso-8859-15' then
    enc = 'latin1'
  end
  return enc
end
...
local function writable_spell_dirs_from_rtp()
  ...
  for _, dir in ipairs(M.config.rtp) do
    local spell = join(vim.fn.fnamemodify(dir, ':p'), 'spell')
...
local function default_spell_dir()
  return join(vim.fn.stdpath('data'), 'site', 'spell')
end
...
function M.directory_choices()
  local opts = {}
  for _, dir in ipairs(M.config.rtp) do
    local spelldir = join(vim.fn.fnamemodify(dir, ':p'), 'spell')
...
function M.parse(lang)
  ...
  for _, fn in ipairs(candidates) do
    if not file_ok(join(dir, fn)) then
...
function M.download(info)
  ...
  local out_utf8 = join(dir, spl_utf8)
  ...
  if not ok then
    ...
    local out_ascii = join(dir, spl_ascii)
  ...
  if not file_ok(join(dir, sug_name)) then
    local url_sug = M.config.url .. '/' .. sug_name
    local out_sug = join(dir, sug_name)
