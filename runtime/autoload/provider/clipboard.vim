...
function! s:clipboard.get(reg) abort
  ...
  " Cached Blob (OSC 52 copy) never matches: comparing Blob with List raises E977.
...
" Blob => List of lines, NUL within a line becomes NL (:help NL-used-for-Nul).
function! s:blob2lines(blob) abort
  return luaeval('vim.tbl_map(function(l) return (l:gsub("%z", "\n")) end, vim.split(_A, "\n", {plain=true}))', a:blob)
endfunction

" {lines} is a List of lines, or a Blob (binary payload, e.g. OSC 52) which is
" converted to a List for function providers that did not set g:clipboard.blob.
