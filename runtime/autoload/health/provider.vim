...
function! s:version_info(python) abort
  ...
  function! s:compare(metapath1, metapath2) abort
    ...
    return luaeval('vim.cmp_versions(_A[1], _A[2])', [a, b])
  endfunction
...
function! s:check_ruby() abort
  ...
  if luaeval('vim.cmp_versions(_A[1], _A[2])', [current_gem, latest_gem]) == -1
    call health#report_warn(
...
function! s:check_node() abort
  ...
  if s:shell_error || luaeval('vim.cmp_versions(_A[1], _A[2])', [node_v[1:], '6.0.0']) < 0
  ...
  if luaeval('vim.cmp_versions(_A[1], _A[2])', [current_npm, latest_npm]) == -1
    call health#report_warn(
...
function! s:check_perl() abort
  ...
  if luaeval('vim.cmp_versions(_A[1], _A[2])', [current_cpan, latest_cpan]) == -1
    call health#report_warn(
