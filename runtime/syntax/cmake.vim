...
syn keyword cmakeKWbuild_command contained
            \ CONFIGURATION PARALLEL_LEVEL TARGET

syn keyword cmakeKWbuild_name contained
            \ CMAKE_CXX_COMPILER

syn keyword cmakeKWcmake_host_system_information contained
            \ AVAILABLE_PHYSICAL_MEMORY AVAILABLE_VIRTUAL_MEMORY DISTRIB_INFO FQDN HAS_AMD_3DNOW HAS_AMD_3DNOW_PLUS HAS_FPU HAS_IA64 HAS_MMX HAS_MMX_PLUS HAS_SERIAL_NUMBER HAS_SSE HAS_SSE2 HAS_SSE_FP HAS_SSE_MMX HOSTNAME ID IS_64BIT NUMBER_OF_LOGICAL_CORES NUMBER_OF_PHYSICAL_CORES OS_NAME OS_PLATFORM OS_RELEASE OS_VERSION PROCESSOR_DESCRIPTION PROCESSOR_NAME PROCESSOR_SERIAL_NUMBER QUERY RESULT TOTAL_PHYSICAL_MEMORY TOTAL_VIRTUAL_MEMORY
...
syn keyword cmakeKWfile contained
            \ ALGO APPEND ARCHIVE_CREATE ARCHIVE_EXTRACT ASCII BASE_DIRECTORY BUNDLE_EXECUTABLE CHMOD CHMOD_RECURSE CMAKE_TLS_CAINFO CMAKE_TLS_VERIFY COMPRESSION COMPRESSION_LEVEL CONDITION CONFIG CONFIGURE_DEPENDS CONFLICTING_DEPENDENCIES_PREFIX CONTENT COPY CR CREATE_LINK DESTINATION DIRECTORIES DIRECTORY_PERMISSIONS DOWNLOAD ENCODING ESCAPE_QUOTES EXCLUDE EXECUTABLES EXPAND_TILDE EXPECTED_HASH EXPECTED_MD5 FILE_PERMISSIONS FILES_MATCHING FOLLOW_SYMLINK_CHAIN FOLLOW_SYMLINKS FORMAT FUNCTION GENERATE GET_RUNTIME_DEPENDENCIES GLOB GLOB_RECURSE GUARD HASH HEX HTTPHEADER IGNORED INACTIVITY_TIMEOUT INPUT INSTALL LENGTH_MAXIMUM LENGTH_MINIMUM LF LIBRARIES LIMIT LIMIT_COUNT LIMIT_INPUT LIMIT_OUTPUT LIST_DIRECTORIES LIST_ONLY LOCK LOG MAKE_DIRECTORY MODULES MTIME NETRC NETRC_FILE NEWLINE_CONSUME NEWLINE_STYLE NO_HEX_CONVERSION NO_REPLACE NO_SOURCE_PERMISSIONS OFFSET OLD @ONLY ONLY_IF_DIFFERENT OPTIONAL OUTPUT PATHS PATTERN PATTERNS PERMISSIONS POST_EXCLUDE_FILES POST_EXCLUDE_REGEXES POST_INCLUDE_FILES POST_INCLUDE_REGEXES PRE_EXCLUDE_REGEXES PRE_INCLUDE_REGEXES PROCESS READ READ_SYMLINK REAL_PATH REGEX RELATIVE RELATIVE_PATH RELEASE REMOVE REMOVE_RECURSE RENAME REQUIRED RESOLVED_DEPENDENCIES_VAR RESULT RESULT_VARIABLE SHOW_PROGRESS SIZE SSL STATUS STRINGS TARGET TIMESTAMP TLS_CAINFO TLS_VERIFY TO_CMAKE_PATH TO_NATIVE_PATH TOUCH TOUCH_NOCREATE UNRESOLVED_DEPENDENCIES_VAR UPLOAD USERPWD USE_SOURCE_PERMISSIONS UTC UTF VERBOSE WRITE
...
syn keyword cmakeKWinstall contained
            \ ARCHIVE BUNDLE CODE COMPONENT CONFIGURATIONS CVS DESTDIR DESTINATION DIRECTORY DIRECTORY_PERMISSIONS DLL EXCLUDE_FROM_ALL EXPORT EXPORT_ANDROID_MK EXPORT_LINK_INTERFACE_LIBRARIES FILE_PERMISSIONS FILES FILES_MATCHING FRAMEWORK GROUP_EXECUTE GROUP_READ GROUP_WRITE IMPORTED_ IMPORTED_RUNTIME_ARTIFACTS INCLUDES INSTALL_PREFIX INTERFACE_INCLUDE_DIRECTORIES LIBRARY MACOSX_BUNDLE MESSAGE_NEVER NAMELINK_COMPONENT NAMELINK_ONLY NAMELINK_SKIP NAMESPACE NDK OBJECTS OPTIONAL OWNER_EXECUTE OWNER_READ OWNER_WRITE PATTERN PERMISSIONS POST_INSTALL_SCRIPT PRE_INSTALL_SCRIPT PRIVATE_HEADER PROGRAMS PUBLIC_HEADER REGEX RENAME RESOURCE RUNTIME RUNTIME_DEPENDENCIES RUNTIME_DEPENDENCY_SET SCRIPT SETGID SETUID SOVERSION TARGETS TRUE USE_SOURCE_PERMISSIONS VERSION WORLD_EXECUTE WORLD_READ WORLD_WRITE
...
syn keyword cmakeKWset_property contained
            \ APPEND APPEND_STRING CACHE DIRECTORY GLOBAL INSTALL PROPERTY SOURCE TARGET TEST VS_SETTINGS WIX
...
syn keyword cmakeKWstring contained
            \ ALPHABET APPEND ASCII CMAKE_MATCH_ COMPARE CONCAT CONFIGURE EQUAL ERROR_VARIABLE ESCAPE_QUOTES FIND GENEX_STRIP GET GREATER GREATER_EQUAL GUID HASH JSON LENGTH LESS LESS_EQUAL MAKE_C_IDENTIFIER MATCH MATCHALL MATCHES MEMBER NAMESPACE NOTEQUAL ONLY PREPEND RANDOM RANDOM_SEED REGEX REMOVE REPLACE REVERSE RFC SET SHA SOURCE_DATE_EPOCH STRIP SUBSTRING SZ TIMESTAMP TOLOWER TOUPPER TYPE US UTC UUID
