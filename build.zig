...
pub fn build(b: *std.Build) !void {
    ...
    _ = test_config_step.add("test/cmakeconfig/paths.lua", test_config(b, gen_headers.getDirectory()));
    ...
    const shell_test = b.addExecutable(.{
        .name = "shell-test",
        .target = target,
        .optimize = optimize,
    });
    shell_test.addCSourceFile(.{ .file = b.path("./test/functional/fixtures/shell-test.c") });
    shell_test.linkLibC();
    const shell_test_install = b.addInstallArtifact(shell_test, .{});
    test_prerequisites.dependOn(&shell_test_install.step);

    const tty_test = b.addExecutable(.{
        .name = "tty-test",
        .target = target,
        .optimize = optimize,
    });
    tty_test.addCSourceFile(.{ .file = b.path("./test/functional/fixtures/tty-test.c") });
    tty_test.linkLibC();
    tty_test.linkLibrary(libuv);
    const tty_test_install = b.addInstallArtifact(tty_test, .{});
    test_prerequisites.dependOn(&tty_test_install.step);
...
pub fn test_config(b: *std.Build, gen_dir: LazyPath) []u8 {
    ...
    return b.fmt(
        ...
        \\
        \\return M
    , .{ .bin_dir = b.install_path, .gen_dir = gen_dir });
}
