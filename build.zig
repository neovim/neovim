...
pub fn build(b: *std.Build) !void {
    ...
    const include_path: []const LazyPath = &.{
        b.path("src/"),
    ...
    if (support_unittests) {
        // TODO: this is the hot fix. in principle we can construct a paths.lua include paths from the lazyPaths directly without copying files again
        for (include_path) |path| {
            test_deps.dependOn(&b.addInstallDirectory(.{ .source_dir = path, .install_dir = .prefix, .install_subdir = "unittest_include/" }).step);
        }
        test_deps.dependOn(&b.addInstallDirectory(.{ .source_dir = gen_headers.getDirectory(), .install_dir = .prefix, .install_subdir = "unittest_include/" }).step);
    }

    try tests.test_steps(b, nvim_exe, test_deps, lua_dev_deps.path("."), test_config_step.getDirectory(), support_unittests);
}
...
pub fn test_config(b: *std.Build) ![]u8 {
    ...
    return b.fmt(
        ...
        \\M.test_lua_prg = ""
        \\M.test_luajit_prg = ""
        \\M.include_paths = {{"{[bin_dir]s}/unittest_include/"}}
        \\
        \\return M
